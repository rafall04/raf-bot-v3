/* global fetchConnectedDevicesData */
        ...
        // halaman ini hanya menampilkan. "Cek titik" = pratinjau (tanpa confirm) → server balas titik
        // LAMA, jarak geser, tetangga/ODP terdekat, dan peringatan. Simpan baru dikirim setelah dilihat.
        ...
        $(document).on('click', '#lokasi_simpan', function() {
            ...
            $.ajax({
                ...
                Swal.fire({ icon: 'success', title: 'Titik tersimpan', timer: 1500, showConfirmButton: false });
                if (typeof dataTableInstance !== 'undefined' && dataTableInstance) dataTableInstance.ajax.reload(null, false);
            }).fail(function(xhr) {
        ...
        $(document).on('click', '.btn-edit', function() {
            ...
            // billing_cycle: 'awal_bulan' (default) | 'akhir_bulan'.
            $('#editModal #edit_billing_cycle').val(String($(this).data('billing_cycle')).toLowerCase() === 'akhir_bulan' ? 'akhir_bulan' : 'awal_bulan');
                            ...
                            if (s.associatedDevices && s.associatedDevices.length > 0) {
                                ...
                                s.associatedDevices.forEach(dev => {
                                    ...
                                    contentHtml += `<li class="list-group-item py-1">
                                                        ${escapeHtml(dev.hostName || 'Tanpa Nama')} <br>
                                                        <small class="text-muted" style="font-size:0.9em;">
                                                            (MAC: ${escapeHtml(dev.mac || '-')}, IP: ${escapeHtml(dev.ip || '-')}, Sinyal: ${dev.signal ? dev.signal + ' dBm' : '-'})
                                                        </small>
                                                    </li>`;
                                });
                                contentHtml += `</ul>`;
        ...
        async function initializePage() {
            ...
            dataTableInstance = $('#dataTable').DataTable({
                ...
                columns: [
                    ...
                    { data: 'device_id' },
                    { data: 'address' },
                    // Kolom TITIK: sekaligus jadi tombol. Sel yang kosong bukan lagi "N/A" pasif —
                    // dia ajakan bertindak, karena mengisi titik adalah pekerjaan yang memang menumpuk.
                    {
                        data: null,
                        ...
                        render: function(data, type, row) {
                            ...
                            let actionButtonsHtml = `
                                <div class="device-action-group">
                                    <button class="btn btn-info btn-sm btn-edit" data-id="${row.id}" data-name="${row.name || ''}" data-phone_number="${row.phone_number || ''}" data-device_id="${deviceIdForActions}" data-address="${row.address || ''}" data-dusun="${row.dusun || ''}" data-subscription="${row.subscription || ''}" data-paid="${row.paid || false}" data-send_invoice="${row.send_invoice || false}" data-notify_outage="${row.notify_outage !== false && row.notify_outage !== 0 ? 'true' : 'false'}" data-account_type="${String(row.account_type || 'pelanggan').toLowerCase() === 'infrastruktur' ? 'infrastruktur' : 'pelanggan'}" data-billing_cycle="${String(row.billing_cycle || 'awal_bulan').toLowerCase() === 'akhir_bulan' ? 'akhir_bulan' : 'awal_bulan'}" data-pppoe_username="${row.pppoe_username || ''}" data-pppoe_password="${row.pppoe_password || ''}" data-latitude="${row.latitude || ''}" data-longitude="${row.longitude || ''}" data-connected_odp_id="${row.connected_odp_id || ''}" data-bulk='${JSON.stringify(Array.isArray(row.bulk) ? row.bulk : (typeof row.bulk === 'string' ? JSON.parse(row.bulk) : []))}' data-toggle="modal" data-target="#editModal" title="Edit User"><i class="fas fa-edit"></i></button>
                        ...
                        result.data.ssid.forEach(s => {
                            const ssidField = `
                                <div class="form-group">
                                    ...
                                    <input type="text" class="form-control form-control-sm" id="modal_ssid_${s.id}" name="ssid_${s.id}" value="${escapeHtml(s.name || '')}">
                                </div>`;
