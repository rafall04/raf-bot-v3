...
const log = require('./logger').logger.child('ALERT_SYSTEM');
...
class AlertSystem {
    ...
    async sendAlert(level, type, details = {}) {
        try {
            ...
            if (this.isInCooldown(type)) {
                log.info(`[ALERT] ${type} is in cooldown`);
                return false;
            }
            
            // Check rate limit
            if (this.isRateLimited(level)) {
                log.info(`[ALERT] Rate limit exceeded for ${level} alerts`);
                return false;
            }
            ...
            log.error('[ALERT] Send error:', error);
            return false;
        }
    }
    ...
    async processAlertQueue() {
        ...
        while (this.alertQueue.length > 0) {
            ...
            try {
                ...
                log.info(`[ALERT] Sent ${alert.level} alert: ${alert.type}`);
            } catch (error) {
                log.error(`[ALERT] Failed to send alert:`, error);
    ...
    async sendWhatsAppAlert(message, _level) {
        try {
            ...
            if (!hasAuthenticatedSession()) {
                log.info('[ALERT] WhatsApp not connected, queuing alert');
            ...
            if (admins.length === 0) {
                log.info('[ALERT] No admin numbers configured');
                return false;
            }
            ...
            const sendPromises = admins.map(async (adminNumber) => {
                ...
                if (isReady()) {
                    try {
                        ...
                        log.info(`[ALERT] Sent to admin: ${adminNumber}`);
                        return true;
                    } catch (error) {
                        ...
                        log.error(`[ALERT] Failed to send to ${adminNumber}:`, error);
                        return false;
                    }
                } else {
            ...
            log.error('[ALERT] WhatsApp send error:', error);
            return false;
        }
    }
    ...
    async sendDailyReport(report) {
        ...
        log.info('[ALERT] Daily report sent');
    }
    ...
    logAlert(alert, message) {
        try {
            ...
            log.error('[ALERT] Log write error:', error);
        }
    }
    ...
    clearQueue() {
        ...
        this.alertQueue = [];
        log.info(`[ALERT] Cleared ${cleared} pending alerts`);
        return cleared;
    }
     ...
     */
    async testAlert() {
        log.info('[ALERT] Running test alert...');
