 ...
 *       jakarta-date (pencocokan hari/bulan WIB terhadap timestamp UTC tersimpan).
 * MainFuncs: `buildCockpit`.
 * SideEffects: Cache in-memory PPP stats + peta profil PPPoE (TTL 90s) — bukan tulis DB.
 */
"use strict";

const { getJakartaParts, jakartaDateStringOf, jakartaYearMonthOf } = require("./jakarta-date");
...
async function buildIncomeCard(deps) {
    ...
    // `t.created_at` berupa datetime('now') UTC; cocokkan lewat kalender Jakarta dua sisi
    // supaya pembayaran pagi WIB tak terhitung sebagai kemarin.
    const todayDate = getJakartaParts(now).date;
    let todayCount = 0;
    let todayAmount = 0;
    for (const t of txs) {
        if (t && t.type === "payment" && jakartaDateStringOf(t.created_at) === todayDate) {
...
async function buildCustomerCard(deps, ppp) {
    const now = new Date(deps.nowMs());
    const monthPrefix = getJakartaParts(now).yearMonth;
    const cust = (deps.getUsers() || []).filter((u) => !deps.isInfrastructure(u));
    const baru = cust.filter((u) => jakartaYearMonthOf(u && u.created_at) === monthPrefix).length;
