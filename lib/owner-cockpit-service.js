...
function buildOltCard(deps, ppp) {
    const st = deps.getLosState() || {};
    const offline = (ppp && ppp.ok && ppp.data && ppp.data.offline != null) ? toInt(ppp.data.offline) : null;
    return { ok: true, activeOutage: toInt(st.activeIncidentCount), recovering: toInt(st.recoveringCount), pending: toInt(st.pendingCount), offline };
}
...
function buildCustomerCard(deps, ppp) {
    ...
    return { ok: true, total: cust.length, aktif, isolir, baru, pppoeOnline };
}
