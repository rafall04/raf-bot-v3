...
function installConsoleBridge() {
    ...
    Object.entries(LEVEL_MAP).forEach(([consoleMethod, loggerMethod]) => {
        console[consoleMethod] = (...args) => {
            // insideBridge: emit dari bridge sendiri. isEmittingToConsole(): emit
            // internal logger (logger.* dipanggil LANGSUNG oleh kode app) — tanpa
            // cek ini baris log terformat diproses ulang bridge → double-prefix +
            // dobel tulis ke file.
            if (insideBridge || logger.isEmittingToConsole()) {
