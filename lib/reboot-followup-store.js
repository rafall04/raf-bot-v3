 ...
 */
"use strict";
const log = require('./logger').logger.child('REBOOT_FOLLOWUP_STORE');
...
function loadJobs(filePath = resolveFilePath()) {
    try {
        ...
        log.warn(`[REBOOTFU_STORE] Gagal membaca ${path.basename(filePath)}: ${err.message}`);
        try {
            if (fs.existsSync(filePath)) {
                const cap = new Date().toISOString().replace(/[:.]/g, "-");
                const karantina = `${filePath}.rusak-${cap}`;
                fs.renameSync(filePath, karantina);
                log.error(`[REBOOTFU_STORE] Berkas rusak DIKARANTINA ke ${path.basename(karantina)} — jangan hapus sebelum diperiksa.`);
            }
        } catch (errKarantina) {
            log.error(`[REBOOTFU_STORE] Gagal mengkarantina berkas rusak: ${errKarantina.message}`);
        }
        return [];
    }
}
...
function saveJobs(jobs, filePath = resolveFilePath()) {
    try {
        ...
        log.error(`[REBOOTFU_STORE] Gagal menulis ${path.basename(filePath)}: ${err.message}`);
        return false;
    }
}
...
function addJob(job, filePath = resolveFilePath()) {
    if (!job || !job.jid || !job.deviceId) {
        log.warn("[REBOOTFU_STORE] addJob ditolak: jid/deviceId wajib.");
        return null;
    }
