 ...
 */
"use strict";
const log = require('./logger').logger.child('REDAMAN_WATCH_STORE');
...
function loadWatches(filePath = resolveFilePath()) {
    try {
        ...
        log.warn(`[REDAMAN_WATCH] Gagal baca ${path.basename(filePath)}: ${err.message}`);
        try {
            if (fs.existsSync(filePath)) {
                const cap = new Date().toISOString().replace(/[:.]/g, "-");
                fs.renameSync(filePath, `${filePath}.rusak-${cap}`);
                log.error(`[REDAMAN_WATCH] Berkas rusak DIKARANTINA (.rusak-${cap}) — jangan hapus sebelum diperiksa.`);
            }
        } catch (e2) { log.error(`[REDAMAN_WATCH] Gagal karantina: ${e2.message}`); }
        return [];
    }
}
...
function saveWatches(watches, filePath = resolveFilePath()) {
    try {
        ...
        log.error(`[REDAMAN_WATCH] Gagal tulis ${path.basename(filePath)}: ${err.message}`);
        return false;
    }
}
