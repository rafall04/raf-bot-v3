...
// TERBUKTI sudah lama tak terlihat — beda dari sekadar "tidak segar". `lastInform` yang tak terbaca
// BUKAN bukti modem sudah bebas; ia cuma ketiadaan data. Pembedaan ini yang menjaga agar modem
// pelanggan tetap ditolak keras saat pembacaannya gagal, alih-alih diturunkan jadi konfirmasi.
function terbuktiLamaHilang(candidate, nowMs) {
    const t = Date.parse((candidate && candidate.lastInform) || "");
...
function classifyModemCandidate(candidate, { users = [], activeUsernames = null, nowMs = Date.now() } = {}) {
    ...
    if (linked) {
        ...
        const tolakKeras = !!linkedByDevice && !terbuktiLamaHilang(candidate, nowMs);
        return {
            ...base,
            state: "terpakai",
            assignable: false,
