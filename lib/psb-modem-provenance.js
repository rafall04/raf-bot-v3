...
function classifyModemCandidate(candidate, { users = [], activeUsernames = null } = {}) {
    ...
    if (pppoe && sessionsKnown && activeUsernames.has(pppoe)) {
    ...
    const linkedByPppoe = pppoe ? list.find((u) => normalizePppoe(u && u.pppoe_username) === pppoe) : null;
    ...
    if (!pppoe || DEFAULT_PPPOE_USERNAMES.has(pppoe)) {
