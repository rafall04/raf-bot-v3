 ...
 */
"use strict";
const log = require('./logger').logger.child('PROCESS_LIFECYCLE');
...
function registerProcessLifecycleHandlers({ runtime, CustomerTrafficUsageService, closeLogsDatabase }) {
    ...
    process.on("uncaughtException", (error) => {
        ...
        log.info("🔄 Restarting process due to uncaught exception...");
        process.exit(1);
    });
    ...
    process.on("SIGTERM", async () => {
        ...
        try {
            try {
                ...
                log.info("   Stopping OLT log scraper...");
            } catch (__error) {
            ...
            try {
                ...
                log.info("   Stopping OLT syslog receiver...");
            } catch (__error) {
            ...
            try {
                ...
                log.info("   Stopping OLT rxPower poller...");
            } catch (__error) {
            ...
            try {
                ...
                log.info("   Stopping OLT SNMP LOS poller...");
            } catch (__error) {
                // Ignore if poller belum diload.
            }
            ...
            try {
                ...
                cctv.stopCctvMonitor();
                log.info("   Stopping CCTV monitor...");
            } catch (__error) {
                // Ignore if monitor belum diload.
            }

            try {
                const tgTeknisi = require("./telegram/telegram-teknisi-bootstrap");
                tgTeknisi.stopTelegramTeknisiBot();
                log.info("   Stopping Telegram teknisi bot...");
            } catch (__error) {
                // Ignore if bot belum diload.
            }

            if (global.sock && typeof global.sock.logout === "function") {
                log.info("   Closing WhatsApp connection...");
            ...
            if (typeof closeLogsDatabase === "function") {
                try {
                    ...
                    log.info("   Closing logs database connection...");
                } catch (error) {
                    log.error("   Error closing logs database:", error);
                }
            }
    ...
    process.on("SIGINT", async () => {
        ...
        try {
            try {
                require("./telegram/telegram-teknisi-bootstrap").stopTelegramTeknisiBot();
            } catch (__error) {
                // Ignore if bot belum diload.
            }
            if (global.sock && typeof global.sock.logout === "function") {
