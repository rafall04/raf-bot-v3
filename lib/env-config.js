...
const log = require('./logger').logger.child('ENV_CONFIG');
const fs = require('fs');
...
const shouldLogEnvBootstrap = () => (
    ...
    if ((NODE_ENV === 'test' || NODE_ENV === 'development') && shouldLogEnvBootstrap()) {
        log.warn(`[ENV] ⚠️  ${NODE_ENV} mode - using test database`);
    }
...
const getConfigPath = () => {
    ...
    if (!fs.existsSync(prodConfig)) {
        ...
        if (fs.existsSync(exampleConfig)) {
            log.warn(`[ENV_CONFIG] config.json not found, using config.example.json`);
...
const ensureConfigFromExample = () => {
    ...
    try {
        ...
        if (typeof fresh.jwt !== 'string' || fresh.jwt.startsWith('ISI_') || fresh.jwt.trim() === '') {
            ...
            // #b345: tulis config.json ATOMIK (torn-write config.json = boot-fatal CONFIG_FATAL).
            const { writeFileAtomicSync } = require('./atomic-file');
            writeFileAtomicSync(prodConfig, JSON.stringify(fresh, null, 2) + '\n');
        }
    } catch (e) {
        log.warn(`[CONFIG_BOOTSTRAP] Gagal generate jwt acak: ${e.message}`);
    }

    const line = '='.repeat(72);
    log.warn(line);
    log.warn('[CONFIG_BOOTSTRAP] config.json tidak ditemukan saat startup.');
    log.warn('[CONFIG_BOOTSTRAP] File dibuat otomatis dari config.example.json.');
    log.warn('[CONFIG_BOOTSTRAP] (jwt secret acak sudah di-generate otomatis)');
    log.warn('[CONFIG_BOOTSTRAP] >> WAJIB isi kredensial asli (ipaymu, tokopay, bank,');
    log.warn('[CONFIG_BOOTSTRAP]    ownerNumber, olt, telegramBackup, genieacs, dll) di');
    log.warn('[CONFIG_BOOTSTRAP]    config.json sebelum dipakai di produksi.');
    log.warn(line);
    return true;
};
    ...
    try {
        if (fs.existsSync(configPath)) {
            ...
            if (NODE_ENV !== 'production' && shouldLogEnvBootstrap()) {
                log.info(`[ENV] ${NODE_ENV} mode - DB: ${getDatabasePath()}`);
            }
            ...
            if (typeof config.jwt !== 'string' || config.jwt.startsWith('ISI_') || config.jwt.trim() === '') {
                const line = '='.repeat(72);
                log.error(line);
                log.error('[CONFIG_SECURITY] config.jwt masih placeholder/kosong!');
                log.error('[CONFIG_SECURITY] Token sesi BISA DIPALSUKAN. Isi config.jwt');
                log.error('[CONFIG_SECURITY] dengan secret acak yang panjang sebelum produksi.');
                log.error(line);
            }
        ...
        log.error(`[ENV_CONFIG_ERROR] Failed to load config:`, error.message);
        throw error;
    }
};
...
const validateEnvironment = () => {
    ...
    if (NODE_ENV === 'production') {
        ...
        if (fs.existsSync(testDbPath)) {
            log.info(`[ENV_CONFIG_INFO] Test database file present but production DB path remains active: ${testDbPath}`);
        }
    ...
    if (warnings.length > 0) {
        log.warn(`[ENV_CONFIG_WARN]`, warnings);
    }
    
    if (errors.length > 0) {
        log.error(`[ENV_CONFIG_ERROR]`, errors);
...
// Field efemeral runtime yang DITAMBAHKAN loadConfig (bukan isi disk) — jangan ikut ter-persist.
const EPHEMERAL_CONFIG_KEYS = ['environment', 'isProduction', 'isTest'];

/**
 * Baca config.json SEGAR dari disk (isi apa adanya, tanpa field efemeral). Basis read-modify-write
 * yang aman: penulis config HANYA mengubah subkey miliknya lalu saveConfigAtomic — supaya subkey
 * yang ditambah penulis lain / hand-edit ops PASCA-BOOT tak terhapus (bahaya merge-key #b336).
 */
const readConfigFresh = () => {
    const configPath = getConfigPath();
    return JSON.parse(fs.readFileSync(configPath, 'utf8'));
};

/**
 * Tulis config ke disk ATOMIK (tmp+rename; config.json terpotong = bot gagal boot total) + buang
 * field efemeral runtime + sinkronkan `global.config`. Melempar bila tulis gagal.
 */
const saveConfigAtomic = (config) => {
    const { writeFileAtomicSync } = require('./atomic-file');
    const toWrite = { ...config };
    for (const k of EPHEMERAL_CONFIG_KEYS) delete toWrite[k];
    writeFileAtomicSync(getConfigPath(), JSON.stringify(toWrite, null, 2));
    // Sinkronkan runtime persis loadConfig (config + field efemeral).
    const runtime = Object.assign(config, {
        environment: NODE_ENV,
        isProduction: NODE_ENV === 'production',
        isTest: NODE_ENV === 'test' || NODE_ENV === 'development',
    });
    if (typeof global !== 'undefined') global.config = runtime;
    return runtime;
};
