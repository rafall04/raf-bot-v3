...
const log = require('./logger').logger.child('MONITORING_SERVICE');
const os = require('os');
...
class MonitoringService {
    ...
    async initializeDatabase() {
        try {
            ...
            this.metricsDb = new sqlite3.Database(this.metricsDbPath);
            {
                ...
                applySqlitePragmas(this.metricsDb).catch((pragmaErr) => {
                    log.warn(`[MONITORING_DB_PRAGMA_WARN] ${pragmaErr.message}`);
                });
            }
            ...
            log.error('[MONITORING] Database initialization error:', error);
        }
    }
    ...
    async collectMetrics() {
        try {
            ...
            log.error('[MONITORING] Collection error:', error);
        }
    }
    ...
    async saveMetricsToDatabase() {
        ...
        try {
            ...
            log.error('[MONITORING] Database save error:', error);
        }
    }
    ...
    async healthCheck() {
        ...
        if (health.status !== 'healthy') {
            log.info(`[MONITORING] Health check: ${health.status} (${health.issues.length} issues)`);
        }
    ...
    async sendDailyReport() {
        try {
            log.info('[MONITORING] Generating daily report...');
            ...
            log.info('[MONITORING] Daily report sent');
        } catch (error) {
            log.error('[MONITORING] Daily report error:', error);
        }
    }
    ...
    async generateDailyReport() {
        return new Promise((resolve) => {
            ...
            `, [dateStr], (err, rows) => {
                if (err) {
                    log.error('[MONITORING] Report query error:', err);
                    resolve(null);
                    return;
                }
    ...
    async saveDailySummary(report) {
        ...
        try {
            ...
            log.error('[MONITORING] Summary save error:', error);
        }
    }
    ...
    async cleanupOldData() {
        ...
        try {
            ...
            log.error('[MONITORING] Cleanup error:', error);
        }
    }
    ...
    async logError(error, context, severity = 'error') {
        ...
        try {
            ...
            log.error('[MONITORING] Error log failed:', dbError);
        }
    }
    ...
    async getMetricsHistory(hours = 24) {
        return new Promise((resolve) => {
            ...
            `, [cutoff.toISOString()], (err, rows) => {
                if (err) {
                    log.error('[MONITORING] History query error:', err);
                    resolve([]);
                    return;
                }
     ...
     */
    async restart() {
        log.info('[MONITORING] Restarting service...');
