...
const log = require('./logger').logger.child('TEMP_CLEANUP');
const fs = require('fs');
        ...
        files.forEach(file => {
            ...
            try {
                ...
                if (age > maxAge && stats.isFile()) {
                    ...
                    log.info(`[TEMP_CLEANUP] Deleted old temp file: ${file}`);
                }
            } catch (err) {
                // File might be locked or already deleted
                if (err.code !== 'ENOENT' && err.code !== 'EBUSY' && err.code !== 'EISDIR') {
                    log.error(`[TEMP_CLEANUP] Error processing ${file}:`, err.message);
                }
            }
        });
    } catch (err) {
        log.error('[TEMP_CLEANUP] Error cleaning temp folder:', err.message);
    }
}
