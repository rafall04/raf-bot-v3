 ...
 */
"use strict";
const log = require('./logger').logger.child('UPLOAD_GUARD');
...
function buatDestinationAman({ namespace, ambilSegmen, ambilTahunBulan, currentDir, saatDitolak }) {
    ...
    return function destination(req, file, cb) {
        ...
        try {
            ...
            log.error(`[UPLOAD_GUARD] Path keluar dari direktori upload: ${error.message}`);
            return cb(new Error("Identitas unggahan tidak valid."), null);
        }
        ...
        try {
            ...
            log.error("[UPLOAD_GUARD] Gagal menyiapkan folder upload:", error.message);
