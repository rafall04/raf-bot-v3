 ...
 */
const refreshingBw = new Set(); // host background-refresh (SWR)

function refreshBandwidth(device, opts = {}) {
...
/**
 * Snapshot bandwidth dengan stale-while-revalidate: ada cache → kembalikan SEKETIKA + refresh
 * di belakang (tak blok UI); `force`/belum-ada-cache → blocking.
 */
function getBandwidthSnapshot(device, opts = {}) {
    const host = device && device.host;
    const ttl = opts.ttlMs || CACHE_TTL_MS;
    const cached = host && cache.get(host);
    if (opts.force || !cached) return refreshBandwidth(device, opts);
    const stale = Date.now() - cached.at >= ttl;
    if (stale && host && !refreshingBw.has(host)) {
        refreshingBw.add(host);
        Promise.resolve()
            .then(() => refreshBandwidth(device, opts))
            .catch(() => {})
            .finally(() => refreshingBw.delete(host));
    }
