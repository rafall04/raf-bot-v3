 ...
 */
const log = require('./logger').logger.child('DEVICE_STATUS');
...
async function isDeviceOnline(deviceId, maxMinutes = DEFAULT_MAX_INFORM_MINUTES) {
    try {
        ...
        if (!deviceIdStr || deviceIdStr.startsWith('DEVICE-')) {
            log.info(`[isDeviceOnline] Skipping check - Mock mode or invalid device ID: ${deviceIdStr}`);
            return {
                online: null,
                lastInform: null,
                minutesAgo: null,
        ...
        if (response.ok && response.data && response.data._lastInform) {
            ...
            log.info(`[isDeviceOnline] Device ${deviceId}: Last inform ${diffMinutes.toFixed(2)} minutes ago`);
        ...
        log.warn(`[isDeviceOnline] Device ${deviceId}: No last inform data`);
        return {
            online: false,
            lastInform: null,
            minutesAgo: null
        ...
        log.error(`[isDeviceOnline] Error checking device ${deviceId}:`, error.message);
...
async function getDeviceLastInform(deviceId) {
    try {
        ...
        log.error(`[getDeviceLastInform] Error:`, error.message);
        return null;
    }
}
