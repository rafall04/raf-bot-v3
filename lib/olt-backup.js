...
'use strict';
const log = require('./logger').logger.child('OLT_BACKUP');
...
function readConfigFile() {
    try {
        ...
        log.error('[OLT-BACKUP] Gagal baca config.json:', e.message);
        return {};
    }
}
    ...
    try {
        ...
        if (method === 'ftp') {
            ...
            try {
                ...
                log.warn(`[OLT-BACKUP] FTP gagal untuk ${device.id} (${ftpErr.message}) → fallback capture running-config`);
        ...
        if (wantTelegram) {
            ...
            try {
                ...
                log.error(`[OLT-BACKUP] Telegram gagal untuk ${device.id}:`, tgErr.message);
            }
        }
        log.info(`[OLT-BACKUP] ✅ ${device.name || device.id} (${result.method}): ${result.lines} baris → ${result.file}`);
    } catch (e) {
        result.error = e.message;
        log.error(`[OLT-BACKUP] ❌ ${device.name || device.id}: ${e.message}`);
    }
    return result;
}
...
async function runBackupAll(opts = {}) {
    ...
    if (opts.notifySummary !== false && failCount > 0) {
        try {
            ...
            log.error('[OLT-BACKUP] Gagal kirim ringkasan Telegram:', e.message);
        }
    }
...
function pruneOldBackups(dir, keep) {
    try {
        ...
        for (const item of files.slice(keep)) {
            ...
            log.info(`[OLT-BACKUP] Retensi: hapus ${item.f}`);
        }
    } catch (e) {
        log.error('[OLT-BACKUP] Gagal pruning retensi:', e.message);
    }
}
