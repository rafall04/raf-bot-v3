/**
 * Header Doc
 * Purpose: Backup konfigurasi OLT — capture `show running-config` via SSH, simpan ke
 *          backups/olt/<deviceId>/, terapkan retensi (hapus terlama), opsional kirim file
 *          ke Telegram (reuse bot telegramBackup). Termasuk konfigurasi auto-backup
 *          (config.olt.backup) dan util listing/download untuk route.
 * Caller: routes/olt-provisioning.js (backup manual + listing), lib/cron/jobs/olt-backup.js (terjadwal).
 * Deps: fs, path, ./olt-zte-provision (captureRunningConfig), ./olt-manager (devices),
 *       ./telegram-backup (sendTelegramDocument/sendTelegramMessage).
...
const { captureRunningConfig } = require('./olt-zte-provision');
...
/** @returns {{enabled, schedule, keep, sendTelegram}} setting backup (merged default) */
...
/**
 * Backup satu OLT: capture running-config → simpan file → retensi → (ops) Telegram.
 * @param {object} device  entry device (butuh sshUsername/sshPassword)
 * @param {object} [opts]  {baseDir, keep, sendTelegram, capture} — capture injectable untuk test
 * @returns {Promise<{ok, deviceId, file, sizeBytes, lines, telegram: {sent, error}|null, error}>}
 */
async function runBackupForDevice(device, opts = {}) {
    ...
    const result = { ok: false, deviceId: device.id, deviceName: device.name, file: null, sizeBytes: 0, lines: 0, telegram: null, error: null };
    try {
        if (!device.sshUsername || !device.sshPassword) {
            throw new Error('Kredensial SSH belum dikonfigurasi untuk OLT ini');
        }
        const configText = await capture(device);
        ...
        const headerLines = [
            ...
            `! Diambil: ${new Date().toLocaleString('id-ID', { timeZone: 'Asia/Jakarta' })}`,
            '!',
        ];
        ...
        console.log(`[OLT-BACKUP] ✅ ${device.name || device.id}: ${result.lines} baris → ${result.file}`);
    } catch (e) {
...
function resolveBackupFile(deviceId, fileName, baseDir) {
    ...
    if (!safeFile.endsWith('.cfg') || safeFile !== fileName) {
