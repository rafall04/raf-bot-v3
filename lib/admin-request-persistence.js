 ...
 */
"use strict";
const log = require('./logger').logger.child('ADMIN_REQUEST_PERSISTENCE');
    ...
    requests.forEach((request) => {
        if (request.status === "pending" && request.createdAt) {
            try {
                ...
                log.error(`[PKG_REQUEST_AUTO_CANCEL_ERROR] Error processing request ${request.id}:`, error.message);
            }
        }
    });
