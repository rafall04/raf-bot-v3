 ...
 */
"use strict";
const log = require('./logger').logger.child('PORT_USAGE_UPDATER');
...
function updateOdpPortUsage(odpId, increment = true, assetsArray = []) {
    ...
    if (odpIndex === -1) {
        log.warn(`[ODP_PORT_UPDATE_WARN] ODP ID ${odpId} tidak ditemukan untuk update port.`);
        return false;
    }
...
function updateOdcPortUsage(odcId, assetsArray = []) {
    ...
    if (odcIndex === -1) {
        log.warn(`[ODC_PORT_UPDATE_WARN] ODC ID ${odcId} tidak ditemukan untuk update port.`);
        return false;
    }
