...
async function getOltData(config) {
    ...
    try {
        const [descRows, serialRows, modelRows, phaseRows, reasonRows, rxRows, ifNameRows] = await Promise.all([
            ...
            snmpWalk(config, OID.ifName, { cap: 4000 }).catch(() => []),
        ]);

        // ifIndex → nama PON (untuk label human-readable).
        const ifNameByIndex = new Map();
        for (const r of ifNameRows) {
            const idx = r.oid.slice(OID.ifName.replace(/^\./, '').length + 1);
            if (r.value) ifNameByIndex.set(idx, r.value);
        }
        ...
        for (const key of keys) {
            ...
            onus.push(normalizeOnu({
                ...
                ponName: ifNameByIndex.get(pon) || null,
