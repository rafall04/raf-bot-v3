...
const OID = {
    ...
    // Kolom .10 = RX optical power ONU. DIKONFIRMASI via matriks DDM (scripts/olt-zte-optical.js):
    //   c10 bervariasi lebar 1020..2841 → −raw/100 = −10.2..−28.4 dBm (rentang GPON tepat).
    //   c14 ~16300 (klaster sempit) = Vcc 3.3V — bukan rx. c1..c9 konstanta/flag.
    onuRxPower: '1.3.6.1.4.1.3902.1012.3.50.12.1.1.10',
};

const ENTERPRISE = '3902';

// ── RX power encoding (DIKONFIRMASI via matriks DDM, scripts/olt-zte-optical.js) ──
// raw 2593→-25.93, 1276→-12.76, 1020→-10.20, 2841→-28.41 dBm — rentang GPON tepat.
...
async function getOltData(config) {
    ...
    try {
        // Strategi transport: KONKUREN TERBATAS (3 sekaligus) + timeout walk DICLAMP ke 8s.
        // 7 walk paralel penuh ke OLT remote sering drop rx-power (walk terberat ~528 entri)
        // → redaman N/A. Sekuensial dgn timeout device 30s terlalu lambat (bisa 2+ menit saat
        // ada packet loss). Konkurensi 3 + timeout pendek + retry = cepat & andal.
        const walkCfg = { ...config, timeout: Math.min(config.timeout || 15000, 8000), retries: 2 };
        const safeWalk = (oid, opts) => snmpWalk(walkCfg, oid, opts).catch(() => []);
        const [
            descRows, serialRows, modelRows, portRows, phaseRows, reasonRows, rxRows,
        ] = await runWithConcurrency([
            () => safeWalk(OID.onuDescr),
            () => safeWalk(OID.onuSerial),
            () => safeWalk(OID.onuModel),
            () => safeWalk(OID.onuPortName),
            () => safeWalk(OID.onuPhaseState),
            () => safeWalk(OID.onuOfflineReason),
            () => safeWalk(OID.onuRxPower),
        ], 3);
