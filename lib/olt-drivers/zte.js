...
// Rentang RX power yang masuk akal secara fisik untuk GPON. ONU yang sudah sync
// (online) tak mungkin di bawah sensitivitas penerima (~-30 dBm; pakai margin -35).
// OLT ZTE mengembalikan nilai sampah untuk ONU yang DDM-nya tak terbaca — terlihat
// sebagai raw besar (mis. 3600 → -36, atau sentinel ~65000 → -653 dBm). Itu BUKAN
// redaman asli → harus N/A. (Verifikasi: dari 488 raw, hanya 339 di rentang valid;
// 149 lainnya -36..-653 dari ONU "online" = bacaan invalid.)
const RX_MIN_DBM = -35;
const RX_MAX_DBM = 0;
...
function parseRxPower(raw) {
    ...
    // Buang bacaan di luar rentang fisik GPON (sampah/sentinel) → N/A.
    if (dbm < RX_MIN_DBM || dbm > RX_MAX_DBM) return 'N/A';
