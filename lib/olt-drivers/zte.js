...
// Rentang RX power fisik GPON. Floor -40 (jangan sembunyikan sinyal lemah nyata spt
// -31.5 dBm); RX_MAX 0 (buang nilai positif/sampah).
const RX_MIN_DBM = -40;
const RX_MAX_DBM = 0;
 ...
 *
 * Encoding ZTE C320 DIVERIFIKASI terhadap CLI `show pon power attenuation` (6 ONU):
 *   dBm = signed16(raw)/500 - 30
 *   - sinyal kuat: raw positif (3406→-23.188, 5126→-19.748).
 *   - sinyal LEMAH (< -30): raw "wrap" ke nilai besar → harus dibaca SIGNED 16-bit
 *     (65052→-484→-30.968, 64762→-774→-31.548, cocok CLI).
 * raw == 65535 = no-signal (CLI "no signal").
 * @param {number|string|null} raw
 */
function parseRxPower(raw) {
    if (raw === null || raw === undefined || raw === '') return 'N/A';
    const n = parseInt(raw, 10);
    if (Number.isNaN(n) || n >= 65535) return 'N/A'; // 65535 = no-signal
    const s16 = n > 32767 ? n - 65536 : n;           // signed 16-bit
    const dbm = s16 / 500 - 30;
