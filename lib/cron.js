 ...
 */
"use strict";
const log = require('./logger').logger.child('CRON');
...
const { initRedamanWatchTask } = require('./cron/jobs/redaman-watch'); // #b353 pantau redaman live
const { initPackageChangeRolloverTask } = require('./cron/jobs/package-change-rollover'); // ganti paket tertunda (opsi B)
...
function initPersonalFinanceBackupTask() {
    ...
    try {
        ...
        log.error('[CRON] Dompet pribadi aktif tapi job backup-nya tak terpasang:', e && e.message);
    }
}
...
function daftarkanTask(nama, fn) {
    try {
        ...
        log.error(`[CRON_INIT_GAGAL] Job "${nama}" tidak terdaftar: ${err && err.message}`);
        return false;
    }
}
...
function initializeAllCronTasks() {
    ...
    const daftar = [
        ...
        // Pantau redaman live saat perbaikan (durabel) — gated config.redamanWatch.enabled (default OFF)
        ["redaman-watch", () => initRedamanWatchTask(config)],
        // Rollover ganti paket tertunda (harga+kecepatan mulai siklus berikutnya) — gated config.packageChangeDeferred.enabled
        ["package-change-rollover", () => initPackageChangeRolloverTask(config)],
    ...
    if (gagal.length) {
        log.error(`[CRON_INIT] ${daftar.length - gagal.length}/${daftar.length} job terdaftar. GAGAL: ${gagal.join(", ")}`);
    } else {
        log.info(`[CRON_INIT] ${daftar.length}/${daftar.length} job terdaftar.`);
    }
...
module.exports = {
    ...
    initRedamanWatchTask,
    initPackageChangeRolloverTask,
    initOltBackupTask,
