 ...
 */
"use strict";
const log = require('./logger').logger.child('HTTP_AUTH_BOOTSTRAP');
...
function registerHttpAuth(app, { runtime, config, authCache, loadJSON }) {
    app.use(async (req, res, next) => {
                ...
                if (decoded.role) {
                    ...
                    if (accounts.length === 0) {
                        try {
                            ...
                            log.error("[AUTH_CRITICAL] Failed to reload accounts.json:", reloadErr);
                        }
                    }
                    ...
                    if (!account) {
                        try {
                            ...
                            log.error("[AUTH_RETRY_ERROR] Failed to reload accounts during retry:", retryErr);
                        }
                    }
                    ...
                    if (account) {
                        ...
                        if (!global.authLogCache.has(cacheKey)) {
                            log.info(`[AUTH] Admin ${account.username} (${account.role}) authenticated. Path: ${req.path}`);
                    ...
                    if (customer) {
                        ...
                        if (!global.authLogCache.has(cacheKey)) {
                            log.info(`[AUTH] Customer ${customer.name} authenticated`);
                            global.authLogCache.add(cacheKey);
                        }
                    } else {
                        log.info(`[AUTH_FAIL] Customer not found for ID ${decoded.id}. Path: ${req.path}`);
                    }
                } else {
                    res.cookie("token", "", { httpOnly: true, maxAge: 0, path: "/" });
                }
            } catch (err) {
                log.info(`[AUTH_ERROR] Invalid token. Error: ${err.message}. Path: ${req.path}`);
        ...
        log.info(`[AUTH_REDIRECT_GUEST] No token and not a public path. Path: ${req.path}. Redirecting to /login.`);
