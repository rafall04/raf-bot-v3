...
const log = require('./logger').logger.child('NOTIFICATION_TRACKER');
...
function isNotificationDuplicate(jid, message) {
    ...
    if (notificationCache.has(key)) {
        ...
        if (DEBUG) {
            log.info(`[NOTIF_TRACKER] Duplicate detected for ${normalizePhone(jid)}`);
            log.info(`[NOTIF_TRACKER] Time since last: ${timeSinceLastSend}ms`);
            log.info(`[NOTIF_TRACKER] Message preview: ${message.text?.substring(0, 50)}...`);
        }
...
function getRecentNotifications(jid) {
    ...
    notificationCache.forEach((entry, _key) => {
...
function clearAll() {
    ...
    if (DEBUG) {
        log.info(`[NOTIF_TRACKER] Cleared ${size} tracked notifications`);
    }
}
...
function deduplicatePhones(phones) {
    ...
    if (DEBUG && phones.length !== result.length) {
        log.info(`[NOTIF_TRACKER] Deduplicated phones: ${phones.length} → ${result.length}`);
    }
