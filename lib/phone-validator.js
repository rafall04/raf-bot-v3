 ...
 */
const log = require('./logger').logger.child('PHONE_VALIDATOR');
    ...
    for (const phone of phones) {
        try {
            ...
            if (existingUser) {
                ...
                existingUser.phone_number.split('|').map(p => p.trim());
            ...
            log.error('[PHONE_VALIDATOR] Error validating phone:', phone, error);
            return {
                valid: false,
...
async function getAllPhoneNumbers(db) {
    return new Promise((resolve, reject) => {
        ...
        db.all('SELECT id, name, phone_number FROM users', [], (err, rows) => {
            if (err) {
                log.error('[PHONE_VALIDATOR] Error getting all phones:', err);
                reject(err);
            } else {
                rows.forEach(row => {
                    if (row.phone_number) {
