 ...
 *   + `routes/public/content` (konten publik read-only: /api/wifi-name, pengumuman, berita —
 *   dipakai halaman /voucher untuk personalisasi & tombol lapor-admin/login-otomatis).
...
function createPublicSiteApp(runtime, express, { projectRoot } = {}) {
    ...
    // Akarnya di `static/uploads/logos` — di situlah routes/invoice.js menyimpannya.
    // Lihat penjelasan panjang di lib/http-security.js (mount kembar untuk app utama).
    ...
    // Konten publik read-only (wifi-name/pengumuman/berita) — tanpa ini halaman /voucher di port
    // ini kehilangan /api/wifi-name: tombol "Laporkan ke Admin", login-otomatis & flag status WA.
