"use strict";
const log = require('./logger').logger.child('TECHNICIAN_COLLECTION_SETTLEMENT');
...
async function resolveTechnicianForSettlement({ teknisiId, teknisiName, userId, periodMonth, periodYear }) {
    ...
    const candidates = requests
        ...
        // #b308: JANGAN saring `newStatus === true` — pengajuan CICILAN menyimpan `false`, jadi syarat
        // itu MENGECUALIKAN penarikan cicilan dan teknisi tak dikredit fee untuk penarikan yang benar-benar
        // ia lakukan (senyap). Jalur utama sudah mencabut syarat ini; fallback ini disamakan agar konsisten.
        // Cukup: approved + punya requested_by_teknisi_id (+ cocok periode di bawah).
        .filter((request) => request.status === 'approved' && request.requested_by_teknisi_id)
        .filter((request) => {
    ...
    try {
        ...
        if (!skipLedgerSync) {
            try {
                ...
                log.error('[TECHNICIAN_COLLECTION_LEDGER_SYNC_ERROR]', ledgerSyncError.message);
            }
        }
...
async function evaluateCollectionSettlement({ user, paid, periodMonth, periodYear, teknisiId = null, teknisiName = null, sourceRequestId = null, sourcePaymentHistoryId = null, createdBy = 'system' }) {
    ...
    const inserted = await insertSettlementEntry({
        ...
        sourceRequestId,
        // #b309 (T8): WARISI source_payment_history_id dari KREDIT yang dibalik bila pemanggil tak
        // mengoper (jalur reversal memang tak punya id pembayaran spesifik). Tanpa ini, baris debit
        // punya source_payment_history_id NULL → LEFT JOIN payment_history memberi collected_amount
        // NULL → summarizeEntries MELEWATI pengurangan → "Total Ditarik" (kas admin↔teknisi) tetap
        // menghitung penarikan yang sudah dibatalkan (berisiko salah-tagih teknisi). Dengan warisan
        // ini, collected_amount debit == kredit → net total_collected untuk penarikan yang dibalik = 0.
        // Idempotensi tetap dijaga getLatestOutstandingCredit (kredit yang sudah dibalik = net 0 = tak
        // ada outstanding), bukan oleh event_key, jadi perubahan komponen key ini aman.
        sourcePaymentHistoryId: sourcePaymentHistoryId || outstandingCredit.source_payment_history_id || null,
        createdBy,
...
async function writeOffCollectionPeriods({ teknisiId, periods = [], keterangan = '', actor = 'system', expectedTotal = null }) {
    ...
    try {
        ...
        log.error('[TECHNICIAN_COLLECTION_CLOSEOUT_SYNC_ERROR]', error.message);
    }
