 ...
 */
"use strict";
const log = require('./logger').logger.child('ADMIN_ALARM');


/**
 * Kirim satu alarm ke semua admin/owner. Never-throw.
 * @param {string} text
 * @param {{label?:string, category?:string, deps?:object}} [opts] category = key notif-router
 *   (mis. 'billing_isolir'): bila diberikan & notifRouting ON+grup → alarm ke grup, jika tidak
 *   FAIL-OPEN ke getAdminJids (perilaku lama). Kirim TETAP sendCritical (alarm harus durable).
...
async function sendAdminAlarm(text, opts = {}) {
    try {
        ...
        const adminFallback = (await getAdminJids()) || [];
        // Routing per-kategori bila diminta; tanpa category = persis perilaku lama (getAdminJids DM).
        let jids = adminFallback;
        if (opts.category) {
            try {
                const { recipientsFor } = require("./notif-router");
                jids = recipientsFor(opts.category, { adminFallback }).recipients || adminFallback;
        ...
        for (const jid of jids) {
            try {
                ...
                log.error(`[ADMIN_ALARM] gagal kirim ke ${jid}:`, e && e.message);
            }
        }
        return { sent, recipients: jids.length };
    } catch (e) {
        log.error("[ADMIN_ALARM_ERROR]", e && e.message);
