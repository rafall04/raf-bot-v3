...
function createCustomerResolver(deps = {}) {
    ...
    return function resolveCustomerByMac(oltMac) {
        ...
        // Fallback GenieACS (device↔MAC↔pppoe/_id) — ANDAL walau modem offline & tanpa
        // caller-id cache. Cocokkan device_id (paling andal, == users.device_id) lalu pppoe.
        try {
            const g = require("./olt-genieacs-resolver").resolveByMacSync(oltMac);
            if (g) {
                if (g.deviceId) {
                    const byDev = users.find((u) => u && u.device_id && String(u.device_id) === String(g.deviceId));
                    if (byDev) return byDev;
                }
                if (g.pppoe) {
                    const byPppoe = users.find((u) => u && u.pppoe_username === g.pppoe);
                    if (byPppoe) return byPppoe;
                }
            }
        } catch (__e) { /* best-effort — jangan throw */ }
        return null;
    };
}
