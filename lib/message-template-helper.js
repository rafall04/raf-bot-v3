 ...
 */
"use strict";
const log = require('./logger').logger.child('MESSAGE_TEMPLATE_HELPER');
...
function saveTemplate(templateId, templateData) {
    try {
        ...
        log.error('[TEMPLATE_HELPER] Error saving template:', error.message);
        return false;
    }
}
...
function getMessage(templateId, data, fallbackMessage = null) {
    ...
    if (!template) {
        log.warn(`[TEMPLATE_HELPER] Template "${templateId}" not found`);
