 ...
 */
// Lazy-require: resolver memuat modul lain; ambil hanya saat dibutuhkan.
...
/**
 * Modem yang ADA DI AREA KITA tapi BELUM DIDAFTARKAN sebagai pelanggan bot (#b288).
 *
 * !! Saringan kepemilikan (#b249) benar untuk masalah yang dipecahkannya: satu ACS dipakai
 * dua bot, jadi modem bot sebelah tak boleh mengalert teknisi kita. Tapi ia menyapu
 * kategori KETIGA yang tak terpikir waktu itu — pelanggan di area KITA SENDIRI yang belum
 * didaftarkan admin (pelanggan gratis/menumpang). Mereka tak punya `device_id` di `users`,
 * jadi redamannya tak pernah diperiksa dan **tak pernah bisa memicu alarm** — sekeruh apa
 * pun sinyalnya. Cron sudah lama MENGHITUNG mereka (`asingRedamanBuruk`) tanpa bisa
 * bertindak.
 *
 * Pembedanya: **sesi PPPoE aktif di MikroTik KITA**. Kalau username PPPoE yang tertanam di
 * modem cocok dengan sesi yang sedang hidup di router kita, modem itu jelas milik kita —
 * terdaftar atau tidak. Modem bot sebelah tak akan pernah cocok karena router kita tak
 * memegang sesinya. TERUKUR di produksi 2026-08-28: dari 3 modem asing berredaman buruk di
 * Dander, 2 milik kita (kastur-rt11 -28,00 · mas-bakir -26,02) dan 1 memang bot sebelah;
 * di Tanjungharjo keempatnya memang bot sebelah — saringan lama benar di sana.
 *
 * Pelanggan sintetis yang dipulangkan sengaja TANPA `phone_number`: alert redaman dikirim
 * ke TEKNISI, bukan ke pelanggan, dan penyusun pesannya sudah punya fallback aman.
 *
 * @param {Array<object>} devices device GenieACS (harus memuat jalur username PPPoE)
 * @param {Set<string>|Array<string>} sesiAktif nama PPPoE yang sedang aktif di router kita
 * @param {Map} petaPelanggan peta device_id→pelanggan terdaftar (untuk dilewati)
 * @param {Array<string>} pppPaths jalur username PPPoE di ACS
 * @returns {Map<string, object>} deviceId → pelanggan sintetis
 */
function bangunPetaAreaSendiri(devices, sesiAktif, petaPelanggan, pppPaths = PPP_PATHS) {
    const peta = new Map();
    if (!Array.isArray(devices)) return peta;
    const aktif = sesiAktif instanceof Set
        ? sesiAktif
        : new Set((Array.isArray(sesiAktif) ? sesiAktif : []).map((n) => String(n || "").trim().toLowerCase()).filter(Boolean));
    if (aktif.size === 0) return peta;   // MikroTik bisu → jangan menebak; perilaku lama.

    for (const d of devices) {
        if (!d || !d._id) continue;
        if (petaPelanggan && petaPelanggan.has(d._id)) continue;   // sudah terdaftar
        let pppoe = null;
        for (const jalur of pppPaths || []) {
            const simpul = ambilJalur(d, jalur);
            const nilai = simpul && typeof simpul === "object" && "_value" in simpul ? simpul._value : simpul;
            const teks = String(nilai == null ? "" : nilai).trim();
            if (teks) { pppoe = teks; break; }
        }
        if (!pppoe || !aktif.has(pppoe.toLowerCase())) continue;
        peta.set(d._id, {
            id: null,
            name: pppoe,
            pppoe_username: pppoe,
            device_id: d._id,
            _belumTerdaftar: true,
        });
    }
    return peta;
}
