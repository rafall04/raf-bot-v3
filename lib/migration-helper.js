...
const log = require('./logger').logger.child('MIGRATION_HELPER');
...
const USER_SCHEMA = {
    ...
    billing_cycle: { type: 'TEXT', default: 'awal_bulan' }, // 'awal_bulan' | 'akhir_bulan' (siklus tagihan akhir bulan)
...
async function prepareUserData(userData) {
    ...
    for (const [fieldName, fieldConfig] of Object.entries(USER_SCHEMA)) {
        const { type: _type, default: defaultValue } = fieldConfig;
...
function transformUserFromDb(dbRow) {
    ...
    try {
        ...
        log.error(`[TRANSFORM_USER] Error transforming user ${dbRow.id}:`, error.message);
...
function parseBulkFromDb(bulk) {
    try {
        ...
        if (typeof bulk === 'string') {
            ...
            // Handle corrupted data like "[object Object]"
            if (trimmed === '[object Object]' || trimmed.startsWith('[object')) {
                log.warn(`[PARSE_BULK] Corrupted bulk data: "${trimmed}", resetting to default`);
                return [];
            }
        ...
        log.warn(`[PARSE_BULK] Failed to parse bulk:`, e.message);
        return [];
    }
}
