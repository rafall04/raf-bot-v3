 ...
 * Deps: `fs`, `path`, `./whatsapp-gateway` (isReady/sendPayload), `./whatsapp-delivery-verdict`
 *   (baca penanda gagal wrapper), `./env-config` (path DB).
 ...
 */
"use strict";
const log = require('./logger').logger.child('WHATSAPP_CRITICAL_DELIVERY');


const fs = require("fs");
const path = require("path");
const gateway = require("./whatsapp-gateway");
const { isDeliverySuccessful, deliveryFailureReason } = require("./whatsapp-delivery-verdict");
...
function loadDeadLetters() {
    try {
        ...
        log.error("[WA_CRITICAL] Gagal baca dead-letter:", err.message);
    }
    return [];
}
...
function saveDeadLetters(entries) {
    try {
        ...
        log.error("[WA_CRITICAL] Gagal tulis dead-letter:", err.message);
    }
}
...
function persistFailed(jid, message, label, errorCode, attempts) {
    ...
    log.error(`[WA_CRITICAL] DEAD-LETTER tersimpan: ${label} → ${maskRecipient(jid)} (${errorCode})`);
}
...
async function sendCritical(recipient, message, options = {}) {
    ...
    for (let attempt = 1; attempt <= cfg.maxAttempts; attempt += 1) {
        try {
            ...
            // penanda. Untuk jalur KRITIS objek itu wajib dibaca sebagai KEGAGALAN (lewat helper
            // verdict bersama); kalau tidak, pesan yang tak pernah keluar dilaporkan `delivered:true`
            // dan tak masuk dead-letter.
            const failReason = deliveryFailureReason(res);
            if (failReason) {
                throw new Error(`WRAPPER_${failReason.toUpperCase()}: ${(res && res.error) || "pesan tidak benar-benar terkirim"}`);
            }
            if (attempt > 1) {
                log.info(`[WA_CRITICAL] ${label} → ${maskRecipient(jid)} terkirim di attempt ${attempt}`);
            }
...
async function retryFailedDeliveries(_options = {}) {
    ...
    for (const entry of entries) {
        ...
        try {
            ...
            // skipDuplicateCheck: retry mengirim teks yang SAMA PERSIS dgn kiriman gagal semula →
            // tanpa ini wrapper mengenalinya sebagai duplikat & memulangkan {status:'blocked_duplicate'}
            // (sukses-palsu). Samakan dgn sendCritical (jalur kritis dijaga retry+dead-letter, bukan dedup).
            const res = await gateway.sendPayload(jid, normalizePayload(entry.message), { skipDuplicateCheck: true });
            // Wrapper TAK melempar pada gagal koneksi/USync — ia memulangkan {status:'error'}. Baca
            // hasilnya: HANYA tandai retried bila benar-benar terkirim. Kalau tidak, entri tetap di
            // dead-letter (unretried) supaya dicoba lagi di reconnect berikutnya — bukan hilang senyap.
            if (!isDeliverySuccessful(res)) {
                log.error(`[WA_CRITICAL] Retry belum terkirim untuk ${maskRecipient(jid)}: ${deliveryFailureReason(res) || "SEND_FAILED"}`);
                continue;
            }
            entry.retried = true;
            ...
            log.error(`[WA_CRITICAL] Retry gagal untuk ${maskRecipient(jid)}:`, err.message);
        }
    }
