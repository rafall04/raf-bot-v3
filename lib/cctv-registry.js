 ...
 */
'use strict';
const log = require('./logger').logger.child('CCTV_REGISTRY');
...
function save(list) {
    ...
    try {
        ...
        log.error('[CCTV] save error:', e.message);
    }
}
 ...
 * @param {object} dev {id?, name, host, phone, customerName?, area?, customMessage?, confirmationMinutes?, enabled?, notifyCustomer?, snoozeUntil?}
 */
function upsert(dev) {
    ...
    const cleaned = {
        ...
        // Maintenance/snooze: epoch ms sampai kapan alert dibisukan (auto-aktif lagi setelah lewat). null = tak snooze.
        snoozeUntil: Number.isFinite(+dev.snoozeUntil) && +dev.snoozeUntil > 0 ? +dev.snoozeUntil : null,
        // .id entri netwatch MikroTik yang dikelola untuk CCTV ini (dicatat auto-sync agar set/hapus
        // menyasar entri PERSIS by-id, bukan menebak by-host — cegah menghajar entri OLT/infra sehost).
        netwatchId: dev.netwatchId != null && String(dev.netwatchId).trim() ? String(dev.netwatchId).trim() : null,
        createdAt: dev.createdAt || new Date().toISOString(),
        updatedAt: new Date().toISOString(),
    };
    if (!cleaned.name || !cleaned.host) {
        throw new Error('CCTV: name, host wajib diisi');
    }
