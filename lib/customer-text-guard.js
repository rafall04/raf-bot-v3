...
//
// DUA dialek slot dipakai di repo ini dan keduanya wajib terjaring:
//   `${jumlah}` — response_templates.json (lib/templating, renderResponseTemplate)
//   `{count}`   — template di config.json yang DIEDIT OPS: `cctvMonitor.message*`
//                 dan `oltLosBroadcast.notifyCustomer.messageTemplate` (renderer sendiri).
// Versi lama hanya mengenali dialek pertama, sehingga `{count} pelanggan terdampak` yang diketik
// admin di halaman Setting lolos tanpa satu pun peringatan — justru di jalur gangguan massal,
// tempat angka itu paling berbahaya karena nyaris sama dengan total pelanggan.
...
// Dua dialek slot (lihat COUNT_SLOT), plus identitas port OLT: `buildCustomerMessage` di
// `lib/olt-los-broadcaster.js` MENAWARKAN `{mac}` / `{slot}` / `{onu}` ke pesan PELANGGAN.
// Template produksi saat ini tidak memakainya, tapi tak ada yang menghalangi admin
// menambahkannya lewat halaman Setting — dan MAC/slot/ONU adalah alamat perangkat di OLT,
// bukan sesuatu yang layak dibaca pelanggan.
const INTERNAL_SLOT = /(?:\$\{|\{)\s*(username_pppoe|pppoe_username|odp|odc|mac|slot|onu)\s*\}/i;
