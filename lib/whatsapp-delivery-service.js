 ...
 */
"use strict";
const log = require('./logger').logger.child('WHATSAPP_DELIVERY_SERVICE');
...
async function sendMessage(recipient, message, options = {}) {
    ...
    try {
        ...
        if (failReason === "error") {
            ...
            log.error("[WA_DELIVERY_ERROR]", { recipient: maskRecipient(jid), error: warning });
            return {
                sent: false,
                successCount: 0,
                recipients: [],
