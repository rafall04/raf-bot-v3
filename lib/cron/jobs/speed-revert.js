 ...
 */
"use strict";
const log = require('../../logger').logger.child('SPEED_REVERT');
...
function initSpeedRevertTask(config) {
    ...
    if (config.status_speed_boost_revert === true) {
        log.info(`[CRON_SPEED_REVERT] Starting/Restarting speed boost revert task with schedule: ${schedule}`);
        cronTaskSpeedRequestRevert = cron.schedule(schedule, async () => {
            ...
            if (speedRevertRunning) {
                log.warn("[CRON_SPEED_REVERT_SKIPPED] Previous speed-revert cycle still running, skipping this tick.");
                return;
            }
            ...
            try {
                ...
                log.error("[CRON_SPEED_REVERT_ERROR] Error reading cron.json:", e);
                return;
            }
            ...
            for (const req of global.speed_requests) {
                ...
                if (!req.expirationDate) {
                    log.warn(`[CRON_SPEED_REVERT_WARN] Request ${req.id || 'unknown'} has no expirationDate, skipping`);
                    continue;
                }

                const expirationDate = new Date(req.expirationDate);
                if (isNaN(expirationDate.getTime())) {
                    log.warn(`[CRON_SPEED_REVERT_WARN] Request ${req.id || 'unknown'} has invalid expirationDate: ${req.expirationDate}, skipping`);
                    continue;
                }
                ...
                if (expirationDate <= now) {
                    ...
                    log.info(`[CRON_SPEED_REVERT] Reverting expired request ${requestId} (user: ${userId})...`);

                    if (!req.userId) {
                        log.error(`[CRON_SPEED_REVERT_ERROR] Request ${requestId} has no userId, skipping`);
                    ...
                    if (!userToRevert) {
                        log.error(`[CRON_SPEED_REVERT_ERROR] User with ID ${req.userId} not found for request ID ${requestId}.`);
                    ...
                    if (!req.currentPackageName) {
                        log.error(`[CRON_SPEED_REVERT_ERROR] Request ${requestId} has no currentPackageName, skipping`);
                        req.status = 'error_revert_invalid_data';
                        requestsModified = true;
                        continue;
                    }
                    ...
                    if (!originalPackage || !originalPackage.profile) {
                        log.error(`[CRON_SPEED_REVERT_ERROR] Original package profile not found for package name: ${req.currentPackageName}.`);
                    ...
                    if (!req.pppoeUsername) {
                        log.error(`[CRON_SPEED_REVERT_ERROR] Request ${requestId} has no pppoeUsername, skipping`);
                        req.status = 'error_revert_invalid_data';
                        requestsModified = true;
                        continue;
                    }
                    ...
                    try {
                        ...
                        try {
                            ...
                            log.warn(`[CRON_SPEED_REVERT_WARN] Could not disconnect active session for ${req.pppoeUsername}: ${disconnectError.message}. This is often not critical.`);
                        }
                                ...
                                for (let number of phoneNumbers) {
                                    ...
                                    if (normalizedNumber.length > 5) {
                                        ...
                                        if (result.success) {
                                            ...
                                            log.error(`[CRON_SPEED_REVERT_NOTIF_ERROR] Failed to send 'reverted' message to ${jid}:`, result.error);
                        ...
                        log.error(`[CRON_SPEED_REVERT_ERROR] Failed to revert Mikrotik profile for ${req.pppoeUsername}:`, mikrotikError.message);
                        requestsModified = true;
                        ...
                        if (isTransientMikrotikError(mikrotikError)) {
                            ...
                            if (req.revertRetryCount >= MAX_REVERT_RETRIES) {
                                ...
                                log.error(`[CRON_SPEED_REVERT_ERROR] ${req.pppoeUsername}: gagal revert ${req.revertRetryCount}x (transien) — finalize error + eskalasi admin.`);
                                await alertRevertStuck('SOD', req.pppoeUsername, req.revertRetryCount);
                            } else {
                                log.warn(`[CRON_SPEED_REVERT_RETRY] ${req.pppoeUsername}: gagal transien (${mikrotikError.code || mikrotikError.message}), tetap 'active' utk retry (${req.revertRetryCount}/${MAX_REVERT_RETRIES}).`);
                            }
                        } else {
            ...
            if (requestsModified) {
                try {
                    ...
                    log.error(`[CRON_SPEED_REVERT_ERROR] Failed to save speed requests:`, saveError.message);
                }
            }

            if (expiredCount > 0) {
                log.info(`[CRON_SPEED_REVERT] Reverted ${expiredCount} expired request(s)`);
            } else if (activeRequests.length > 0) {
                log.info(`[CRON_SPEED_REVERT] ${activeRequests.length} active request(s) monitored`);
            }
            } finally {
        ...
        log.info("[CRON_SPEED_REVERT] Task started successfully!");
    } else {
