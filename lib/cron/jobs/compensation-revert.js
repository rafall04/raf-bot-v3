    ...
    if (config.status_compensation_revert === true && isValidCron(config.schedule_compensation_revert)) {
        ...
        cronTaskCompensationRevert = cron.schedule(config.schedule_compensation_revert, async () => {
                    ...
                    try {
                        ...
                        if (isTransientMikrotikError(mikrotikError)) {
                            ...
                            if (comp.revertRetryCount >= MAX_REVERT_RETRIES) {
                                ...
                                console.error(`[CRON_COMPENSATION_REVERT_ERROR] ${comp.pppoeUsername}: gagal revert ${comp.revertRetryCount}x (transien) — finalize error + eskalasi admin.`);
                                await alertRevertStuck(comp.pppoeUsername, comp.revertRetryCount);
                            } else {
                                console.warn(`[CRON_COMPENSATION_REVERT_RETRY] ${comp.pppoeUsername}: gagal transien (${mikrotikError.code || mikrotikError.message}), tetap 'active' utk retry (${comp.revertRetryCount}/${MAX_REVERT_RETRIES}).`);
                            }
                        } else {
                            comp.status = 'error_revert';
                        }
            ...
            if (expiredCount > 0) {
                ...
                console.error('[CRON_COMPENSATION_TICK_ERROR] Tick dibatalkan oleh error tak terduga:', tickError);
                try {
                    if (compensationsModifiedInThisRun) {
                        saveCompensations();
                    }
                } catch (saveErr) {
                    console.error('[CRON_COMPENSATION_TICK_ERROR] Gagal persist progres parsial:', saveErr);
                }
            } finally {
