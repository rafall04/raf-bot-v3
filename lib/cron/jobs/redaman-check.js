...
const { pilihNilaiRedaman } = require('../../redaman-sumber-silang');
const {
    ALASAN,
    bacaSetelanLingkup,
        ...
        checkTask = cron.schedule(schedule, async () => {
            try {
                ...
                for (const device of devicesWithRedaman) {
                    ...
                    const acsDipakai = putusan.alasan === ALASAN.ALERT || putusan.alasan === ALASAN.NILAI_BASI
                        ? putusan.angka
                        : null;
                    const silang = pilihNilaiRedaman({
                        acs: putusan.alasan === ALASAN.NILAI_BASI ? null : acsDipakai,
                        olt: nilaiOlt,
                    });

                    if (silang.integritasMencurigakan) {
                        // Redaman pelanggan ini TIDAK diketahui — jangan mengalert dari angka
                        // mana pun. Dilaporkan lewat ringkasan siklus supaya bisa dibetulkan.
                        devicesIntegritas++;
                        console.warn('[CRON_REDAMAN_INTEGRITAS] ' + (device._id || '?')
                            + ' ACS ' + silang.acs + ' vs OLT ' + silang.olt
                            + ' (beda ' + silang.beda.toFixed(2) + ' dB) — pemetaan pelanggan/ONU patut dicek.');
                        devicesChecked++;
                        continue;
                    }

                    if (putusan.alasan === ALASAN.TANPA_NILAI && silang.angka === null) { devicesSkipped++; continue; }

                    devicesChecked++;

                    if (putusan.alasan === ALASAN.NILAI_BASI && silang.angka === null) {
                        // Modem tak menjawab refresh DAN OLT tak punya angkanya — benar-benar buta.
                        devicesNilaiBasi++;
                        devicesWithBadRedaman++;
                        continue;
                    }

                    const angkaFinal = silang.angka;
                    const layakAlert = angkaFinal !== null && angkaFinal <= rxTolerance;
                    if (putusan.alasan === ALASAN.NILAI_BASI || putusan.alasan === ALASAN.TANPA_NILAI) {
                        if (!layakAlert) { devicesSkipped++; continue; }
                        devicesDitambalOlt++;   // alarm ini hanya ada berkat OLT
                    }

                    if (layakAlert) {
                        const redamanInt = angkaFinal;
                ...
                if (devicesDitambalOlt > 0 || devicesIntegritas > 0 || petaOlt) {
                    console.log('[CRON] Redaman sumber-ganda: ' + devicesDitambalOlt
                        ...
                        + ' ditahan karena ACS vs OLT beda jauh (pemetaan patut dicek)'
