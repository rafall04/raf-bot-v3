/**
 * Header Doc
 * Purpose: Cron job redaman-check — periodically poll GenieACS untuk semua device, fetch metrik redaman (RX power), filter device dengan signal lebih buruk dari `rx_tolerance`, kemudian kirim notifikasi WA ke penerima yang DIATUR pemilik (peran + nomor tambahan). State holder `checkTask` ter-encapsulasi di module ini. Function di-export sebagai `startCheck` (legacy name) untuk backward compat dengan `routes/admin-config-routes.js` dan composer.
 ...
 * SideEffects: Jadwalkan job background, panggil GenieACS API (refresh + query devices), kirim WhatsApp ke PENERIMA yang diatur di halaman Konfigurasi (per peran + nomor tambahan) — lihat lib/redaman-alert-recipients.js.
 */
"use strict";
...
let checkTask = null;
// Tracker cooldown per-device agar alert redaman tidak spam tiap jam untuk device buruk yang sama.
// Map<deviceId, lastAlertedAtEpochMs>
const lastAlertSentByDeviceId = new Map();
    ...
    try {
        ...
        checkTask = cron.schedule(schedule, async () => {
            try {
                ...
                // Cooldown agar device buruk yang sama tidak di-alert tiap cycle.
                // Default 12 jam; bisa di-override via config `redaman_alert_cooldown_hours`.
                const cooldownHours = parseFloat(global.config.redaman_alert_cooldown_hours);
                const cooldownMs = Number.isFinite(cooldownHours) && cooldownHours >= 0
                    ? cooldownHours * 3600000
                    : 12 * 3600000;
                ...
                for (const device of devicesWithRedaman) {
                    ...
                    if (redamanInt < rxTolerance) {
                        ...
                        const lastAlertedAt = lastAlertSentByDeviceId.get(device._id) || 0;
                        if (cooldownMs > 0 && nowMs - lastAlertedAt < cooldownMs) {
                        ...
                        if (alertedAtLeastOnce) {
                            lastAlertSentByDeviceId.set(device._id, Date.now());
                        }
                ...
                if (devicesWithBadRedaman > 0) {
                    const cooldownNote = devicesAlertedSkippedByCooldown > 0
                        ? ` (${devicesAlertedSkippedByCooldown} skipped by cooldown)`
                        : '';
                    console.log(`[CRON] Redaman: ${devicesWithBadRedaman}/${devicesChecked} device buruk${cooldownNote}`);
                }
