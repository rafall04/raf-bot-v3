 ...
 * !! KEPEMILIKAN PUNYA DUA SUMBER (#b288): pelanggan terdaftar (device_id) ATAU modem di
 *    AREA KITA yang belum didaftarkan, dikenali dari sesi PPPoE aktif di MikroTik kita.
 ...
 */
"use strict";
const log = require('../../logger').logger.child('REDAMAN_CHECK');
...
const { ringkasDuaSumber } = require('../../redaman-sumber-silang');
const {
    ALASAN,
    bacaSetelanLingkup,
    bangunPetaDevicePelanggan,
    bangunPetaAreaSendiri,
    bacaNilaiRedaman,
    evaluasiDevice,
} = require('../../redaman-alert-scope');
const cooldownStore = require('../../redaman-alert-cooldown-store');
const { PPP_PATHS } = require('../../olt-genieacs-resolver');
...
async function alarmkanKegagalanBeruntun(pesanGalat) {
    try {
        ...
        if (!penerima.aktif || !daftar.length) {
            log.warn('[CRON_REDAMAN_ALARM] gagal beruntun tapi tak ada penerima alert redaman yang aktif.');
            return;
        }
        const teks = [
        ...
        if (terkirim === 0) {
            log.warn('[CRON_REDAMAN_ALARM] alarm GAGAL terkirim ke semua penerima — cooldown TIDAK dipasang, akan dicoba lagi run berikutnya.');
            return;
        }
        alarmTerakhirMs = Date.now();
        log.warn(`[CRON_REDAMAN_ALARM] alarm terkirim ke ${terkirim}/${daftar.length} penerima (gagal beruntun ${gagalBeruntun}).`);
    } catch (e) {
        // Alarm TIDAK BOLEH menjatuhkan cron — kegagalan kirim cukup dicatat.
        log.error('[CRON_REDAMAN_ALARM_ERROR]', e.message);
    }
}
...
function startCheck() {
    ...
    try {
        ...
        if (!isEnabled || schedule.startsWith('#')) {
            log.info(`[CRON_REDAMAN] Redaman check task is DISABLED (status: ${isEnabled}, schedule: ${schedule})`);
            return;
        }

        // Validate the schedule
        if (!cron.validate(schedule)) {
            log.error(`[CRON_REDAMAN_ERROR] Invalid cron expression: "${schedule}". Job not started.`);
            return;
        }
        ...
        checkTask = cron.schedule(schedule, async () => {
            try {
                ...
                const allDevicesResult = await queryDevices({
                    // Username PPPoE ikut diminta di panggilan yang SAMA — dipakai mengenali
                    // modem di AREA KITA yang belum didaftarkan (#b288).
                    projection: ['_id', '_lastInform', ...REDAMAN_PATHS, ...PPP_PATHS].join(','),
                    timeoutMs: 30000,
                ...
                // Modem di AREA KITA yang belum didaftarkan tetap milik kita (#b288).
                // Pembedanya sesi PPPoE aktif di MikroTik KITA — modem bot sebelah tak akan
                // pernah cocok. NON-FATAL: MikroTik bisu ⇒ `sesiAktif` kosong ⇒ perilaku
                // lama persis, alarm yang sudah bekerja tidak ikut rusak.
                // #b347: sesi PPPoE aktif (objek PENUH dgn caller_id/MAC) diambil SEKALI utk seluruh
                // tick — dipakai (a) perluasan area-sendiri DAN (b) resolveOlt→resolveByCustomer sebagai
                // `pppoeActive`. MAC live = sumber utama pemetaan ONU EPON/Hioso (jalur saudara #b340
                // yg terlewat: 3 pemanggil Telegram sudah oper pppoeActive, cron alarm ini belum).
                // JANGAN buang caller_id dengan .map(s=>s.name). Diambil di luar gerbang hanyaPelangganSendiri.
                let sesiAktifPenuh = [];
                try {
                    const rSesi = await getActivePPPoEUsers({ caller: 'cron.redaman' });
                    if (rSesi && rSesi.ok && Array.isArray(rSesi.data)) sesiAktifPenuh = rSesi.data;
                } catch (e) {
                    log.warn('[CRON_REDAMAN] Gagal baca sesi PPPoE aktif (lanjut tanpa MAC live):', e.message);
                }
                let ditambahAreaSendiri = 0;
                if (setelanLingkup.hanyaPelangganSendiri) {
                    const sesiAktif = sesiAktifPenuh.map((s) => s && s.name).filter(Boolean);
                    const petaArea = bangunPetaAreaSendiri(allDevices, sesiAktif, petaPelanggan, PPP_PATHS);
                    for (const [id, pel] of petaArea) { petaPelanggan.set(id, pel); ditambahAreaSendiri++; }
                    if (ditambahAreaSendiri > 0) {
                        log.info(`[CRON_REDAMAN_AREA] ${ditambahAreaSendiri} modem di area kita BELUM DIDAFTARKAN ikut dipantau `
                            + `(dikenali dari sesi PPPoE aktif di router kita).`);
                    }
                }

                // Penyaringan dijalankan SETELAH perluasan di atas — kalau dibalik, modem
                // area sendiri yang baru dikenali akan tersaring keluar sebelum sempat masuk.
                if (setelanLingkup.hanyaPelangganSendiri) {
                ...
                if (dilewatiDiam > 0) {
                    ...
                    log.info(`[CRON_REDAMAN_LEWAT_DIAM] ${dilewatiDiam} modem dilewati — tak menyapa ACS > ${DEFAULT_MAX_INFORM_MINUTES} menit (refresh-nya hanya akan mengendap di antrean).`);
                }

                if (deviceIDs.length === 0) {
                    // JANGAN BISU: tak ada device milik sendiri terlihat sama persis dengan
                    // "semua modem sehat" kalau tidak dicetak.
                    log.warn(`[CRON_REDAMAN_WARN] Tak ada modem pelanggan bot ini di ACS (${totalDiAcs} device asing dilewati) — tidak ada yang diperiksa.`);
                    return;
                }
                ...
                if (isNaN(rxTolerance)) {
                    log.error("[CRON_REDAMAN_ERROR] `rx_tolerance` in config.json is not a valid number. Skipping checks.");
                    return;
                }
                ...
                // SUMBER KEDUA: redaman dari OLT (dibaca lewat web, tanpa SNMP). Satu tarikan
                // untuk seluruh siklus. Gunanya BUKAN verifikasi silang — terukur keduanya
                // mengukur besaran yang sama (r=0,991) — melainkan CAKUPAN: saat modem tak
                // inform, ACS diam dan alarm ini dulu buta sebelah.
                let petaOlt = null;
                try {
                    const snap = await require('../../olt-optical-resolver').getOltSnapshot({ forceRefresh: true });
                    if (snap && snap.status === 'success' && Array.isArray(snap.onus)) petaOlt = snap;
                } catch (_e) {
                    petaOlt = null;   // never-throw: tanpa OLT, alarm jalan seperti sebelumnya
                }
                const resolveOlt = (user) => {
                    if (!petaOlt || !user) return null;
                    try {
                        const r = require('../../olt-optical-resolver').resolveByCustomer(user, { oltSnapshot: petaOlt, pppoeActive: sesiAktifPenuh });
                        if (!r || !r.matched) return null;
                        // Hanya ONU yang benar-benar Online — OLT tetap memamerkan angka
                        // terakhir milik ONU yang sudah mati.
                        if (String(r.status) !== 'Online') return null;
                        const v = Number.parseFloat(r.rxPower);
                        return Number.isFinite(v) ? v : null;
                    } catch (_e) {
                        return null;
                    }
                };
                let devicesIntegritas = 0;
                let devicesDitambalOlt = 0;
                ...
                for (const device of devicesWithRedaman) {
                    ...
                    // ---- SILANG DUA SUMBER (#b277) --------------------------------------
                    // ACS diam atau basi tidak lagi berarti buta: OLT punya angkanya sendiri.
                    // Sebaliknya, beda besar antar-sumber berarti kita membandingkan DUA
                    // PERANGKAT BERBEDA — pemetaan pelanggan↔ONU yang salah, bukan optik.
                    const nilaiOlt = resolveOlt(putusan.pelanggan);
                    // ACS dipakai apa adanya, KECUALI sudah basi (modem tak menjawab refresh) —
                    // bacaan lama tak layak jadi dasar alarm.
                    const acsDipakai = putusan.alasan === ALASAN.NILAI_BASI ? null : putusan.angka;
                    const dua = ringkasDuaSumber({ acs: acsDipakai, olt: nilaiOlt, ambangAlert: rxTolerance });

                    if (!dua.adaData) {
                        // Tak ada sumber sama sekali → benar-benar buta. Bukan berarti buruk.
                    ...
                    if (dua.bedaBesar) {
                        // Dicatat DAN ditempelkan ke pesan — TIDAK menahan alarm. Menahan berarti
                        // berpotensi menyembunyikan gangguan nyata; teknisi melihat dua angka
                        // sekaligus dan bisa menilai sendiri.
                        devicesIntegritas++;
                        log.warn('[CRON_REDAMAN_BEDA_JAUH] ' + (device._id || '?')
                            + ' ACS ' + dua.acs + ' vs OLT ' + dua.olt
                            + ' (beda ' + dua.beda.toFixed(2) + ' dB) — pemetaan pelanggan/ONU patut dicek.');
                    }

                    if (dua.layakAlert && acsDipakai === null) devicesDitambalOlt++;

                    if (dua.layakAlert) {
                        const redamanTeks = dua.teks;
                        ...
                        const templateData = {
                            // Modem area sendiri yang belum didaftarkan tak punya nama/alamat/HP
                            // di `users` (#b288). Tanpa penanda ini teknisi cuma melihat nama
                            // PPPoE dengan alamat & nomor kosong, dan mengira datanya rusak.
                            //
                            // Penandanya dititipkan di slot yang SUDAH ADA — menambah slot baru
                            // berarti template produksi yang belum diperbarui mengirim `${slot}`
                            // MENTAH ke teknisi.
                            nama_pelanggan: findUser?._belumTerdaftar
                                ? `${findUser.pppoe_username} (BELUM DIDAFTARKAN)`
                                : (findUser?.name?.split("|")[0] || "(Tidak Terdaftar)"),
                            ...
                            // Dua sumber APA ADANYA — tidak disinkronkan, tidak ada yang dibuang.
                            redaman: redamanTeks
                        };
                        ...
                        if (!templatesCache.notificationTemplates?.redaman_alert?.template) {
                            log.error(`[CRON_REDAMAN_ERROR] Template 'redaman_alert' tidak ditemukan. Skip device ${device._id}.`);
                        ...
                        if (!notificationText || notificationText.startsWith('Error:')) {
                            log.error(`[CRON_REDAMAN_ERROR] Gagal render template untuk device ${device._id}. Skip notifikasi.`);
                            continue; // Skip to the next device in the loop
                        }
                        ...
                        const penerima = resolvePenerimaRedaman();
                        if (!penerima.aktif) {
                            log.info('[CRON_REDAMAN_NOTIF] Alert redaman dimatikan dari konfigurasi — tak ada yang dikirimi.');
                            ...
                            log.warn('[CRON_REDAMAN_NOTIF_WARN] Setelan penerima menghasilkan NOL penerima — alert tidak terkirim ke siapa pun.');
                        }
                        ...
                        for (const target of penerima.rincian) {
                            ...
                            if (result.success) {
                                ...
                                log.error(`[CRON_REDAMAN_NOTIF_ERROR] Gagal kirim ke ${target.label} (${target.jid}):`, result.error);
                                // Jika error adalah connection error, stop sending ke account lain
                                if (result.shouldStop) {
                                    log.warn(`[CRON_REDAMAN_NOTIF_WARN] Connection error detected, stopping notifications for this cycle.`);
                                    break;
                                }
                            }
                        }
                ...
                // Dua sumber (#b277) — dicetak walau nol, supaya efeknya terlihat dan tidak
                // jadi fitur yang diam-diam tak pernah jalan.
                if (devicesDitambalOlt > 0 || devicesIntegritas > 0 || petaOlt) {
                    log.info('[CRON] Redaman sumber-ganda: ' + devicesDitambalOlt
                        + ' alarm ditambal OLT (ACS diam/basi), ' + devicesIntegritas
                        + ' ditandai beda jauh ACS vs OLT (pemetaan patut dicek, alarm TETAP dikirim)'
                        + (petaOlt ? '' : ' — OLT tak terbaca siklus ini'));
                }
                ...
                if (devicesWithBadRedaman > 0) {
                    ...
                    const basiNote = devicesNilaiBasi > 0
                        ...
                        : '';
                    log.info(`[CRON] Redaman: ${devicesWithBadRedaman}/${devicesChecked} device buruk${cooldownNote}${basiNote}`);
                }
                ...
                log.error(`[CRON_REDAMAN_ERROR] Error: ${errorMessage}`);
                await alarmkanKegagalanBeruntun(errorMessage);
            }
        });
        // Redaman check scheduled (silent)
    } catch (e) {
        log.error("[CRON_REDAMAN_SETUP_ERROR] Error setting up redaman check cron job:", e);
    }
}
