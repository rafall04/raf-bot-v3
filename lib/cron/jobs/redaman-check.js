        ...
        checkTask = cron.schedule(schedule, async () => {
            try {
                ...
                if (setelanLingkup.hanyaPelangganSendiri) {
                    let sesiAktif = [];
                    try {
                        const r = await getActivePPPoEUsers({ caller: 'cron.redaman.areaSendiri' });
                        if (r && r.ok && Array.isArray(r.data)) sesiAktif = r.data.map((s) => s && s.name).filter(Boolean);
                    } catch (e) {
                        console.warn('[CRON_REDAMAN_AREA] Gagal baca sesi PPPoE, lanjut tanpa perluasan:', e.message);
                    }
                ...
                const resolveOlt = (user) => {
                    ...
                    try {
                        const r = require('../../olt-optical-resolver').resolveByCustomer(user, { oltSnapshot: petaOlt });
