 ...
 */
"use strict";
const log = require('../../logger').logger.child('SET_UNPAID');
...
function initSetUnpaidTask(config) {
    ...
    if (config.status_unpaid_schedule === true && isValidCron(config.unpaid_schedule)) {
        log.info("[CRON_SET_UNPAID] Starting/Restarting set-unpaid task with schedule:", config.unpaid_schedule);
        cronTaskSetUnpaid = cron.schedule(config.unpaid_schedule, async () => {
            ...
            if (setUnpaidRunning) {
                log.warn("[CRON_SET_UNPAID_SKIPPED] Previous set-unpaid cycle still running, skipping this tick.");
                return;
            }
            ...
            try {
                ...
                for (const user of users) {
                    ...
                    try {
                        ...
                        log.error(`[CRON_UNPAID_RESET_ERROR] Failed to sync paid status for user ${user.id}: ${err.message}`);
                        failedCount++;
                    }
                }
                log.info(`[CRON_SET_UNPAID] Summary: ${syncedCount} synced, ${failedCount} failed.`);
                // Reset flag paid gagal = periode billing baru tak konsisten utk sebagian user. Dulu
                // hanya di-console; kini eskalasi TERJAMIN ke admin agar tak senyap. Never-throw.
                if (failedCount > 0) {
                    try {
                        await require('../../admin-alarm').sendAdminAlarm(
                            `⚠️ *Set-unpaid: ${failedCount} GAGAL* dari ${syncedCount + failedCount}. Sebagian status paid periode baru mungkin tak konsisten — cek log [CRON_UNPAID_RESET_ERROR].`,
                            { label: 'cron-set-unpaid-gagal', category: 'billing_isolir' }
                        );
