 ...
 */
"use strict";
const log = require('../../logger').logger.child('PAID_RECONCILE');
...
function initPaidReconcileTask(config = {}) {
    ...
    if (config && config.paid_reconcile_enabled === false) {
        log.info('[CRON_PAID_RECONCILE] Dinonaktifkan via config (paid_reconcile_enabled=false).');
        return;
    }
    ...
    log.info('[CRON_PAID_RECONCILE] Starting/Restarting paid-reconcile task with schedule:', schedule);
    cronTaskPaidReconcile = cron.schedule(schedule, async () => {
        // Overlap guard — cegah dua siklus jalan bersamaan.
        if (paidReconcileRunning) {
            log.warn('[CRON_PAID_RECONCILE_SKIPPED] Previous reconcile cycle still running, skipping this tick.');
            return;
        }
        if (!global.db) {
            log.warn('[CRON_PAID_RECONCILE_SKIPPED] global.db belum siap.');
            return;
        }
        ...
        try {
            ...
            log.error('[CRON_PAID_RECONCILE_ERROR]', error.message);
        } finally {
