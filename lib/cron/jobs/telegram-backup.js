...
function initTelegramBackupTask(config) {
    ...
    if (!isEnabled || schedule.startsWith('#')) {
        log.info(`[CRON_TELEGRAM_BACKUP] Telegram backup task is DISABLED`);
        return;
    }

    // Validate the schedule
    if (!isValidCron(schedule)) {
        log.error(`[CRON_TELEGRAM_BACKUP_ERROR] Invalid cron expression: "${schedule}". Job not started.`);
        return;
    }
    ...
    if (!telegramConfig.enabled || !telegramConfig.botToken || !telegramConfig.chatId) {
        log.info(`[CRON_TELEGRAM_BACKUP] Telegram backup tidak aktif atau belum dikonfigurasi`);
        return;
    }

    log.info(`[CRON_TELEGRAM_BACKUP] Starting backup task with schedule: ${schedule}`);

    cronTaskTelegramBackup = cron.schedule(schedule, async () => {
        log.info(`[CRON_TELEGRAM_BACKUP] Executing scheduled backup at ${new Date().toLocaleString('id-ID', { timeZone: 'Asia/Jakarta' })}`);
        ...
        try {
            ...
            if (result.success) {
                log.info(`[CRON_TELEGRAM_BACKUP] ✅ Backup completed successfully`);
            } else {
                log.error(`[CRON_TELEGRAM_BACKUP] ⚠️ Backup completed with issues: ${result.message}`);
                await alarmBackup(`Selesai dengan masalah: ${result.message || 'tidak diketahui'}`);
            }
        } catch (error) {
            log.error(`[CRON_TELEGRAM_BACKUP] ❌ Backup failed:`, error.message);
    ...
    log.info(`[CRON_TELEGRAM_BACKUP] ✅ Task started successfully!`);
}
