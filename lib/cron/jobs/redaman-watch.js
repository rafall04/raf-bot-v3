 ...
 */
"use strict";
const log = require('../../logger').logger.child('REDAMAN_WATCH');
...
async function logToTicket(ticketId, info) {
    try {
        ...
        log.warn(`[CRON_REDAMAN_WATCH] Gagal log tiket ${ticketId}: ${e.message}`);
    }
}
...
function initRedamanWatchTask(_config) {
    ...
    cronTaskRedamanWatch = cron.schedule("* * * * *", async () => {
        ...
        if (redamanWatchRunning) {
            log.warn("[CRON_REDAMAN_WATCH_SKIPPED] tick sebelumnya masih jalan — dilewati.");
            return;
        }
        ...
        try {
            ...
            if (res && (res.notified || res.finalized)) {
                log.info(`[CRON_REDAMAN_WATCH] proses=${res.processed} notif=${res.notified} selesai=${res.finalized}`);
            }
        } catch (error) {
            log.error(`[CRON_REDAMAN_WATCH_ERROR] ${error.message}`);
        } finally {
