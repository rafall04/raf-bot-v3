 ...
 */
"use strict";
const log = require('../../logger').logger.child('BILLING_AKHIR_BULAN');
...
// #b347: dedup profil LIVE (samakan dgn isolir-paket.js) — anti re-isolir+reboot berulang.
...
function collectReminderItems(templateKey, ctx) {
    ...
    for (const user of global.users || []) {
        ...
        if (tagihan.reason === "katalog-tak-terbaca") {
            log.warn(`[${ctx.tag}] Harga efektif tak terbaca untuk ${user.name} — pakai harga paket.`);
        }
        ...
        try {
            ...
            log.error(`[${ctx.tag}] Gagal buat link bayar untuk ${user.name}: ${linkErr.message}`);
        }
...
async function sendItems(items, tag) {
    ...
    if (items.length > 0 && !isReady()) {
        log.warn(`[${tag}] WhatsApp belum siap (state: ${getConnectionState()}) — antrian ${items.length} pesan akan menunggu & di-retry.`);
    }
...
async function runReminderPhase(templateKey, ctx) {
    ...
    for (const s of skipped) log.info(`[${ctx.tag}_SKIP] ${s.name}: ${s.reason}`);
    log.info(`[${ctx.tag}] === SUMMARY === kohort_unpaid=${unpaid} whitelisted=${whitelisted} target=${items.length} sent=${res.sent} gagal=${res.failed.length} skipped=${skipped.length}`);
...
async function runIsolirPhase(ctx) {
    ...
    if (cfg.sync_to_mikrotik === false) {
        log.info(`[${TAG}] Sync ke MikroTik DINONAKTIFKAN — lewati isolir.`);
        return { isolated: 0, failed: 0, notified: 0 };
    }
    if (cfg.isolirFeatureEnabled === false) {
        log.info(`[${TAG}] Fitur isolir dinonaktifkan — lewati.`);
        return { isolated: 0, failed: 0, notified: 0 };
    }
    const isolirProfile = cfg.isolir_profile;
    if (!isolirProfile) {
        log.error(`[${TAG}] isolir_profile tak terdefinisi di config — batal.`);
        return { isolated: 0, failed: 0, notified: 0 };
    }
    ...
    if (candidates.length === 0) {
        log.info(`[${TAG}] Tak ada kandidat isolir kohort akhir-bulan.`);
        return { isolated: 0, failed: 0, notified: 0 };
    }

    const throttleMs = (cfg && parseInt(cfg.isolir_action_delay, 10)) || 500;
    log.info(`[${TAG}] ${candidates.length} kandidat. Proses sequential throttle ${throttleMs}ms.`);
    ...
    for (const user of candidates) {
        // #b347: DEDUP PROFIL LIVE — kandidat di atas hanya menyaring profil PAKET statis
        // (getProfileBySubscription = '10M') yang TAK PERNAH == isolir_profile, jadi pelanggan yang
        // SUDAH terisolir tetap lolos → executeProfileAction (disconnect+REBOOT) tiap hari selama
        // jendela isolirDaysBefore (mis. 3x reboot berturut). Baca profil live seperti isolir-paket.js:
        // sudah terisolir → skip; tak terbaca → FAIL-SAFE skip (jangan isolir buta).
        let liveProfile = null;
        try {
            const res = await getPPPoEUserProfile(user.pppoe_username, { caller: "cron.billing-akhir-bulan" });
            const data = assertMikrotikResult(res);
            liveProfile = data && data.data ? data.data.profile : null;
        } catch (error) {
            fetchFail++;
            log.error(`[${TAG}] Gagal baca profil live ${user.name} (${user.pppoe_username}): ${error.message} — dilewati (fail-safe).`);
            continue;
        }
        if (liveProfile === isolirProfile) { alreadyIsolir++; continue; }
        ...
        try {
            ...
            log.info(`[${TAG}_SUCCESS] ${user.pppoe_username} berhasil diisolir.`);
            isolated++;
            isolatedUsers.push(user);
        } catch (e) {
            log.error(`[${TAG}_ERROR] Gagal isolir ${user.name}: ${e.message || e}`);
            failed++;
        }
    ...
    for (const user of isolatedUsers) {
        ...
        try {
            ...
            log.error(`[${TAG}] Gagal buat link bayar untuk ${user.name}: ${linkErr.message}`);
        }
    ...
    log.info(`[${TAG}] === SUMMARY === kandidat=${candidates.length} isolir=${isolated} sudah-terisolir=${alreadyIsolir} gagal-baca=${fetchFail} gagal=${failed} notif=${notifRes.sent}`);
    return { isolated, failed, notified: notifRes.sent, alreadyIsolir, fetchFail };
}
...
async function runBillingAkhirBulanCycle(nowOverride = null) {
    ...
    // #b320: ISOLIR pakai `<=` (self-healing). Bila tick hari-aksi terlewat (restart tepat di menit
    // terjadwal — prod restart 7-13x/hari), hari berikutnya DALAM jendela schedule masih menyusul.
    // AMAN dari dobel-isolir karena runIsolirPhase IDEMPOTEN (kandidat = profil live != isolir & belum
    // bayar → yang sudah terisolir dilewati). Reminder/tenggang di atas SENGAJA tetap `===`: keduanya
    // tak punya dedup per-periode, jadi `<=` akan mengirim ulang tiap hari (spam); missed reminder jauh
    // lebih murah daripada missed isolir (kebocoran pendapatan), jadi trade-off ini disengaja.
    ...
    if (hasil.phases.length === 0) {
        log.info(`[CRON_BILLING_AKHIR_BULAN] Bukan hari aksi (sisa=${daysUntilEnd}) — no-op.`);
    }
    return hasil;
}
...
function initBillingAkhirBulanTask(config) {
    ...
    if (config.status_billing_akhir_bulan === true && isValidCron(config.schedule_billing_akhir_bulan)) {
        log.info("[CRON_BILLING_AKHIR_BULAN] Start/restart schedule:", config.schedule_billing_akhir_bulan);
        cronTaskBillingAkhirBulan = cron.schedule(config.schedule_billing_akhir_bulan, async () => {
            if (billingAkhirBulanRunning) {
                log.warn("[CRON_BILLING_AKHIR_BULAN_SKIPPED] Siklus sebelumnya masih jalan, lewati tick ini.");
                return;
            }
            ...
            try {
                ...
                log.error("[CRON_BILLING_AKHIR_BULAN_ERROR]", err.message || err);
            } finally {
                billingAkhirBulanRunning = false;
            }
        });
    } else {
        // GERBANG GANDA (#b304): eksklusi kohort dari 3 job standar dibaca dari config.json
        // (billingAkhirBulan.enabled + cron status, lihat isEndOfMonthBillingActive), sedangkan
        // penjadwalan job INI dari cron.json. Bila fitur ENABLED tapi job tak terjadwal, teriak
        // KERAS — jangan diam (pola warnIfScheduleCanNeverAct di isolir.js). Eksklusi sudah
        // fail-closed (mati bila cron mati), jadi ini peringatan operasional, bukan pencegah bocor.
        const fiturOn = !!(global.config && global.config.billingAkhirBulan && global.config.billingAkhirBulan.enabled === true);
        if (fiturOn) {
            log.error(
                "[CRON_BILLING_AKHIR_BULAN_MISCONFIG] config.billingAkhirBulan.enabled=true TAPI job tak terjadwal " +
                `(status_billing_akhir_bulan=${config.status_billing_akhir_bulan}, schedule=${JSON.stringify(config.schedule_billing_akhir_bulan)}, ` +
                `valid=${isValidCron(config.schedule_billing_akhir_bulan)}). Kohort akhir-bulan JATUH-AMAN ke siklus standar. ` +
                "Set status_billing_akhir_bulan=true + schedule valid di database/cron.json agar mereka ditagih relatif akhir bulan."
            );
        }
        // fitur off → benar-benar diam (perilaku lama).
    }
}
