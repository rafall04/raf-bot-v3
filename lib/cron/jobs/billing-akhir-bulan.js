...
async function runIsolirPhase(ctx) {
    ...
    for (const user of candidates) {
        ...
        try {
            ...
            fetchFail++;
            console.error(`[${TAG}] Gagal baca profil live ${user.name} (${user.pppoe_username}): ${error.message} — dilewati (fail-safe).`);
            continue;
        }
    ...
    console.log(`[${TAG}] === SUMMARY === kandidat=${candidates.length} isolir=${isolated} sudah-terisolir=${alreadyIsolir} gagal-baca=${fetchFail} gagal=${failed} notif=${notifRes.sent}`);
