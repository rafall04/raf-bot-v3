 ...
 * Deps: `node-cron`, `../shared` (isValidCron, loadCronConfig, delay), `../../myfunc` (getProfileBySubscription), `../../services/isolir-service` (executeProfileAction).
 ...
 */
"use strict";
const log = require('../../logger').logger.child('ISOLIR');


const cron = require('node-cron');

const { delay, isValidCron, loadCronConfig } = require('../shared');
...
function initIsolirTask(config) {
    ...
    if (config.status_schedule_unpaid_action === true && isValidCron(config.schedule_unpaid_action)) {
        log.info("[CRON_ISOLIR_ACTION] Starting/Restarting isolir action task with schedule:", config.schedule_unpaid_action);
        ...
        cronTaskUnpaidAction = cron.schedule(config.schedule_unpaid_action, async () => {
            if (isolirRunning) {
                log.warn("[CRON_ISOLIR_ACTION_SKIPPED] Previous isolir cycle still running, skipping this tick.");
                return;
            }

            // Hormati toggle cron.json SAAT TICK (bukan hanya saat penjadwalan). Mematikan
            // status_schedule_unpaid_action lewat edit cron.json harus langsung menghentikan isolir
            // tanpa wajib re-schedule/restart. Tanpa ini, task lama TETAP jalan = "bom isolir massal"
            // yang dikira sudah mati. Mirror pola sibling (isolir-paket.js, grace-reminder.js).
            const currentCronConfig = loadCronConfig();
            if (currentCronConfig.status_schedule_unpaid_action !== true) {
                log.info("[CRON_ISOLIR_ACTION_SKIPPED] status_schedule_unpaid_action dimatikan di cron.json — lewati tick ini.");
                return;
            }
            ...
            if (currentDay < isolirDay) {
                log.info(`[CRON_ISOLIR_ACTION_SKIPPED] Today (day ${currentDay}) is before the configured isolation day (${isolirDay}). No action will be taken.`);
                return;
            }

            log.info(`[CRON_ISOLIR_ACTION_STARTED] Today is day ${currentDay}, which is on or after the isolation day (${isolirDay}). Starting isolation process for unpaid users.`);
            const isolirProfileToUse = global.config.isolir_profile;
            if (!isolirProfileToUse) {
                log.error("[CRON_ISOLIR_ACTION_ERROR] `isolir_profile` is not defined in config.json. Cannot proceed.");
                return;
            }
            ...
            if (!syncToMikrotik) {
                log.info("[CRON_ISOLIR_ACTION] Sync to MikroTik is DISABLED - skipping isolir action.");
                return;
            }

            if (global.config && global.config.isolirFeatureEnabled === false) {
                log.info("[CRON_ISOLIR_ACTION] Fitur isolir dinonaktifkan - skipping isolir action.");
                return;
            }
            ...
            try {
                ...
                if (candidates.length === 0) {
                    log.info("[CRON_ISOLIR_ACTION] No candidate users to isolate.");
                    return;
                }

                const throttleMs = (global.config && parseInt(global.config.isolir_action_delay)) || 500;
                log.info(`[CRON_ISOLIR_ACTION] Found ${candidates.length} candidate(s). Processing sequentially with ${throttleMs}ms throttle.`);
                ...
                for (const user of candidates) {
                    log.info(`[CRON_ISOLIR_ACTION] Attempting to isolate user: ${user.pppoe_username} with profile: "${isolirProfileToUse}"`);
                    try {
                        ...
                        log.info(`[CRON_ISOLIR_SUCCESS] ${user.pppoe_username} berhasil diisolir.`);
                        isolatedCount++;
                    } catch (e) {
                        log.error(`[CRON_ISOLIR_ACTION_ERROR] Full process failed for user ${user.name}: ${e.message || e}`);
                        failedCount++;
                        failedNames.push(user.name || user.pppoe_username || `#${user.id}`);
                    }
                    if (throttleMs > 0) {
                ...
                log.info(`[CRON_ISOLIR_ACTION] Summary: ${isolatedCount} isolated, ${failedCount} failed, ${candidates.length} total.`);
                // GAGAL ISOLIR = pelanggan unpaid TETAP menikmati layanan (bocor pendapatan senyap).
                // Dulu hanya di-console; kini eskalasi TERJAMIN ke admin. Never-throw.
                if (failedCount > 0) {
                    try {
                        const { sendAdminAlarm } = require('../../admin-alarm');
                        const daftar = failedNames.slice(0, 30).join(', ') + (failedNames.length > 30 ? `, +${failedNames.length - 30} lagi` : '');
                        await sendAdminAlarm(
                            `🚨 *ISOLIR: ${failedCount} GAGAL* dari ${candidates.length}.\n\nPelanggan berikut BELUM bayar tapi GAGAL diisolir (masih menikmati layanan): ${daftar}.\n\nProses manual / cek MikroTik.`,
                            { label: 'cron-isolir-gagal', category: 'billing_isolir' }
                        );
