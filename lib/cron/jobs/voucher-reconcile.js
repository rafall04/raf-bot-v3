 ...
 */
"use strict";
const log = require('../../logger').logger.child('VOUCHER_RECONCILE');
...
async function runReconcileOnce(deps = {}) {
    ...
    if (keptUnparsed > 0) {
        log.warn(`[CRON_VOUCHER_RECONCILE] ${keptUnparsed} log gagal di-parse DIBIARKAN di router (tak dipangkas) — perbaiki parser lalu ingest ulang.`);
    }
...
function initVoucherReconcileTask(config) {
    ...
    if (config.status_voucher_reconcile === true && isValidCron(config.voucher_reconcile_schedule)) {
        log.info("[CRON_VOUCHER_RECONCILE] Starting schedule:", config.voucher_reconcile_schedule);
        task = cron.schedule(config.voucher_reconcile_schedule, async () => {
            if (running) {
                log.warn("[CRON_VOUCHER_RECONCILE_SKIP] Previous cycle still running, skipping.");
                return;
            }
            ...
            try {
                ...
                if (r.ok && r.found > 0) {
                    log.info(`[CRON_VOUCHER_RECONCILE] found:${r.found} ingested:${r.ingested} skipped:${r.skipped} pruned:${r.pruned}`);
                } else if (!r.ok) {
                    log.warn("[CRON_VOUCHER_RECONCILE] gagal:", r.message);
                }
            } catch (error) {
                log.error("[CRON_VOUCHER_RECONCILE_ERROR]", error.message);
            } finally {
                running = false;
            }
        });
