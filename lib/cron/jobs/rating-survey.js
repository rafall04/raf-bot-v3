...
async function runRatingSurveyCycle(overrides = {}) {
    ...
    for (const user of d.getUsers()) {
        ...
        if (whitelistedProfile.includes(userProfile)) { skipped.push({ name: user.name, reason: `paket whitelist/gratis (${user.subscription})` }); continue; }
        ...
        if (await repo.hasSurveyForPeriod(user.id, period)) { skipped.push({ name: user.name, reason: "sudah disurvei periode ini" }); continue; }
    ...
    // === FASE 3: tandai hasil per-survei (sent bila ≥1 nomor sukses) + ringkasan ===
    ...
    for (const [surveyId, s] of perSurvey.entries()) {
        if (s.failed < s.total) { await repo.markSent(surveyId, { sent_at: sentAt, expires_at: expiresAt }); markedSent += 1; }
        else { await repo.markUndelivered(surveyId); markedUndelivered += 1; }
    }
