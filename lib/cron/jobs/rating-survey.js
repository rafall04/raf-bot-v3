 ...
 */
"use strict";
const log = require('../../logger').logger.child('RATING_SURVEY');
...
function defaultDeps() {
    return {
        ...
        now: sqlNow,
        logger: console,
        // Diinjeksi supaya FASE 3 (revert not-sent) bisa diuji dgn keluaran skipped/deferred.
        sendQueue: sendQueueWithRetry,
    };
}
...
async function runRatingSurveyCycle(overrides = {}) {
    ...
    // Re-entrancy guard yang dihormati SEMUA pemanggil: cron bulanan DAN endpoint manual
    // (POST /api/owner/csat/run) & csat-recovery. Blast ber-jeda anti-ban makan 6-10 menit; dua
    // siklus tumpang-tindih = pelanggan disurvei DOBEL — justru risiko ban yang subsistem anti-ban
    // ada untuk mencegah. Dulu guard `surveyRunning` HANYA di dalam closure cron.
    if (surveyRunning) {
        d.logger.warn("[CRON_CSAT_SKIPPED] Siklus survei masih berjalan — permintaan ini dilewati.");
        return { ok: false, reason: "already_running" };
    }
    surveyRunning = true;
    try {
        return await runRatingSurveyCore(d);
    } finally {
        surveyRunning = false;
    }
}

async function runRatingSurveyCore(d) {
    ...
    for (const user of d.getUsers()) {
        ...
        const { id, created } = await repo.upsertPending({
            user_id: user.id,
        ...
        // upsertPending idempoten (UNIQUE user_id,period). `created:false` = baris SUDAH ada
        // (siklus lain/tumpang-tindih membuatnya). hasSurveyForPeriod di atas non-atomik terhadap
        // upsert, jadi dua run bisa lolos cek lalu keduanya di sini — TANPA guard `created` keduanya
        // markSent + push → pesan DOBEL. Skip baris yang BUKAN kita buat. (MODE TES dikecualikan:
        // di sana dedup memang sengaja dilewati agar bisa kirim ulang ke nomor uji.)
        if (!testJid && created === false) { skipped.push({ name: user.name, reason: "baris survei periode ini sudah ada (idempoten)" }); continue; }
    ...
    // "Tidak terkirim" mencakup TIGA keluaran, bukan cuma `failed`:
    //  - res.failed   : gagal kirim total (retry habis / breaker).
    //  - res.deferred : kena batas maxPerRun broadcastGuard → ditunda (belum dikirim run ini).
    //  - res.skipped  : ledger-block / minGap / tak terdaftar WA (broadcastGuard).
    // Dulu HANYA `failed` yang diperiksa, jadi survei yang nomornya di-skip/deferred tetap 'sent'
    // (hantu terkirim: masuk denominator response-rate, tak pernah dikirim ulang). Dicocokkan per
    // JID (skipped hanya membawa {jid,label,reason}, bukan objek item asli).
    const notSentJids = new Set();
    for (const it of (res.failed || [])) if (it && it.jid) notSentJids.add(it.jid);
    for (const it of (res.deferred || [])) if (it && it.jid) notSentJids.add(it.jid);
    for (const sk of (res.skipped || [])) if (sk && sk.jid) notSentJids.add(sk.jid);

    const perSurvey = new Map(); // surveyId -> {total, delivered}
    for (const it of items) {
        const s = perSurvey.get(it.surveyId) || { total: 0, delivered: 0 };
        s.total += 1;
        if (!notSentJids.has(it.jid)) s.delivered += 1; // benar-benar terkirim (bukan gagal/skip/tunda)
        perSurvey.set(it.surveyId, s);
    }
    let markedUndelivered = 0;
    for (const [surveyId, s] of perSurvey.entries()) {
        // Tak ADA satu pun nomor survei ini yang benar-benar terkirim → turunkan ke 'undelivered'
        // supaya tak dihitung terkirim & tetap layak dikirim ulang.
        if (s.delivered === 0) { await repo.markUndelivered(surveyId); markedUndelivered += 1; }
    }
...
function initRatingSurveyTask(config) {
    ...
    if (config.status_rating_survey === true && isValidCron(config.schedule_rating_survey)) {
        log.info("[CRON_CSAT] Menjadwalkan survei rating:", config.schedule_rating_survey);
        cronTaskSurvey = cron.schedule(config.schedule_rating_survey, async () => {
            // Guard re-entrancy kini DI DALAM runRatingSurveyCycle (dihormati cron + endpoint manual).
            // Menyetel surveyRunning di sini lagi akan menabrak guard itu (siklus asli terlihat "sedang jalan").
            try { await runRatingSurveyCycle(); }
            catch (error) { log.error("[CRON_CSAT_ERROR] Siklus survei gagal:", error.message); }
        });
    }

    if (config.status_rating_digest === true && isValidCron(config.schedule_rating_digest)) {
        log.info("[CRON_CSAT_DIGEST] Menjadwalkan digest rekap:", config.schedule_rating_digest);
        cronTaskDigest = cron.schedule(config.schedule_rating_digest, async () => {
            if (digestRunning) { log.warn("[CRON_CSAT_DIGEST_SKIPPED] Siklus digest sebelumnya masih jalan."); return; }
            digestRunning = true;
            try { await runRatingDigestCycle(); }
            catch (error) { log.error("[CRON_CSAT_DIGEST_ERROR] Siklus digest gagal:", error.message); }
