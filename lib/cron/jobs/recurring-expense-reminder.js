 ...
 */
"use strict";
const log = require('../../logger').logger.child('RECURRING_EXPENSE_REMINDER');
...
function initRecurringExpenseReminderTask() {
    ...
    const cfg = (global.config && global.config.businessExpense) || {};
    if (cfg.enabled !== true) {
        log.info("[CRON_KAS_RUTIN] Kas usaha OFF — pengingat biaya rutin tidak dijadwalkan");
        return;
    }
    ...
    if (!isValidCron(jadwal)) {
        log.error(`[CRON_KAS_RUTIN_ERROR] Jadwal tidak valid: "${jadwal}". Job tidak dijalankan.`);
        return;
    }

    log.info(`[CRON_KAS_RUTIN] Pengingat biaya rutin aktif, jadwal: ${jadwal}`);
    task = cron.schedule(jadwal, async () => {
        if (sedangJalan) {
            log.warn("[CRON_KAS_RUTIN] Siklus sebelumnya masih berjalan — dilewati");
            return;
        }
        sedangJalan = true;
        try {
            const r = await kirimPengingat();
            if (r.jumlah) log.info(`[CRON_KAS_RUTIN] ${r.jumlah}/${r.dariTotal} pengingat terkirim`);
        } catch (e) {
            log.error("[CRON_KAS_RUTIN_ERROR]", e.message);
        } finally {
            sedangJalan = false;
        }
    });
