 ...
 */
"use strict";
const log = require('../../logger').logger.child('MONEY_DIGEST');
...
async function kirimRingkasan(deps = {}) {
    ...
    try {
        ...
        log.error("[CRON_RINGKASAN_UANG_ERROR]", e && e.message);
...
function initMoneyDigestTask() {
    ...
    if (cfg.enabled !== true || cfg.digest !== true) {
        log.info("[CRON_RINGKASAN_UANG] Ringkasan uang tidak dijadwalkan (kas OFF atau digest OFF)");
        return;
    }
    ...
    if (!isValidCron(jadwal)) {
        log.error(`[CRON_RINGKASAN_UANG_ERROR] Jadwal tidak valid: "${jadwal}". Job tidak dijalankan.`);
        return;
    }

    log.info(`[CRON_RINGKASAN_UANG] Ringkasan uang aktif, jadwal: ${jadwal}`);
    task = cron.schedule(jadwal, async () => {
        if (sedangJalan) {
            log.warn("[CRON_RINGKASAN_UANG] Siklus sebelumnya masih berjalan — dilewati");
            return;
        }
        sedangJalan = true;
        try {
            const r = await kirimRingkasan();
            if (r.terkirim) log.info("[CRON_RINGKASAN_UANG] Ringkasan terkirim");
        } catch (e) {
            log.error("[CRON_RINGKASAN_UANG_ERROR]", e && e.message);
        } finally {
            sedangJalan = false;
