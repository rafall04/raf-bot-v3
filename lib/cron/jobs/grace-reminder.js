 ...
 */
"use strict";
const log = require('../../logger').logger.child('GRACE_REMINDER');
...
const { isInfrastructure, getWhitelistedPackageNames, isEndOfMonthBillingActive, isPerPackageIsolirActive, getPackageIsolirDay } = require('../../account-classification');
...
async function runGraceReminderCycle() {
    ...
    log.info(`[CRON_GRACE] Task executed at: ${now.toLocaleString('id-ID')}. jatuh_tempo=${jatuh_tempo} isolir=${tanggal_isolir}`);
    ...
    for (const user of (global.users || [])) {
        ...
        if (tagihan.reason === 'katalog-tak-terbaca') {
            log.warn(`[CRON_GRACE] Harga efektif tak terbaca untuk ${user.name} — pakai harga paket.`);
        }
        ...
        try {
            ...
            log.error(`[CRON_GRACE] Gagal buat link bayar untuk ${user.name}: ${linkErr.message}`);
        }

        // #b305: bila pelanggan pakai isolir per-PAKET, tunjukkan tanggal isolir PAKETnya (bukan
        // tanggal global) agar surat tenggang tak menyebut tanggal yang salah. Akhir-bulan sudah
        // dikecualikan di atas, jadi di sini cuma standar vs per-paket.
        let tanggalIsolirUser = tanggal_isolir;
        if (isPerPackageIsolirActive(user)) {
            const uDay = getPackageIsolirDay(user);
            if (uDay) tanggalIsolirUser = fmt(new Date(now.getFullYear(), now.getMonth(), uDay));
        }
        ...
        const messageText = renderTemplate('masa_tenggang_reminder', {
            ...
            jatuh_tempo,
            tanggal_isolir: tanggalIsolirUser,
            periode,
    ...
    if (items.length > 0 && !isReady()) {
        log.warn(`[CRON_GRACE] WhatsApp belum siap (state: ${getConnectionState()}) — antrian ${items.length} pesan akan menunggu & di-retry.`);
    }
    ...
    for (const s of skipped) log.info(`[CRON_GRACE_SKIP] ${s.name}: ${s.reason}`);
    log.info(`[CRON_GRACE] === SUMMARY === unpaid=${unpaidCount} whitelisted=${whitelistedCount} target=${items.length} sent=${res.sent} gagal=${res.failed.length} skipped=${skipped.length}`);
...
function initGraceReminderTask(config) {
    ...
    if (!isValidCron(schedule)) {
        log.error(`[CRON_GRACE] Jadwal tidak valid: "${schedule}". Task tidak dijalankan.`);
        return;
    }

    log.info("[CRON_GRACE] Starting/Restarting grace (masa tenggang) task with schedule:", schedule);
    cronTaskGraceReminder = cron.schedule(schedule, async () => {
        // Overlap guard — cegah dua siklus jalan bersamaan.
        if (graceRunning) {
            log.warn("[CRON_GRACE_SKIPPED] Previous grace cycle still running, skipping this tick.");
            return;
        }
        ...
        try {
            ...
            log.error('[CRON_GRACE_ERROR] Siklus masa tenggang gagal:', err.message);
        } finally {
            graceRunning = false;
        }
    });
