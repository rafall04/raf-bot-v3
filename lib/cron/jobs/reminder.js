 ...
 */
"use strict";
const log = require('../../logger').logger.child('REMINDER');
...
async function runReminderCycle() {
    // Dikunci pada NAMA PAKET, bukan profil — profil MikroTik wajar dipakai bersama
    // paket gratis DAN berbayar. Lihat alasan lengkap di lib/account-classification.js.
    ...
    log.info(`[CRON_REMINDER] Total users: ${global.users ? global.users.length : 0}`);
    log.info(`[CRON_REMINDER] Whitelisted packages: ${[...paketWhitelist].join(', ') || 'none'}`);
    ...
    for (const user of (global.users || [])) {
        ...
        // Pelanggan siklus AKHIR BULAN ditangani cron billing-akhir-bulan (jadwal relatif akhir
        // bulan). Kecualikan di sini agar tak dapat pengingat awal-bulan yang keliru.
        ...
        if (tagihan.reason === 'katalog-tak-terbaca') {
            log.warn(`[CRON_REMINDER] Harga efektif tak terbaca untuk ${user.name} — pakai harga paket.`);
        }
        ...
        try {
            ...
            log.error(`[CRON_REMINDER] Gagal buat link bayar untuk ${user.name}: ${linkErr.message}`);
        }
    ...
    if (items.length > 0 && !isReady()) {
        log.warn(`[CRON_REMINDER] WhatsApp belum siap (state: ${getConnectionState()}) — antrian ${items.length} pesan akan menunggu & di-retry.`);
    }
    ...
    for (const s of skipped) log.info(`[CRON_REMINDER_SKIP] ${s.name}: ${s.reason}`);
    log.info(`[CRON_REMINDER] === SUMMARY === unpaid=${unpaidCount} whitelisted=${whitelistedCount} target=${items.length} sent=${res.sent} gagal=${res.failed.length} skipped=${skipped.length}`);
...
function initReminderTask(config) {
    ...
    if (config.status_schedule === true && isValidCron(config.schedule)) {
        log.info("[CRON_REMINDER] Starting/Restarting reminder task with schedule:", config.schedule);
        cronTaskReminder = cron.schedule(config.schedule, async () => {
            ...
            if (reminderRunning) {
                log.warn("[CRON_REMINDER_SKIPPED] Previous reminder cycle still running, skipping this tick.");
                return;
            }
            ...
            log.info(`[CRON_REMINDER] Task executed at: ${new Date().toLocaleString('id-ID')}. tanggal_pengingat config: ${reminderDayInfo} (for template only, schedule is authoritative).`);

            reminderRunning = true;
            try {
                await runReminderCycle();
            } catch (err) {
                log.error('[CRON_REMINDER_ERROR] Siklus reminder gagal:', err.message);
            } finally {
