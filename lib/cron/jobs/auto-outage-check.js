 ...
 */
"use strict";
const log = require('../../logger').logger.child('AUTO_OUTAGE_CHECK');
...
function initAutoOutageCheckTask(config = {}, deps = {}) {
    ...
    autoOutageTask = cron.schedule(schedule, async () => {
        if (autoOutageRunning) {
            log.warn("[CRON_AUTO_OUTAGE_SKIPPED] tick sebelumnya masih berjalan — dilewati (anti dobel-DM).");
            return;
        }
        ...
        try {
            ...
            log.error(`[CRON_AUTO_OUTAGE_ERROR] ${error.message}`);
        } finally {
