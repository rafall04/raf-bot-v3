 ...
 */
"use strict";
const log = require('../../logger').logger.child('ISOLIR_NOTIFICATION');
...
async function runIsolirNotificationCycle(overrides = {}) {
    ...
    for (const user of d.getUsers()) {
        ...
        // #b304: kohort siklus AKHIR BULAN TIDAK lagi dikecualikan di sini (dulu #b303 mengecualikan).
        // Job billing-akhir-bulan mengirim notif isolir INLINE saat mengisolir, tapi bila WA mati saat
        // tick itu notif hilang tanpa jaring pengaman. Dengan menyertakan kohort, siapa pun yang profil
        // LIVE-nya masih = isolir_profile & belum bayar tetap dapat notif+link bayar di tick BULANAN
        // (tgl 12) → jaring durabel pulih. Cadence bulanan, bukan harian, jadi bukan spam.
        if (!user.pppoe_username) continue;       // tanpa PPPoE → bukan target isolir

        // Sudah BAYAR (selaras gate isolir.js `!user.paid`) tapi profil live mungkin masih isolir
        // karena reaktivasi tertunda/gagal → JANGAN dun "silakan bayar" (menyinggung pelanggan yang
        // sudah bayar). Lewati + catat (never-silent) supaya reaktivasi tertunda tetap terlihat di log.
        if (user.paid) {
            skipped.push({ name: user.name, reason: 'sudah bayar (user.paid) — tak di-dun; jika profil live masih isolir, itu soal reaktivasi tertunda' });
            continue;
        }
...
function initIsolirNotificationTask(config) {
    ...
    if (config.status_message_isolir_notification === true && isValidCron(config.schedule_isolir_notification)) {
        log.info("[CRON_ISOLIR_NOTIF] Starting/Restarting isolir notification task with schedule:", config.schedule_isolir_notification);
        cronTaskIsolirNotification = cron.schedule(config.schedule_isolir_notification, async () => {
            if (notifRunning) {
                log.warn("[CRON_ISOLIR_NOTIF_SKIPPED] Previous notification cycle still running, skipping this tick.");
                return;
            }
            notifRunning = true;
            try {
                ...
                log.error('[CRON_ISOLIR_NOTIF_ERROR] Siklus notifikasi isolir gagal:', error.message);
            } finally {
                notifRunning = false;
            }
        });
