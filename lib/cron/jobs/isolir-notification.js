...
const { isInfrastructure: isInfra, getWhitelistedPackageNames, isEndOfMonthBillingActive: isEomActive } = require('../../account-classification');
...
function defaultDeps() {
    return {
        ...
        isEndOfMonthBillingActive: isEomActive,
...
async function runIsolirNotificationCycle(overrides = {}) {
    ...
    for (const user of d.getUsers()) {
        ...
        // Kohort siklus AKHIR BULAN: notifikasi isolirnya dikirim langsung oleh cron
        // billing-akhir-bulan saat mengisolir (di akhir bulan). Kecualikan di sini agar tak
        // dapat notif ganda / di hari yang salah.
        if (d.isEndOfMonthBillingActive && d.isEndOfMonthBillingActive(user)) continue;
