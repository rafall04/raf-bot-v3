    ...
    if (config.status_message_isolir_notification === true && isValidCron(config.schedule_isolir_notification)) {
        ...
        cronTaskIsolirNotification = cron.schedule(config.schedule_isolir_notification, async () => {
            ...
            for (let user of global.users) {
                // Akun infrastruktur (mis. modem CCTV/monitoring) tidak diisolir → lewati notifikasi isolir.
                        ...
                        if (user.phone_number && isReady()) {
                            ...
                            for (let number of phoneNumbers) {
                                ...
                                if (normalizedNumber.length > 5) { // Basic validation
                                    ...
                                    let linkBayar = '';
                                    try {
                                        linkBayar = buildBillPayUrl(user, { periodMonth: nowDate.getMonth() + 1, periodYear: nowDate.getFullYear() });
                                    } catch (linkErr) {
                                        console.error(`[CRON_ISOLIR_NOTIF] Gagal buat link bayar untuk ${user.name}: ${linkErr.message}`);
                                    }
