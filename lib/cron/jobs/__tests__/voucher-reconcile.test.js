...
describe("voucher-reconcile runReconcileOnce", () => {
    ...
    test("#b325 log gagal di-parse DIBIARKAN di router (tak dipangkas); hanya yang valid dipangkas", async () => {
        const repo = tmpRepo();
        let removedIds = null;
        const BAD = "format-log-ngawur-tanpa-separator"; // parseMikhmonLogName → null → tak boleh dihapus
        const r = await runReconcileOnce({
        ...
        expect(r.ok).toBe(true);
        expect(r.ingested).toBe(1);
        expect(r.keptUnparsed).toBe(1);
        expect(removedIds).toEqual(["*1"]); // "*2" (parse gagal) TIDAK ikut dihapus → revenue tak hilang
        await repo.close();
    });
