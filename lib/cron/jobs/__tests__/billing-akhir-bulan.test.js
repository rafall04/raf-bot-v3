...
describe("billing-akhir-bulan — pemilihan fase relatif AKHIR bulan", () => {
    ...
    test("#b320: isolir SELF-HEAL (isolirDaysBefore=2) → fase isolir menyala di H-2,H-1,H-0; tick terlewat tersusul", async () => {
        aktifkanFitur({ billingAkhirBulan: { enabled: true, reminderDaysBefore: 5, graceDaysBefore: 2, isolirDaysBefore: 2 } });
        // Agustus 31 hari: H-2=29, H-1=30, H-0=31 → semuanya memicu isolir (idempoten via profil-live).
        expect((await runBillingAkhirBulanCycle(new Date(2026, 7, 29))).phases).toContain("isolir");
        expect((await runBillingAkhirBulanCycle(new Date(2026, 7, 30))).phases).toContain("isolir");
        expect((await runBillingAkhirBulanCycle(new Date(2026, 7, 31))).phases).toContain("isolir");
        // Sebelum jendela (H-3=28) TIDAK isolir; reminder/tenggang tetap === (tak ikut self-heal).
        expect((await runBillingAkhirBulanCycle(new Date(2026, 7, 28))).phases).not.toContain("isolir");
    });
...
describe("billing-akhir-bulan — penjaga struktural: 3 job standar mengecualikan kohort (#b304)", () => {
    // Isolir ACTION + reminder + tenggang mengecualikan kohort (mereka salah-hari utk akhir-bulan).
    const JOBS = [
    ...
    test("#b304: isolir-notification.js TIDAK mengecualikan kohort (jaring durabel bulanan dipulihkan)", () => {
        const s = fs.readFileSync(path.join(AKAR, "lib/cron/jobs/isolir-notification.js"), "utf8");
        // tak ada lagi skip continue berbasis isEndOfMonthBillingActive di sini
        expect(s).not.toMatch(/isEndOfMonthBillingActive\s*\(\s*user\s*\)\s*\)?\s*continue/);
        expect(s).not.toMatch(/isEndOfMonthBillingActive\)\s*&&\s*d\.isEndOfMonthBillingActive/);
    });
});

describe("billing-akhir-bulan — #b304 gerbang misconfig (enabled tapi tak terjadwal → teriak)", () => {
    test("initBillingAkhirBulanTask: cabang else meneriakkan MISCONFIG saat fitur enabled", () => {
        const s = fs.readFileSync(path.join(AKAR, "lib/cron/jobs/billing-akhir-bulan.js"), "utf8");
        expect(s).toMatch(/CRON_BILLING_AKHIR_BULAN_MISCONFIG/);
        // dipicu hanya bila fitur enabled (bukan spam saat fitur off)
        expect(s).toMatch(/billingAkhirBulan\s*&&\s*global\.config\.billingAkhirBulan\.enabled\s*===\s*true/);
    });
});
