 ...
 */
"use strict";
const log = require('../../logger').logger.child('ISOLIR_PAKET');
...
async function runIsolirPaketCycle(nowOverride = null) {
    ...
    if (cfg.sync_to_mikrotik === false) {
        log.info("[CRON_ISOLIR_PAKET] Sync MikroTik DINONAKTIFKAN — lewati.");
        return { skipped: "sync-off" };
    }
    if (cfg.isolirFeatureEnabled === false) {
        log.info("[CRON_ISOLIR_PAKET] Fitur isolir dinonaktifkan — lewati.");
        return { skipped: "isolir-off" };
    }
    const isolirProfile = cfg.isolir_profile;
    if (!isolirProfile) {
        log.error("[CRON_ISOLIR_PAKET] isolir_profile tak terdefinisi di config — batal.");
    ...
    if (candidates.length === 0) {
        log.info(`[CRON_ISOLIR_PAKET] hari=${today}: tak ada kandidat isolir per-paket.`);
    ...
    log.info(`[CRON_ISOLIR_PAKET] hari=${today}: ${candidates.length} kandidat. Throttle ${throttleMs}ms.`);
    ...
    for (const { user, isolirDay } of candidates) {
        ...
        try {
            ...
            fetchFail++;
            log.error(`[CRON_ISOLIR_PAKET_FETCH_ERROR] ${user.name} (${user.pppoe_username}): ${error.message}`);
            continue;
        }
        ...
        try {
            ...
            log.info(`[CRON_ISOLIR_PAKET_SUCCESS] ${user.pppoe_username} diisolir (isolir_day=${isolirDay}).`);
            isolated++;
            isolatedUsers.push(user);
        } catch (e) {
            failed++;
            log.error(`[CRON_ISOLIR_PAKET_ERROR] Gagal isolir ${user.name}: ${e.message || e}`);
        }
    ...
    for (const user of isolatedUsers) {
        ...
        try {
            ...
            log.error(`[CRON_ISOLIR_PAKET] Gagal buat link bayar ${user.name}: ${linkErr.message}`);
        }
    ...
    if (notifItems.length) {
        if (!isReady()) log.warn(`[CRON_ISOLIR_PAKET] WhatsApp belum siap (state: ${getConnectionState()}) — antrian ${notifItems.length} di-retry.`);
    ...
    log.info(`[CRON_ISOLIR_PAKET] === SUMMARY === hari=${today} kandidat=${candidates.length} isolir=${isolated} gagal=${failed} sudah-isolir=${alreadyIsolir} gagal-baca-profil=${fetchFail} notif=${notified}`);
...
function initIsolirPaketTask(config) {
    ...
    if (config.status_isolir_paket === true && isValidCron(config.schedule_isolir_paket)) {
        log.info("[CRON_ISOLIR_PAKET] Start/restart schedule:", config.schedule_isolir_paket);
        cronTaskIsolirPaket = cron.schedule(config.schedule_isolir_paket, async () => {
            if (isolirPaketRunning) {
                log.warn("[CRON_ISOLIR_PAKET_SKIPPED] Siklus sebelumnya masih jalan, lewati tick ini.");
                return;
            }
            ...
            try {
                ...
                log.error("[CRON_ISOLIR_PAKET_ERROR]", err.message || err);
            } finally {
