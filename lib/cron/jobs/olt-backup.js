...
function initOltBackupTask() {
    ...
    if (!settings.enabled) {
        log.info('[CRON_OLT_BACKUP] Auto-backup OLT DISABLED');
        return;
    }
    if (!isValidCron(settings.schedule)) {
        log.error(`[CRON_OLT_BACKUP_ERROR] Cron expression tidak valid: "${settings.schedule}". Job tidak dijalankan.`);
        return;
    }

    log.info(`[CRON_OLT_BACKUP] Auto-backup OLT aktif, jadwal: ${settings.schedule}`);
    cronTaskOltBackup = cron.schedule(settings.schedule, async () => {
        if (isRunning) {
            log.warn('[CRON_OLT_BACKUP] Run sebelumnya masih berjalan — skip siklus ini');
            return;
        }
        isRunning = true;
        log.info(`[CRON_OLT_BACKUP] Mulai backup OLT terjadwal ${new Date().toLocaleString('id-ID', { timeZone: 'Asia/Jakarta' })}`);
        try {
            const { okCount, failCount } = await runBackupAll();
            log.info(`[CRON_OLT_BACKUP] Selesai: ✅ ${okCount} • ❌ ${failCount}`);
        } catch (error) {
            log.error('[CRON_OLT_BACKUP] ❌ Gagal:', error.message);
        } finally {
            isRunning = false;
        }
