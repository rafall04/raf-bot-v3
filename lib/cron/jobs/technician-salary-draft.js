 ...
 */
"use strict";
const log = require('../../logger').logger.child('TECHNICIAN_SALARY_DRAFT');
...
async function jalankanSiklus() {
    ...
    try {
        ...
        if (hasil.dijalankan) {
            log.info(`[CRON_GAJI_TETAP] ${hasil.periode}: ${hasil.dibuat} dibuat, ${hasil.sudahAda} sudah ada, ${hasil.dilewati} dilewati`);
        }
        return hasil;
    } catch (error) {
        log.error("[CRON_GAJI_TETAP_ERROR]", error.message);
...
function initTechnicianSalaryDraftTask() {
    ...
    if (!cfg.autoDraft) {
        log.info("[CRON_GAJI_TETAP] Draft otomatis MATI (config.technicianSalary.autoDraft)");
        return null;
    }
    ...
    log.info(`[CRON_GAJI_TETAP] Aktif — jadwal '${jadwal}', pemicu tanggal ${cfg.draftDay}`);
    return task;
}
