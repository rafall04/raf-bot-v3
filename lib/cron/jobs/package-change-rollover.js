 ...
 */
"use strict";
const log = require('../../logger').logger.child('PACKAGE_CHANGE_ROLLOVER');
...
// response-template-helper → template-service memanggil loadAllCategories() saat LOAD (loadJSON).
// Require LAZY (di dalam tick) supaya tak menyeret template-service ke graph load cron.js — kalau
// tidak, test cron yang mem-mock lib/database (tanpa loadJSON) gagal saat suite-load.
function renderResponseTemplate(...args) {
    return require("../../response-template-helper").renderResponseTemplate(...args);
}
...
async function alertAdmins(text) {
    try {
        ...
        log.error("[CRON_PKG_ROLLOVER] alertAdmins gagal:", e && e.message);
    }
}
...
async function runRolloverTick(deps = {}) {
    ...
    // Notif rollover = SATU-KALI (bukan berulang). safeSendMessage "success" cuma berarti diterima
    // antrian LOKAL Baileys (bukan terkirim) — saat socket blip pesan HILANG tanpa jejak, dan pelanggan
    // tak pernah tahu paketnya berganti. Pakai sendCritical (retry + dead-letter) agar durabel.
    const send = deps.sendCritical || deps.safeSendMessage || require("../../whatsapp-critical-delivery").sendCritical;
    ...
    for (let i = 0; i < requests.length; i++) {
        ...
        try {
            ...
            failed += 1;
            log.error(`[CRON_PKG_ROLLOVER_ERROR] #${r.id}: ${error.message}`);
...
function initPackageChangeRolloverTask(_config) {
    ...
    cronTaskPkgRollover = cron.schedule("0 * * * *", async () => {
        ...
        if (pkgRolloverRunning) { log.warn("[CRON_PKG_ROLLOVER_SKIPPED] tick sebelumnya masih jalan."); return; }
        pkgRolloverRunning = true;
        try {
            const res = await runRolloverTick();
            if (res.applied || res.failed) {
                log.info(`[CRON_PKG_ROLLOVER] proses=${res.processed} terapkan=${res.applied} gagal=${res.failed}`);
            }
        } catch (error) {
            log.error(`[CRON_PKG_ROLLOVER_ERROR] ${error.message}`);
        } finally {
