...
describe("tick isolir HORMATI toggle cron.json saat berjalan (#b319 — anti bom isolir massal)", () => {
    // Mematikan status_schedule_unpaid_action lewat edit cron.json harus langsung menghentikan
    // isolir tanpa re-schedule. Guard harus di DALAM callback terjadwal, bukan hanya saat init.
    const src = fs.readFileSync(path.join(__dirname, "..", "jobs", "isolir.js"), "utf8");
    test("callback terjadwal loadCronConfig() + skip bila status_schedule_unpaid_action != true", () => {
        const body = src.slice(src.indexOf("cron.schedule("));
        expect(body).toMatch(/loadCronConfig\(\)/);
        expect(body).toMatch(/status_schedule_unpaid_action\s*!==\s*true/);
    });
});
