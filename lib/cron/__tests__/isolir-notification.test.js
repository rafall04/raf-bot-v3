...
describe("isolir-notification (hardened)", () => {
    ...
    test("#b319: sudah BAYAR (user.paid) walau profil live ISOLIR → TIDAK di-dun, skip TERCATAT", async () => {
        const users = [CUST({ name: "Sudah Bayar", pppoe_username: "paid@r", paid: true })];
        const { deps, sent, logs } = harness({ users, liveProfiles: { "paid@r": "ISOLIR" } });
        const res = await runIsolirNotificationCycle(deps);
        expect(sent).toHaveLength(0);
        expect(res.sent).toBe(0);
        expect(res.skipped).toBe(1);
        expect(logs.join("\n")).toMatch(/Sudah Bayar.*sudah bayar/i);
    });
