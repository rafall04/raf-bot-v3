...
// #b305 — isolir per-PAKET. Fitur gated + fail-closed (spt #b304). Jaga cron.json yang DIKIRIM repo
// selalu menjadwalkan job, dan blok config contoh berbentuk benar.
describe("koherensi ISOLIR PER-PAKET (#b305)", () => {
    test("cron.json menjadwalkan job isolir-paket (status true + schedule 5-field)", () => {
        expect(cronCfg.status_isolir_paket).toBe(true);
        expect(typeof cronCfg.schedule_isolir_paket).toBe("string");
        expect(cronCfg.schedule_isolir_paket.trim().split(/\s+/).length).toBe(5);
    });
    test("config.example memuat blok isolirPerPaket (enabled boolean)", () => {
        expect(exampleCfg.isolirPerPaket).toBeTruthy();
        expect(typeof exampleCfg.isolirPerPaket.enabled).toBe("boolean");
    });
    test("bila contoh MENGAKTIFKAN fitur, cron WAJIB terjadwal (cegah split-brain)", () => {
        if (exampleCfg.isolirPerPaket && exampleCfg.isolirPerPaket.enabled === true) {
            expect(cronCfg.status_isolir_paket).toBe(true);
            expect(cronCfg.schedule_isolir_paket.trim().split(/\s+/).length).toBe(5);
