...
describe('cron set-unpaid hardening', () => {
    beforeEach(() => {
        ...
        // Paket NYATA selalu punya `name`; whitelist dikunci pada nama itu (#b241),
        // bukan pada profil. Fixture lama tanpa `name` memodelkan implementasi lama.
        global.packages = [
            { name: 'VIP', profile: 'profile-VIP', whitelist: true },
            { name: 'REG', profile: 'profile-REG', whitelist: false }
        ];
        global.users = [];
    });
    ...
    test('skip akun infrastruktur (account_type=infrastruktur) dari set-unpaid', async () => {
        global.users = [
            { id: 1, subscription: 'REG', paid: true },                                // → sync
            { id: 5, subscription: 'REG', paid: true, account_type: 'infrastruktur' }, // infra → skip
            { id: 6, subscription: 'REG', paid: true, account_type: 'INFRASTRUKTUR' }  // infra (kapital) → skip
        ];
        initSetUnpaidTask({ status_unpaid_schedule: true, unpaid_schedule: '0 0 1 * *' });
        await getScheduledCallback()();
