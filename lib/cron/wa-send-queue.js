 ...
 * Deps: `../utils` (normalizePhoneNumber 0→62). Dependency inti (safeSendMessage/isReady/delay/logger) DIINJEKSI. ANTI-BAN (opt-in
 ...
 * WAJIB pakai normalizePhoneNumber (0→62) — nomor pelanggan tersimpan mentah '08xxx' (write-path tak
 * menormalkan). Tanpa ini JID jadi '08xxx@s.whatsapp.net' yang DITERIMA Baileys tanpa error tapi TAK
 * PERNAH sampai → semua cron tagihan/tenggang/isolir "sukses" ke nomor 0-awalan padahal buta. Jalur
 * welcome (create-user-persist) sudah pakai normalizer yang sama → kini konsisten.
 */
function buildJid(raw) {
    const digits = normalizePhoneNumber(String(raw || ""));
