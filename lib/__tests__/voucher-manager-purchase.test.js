...
jest.mock('../voucher-orphan', () => ({ recordVoucherOrphan: jest.fn() }));
...
describe('purchaseVoucherWithSaldo', () => {
    ...
    test('deduct gagal: kembalikan gagal (tidak menganggap sukses) + CATAT orphan (#b325)', async () => {
        recordVoucherOrphan.mockClear();
        const saldoManager = makeSaldoManager({ saldo: 10000, deductResult: false });
        const result = await purchaseVoucherWithSaldo('user-1', 'P1', saldoManager);

        expect(result.success).toBe(false);
        expect(result.message).toMatch(/Gagal memotong saldo/i);
        // Voucher terlanjur dibuat tapi saldo gagal dipotong → WAJIB masuk worklist orphan (bukan cuma log).
        expect(recordVoucherOrphan).toHaveBeenCalledWith(expect.objectContaining({ reason: 'admin_purchase_deduct_failed', voucherCode: 'v123' }));
    });
