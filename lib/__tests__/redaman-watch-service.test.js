...
describe("decideNotify (#b353)", () => {
    ...
    // #b356 override PRIBADI (prefs.pantau)
    test("targetDbm pribadi: RX ≥ target → kind target walau label belum BAIK", () => {
        const w = { ...base, targetDbm: -24 };
        // -24 (WASPADA) ≥ target -24 → target tercapai versi pribadi
        expect(decideNotify(w, diagOlt(-24, "WASPADA"), Date.now())).toEqual({ notify: true, kind: "target" });
    });
    test("targetDbm pribadi: RX belum capai target → BUKAN target (jatuh ke cek lain, DIAM)", () => {
        const w = { ...base, targetDbm: -22, targetAnnounced: false, lastRx: -25 };
        expect(decideNotify(w, diagOlt(-25, "WASPADA"), Date.now())).toEqual({ notify: false, kind: null });
    });
    test("changeThresholdDb pribadi menang atas config: ambang 5 → perubahan 3 dB DIAM", () => {
        const w = { ...base, targetAnnounced: true, changeThresholdDb: 5, lastRx: -27 };
        // config default berubah pada ~1.5; dgn ambang pribadi 5, selisih 3 (−27→−24) tak cukup
        expect(decideNotify(w, diagOlt(-24, "WASPADA"), Date.now(), { changeThresholdDb: 1.5 })).toEqual({ notify: false, kind: null });
    });
});
