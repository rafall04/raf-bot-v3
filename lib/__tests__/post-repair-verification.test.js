...
describe("#b276 — verifikasi memakai nilai PER-ONU yang segar, bukan halaman daftar", () => {
    function jalankan(bacaSegarPerOnu) {
        const timers = [];
        const { verifier, sent } = makeVerifier({
            getConfig: () => ({ enabled: true, settleDelayMs: 1, rxWarnDbm: -25, rxDegradeDb: 3 }),
            getOltSnapshot: async () => ({ status: "success", onus: [onu("aa:bb:cc:dd:01:01", "Online", "-24.90")], failedOlts: [] }),
            bacaSegarPerOnu,
            setTimeoutFn: (fn) => { timers.push(fn); return { unref: () => {} }; },
        });
        verifier.reportAfterRecovery({ oltKey: "olt-1", recovered: [AFFECTED[0]], stillDown: [] });
    ...
    test("!! nilai segar MENANG atas nilai halaman daftar", async () => {
        // Terukur: halaman daftar di-cache OLT (6 dari 8 ONU beda, s/d 0,23 dB). Di ambang
        // batas selisih sekecil itu membalik vonis — daftar -24,90 (aman) vs segar -25,40
        // (perlu dicek).
        const { timers, sent } = jalankan(async () => ({ ok: true, rxPower: -25.4 }));
        await timers[0]();
        expect(sent[0].text).toMatch(/-25[.,]40/);
        expect(sent[0].text).not.toMatch(/-24[.,]90/);
        expect(sent[0].text).toMatch(/REDAMAN PERLU DICEK/i);
    });

    test("gagal baca segar → pakai nilai daftar, laporan TETAP terkirim", async () => {
        const { timers, sent } = jalankan(async () => null);
        await timers[0]();
        expect(sent[0].text).toMatch(/-24[.,]90/);
    });

    test("pembaca segar melempar → tidak menjatuhkan laporan", async () => {
        const { timers, sent } = jalankan(async () => { throw new Error("boom"); });
        await expect(timers[0]()).resolves.toBeDefined();
        expect(sent.length).toBeGreaterThan(0);
    });
});

describe("#b278 — laporan verifikasi menampilkan DUA sumber", () => {
    function jalankan(bacaAcsRedaman) {
        const timers = [];
        const { verifier, sent } = makeVerifier({
            getConfig: () => ({ enabled: true, settleDelayMs: 1, rxWarnDbm: -25, rxDegradeDb: 3 }),
            getOltSnapshot: async () => ({ status: "success", onus: [onu("aa:bb:cc:dd:01:01", "Online", "-20.40")], failedOlts: [] }),
            bacaSegarPerOnu: async () => null,
            bacaAcsRedaman,
            setTimeoutFn: (fn) => { timers.push(fn); return { unref: () => {} }; },
        });
        verifier.reportAfterRecovery({
            oltKey: "olt-1",
            recovered: [{ ...AFFECTED[0], customerId: 7 }],
            stillDown: [],
        });
        return { timers, sent };
    }

    test("!! keduanya ada → dua-duanya tampil, tak ada yang dibuang", async () => {
        const { timers, sent } = jalankan(async () => new Map([["7", -20]]));
        await timers[0]();
        expect(sent[0].text).toMatch(/GenieACS -20.00 dBm · OLT -20.40 dBm/);
    });

    test("query ACS menerima customerId yang terdampak", async () => {
        let diminta = null;
        const { timers } = jalankan(async (ids) => { diminta = ids; return new Map(); });
        await timers[0]();
        expect(diminta).toEqual([7]);
    });

    test("ACS tak punya angka → tetap tampil sebagai \"-\", angka OLT utuh", async () => {
        const { timers, sent } = jalankan(async () => new Map());
        await timers[0]();
        expect(sent[0].text).toMatch(/GenieACS - · OLT -20.40 dBm/);
    });

    test("pembaca ACS meledak → laporan TETAP terkirim", async () => {
        const { timers, sent } = jalankan(async () => { throw new Error("acs mati"); });
        await expect(timers[0]()).resolves.toBeDefined();
        expect(sent.length).toBeGreaterThan(0);
        expect(sent[0].text).toMatch(/OLT -20.40 dBm/);
    });
});
