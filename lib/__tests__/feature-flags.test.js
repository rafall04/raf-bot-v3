...
test("defaultEnabled: gate default-AKTIF nyala walau key absen (anti 404-senyap merge-key)", () => {
    const vsd = flagByKey("voucherSalesDashboard");
    expect(vsd.defaultEnabled).toBe(true);
    expect(getFlagEnabled({}, vsd)).toBe(true);                       // absen → default true
    expect(getFlagEnabled({ voucherSalesDashboard: { enabled: false } }, vsd)).toBe(false); // eksplisit false menang
    const tp = flagByKey("teknisiPrefs");
    expect(getFlagEnabled({}, tp)).toBe(false);                      // deploy-gelap absen → false
});

test("isFeatureEnabled: kanonik + fallback untuk key tak dikenal", () => {
    const { isFeatureEnabled } = require("../feature-flags");
    expect(isFeatureEnabled("voucherSalesDashboard", { config: {} })).toBe(true);
    expect(isFeatureEnabled("teknisiPrefs", { config: {} })).toBe(false);
    expect(isFeatureEnabled("teknisiPrefs", { config: { teknisiPrefs: { enabled: true } } })).toBe(true);
    expect(() => isFeatureEnabled("ngawur", { config: {} })).toThrow(/tak dikenal/i);
    expect(isFeatureEnabled("ngawur", { config: {}, fallback: true })).toBe(true);
});
