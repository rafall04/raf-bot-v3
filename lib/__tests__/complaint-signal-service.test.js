...
// Default pool resolver: 192.168.61.0/24 → mni (lihat lib/upstream-path-resolver).
...
test("IP di luar semua pool / tak dikenal → klaster 'unknown' tetap bisa alert", async () => {
    const { deps, send } = makeDeps();
    for (let i = 1; i <= 3; i += 1) {
