 ...
 *          ISP (overall + trafik agregat), PSB (belum-kepasang+terpasang+komisi), Tiket (aktif/belum-diambil/
 *          lama/per-status), OLT (outage+offline), Pelanggan (aktif/isolir/baru/PPPoE online), Perlu Tindakan
 *          (bukti bayar/ganti paket/topup/approval), gate ISP off, dan ISOLASI kegagalan per kartu.
 * Caller: jest.
...
const DEPS = {
    ...
    getPppStats: async () => ({ ok: true, data: { online: 120, offline: 8, total: 128 } }),
    getUsers: () => [
        { status: "aktif", paid: 1, created_at: "2026-07-05", price: 100000 },
        { status: "aktif", paid: 0, created_at: "2026-06-01", price: 150000 },
        { status: "isolir", paid: 0, created_at: "2026-01-01", price: 100000 },
...
test("cockpit diperkaya: semua kartu + metrik baru benar", async () => {
    const c = await buildCockpit(DEPS);

    // Pemasukan: net_paid, today, MRR (active non-infra), pelunasan, tunggakan
    ...
    // ISP: overall + trafik agregat
    expect(c.isp).toMatchObject({ ok: true, overall: "WARN", rxMbps: 73.7, txMbps: 4.6 });

    // PSB: belum kepasang + terpasang + komisi
    expect(c.psb).toMatchObject({ ok: true, belumKepasang: 5, terpasangBulanIni: 7, komisiBulanIni: 150000 });

    // Tiket: aktif/belum-diambil/lama + per-status
    expect(c.tickets).toMatchObject({ ok: true, active: 3, belumDiambil: 1, lama: 1 });
    expect(c.tickets.byStatus).toMatchObject({ process: 1, reported: 1, otw: 1 });

    // OLT: outage + offline (dari PPP)
    expect(c.olt).toMatchObject({ ok: true, activeOutage: 1, offline: 8 });

    // Pelanggan: aktif/isolir/baru/PPPoE online (infra dikecualikan)
    expect(c.customers).toMatchObject({ ok: true, total: 3, aktif: 2, isolir: 1, baru: 1, pppoeOnline: 120 });
...
test("cockpit: PPP gagal → online/offline null tapi kartu tetap ok", async () => {
    ...
    expect(c.customers).toMatchObject({ ok: true, pppoeOnline: null });
    expect(c.olt).toMatchObject({ ok: true, offline: null });
});
