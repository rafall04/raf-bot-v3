...
describe('wifi-bulk-reconcile', () => {
    ...
    test('resolveModel fallback ke segmen device_id saat model bukan string', () => {
        // extractDeviceModel mengembalikan objek (path TR-069 tanpa _value) -> "[object Object]"
        const objModelDevice = {
            _id: 'd',
            InternetGatewayDevice: { DeviceInfo: { ModelName: { garbage: 1 } }, LANDevice: { 1: { WLANConfiguration: { 1: { SSID: 'X' } } } } },
        };
        expect(resolveModel(objModelDevice, 'AABBCC-HG8546M-0001')).toBe('HG8546M');
        expect(resolveModel(dualBandDevice('d'), 'X-Y-Z')).toBe('HG8145V5'); // model string tetap dipakai
        expect(resolveModel({}, 'noseg')).toBe('-');
    });
