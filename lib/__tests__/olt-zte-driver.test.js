...
describe('zte: parseRxPower', () => {
    ...
    test('di luar rentang fisik GPON (sampah/sentinel) → N/A', () => {
        // ONU "online" tak mungkin < -35 dBm (di bawah sensitivitas). Verifikasi live:
        // OLT ZTE keluarkan raw besar utk ONU tanpa DDM (mis. 6900→-69, 65311→-653).
        expect(parseRxPower(6900)).toBe('N/A');   // -69 dBm
        expect(parseRxPower(65311)).toBe('N/A');  // -653.11 dBm (sentinel)
        expect(parseRxPower(3600)).toBe('N/A');   // -36 dBm (tepat di luar batas -35)
        // Batas masih diterima:
        expect(parseRxPower(3494)).toBe('-34.94 dBm'); // -34.94 dBm (valid, mepet batas)
    });
});
