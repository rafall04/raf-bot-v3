...
describe('zte: parseRxPower (encoding raw/500-30, diverifikasi vs CLI)', () => {
    test('raw → dBm = raw/500 - 30 (cocok CLI show pon power)', () => {
        expect(parseRxPower(3406)).toBe('-23.19 dBm'); // CLI onu14 -23.188
        expect(parseRxPower(3741)).toBe('-22.52 dBm'); // CLI onu18 -22.518
        expect(parseRxPower(5105)).toBe('-19.79 dBm'); // CLI onu20 -19.790
    ...
    test('raw sampah (dBm > 0) → N/A', () => {
        expect(parseRxPower(65311)).toBe('N/A'); // → +100.6 dBm (sentinel near-65535)
        expect(parseRxPower(30000)).toBe('N/A'); // → +30 dBm (mustahil)
    });

    test('rentang valid GPON terisi (yang dulu salah di-N/A-kan)', () => {
        expect(parseRxPower(6900)).toBe('-16.20 dBm'); // dulu (-raw/100) salah jadi N/A
        expect(parseRxPower(1000)).toBe('-28.00 dBm');
    });
});
