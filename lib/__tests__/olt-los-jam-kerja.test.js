...
describe("#b269 — notif LOS pelanggan menyesuaikan jam kerja teknisi", () => {
    ...
    test("!! pesan menyebut RENTANG jam kerja, bukan satu jam mulai", async () => {
        // Ini permintaan pemilik: jam mulai membuat pelanggan menunggu di jam itu persis.
        // Rentang memberi teknisi ruang mendahulukan gangguan yang paling luas.
        waktuMulaiKerjaBerikutnya.mockReturnValue({
            dalamJamKerja: false,
            teks: "besok pukul 08:00 WIB",
            teksRentang: "besok pada jam kerja, pukul 08:00–17:00 WIB",
        });
        const teks = await kirimPesanPelanggan(NOTIFY);
        expect(teks).toMatch(/pada jam kerja, pukul 08:00–17:00 WIB/);
        expect(teks).toMatch(/dahulukan gangguan yang paling luas/);
    });

    test("slot {jam_kerja} tak pernah bocor mentah", async () => {
        waktuMulaiKerjaBerikutnya.mockReturnValue({
            dalamJamKerja: false, teks: "besok pukul 08:00 WIB", teksRentang: "besok pada jam kerja, pukul 08:00–17:00 WIB",
        });
        const teks = await kirimPesanPelanggan(NOTIFY);
        expect(teks).not.toMatch(/{jam_kerja}/);
        expect(teks).not.toMatch(/{waktu_mulai}/);
    });

    test("teksRentang kosong (jadwal tak terbaca) → jatuh ke jam mulai, bukan slot mentah", async () => {
        waktuMulaiKerjaBerikutnya.mockReturnValue({ dalamJamKerja: false, teks: "besok pukul 08:00 WIB", teksRentang: null });
        const teks = await kirimPesanPelanggan(NOTIFY);
        expect(teks).toMatch(/besok pukul 08:00 WIB/);
        expect(teks).not.toMatch(/{jam_kerja}/);
    });
