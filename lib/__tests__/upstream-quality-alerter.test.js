...
describe("upstream-quality-alerter", () => {
    ...
    test("buildAffectedLine: live (jumlah+nama) / unknown (fail-closed) / fallback statis / 0 pelanggan", () => {
        ...
        const live = bal(cfg, "mni", {
            path: "mni", count: 5, confidence: "live",
            customers: [{ name: "andik" }, { name: "budi" }, { name: "cici" }, { name: "dedi" }]
        });
        expect(live).toContain("*5 pelanggan*");
        expect(live).toContain("andik, budi, cici");
