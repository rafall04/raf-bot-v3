...
describe("upstream-quality-alerter", () => {
    ...
    // #b332 — jalur "irisan-layanan" (IH/FREE): trafik WA/game pelanggan MNI lewat sini, tapi
    // resolveCustomerPath memetakan mereka ke 'mni'. Hitung live utk 'ih' = ~0 walau semua kena.
    // "0 pelanggan" menyesatkan → tampilkan anotasi cakupan. Opt-in serviceSlice=true.
    test("buildAffectedLine: jalur serviceSlice + 0 live → anotasi cakupan, BUKAN '0 pelanggan'", () => {
        const bal = alerter._internal.buildAffectedLine;
        const cfg = {
            paths: [{ key: "ih", label: "IndiHome direct", affects: "trafik WA/game pelanggan 110k-125k", serviceSlice: true }]
        };
        const line = bal(cfg, "ih", { path: "ih", count: 0, confidence: "live", customers: [] });
        expect(line).toContain("trafik WA/game pelanggan 110k-125k");
        expect(line).toContain("irisan-layanan");
        expect(line).not.toContain("*0 pelanggan*");
    });

    test("buildAffectedLine: jalur NON-serviceSlice + 0 live → tetap jujur '0 pelanggan'", () => {
        const bal = alerter._internal.buildAffectedLine;
        const cfg = { paths: [{ key: "sf", label: "SF backup", affects: "cadangan", serviceSlice: false }] };
        expect(bal(cfg, "sf", { path: "sf", count: 0, confidence: "live", customers: [] })).toContain("*0 pelanggan*");
    });
