...
describe("ringkasan uang: membaca, bukan menghitung ulang", () => {
    ...
    test("proyeksi sisa = masuk - keluar - tagihan rutin yang belum tuntas", async () => {
        const data = await buildMoneySummary({
            ...
            deps: depsLengkap({
                ...
                // #b311: "keluar"/"sisa" kini dari BUKU BESAR. Buku besar konsisten dgn expense (debit
                // 500rb yang sama tersync) → angka sama, tapi sumbernya kini ledger.
                ledger: ledgerPalsu([{ direction: "debit", amount: 500000 }]),
                recurring: {
                    listAll: async () => [
    ...
    test("#b311 (T3/T6): keluar/sisa memakai BUKU BESAR (termasuk gaji), bukan expense_entries saja", async () => {
        const data = await buildMoneySummary({
            period: { periodMonth: 8, periodYear: 2026 },
            deps: depsLengkap({
                // Kas biasa (expense_entries) hanya 500rb...
                expenseManager: { listExpenses: async () => [{ category: "operasional", amount: 500000 }] },
                // ...tapi buku besar juga memuat GAJI dibayar 600rb (technician_payroll_paid) yang TAK
                // pernah jadi expense_entry. Dulu WA menghilangkan gaji ini; kini ikut terhitung.
                ledger: ledgerPalsu([
                    { direction: "credit", amount: 12500000 },
                    { direction: "debit", amount: 500000 },
                    { direction: "debit", amount: 600000 }
                ])
            })
        });
        expect(data.keluar).toBe(500000);        // rincian per-kategori (expense_entries)
        expect(data.keluarTotal).toBe(1100000);  // headline kanonik: buku besar (kas + gaji)
        expect(data.sisa).toBe(12500000 - 1100000);
        const teks = String(buildMoneySummaryText(data));
        expect(teks).toContain("1.100.000"); // "Sudah keluar" menampilkan angka buku besar
    });
