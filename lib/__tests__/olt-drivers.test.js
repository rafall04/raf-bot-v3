...
describe('hioso driver (baca lewat WEB — #b283)', () => {
    // Dulu driver ini mendelegasi ke `olt-hioso` yang berjalan di atas SNMP. SNMP membuat
    // OLT HIOSO hang, jadi sumbernya dipindah ke web. Perilaku yang dijaga di sini tetap
    // sama: hasil `success` dinormalisasi (mis. `serial` diisi null, bukan dibiarkan hilang).
    test('getOltData menormalisasi onus (menambah serial) saat success', async () => {
        jest.resetModules();
        jest.doMock('../olt-web-optical', () => ({
            getWebOpticalSnapshot: jest.fn().mockResolvedValue({
                status: 'success',
                failedOlts: [],
        ...
        jest.doMock('../olt-manager', () => ({
            getOltDevices: () => [{ id: 1, host: '1.2.3.4', webUsername: 'u', webPassword: 'p' }],
        }));
