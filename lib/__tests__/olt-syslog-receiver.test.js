...
describe('lib/olt-syslog-receiver', () => {
    ...
    describe('packet handler', () => {
        ...
        test('Lost following DG writes DG event to file', () => {
            ...
            receiver._flushForTest(); // persist di-batch (debounced) → flush manual di test
        ...
        test('BURST: banyak event di-batch jadi SATU write (anti event-loop stall saat mass-outage)', () => {
            const { receiver } = setupReceiver();
            // 50 ONU LOS serempak (grace=0 → emit langsung). Tanpa batching = 50 write.
            for (let i = 0; i < 50; i++) {
                const mac = `c0:f6:ec:00:00:${String(i).padStart(2, '0')}`;
                const lost = `<14>Jan 18 14:53:59 OLT EPON: Slot 0/1/1:${i} Onu ${mac}[Na] Lost`;
                receiver._handleMessageForTest(Buffer.from(lost), { address: '10.0.0.5' });
            }
            // Belum di-flush → belum ada write (semua ter-stage di memori).
            expect(fs.writeFileSync).not.toHaveBeenCalled();
            receiver._flushForTest();
            // Satu flush → SATU write berisi seluruh 50 event.
            expect(fs.writeFileSync).toHaveBeenCalledTimes(1);
            const events = JSON.parse(fs.writeFileSync.mock.calls[0][1]);
            expect(Object.keys(events)).toHaveLength(50);
        });
