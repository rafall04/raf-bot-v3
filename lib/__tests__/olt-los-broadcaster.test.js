// #b355: prefs per-teknisi dikontrol via map; defaultTeknisiRecipients menyaring lewat resolver ASLI.
const PREFS = {};
...
const { createLosBroadcaster, defaultTeknisiRecipients } = require("../olt-los-broadcaster");
...
describe("defaultTeknisiRecipients — filter kelas LOS (#b355)", () => {
    const savedAccounts = global.accounts;
    const savedConfig = global.config;
    afterEach(() => { global.accounts = savedAccounts; global.config = savedConfig; for (const k of Object.keys(PREFS)) delete PREFS[k]; });

    test("gate OFF → semua teknisi (perilaku lama, string telepon)", () => {
        global.config = {};
        global.accounts = [
            { id: "t1", role: "teknisi", phone_number: "081200000001" },
            { id: "t2", role: "teknisi", phone_number: "081200000002" },
        ];
        expect(defaultTeknisiRecipients().sort()).toEqual(["081200000001", "081200000002"]);
    });

    test("gate ON → teknisi dgn alerts.los=false DIKELUARKAN dari broadcast LOS", () => {
        global.config = { teknisiPrefs: { enabled: true } };
        global.accounts = [
            { id: "t1", role: "teknisi", phone_number: "081200000001" },
            { id: "t2", role: "teknisi", phone_number: "081200000002" },
        ];
        PREFS.t1 = { alerts: { los: false } };
        expect(defaultTeknisiRecipients()).toEqual(["081200000002"]);
    });

    test("gate ON → area TAK menyaring LOS (kritis, lintas-ODP); hanya kelas & master", () => {
        global.config = { teknisiPrefs: { enabled: true } };
        global.accounts = [{ id: "t1", role: "teknisi", phone_number: "081200000001" }];
        PREFS.t1 = { areas: ["ODP-01"] }; // area lain, TAPI LOS tetap sampai (tak difilter area)
        expect(defaultTeknisiRecipients()).toEqual(["081200000001"]);
    });
});
