...
describe("#b288 — wiring cron", () => {
    ...
    test("!! panggilan MikroTik NON-FATAL — alarm yang sudah bekerja tak boleh ikut rusak", () => {
        // #b347: fetch sesi PPPoE di-hoist ke scope tick (caller 'cron.redaman') & dipakai bersama
        // (area-sendiri + resolveOlt.pppoeActive). Tetap dibungkus try/catch (MikroTik bisu → lanjut).
        const i = src.indexOf("getActivePPPoEUsers({ caller: 'cron.redaman' })");
