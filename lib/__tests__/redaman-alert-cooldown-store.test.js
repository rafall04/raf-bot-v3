...
describe("cron memakai store durabel ini, bukan Map in-memory", () => {
    test("redaman-check.js tak lagi memegang Map cooldown", () => {
        ...
        // Maksud guard ini: cooldown TIDAK boleh disimpan di Map in-memory (hilang saat restart).
        // Bukan melarang Map untuk apa pun — `redaman-check` memakai Map transien sebagai lookup
        // umur inform DALAM SATU RUN (#b260), yang memang tak perlu durabel. Yang dijaga adalah
        // Map yang hidup di MODULE-SCOPE (bertahan antar-run), bukan yang lahir & mati di dalam run.
        expect(src).not.toMatch(new RegExp("^(const|let|var)\s+\w+\s*=\s*new Map\(\)", "m"));
    });
});
