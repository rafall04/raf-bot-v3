...
describe("validateTargets", () => {
    test("valid → normalized; kosong/dobel/alamat rusak/terlarang → error", () => {
        // Bentuk target kini menyertakan `addresses` (#b266): satu layanan boleh punya banyak IP,
        // karena satu alamat bukan sampel yang sah untuk sebuah layanan. `address` tetap diisi
        // alamat pertama supaya pembaca lama tak pecah.
        expect(validateTargets([{ key: "google", label: "Google DNS", address: "8.8.4.4" }]).value)
            .toEqual([{ key: "google", label: "Google DNS", address: "8.8.4.4", addresses: ["8.8.4.4"] }]);
...
describe("applyUpstreamConfigPatch", () => {
    ...
    test("sukses: tulis config + live-apply + insiden; paths dimaterialisasi dari efektif", () => {
        ...
        expect(saved.upstreamMonitor.targets).toEqual([{ key: "quad9", label: "Quad9", address: "9.9.9.9", addresses: ["9.9.9.9"] }]);
