...
describe("classifyModemCandidate", () => {
    test("modem polos (tes@hw) → BARU & boleh dipakai", () => {
        const r = classifyModemCandidate(dev({ currentPPPUsername: "tes@hw" }), { users: [], activeUsernames: new Set() });
...
describe("enrichPreviousOwner", () => {
    ...
    test("modem BARU tak perlu lookup OLT sama sekali", async () => {
        ...
        const r = await describeCandidate(dev({ currentPPPUsername: "tes@hw" }), { users: [], activeUsernames: new Set() }, { oltRepository });
