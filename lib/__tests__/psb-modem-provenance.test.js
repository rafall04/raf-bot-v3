...
describe("gerbang lintas-area: modem yang sesinya hidup di router area lain", () => {
    ...
    const segar = new Date(NOW - 5 * 60 * 1000).toISOString();
    const basi = new Date(NOW - 38 * 24 * 60 * 60 * 1000).toISOString();  // pola ZTE 38 hari

    test("PPPoE asing + sesi hidup + inform SEGAR → ditahan untuk DITEGASKAN, bukan ditolak", () => {
        const hasil = classifyModemCandidate(
            dev({ currentPPPUsername: "mika_andriani-karang@rafcybernet", pppUptimeSeconds: 160466, lastInform: segar }),
            { ...KONTEKS_KOSONG, nowMs: NOW }
        );
        expect(hasil.state).toBe("terpakai");
        expect(hasil.assignable).toBe(false);
        expect(hasil.butuhKonfirmasi).toBe(true);   // teknisi yang memutuskan, bukan bot
    ...
    // Ini kasus yang membuat gerbang berbasis-uptime saja SALAH. Nilai ACS hanya sesegar inform
    // terakhir, jadi modem yang dicabut menyimpan uptime besar selamanya. Terukur di produksi:
    // uptime 425.221 detik pada modem yang sudah berhenti inform 38 HARI. Kalau uptime dipercaya
    // sendirian, modem bebas itu terblokir permanen.
    test("uptime besar TAPI inform sudah 38 hari basi → BEBAS dipakai (bukti kedaluwarsa)", () => {
        const hasil = classifyModemCandidate(
            dev({ currentPPPUsername: "muchammad_nur_chozin-karang@rafcybernet", pppUptimeSeconds: 425221, lastInform: basi }),
            { ...KONTEKS_KOSONG, nowMs: NOW }
        );
...
// ── Pola TUKAR MODEM ──────────────────────────────────────────────────────────────────────────
// Pelanggan A diganti modem baru; modem lamanya dipakai untuk PSB pelanggan lain. Saat teknisi
// memasangnya, kredensial lama masih tertulis di dalamnya — jadi bot melihat "milik pelanggan A".
// Pembedanya: baris pelanggan A menunjuk device_id modem BARU, bukan yang di tangan teknisi.
// Terbukti ada di ACS produksi (satu username di dua device; yang lama berhenti inform 38 hari).
describe("pola tukar modem: baris pelanggan menunjuk modem LAIN", () => {
    const NOW = Date.parse("2026-08-14T08:00:00.000Z");
    const segar = new Date(NOW - 5 * 60 * 1000).toISOString();
    const PELANGGAN = { name: "Muchammad Nur Chozin", pppoe_username: "chozin@rafcybernet", device_id: "dev-MODEM-BARU" };

    test("modem lama (device_id beda) + sesi aktif → DITEGASKAN, bukan ditolak", () => {
        const hasil = classifyModemCandidate(
            dev({ deviceId: "dev-MODEM-LAMA", currentPPPUsername: "chozin@rafcybernet", lastInform: segar }),
            { users: [PELANGGAN], activeUsernames: new Set(["chozin@rafcybernet"]), nowMs: NOW }
        );
        expect(hasil.assignable).toBe(false);
        expect(hasil.butuhKonfirmasi).toBe(true);
        expect(hasil.deviceTercatatPemilik).toBe("dev-MODEM-BARU");
        expect(hasil.reason).toMatch(/modem LAIN/);
    });

    test("modem yang MEMANG tercatat utk pelanggan itu + masih terlihat → TOLAK KERAS", () => {
        const hasil = classifyModemCandidate(
            dev({ deviceId: "dev-MODEM-BARU", currentPPPUsername: "chozin@rafcybernet", lastInform: segar }),
            { users: [PELANGGAN], activeUsernames: new Set(), nowMs: NOW }
        );
        expect(hasil.assignable).toBe(false);
        expect(hasil.butuhKonfirmasi).toBe(false);   // tak ada yang perlu ditanyakan — ini modemnya
    });

    test("modem tercatat pelanggan TAPI terbukti lama hilang → turun jadi penegasan", () => {
        const hasil = classifyModemCandidate(
            dev({ deviceId: "dev-MODEM-BARU", currentPPPUsername: "chozin@rafcybernet", lastInform: new Date(NOW - 40 * 60 * 1000).toISOString() }),
            { users: [PELANGGAN], activeUsernames: new Set(), nowMs: NOW }
        );
        expect(hasil.butuhKonfirmasi).toBe(true);
    });

    test("lastInform TAK TERBACA bukan alasan melonggarkan — tetap TOLAK KERAS", () => {
        // Ketiadaan data ≠ bukti modem sudah bebas. Baris pelanggan sudah cukup jadi alasan menolak.
        const hasil = classifyModemCandidate(
            dev({ deviceId: "dev-MODEM-BARU", currentPPPUsername: "chozin@rafcybernet" }),
            { users: [PELANGGAN], activeUsernames: new Set(), nowMs: NOW }
        );
        expect(hasil.assignable).toBe(false);
        expect(hasil.butuhKonfirmasi).toBe(false);
    });
});
