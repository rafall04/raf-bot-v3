...
describe("extractAppEntities — korpus verbatim", () => {
    ...
    test("app tambahan: Telegram/ojol/musik/discord + game & marketplace diperluas", () => {
        expect(topAppEntity("telegram gabisa kirim")).toMatchObject({ key: "telegram", kind: "chat" });
        expect(topAppEntity("gojek loading terus")).toMatchObject({ key: "ojol", serviceKey: "google", symptom: "buffering" });
        expect(topAppEntity("google maps muter")).toMatchObject({ key: "ojol" });
        expect(topAppEntity("spotify putus putus")).toMatchObject({ key: "musik", kind: "streaming" });
        expect(topAppEntity("discord ngelag pas mabar")).toMatchObject({ key: "discord", kind: "call", symptom: "lag" });
        expect(topAppEntity("mau disko malam ini")).toBe(null); // "disko" (dansa) BUKAN discord
        expect(topAppEntity("point blank ngelag")).toMatchObject({ key: "game", kind: "game" });
        expect(topAppEntity("codm ping tinggi")).toMatchObject({ key: "game", symptom: "lag" });
        expect(topAppEntity("bukalapak gabisa checkout")).toMatchObject({ key: "shopee" });
    });

    test("gejala dialek Jawa dikenali: medhot/mandek/alon/ora mlebu/ngebuffer", () => {
        expect(topAppEntity("youtube medhot terus")).toMatchObject({ key: "youtube", symptom: "putus" });
        expect(topAppEntity("tiktok alon banget")).toMatchObject({ key: "tiktok", symptom: "lambat" });
        expect(topAppEntity("ig ora mlebu")).toMatchObject({ key: "instagram", symptom: "gagal" });
        expect(topAppEntity("netflix ngebuffer")).toMatchObject({ key: "streaming", symptom: "buffering" });
    });
});
