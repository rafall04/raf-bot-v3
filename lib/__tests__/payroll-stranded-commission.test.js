...
describe("komisi periode lain yang belum pernah masuk payroll harus terbaca", () => {
    ...
    test("periode yang payroll-nya SUDAH ada ditandai has_payroll (walau masih draft)", async () => {
        // Beda yang menentukan: uang di periode ini SUDAH terlihat di tabel halaman, tinggal
        // difinalisasi. Menandainya sebagai "hilang" membuat peringatan menyala untuk hal yang
        // kelihatan — dan peringatan yang sering salah nyala akan berhenti dibaca.
        await jalankan(
            `INSERT INTO technician_gaji (teknisi_id, teknisi_name, period_month, period_year, gaji_pokok, status, created_at)
             VALUES (?, 'Teknisi Uji', 7, 2026, 250000, 'draft', datetime('now'))`,
            [TEKNISI]
        );
        const per = await financeService.getUnsettledCollectionByPeriod({ teknisiId: TEKNISI });
        const juli = per.find((r) => Number(r.period_month) === 7);
        const juni = per.find((r) => Number(r.period_month) === 6);
        expect(Number(juli.has_payroll)).toBe(1);
        expect(Number(juni.has_payroll)).toBe(0);
    });

    test("teknisi_id INTEGER di payroll tetap cocok dengan TEXT di ledger", async () => {
        // Kedua tabel menyimpan id teknisi dengan tipe BERBEDA (technician_gaji INTEGER,
        // ledger TEXT). Tanpa CAST, join-nya diam-diam tak pernah cocok dan has_payroll
        // selalu 0 — peringatan lalu menyala untuk semua periode.
        const src = baca("lib", "technician-finance-service.js");
        expect(src).toMatch(/CAST\(g\.teknisi_id AS TEXT\) = CAST\(l\.teknisi_id AS TEXT\)/);
    });
...
describe("jalur tampil: route dan halaman benar-benar membawa peringatannya", () => {
    ...
    test("periode yang payroll-nya sudah ada juga dikecualikan", () => {
        expect(route).toMatch(/!Number\(r\.has_payroll\)/);
    });
