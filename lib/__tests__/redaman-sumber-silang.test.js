...
describe("#b277 — memilih angka redaman dari ACS + OLT", () => {
    ...
    test("selisih khas lapangan (s/d p90 = 1,02 dB) TIDAK dianggap mencurigakan", () => {
        for (const [acs, olt] of [[-18, -18.93], [-23, -23.28], [-20, -20.99]]) {
    ...
    test("!! beda > 1,5 dB → DITAHAN, ditandai masalah pemetaan (bukan optik)", () => {
    ...
    test("ambang berada DI ATAS p90 terukur, bukan di bawahnya", () => {
        expect(AMBANG_BEDA_DB).toBeGreaterThan(1.02);
        expect(AMBANG_BEDA_DB).toBeLessThanOrEqual(3);
    });

    test("tepat di ambang belum mencurigakan; sedikit di atasnya baru ya", () => {
        expect(pilihNilaiRedaman({ acs: -20, olt: -21.5 }).integritasMencurigakan).toBe(false);
        expect(pilihNilaiRedaman({ acs: -20, olt: -21.6 }).integritasMencurigakan).toBe(true);
    });
    ...
    test("ambang bisa disetel pemanggil", () => {
        expect(pilihNilaiRedaman({ acs: -20, olt: -22, ambangBedaDb: 3 }).integritasMencurigakan).toBe(false);
        expect(pilihNilaiRedaman({ acs: -20, olt: -22, ambangBedaDb: 1 }).integritasMencurigakan).toBe(true);
    });
});
