...
function makeDeps({ entries, isp24 = [], isp7 = [], incidents = [], alertStates = {} } = {}) {
    return {
        ...
        // Default TANPA data terdampak (cegah I/O MikroTik nyata via defaultDeps.getAffectedSet).
...
test("data <isp>: getAffectedSet LIVE diteruskan ke buildDirectionText (baris Terdampak akurat)", async () => {
    let received;
    const deps = makeDeps({ entries: [entryFor("mni", { status: "GANGGUAN", segment: "UPSTREAM_ISP", loss: 45, rtt: 90 })] });
    deps.getAffectedSet = async () => ({ path: "mni", count: 20, confidence: "live", customers: [] });
    deps.buildDirectionText = (cfg, pathKey, entry, affected) => { received = affected; return "\n[ARAH]"; };
    await summary.buildIspDataSummary("mni", deps);
    expect(received).toEqual({ path: "mni", count: 20, confidence: "live", customers: [] });
});
...
test("toggle seksi report: rincianArah/layananPopuler/tujuhHari/insidenTerakhir OFF → bloknya hilang", async () => {
    const base = makeDeps({
        entries: [entryFor("gmdp", { status: "NORMAL", loss: 0.5 })],
        isp24: [{ path: "gmdp", availability_pct: 100, loss_avg: 0.7, rtt_avg: 17, sick_pct: 3, flaps: 0 }],
        isp7: [{ path: "gmdp", availability_pct: 99.6, loss_avg: 1, rtt_avg: 17, sick_pct: 3, flaps: 0 }],
        incidents: [{ path: "gmdp", kind: "alert", created_at: new Date(NOW - 60 * 60 * 1000).toISOString() }]
    });
    const cfgOff = { ...CFG, report: { sections: { rincianArah: false, layananPopuler: false, tujuhHari: false, insidenTerakhir: false } } };
    const teks = await summary.buildIspDataSummary("gmdp", { ...base, getMonitorConfig: () => cfgOff });
    expect(teks).not.toContain("[ARAH]");      // buildDirectionText di-gate rincianArah
    expect(teks).not.toContain("[LAYANAN]");   // buildServiceSection di-gate layananPopuler
    expect(teks).not.toContain("7 HARI");
    expect(teks).not.toContain("Insiden terakhir");
    expect(teks).toContain("24 JAM TERAKHIR"); // blok inti tetap tampil
});

test("tanpa config report → semua seksi tampil (default ON, kompat mundur)", async () => {
    const deps = makeDeps({
        entries: [entryFor("gmdp", { status: "NORMAL", loss: 0.5 })],
        isp24: [{ path: "gmdp", availability_pct: 100, loss_avg: 0.7, rtt_avg: 17, sick_pct: 3, flaps: 0 }],
        isp7: [{ path: "gmdp", availability_pct: 99.6, loss_avg: 1, rtt_avg: 17, sick_pct: 3, flaps: 0 }]
    });
    const teks = await summary.buildIspDataSummary("gmdp", deps);
    expect(teks).toContain("[ARAH]");
    expect(teks).toContain("[LAYANAN]");
    expect(teks).toContain("7 HARI");
});
