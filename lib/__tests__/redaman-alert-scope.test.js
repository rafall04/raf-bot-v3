...
describe("menyaring tak boleh membuat modem hilang diam-diam", () => {
    ...
    test("remah-remah TIDAK menambah panggilan ke ACS (nilai ikut di kueri daftar)", () => {
        ...
        // `_lastInform` boleh ikut di projection yang SAMA (#b260): dipakai melewati modem yang
        // sedang diam, dan menumpang kueri ini justru MENGHINDARI panggilan tambahan — persis
        // maksud guard ini. Yang dilarang tetap: REDAMAN_PATHS hilang dari kueri daftar.
        // `...PPP_PATHS` juga boleh menumpang (#b288): username PPPoE dipakai mengenali modem
        // di area kita yang belum didaftarkan, dan menumpang kueri ini persis maksud guard ini.
        expect(src).toMatch(/projection:\s*\['_id',(\s*'_lastInform',)?\s*\.\.\.REDAMAN_PATHS(,\s*\.\.\.PPP_PATHS)?\]/);
        // Yang tetap DILARANG: kueri KEDUA ke ACS hanya untuk remah-remah ini.
        const jumlahQuery = (src.match(/await queryDevices\(/g) || []).length;
        expect(jumlahQuery).toBeLessThanOrEqual(2);   // 1 daftar + 1 setelah refresh
...
describe("#b260 — modem yang DIAM tidak boleh di-refresh (tugasnya mengendap selamanya)", () => {
    const fs = require("fs");
    const path = require("path");
    const src = fs.readFileSync(path.join(__dirname, "..", "cron", "jobs", "redaman-check.js"), "utf8");

    test("cron menyaring berdasarkan umur inform sebelum me-refresh", () => {
        // TERUKUR: satu modem yang DIGANTI 6 Juli mengumpulkan 3.797 tugas sampai 23 Agustus,
        // karena cron ini menambah ~48/hari ke record yang tak akan pernah menjawab. GenieACS
        // tidak punya kedaluwarsa tugas, jadi antreannya tumbuh selamanya.
        expect(src).toMatch(/_lastInform/);
        // !! Dipatok ke penanda KHAS saringan inform, bukan `deviceIDs.filter` — pola itu juga
        // cocok dengan saringan KEPEMILIKAN di atasnya, sehingga guard-nya lulus hampa walau
        // saringan inform dicabut (terbukti saat diuji-mutasi).
        expect(src).toMatch(/batasInformMs/);
        expect(src).toMatch(/umurInform/);
        expect(src).toMatch(/umur <= batasInformMs/);
    });

    test("ambangnya MEMINJAM konstanta bersama, bukan angka baru yang ditulis di sini", () => {
        // Ambang inform sudah punya satu pemilik (#b257). Menulis angka sendiri di sini adalah
        // persis cara pelajaran itu dulu gagal menyebar: diperbaiki di satu modul, terlewat di
        // modul lain, dan jalur pelanggan tetap memakai angka yang sudah terbukti salah.
        expect(src).toMatch(/DEFAULT_MAX_INFORM_MINUTES/);
        expect(src).toMatch(/require\(['"]\.\.\/\.\.\/device-status['"]\)/);
    });

    test("melewati modem TIDAK boleh senyap — wajib dicetak", () => {
        // "Dilewati karena diam" terlihat sama persis dengan "diperiksa dan sehat" kalau tak
        // dicetak — aturan anti-bisu yang sama dengan penyaringan kepemilikan di atasnya.
        expect(src).toMatch(/CRON_REDAMAN_LEWAT_DIAM/);
    });
});
