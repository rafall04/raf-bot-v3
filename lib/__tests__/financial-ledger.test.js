...
describe('financial ledger', () => {
  ...
  afterEach(async () => {
    ...
    // Reader dipakai bersama & sengaja berumur panjang (lib/sqlite-shared-reader), jadi ia
    // memegang file DB sampai ditutup eksplisit — tanpa ini rmSync gagal EBUSY di Windows.
...
describe('classifyLedgerCashflow — domain tak dikenal disuarakan (#b308)', () => {
  const { classifyLedgerCashflow } = require('../financial-ledger');

  test('domain BARU yang belum terdaftar → warning + classification "unknown", angka dipertahankan', () => {
    const warn = jest.spyOn(console, 'warn').mockImplementation(() => {});
    const credit = classifyLedgerCashflow({ domain: 'domain_baru_lupa_daftar', direction: 'credit', amount: 5000 });
    expect(credit).toEqual({ incomeDelta: 5000, expenseDelta: 0, classification: 'unknown' });
    const debit = classifyLedgerCashflow({ domain: 'domain_baru_lupa_daftar', direction: 'debit', amount: 5000 });
    expect(debit).toEqual({ incomeDelta: 0, expenseDelta: 5000, classification: 'unknown' });
    expect(warn).toHaveBeenCalled();
    warn.mockRestore();
  });

  test('domain yang SUDAH dikenal tidak memicu warning', () => {
    const warn = jest.spyOn(console, 'warn').mockImplementation(() => {});
    classifyLedgerCashflow({ domain: 'customer_payment', direction: 'credit', amount: 5000 });
    classifyLedgerCashflow({ domain: 'technician_collection_commission', direction: 'credit', amount: 5000 });
    classifyLedgerCashflow({ domain: 'expense_entry', direction: 'debit', amount: 5000 });
    expect(warn).not.toHaveBeenCalled();
    warn.mockRestore();
  });

  test('#b310 (T16): voucher berbayar SALDO = internal (tak double-count topup); non-SALDO = pemasukan', () => {
    // SALDO: uangnya sudah dihitung saat topup → jangan hitung lagi.
    expect(classifyLedgerCashflow({ domain: 'voucher_purchase', direction: 'credit', amount: 20000, payment_method: 'SALDO' }))
      .toEqual({ incomeDelta: 0, expenseDelta: 0, classification: 'internal' });
    // Tunai/gateway: pemasukan usaha nyata.
    expect(classifyLedgerCashflow({ domain: 'voucher_purchase', direction: 'credit', amount: 20000, payment_method: 'TUNAI' }))
      .toEqual({ incomeDelta: 20000, expenseDelta: 0, classification: 'cash_in' });
  });

  test('#b346: domain "tagihan" (kelebihan bayar) pending_review → internal (BUKAN pemasukan), tanpa warning', () => {
    const warn = jest.spyOn(console, 'warn').mockImplementation(() => {});
    // pending_review = uang belum diputuskan (refund vs dipakai periode depan) → jangan gelembungkan laba.
    expect(classifyLedgerCashflow({ domain: 'tagihan', referenceType: 'kelebihan_bayar', direction: 'credit', amount: 75000, status: 'pending_review' }))
      .toEqual({ incomeDelta: 0, expenseDelta: 0, classification: 'internal' });
    // Sudah diputuskan (status non-pending) → baru dihitung pemasukan.
    expect(classifyLedgerCashflow({ domain: 'tagihan', direction: 'credit', amount: 75000, status: 'kept' }))
      .toEqual({ incomeDelta: 75000, expenseDelta: 0, classification: 'cash_in' });
    // Refund manual (debit) → pengeluaran (koreksi lawan).
    expect(classifyLedgerCashflow({ domain: 'tagihan', direction: 'debit', amount: 75000, status: 'refunded' }))
      .toEqual({ incomeDelta: 0, expenseDelta: 75000, classification: 'cash_out' });
    // Domain 'tagihan' kini TERDAFTAR → tak lagi memicu LEDGER_UNKNOWN_DOMAIN.
    expect(warn).not.toHaveBeenCalled();
    warn.mockRestore();
  });
});
