...
describe("setelan BENAR-BENAR mengubah yang tercetak (sumbu yang dulu terlewat)", () => {
    test("'cash' TIDAK mencantumkan Transfer Bank", () => {
        const blok = blokMetode(generateInvoiceHTML(buatInvoice(), { showPaymentMethods: true, paymentMethods: "cash" }));
        expect(blok).toContain("Tunai");
        expect(blok).not.toContain("Transfer Bank");
    });

    test("'transfer' TIDAK mencantumkan Tunai", () => {
        const blok = blokMetode(generateInvoiceHTML(buatInvoice(), { showPaymentMethods: true, paymentMethods: "transfer" }));
    ...
    test("'cash_transfer' mencantumkan keduanya, tanpa Pembayaran Online", () => {
        const blok = blokMetode(generateInvoiceHTML(buatInvoice(), { showPaymentMethods: true, paymentMethods: "cash_transfer" }));
        expect(blok).toContain("Tunai");
        expect(blok).toContain("Transfer Bank");
    ...
    test("'all' mencantumkan ketiganya", () => {
        const blok = blokMetode(generateInvoiceHTML(buatInvoice(), { showPaymentMethods: true, paymentMethods: "all" }));
        expect(blok).toContain("Tunai");
        expect(blok).toContain("Transfer Bank");
...
describe("nomor rekening yang sampai ke pelanggan", () => {
    ...
    test("JATUH ke bankAccounts jamak bila tunggal kosong — beda nama field ikut ditangani", () => {
        ...
        const blok = blokMetode(generateInvoiceHTML(inv, { showPaymentMethods: true, paymentMethods: "transfer" }));
    ...
    test("Transfer Bank TIDAK diklaim diterima bila tak ada satu pun rekening", () => {
        ...
        const blok = blokMetode(generateInvoiceHTML(inv, { showPaymentMethods: true, paymentMethods: "cash_transfer" }));
        expect(blok).not.toContain("Transfer Bank");
        expect(blok).toContain("Tunai");
    });
...
describe("blok 'Metode Pembayaran yang Diterima' hanya untuk invoice sebagai TAGIHAN", () => {
    const { buatCustomizationInvoice } = require("../invoice-generator");

    test("BAWAANNYA MATI — invoice di sistem ini selalu bukti LUNAS", () => {
        // createInvoice menetapkan payment.status='PAID' tanpa syarat, dan kedua
        // pemanggilnya baru jalan SESUDAH uang diterima. Terukur: 35 dari 35 record
        // produksi Tanjungharjo berstatus PAID. Mendaftarkan cara membayar beserta
        // nomor rekening di dokumen yang barusan bilang "LUNAS" hanya membingungkan.
        const c = buatCustomizationInvoice({});
        expect(c.showPaymentMethods).toBe(false);

        const html = generateInvoiceHTML(buatInvoice(), c);
        expect(html).not.toContain("Metode Pembayaran yang Diterima");
        // Baris "Metode:" yang menyatakan cara bayar SEBENARNYA tetap ada.
        expect(html).toContain("Metode:");
    });

    test("bisa dinyalakan dari halaman pengaturan", () => {
        const c = buatCustomizationInvoice({ pdfCustomization: { showPaymentMethods: true } });
        expect(c.showPaymentMethods).toBe(true);
        const html = generateInvoiceHTML(buatInvoice(), c);
        expect(html).toContain("Metode Pembayaran yang Diterima");
    });

    test("nilai form berupa string 'true'/'false' ikut dipahami", () => {
        expect(buatCustomizationInvoice({}, { showPaymentMethods: "true" }).showPaymentMethods).toBe(true);
        expect(buatCustomizationInvoice({}, { showPaymentMethods: "false" }).showPaymentMethods).toBe(false);
    });

    test("saat mati, nomor rekening TIDAK ikut tercetak", () => {
        const inv = buatInvoice({ bankAccounts: [{ bank: "BCA", number: "8640114", name: "RAF NET" }] });
        const html = generateInvoiceHTML(inv, buatCustomizationInvoice({}));
        expect(html).not.toContain("8640114");
    });
});
