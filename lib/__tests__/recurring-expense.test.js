...
const { listExpenses, cancelExpense } = require("../expense-manager");
...
describe("biaya rutin", () => {
    ...
    test("#b310 (T18): batal expense biaya rutin me-reset penanda → bisa dikonfirmasi/diingatkan lagi", async () => {
        const it = await buatItem();
        await recurring.tandaiDiingatkan(it.id);
        const { expense } = await recurring.konfirmasi(it.id);
        expect((await recurring.cari(it.id)).last_settled_period).toBe(PERIODE);
        // expense tertaut ke biaya rutin sumbernya (#b310).
        expect(expense.source_recurring_id).toBe(it.id);

        // Batalkan (mis. salah nominal) → penanda periode HARUS lepas, bukan tertinggal "tuntas".
        await cancelExpense(expense.id, { name: "Uji" }, "salah nominal");
        const after = await recurring.cari(it.id);
        expect(after.last_settled_period).toBeNull();
        expect(after.last_reminded_period).toBeNull();
        // Muncul lagi sebagai jatuh tempo (bisa dikonfirmasi ulang).
        expect(await recurring.jatuhTempoHariIni()).toHaveLength(1);
    });
