...
describe('lib/mikrotik', () => {
    ...
    test('M3: addpppoe uses POST with JSON body (password NOT in URL query)', async () => {
        global.config = { site_url_bot: 'http://localhost' };
        getAxiosMock().post.mockResolvedValue({ status: 200, data: { data: {} } });

        const { addpppoe } = require('../mikrotik');
        await addpppoe('user1', 'SecretPass!', 'PROFILE_A');

        expect(getAxiosMock().get).not.toHaveBeenCalled();
        const [url, body, opts] = getAxiosMock().post.mock.calls[0];
        expect(url).toBe('http://localhost/adduserpppoe.php');
        expect(body).toEqual({ user: 'user1', pw: 'SecretPass!', profil: 'PROFILE_A' });
        // Pastikan tidak ada params di config (yang akan jadi URL query).
