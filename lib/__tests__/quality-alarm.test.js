...
describe("#b255 — alarm kestabilan", () => {
    ...
    test("penyaring jalur: hanya jalur yang didaftar yang boleh membangunkan admin", () => {
        // TERUKUR ~1,0-1,4 alarm/hari PER JALUR. Dander punya 5 jalur, sebagian cadangan yang
        // memang kronis — tanpa penyaring, alarm jadi kebisingan harian dan berhenti dibaca.
        const cfg = {
            paths: [
                { key: "gmdp", label: "Jalur Utama" },
                { key: "sf", label: "Jalur Cadangan Kronis" }
            ],
            alarmKestabilan: { enabled: true, consecutiveCycles: 1, cooldownMinutes: 120, paths: ["gmdp"] }
        };
        const { deps, dikirim } = buatDeps({ rows: enam(20, 30) });
        return evaluasiKestabilan(cfg, deps).then(() => {
            expect(dikirim.length).toBe(1);
            expect(dikirim[0].teks).toMatch(/Jalur Utama/);
            expect(dikirim[0].teks).not.toMatch(/Cadangan/);
        });
    });

    test("daftar jalur KOSONG = semua jalur (menambah kunci tak boleh mematikan alarm lama)", () => {
        const cfg = {
            paths: [{ key: "gmdp", label: "Jalur Utama" }, { key: "sf", label: "Cadangan" }],
            alarmKestabilan: { enabled: true, consecutiveCycles: 1, cooldownMinutes: 120, paths: [] }
        };
        const { deps, dikirim } = buatDeps({ rows: enam(20, 30) });
        return evaluasiKestabilan(cfg, deps).then(() => expect(dikirim.length).toBe(2));
    });
