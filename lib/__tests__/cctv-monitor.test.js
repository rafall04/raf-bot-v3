...
function setup({ enabled = true, devices = [], confirmationMinutes = 1, cooldown = 30*60_000, notifyRecovery = true, massOutageThreshold = 0, massOutageAdminPhone = '', quietHoursEnabled = false, quietStart = '22:00', quietEnd = '06:00', quietApplyCustomer = true, quietApplyCoordinator = true, quietApplyGroup = true, coordinators = {}, seedIncidents = [] } = {}) {
    let nw = [];
    const sent = [];
    ...
    const m = createCctvMonitor({
        getConfig: () => ({ enabled, pollIntervalMs: 60_000, confirmationMinutes, rebroadcastCooldownMs: cooldown, notifyRecovery, massOutageThreshold, massOutageAdminPhone, quietHoursEnabled, quietStart, quietEnd, quietApplyCustomer, quietApplyCoordinator, quietApplyGroup }),
        getDevices: () => devices,
...
describe('cctv-monitor: semantik transisi', () => {
    ...
    test('anti tak-sinkron: pulih saat DOWN masih di antrean agregasi → DOWN dibatalkan, tak ada UP nyusul', async () => {
        ...
        expect(t.sent.filter(x => x.label === 'cctv_down')).toHaveLength(0);
        t.setNetwatch([{ host: '10.0.0.1', status: 'up' }]);
        await t.m._pollOnceForTest(); // pulih SAAT DOWN masih di antrean
        t.fireKind('batch'); // jendela agregasi habis → flush
        expect(t.sent.filter(x => x.label === 'cctv_down')).toHaveLength(0); // DOWN dibatalkan (tak nyusul setelah pulih)
        expect(t.sent.filter(x => x.label === 'cctv_up')).toHaveLength(0);   // tak ada UP (DOWN tak pernah keluar)
    });
