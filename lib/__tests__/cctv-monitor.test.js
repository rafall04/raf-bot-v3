...
describe('cctv-monitor: durabilitas restart & prune (#b337)', () => {
    const NOWMS = 1_700_000_000_000; // ≈2023-11-15 05:13 WIB

    test('#5: broadcasted TANPA deliveredAt + host masih down → DOWN dikirim ULANG (nyaris hilang restart)', async () => {
        const dev = { id: 'c1', name: 'CCTV X', host: '192.168.99.1', phone: '628111', customerName: 'Pak Joko', enabled: true };
        const t = setup({ devices: [dev], seedIncidents: [
            { incidentId: 'i1', host: '192.168.99.1', detectedAt: new Date(NOWMS - 20 * 60_000).toISOString(), broadcastedAt: new Date(NOWMS - 19 * 60_000).toISOString(), status: 'broadcasted' }, // TANPA deliveredAt
        ] });
        t.setNetwatch([{ host: '192.168.99.1', status: 'down' }]);
        await t.m._pollOnceForTest();     // restore → re-enqueue DOWN (deliveredAt kosong)
        t.fireKind('batch');              // flush agregasi
        expect(t.sent.filter(x => x.label === 'cctv_down')).toHaveLength(1);
    });

    test('#5: broadcasted DENGAN deliveredAt + host down → TIDAK kirim ulang (sudah terkirim)', async () => {
        const dev = { id: 'c1', name: 'CCTV X', host: '192.168.99.1', phone: '628111', customerName: 'Pak Joko', enabled: true };
        const t = setup({ devices: [dev], seedIncidents: [
            { incidentId: 'i1', host: '192.168.99.1', detectedAt: new Date(NOWMS - 20 * 60_000).toISOString(), broadcastedAt: new Date(NOWMS - 19 * 60_000).toISOString(), deliveredAt: new Date(NOWMS - 19 * 60_000).toISOString(), status: 'broadcasted' },
        ] });
        t.setNetwatch([{ host: '192.168.99.1', status: 'down' }]);
        await t.m._pollOnceForTest();
        t.fireKind('batch');
        expect(t.sent.filter(x => x.label === 'cctv_down')).toHaveLength(0); // tak dobel
    });

    test('#5: flushBatch menandai insiden deliveredAt saat DOWN benar-benar terkirim', async () => {
        const dev = { id: 'c1', name: 'CCTV X', host: '192.168.99.1', phone: '628111', enabled: true };
        const t = setup({ devices: [dev], confirmationMinutes: 1 });
        t.setNetwatch([{ host: '192.168.99.1', status: 'up' }]); await t.m._pollOnceForTest();
        t.setNetwatch([{ host: '192.168.99.1', status: 'down' }]); await t.m._pollOnceForTest();
        t.fireKind('confirm'); t.fireKind('batch'); // benar-benar terkirim
        const inc = t.incidents.find(i => i.status === 'broadcasted' || i.deliveredAt);
        expect(inc && inc.deliveredAt).toBeTruthy();
    });

    test('#6: broadcasted + jam tenang per-jenis + restart → penerima DITAHAN direkonstruksi & dikirimi saat pagi', async () => {
        const dev = { id: 'c1', name: 'CCTV X', host: '192.168.99.1', phone: '628111', customerName: 'Pak Joko', area: 'DANDER', enabled: true };
        const t = setup({
            devices: [dev],
            quietHoursEnabled: true, quietStart: '00:00', quietEnd: '06:00', // 05:13 WIB → dalam jam tenang
            quietApplyCustomer: true, quietApplyCoordinator: false,
            coordinators: { dander: { coordinatorPhone: '628RT', coordinatorName: 'Pak RT', enabled: true } },
            seedIncidents: [
                { incidentId: 'i1', host: '192.168.99.1', detectedAt: new Date(NOWMS - 20 * 60_000).toISOString(), broadcastedAt: new Date(NOWMS - 19 * 60_000).toISOString(), deliveredAt: new Date(NOWMS - 19 * 60_000).toISOString(), status: 'broadcasted' },
            ],
        });
        t.setNetwatch([{ host: '192.168.99.1', status: 'down' }]);
        await t.m._pollOnceForTest(); // restore → s.deferred={pelanggan} direkonstruksi (deliveredAt ada → tak re-send)
        expect(t.sent.filter(x => x.label === 'cctv_down')).toHaveLength(0); // pelanggan masih ditahan
        t.advance(60 * 60_000); // maju 1 jam → jam tenang berakhir (≈06:13 WIB)
        await t.m._pollOnceForTest(); // sweep → kirim pelanggan yang tertahan
        t.fireKind('batch');
        const downs = t.sent.filter(x => x.label === 'cctv_down');
        expect(downs.find(x => x.phone === '628111')).toBeTruthy(); // pelanggan AKHIRNYA dapat "mati"
    });

    test('#7: CCTV dihapus dari registry saat down → state di-prune (tak salah picu gangguan-massal)', async () => {
        const A = { id: 'a', name: 'CCTV A', host: '10.0.0.1', phone: '628a', enabled: true };
        const B = { id: 'b', name: 'CCTV B', host: '10.0.0.2', phone: '628b', enabled: true };
        const devices = [A, B];
        const t = setup({ devices, massOutageThreshold: 2, confirmationMinutes: 1 });
        t.setNetwatch([{ host: '10.0.0.1', status: 'up' }, { host: '10.0.0.2', status: 'up' }]);
        await t.m._pollOnceForTest(); // seed
        t.setNetwatch([{ host: '10.0.0.1', status: 'down' }, { host: '10.0.0.2', status: 'down' }]);
        await t.m._pollOnceForTest(); // A & B pending

        // B di-dekomisioning (dihapus dari registry) sementara masih 'down'.
        devices.splice(1, 1);
        t.setNetwatch([{ host: '10.0.0.1', status: 'down' }]);
        await t.m._pollOnceForTest(); // prune → state B dibuang

        // getStatus tak lagi melaporkan host hantu B.
        const st = t.m.getStatus();
        expect(st.devices.find(d => d.host === '10.0.0.2')).toBeFalsy();
        expect(st.tracked).toBe(1);

        // countDown kini 1 (bukan 2) → A tidak salah diredam sbg gangguan-massal.
        t.fireKind('confirm'); t.fireKind('batch');
        const downs = t.sent.filter(x => x.label === 'cctv_down');
        expect(downs.find(x => x.phone === '628a')).toBeTruthy(); // A tetap di-broadcast (tak mass-suppressed)
    });
});
