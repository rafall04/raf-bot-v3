...
// #b336 — recovery PERSISTEN & year-wrap.
describe('olt-log-scraper: recovery persisten lintas-siklus (#b336)', () => {
    const { processLog } = oltScraper.__testHooks;
    const norm = oltScraper.normalizeMAC;
    const TS = 'Jun 10 11:00:53';

    // Replikasi merge scrapeOltLog: allEvents (persisten) + hasil per-OLT.
    function applyMerge(allEvents, result) {
        if (result.events && Object.keys(result.events).length > 0) Object.assign(allEvents, result.events);
        for (const mac of (result.recoveredMacs || [])) {
            if (!(result.events && result.events[mac])) delete allEvents[mac];
        }
    }

    test('Discovery utk MAC yang tak ada di events fresh tetap dilaporkan recoveredMacs', () => {
        const MAC = '44:44:44:44:44:44';
        const events = {}; const report = {};
        processLog([`Jun 10 11:01:30 EPON: Onu 0/1/1:4 ${MAC} [Na] Discovery`], events, 10, { report });
        expect(events[norm(MAC)]).toBeUndefined();     // memang tak ada di siklus ini
        expect(report.recoveredMacs).toContain(norm(MAC)); // TAPI dilaporkan pulih
    });

    test('LOS siklus-1 lalu Discovery siklus-2 → allEvents PERSISTEN dibersihkan (bukan nyangkut 2 hari)', () => {
        const MAC = '55:55:55:55:55:55';
        const allEvents = {};

        // Siklus 1: LOS
        const c1 = {}; const r1 = {};
        processLog([`${TS} EPON: Slot 0/1/1:4 Onu ${MAC}[Na] Lost`], c1, 10, { report: r1 });
        applyMerge(allEvents, { events: c1, recoveredMacs: r1.recoveredMacs });
        expect(allEvents[norm(MAC)]).toBeDefined();

        // Siklus 2: Discovery (fresh events kosong; LOS ada di allEvents dari siklus 1)
        const c2 = {}; const r2 = {};
        processLog([`Jun 10 11:05:00 EPON: Onu 0/1/1:4 ${MAC} [Na] Discovery`], c2, 10, { report: r2 });
        applyMerge(allEvents, { events: c2, recoveredMacs: r2.recoveredMacs });

        expect(allEvents[norm(MAC)]).toBeUndefined(); // insiden LOS DITUTUP
    });

    test('Discovery lalu Lost di siklus SAMA → tetap down (jangan terhapus oleh recovery)', () => {
        const MAC = '66:66:66:66:66:66';
        const allEvents = { [norm(MAC)]: { mac: norm(MAC), event_type: 'los' } };
        const c = {}; const r = {};
        // Discovery lebih awal, lalu Lost lagi (turun ulang) — sortir kronologis menjaga urutan.
        processLog([
            `Jun 10 11:00:00 EPON: Onu 0/1/1:4 ${MAC} [Na] Discovery`,
            `Jun 10 11:05:00 EPON: Slot 0/1/1:4 Onu ${MAC}[Na] Lost`,
        ], c, 10, { report: r });
        applyMerge(allEvents, { events: c, recoveredMacs: r.recoveredMacs });
        expect(allEvents[norm(MAC)]).toBeDefined(); // masih down — recovery TIDAK menghapusnya
        expect(allEvents[norm(MAC)].event_type).toBe('los');
    });

    test('scrapeOltLog benar-benar menghapus recoveredMacs dari allEvents (source-scan)', () => {
        const fs = require('fs');
        const path = require('path');
        const src = fs.readFileSync(path.join(__dirname, '..', 'olt-log-scraper.js'), 'utf8');
        expect(src).toMatch(/recoveredMacs/);
        expect(src).toMatch(/delete allEvents\[mac\]/);
    });
});

describe('olt-log-scraper: parseTimestamp year-wrap Des/Jan (#b336)', () => {
    const { parseTimestamp } = oltScraper.__testHooks;

    afterEach(() => { if (Date.now.mockRestore) Date.now.mockRestore(); });

    test('baris Desember diparse saat "now"=Januari → tahun SEBELUMNYA', () => {
        // now = 2027-01-01 06:00 lokal.
        jest.spyOn(Date, 'now').mockReturnValue(new Date('2027-01-01T06:00:00').getTime());
        const iso = parseTimestamp('Dec 31 23:00:00', 2027);
        expect(iso.startsWith('2026-12-31')).toBe(true); // bukan 2027 (masa depan)
    });

    test('baris Januari saat "now"=Januari → tetap tahun berjalan', () => {
        jest.spyOn(Date, 'now').mockReturnValue(new Date('2027-01-01T06:00:00').getTime());
        const iso = parseTimestamp('Jan 01 05:00:00', 2027);
        expect(iso.startsWith('2027-01-01')).toBe(true);
    });

    test('Des sebelum Jan setelah wrap → urutan kronologis benar (DG tak meleset)', () => {
        jest.spyOn(Date, 'now').mockReturnValue(new Date('2027-01-01T06:00:00').getTime());
        const des = new Date(parseTimestamp('Dec 31 23:59:00', 2027)).getTime();
        const jan = new Date(parseTimestamp('Jan 01 00:01:00', 2027)).getTime();
        expect(des).toBeLessThan(jan);
    });
});
