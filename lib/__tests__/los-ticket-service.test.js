...
// Prefs per-teknisi dikontrol lewat map; resolver ASLI (../teknisi-recipient-resolver) berjalan di atasnya.
const PREFS = {};
...
describe("los-ticket-service", () => {
    ...
    test("#b355 gate ON → assign ke teknisi yang area-nya mencakup pelanggan (sadar-area)", () => {
        global.config.teknisiPrefs = { enabled: true };
        global.users = [{ id: 7, name: "Mbah Uti", phone_number: "0812", connected_odp_id: "ODP-09", subscription: "X", pppoe_username: "mbah-uti" }];
        global.accounts = [
            { id: "t1", username: "budi", name: "Budi", role: "teknisi", phone_number: "0899" },
            { id: "t2", username: "sari", name: "Sari", role: "teknisi", phone_number: "0877" },
        ];
        PREFS.t1 = { areas: ["ODP-01"] };  // luar area
        PREFS.t2 = { areas: ["ODP-09"] };  // cocok
        svc.maybeCreateLosTicket(INCIDENT);
        expect(tw.processTicket).toHaveBeenCalledTimes(1);
        expect(tw.processTicket.mock.calls[0][0].actor.id).toBe("t2");
    });

    test("#b355 gate ON → tak ada teknisi cocok area → FAIL-OPEN (tetap ter-assign)", () => {
        global.config.teknisiPrefs = { enabled: true };
        global.users = [{ id: 7, connected_odp_id: "ODP-99", phone_number: "0812", subscription: "X", pppoe_username: "mbah-uti" }];
        global.accounts = [{ id: "t1", username: "budi", name: "Budi", role: "teknisi", phone_number: "0899" }];
        PREFS.t1 = { areas: ["ODP-01"] }; // tak mencakup ODP-99
        svc.maybeCreateLosTicket(INCIDENT);
        expect(tw.processTicket).toHaveBeenCalledTimes(1); // LOS jangan pernah tak ter-assign
        expect(tw.processTicket.mock.calls[0][0].actor.id).toBe("t1");
    });
