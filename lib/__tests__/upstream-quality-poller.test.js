...
describe("detectFailover", () => {
    ...
    // Tangga nyata sudah lebih dari 2 rung: Dander GMDP→GMDP2→MNI, Tanjungharjo
    // VLAN62→VLAN401→radio→Indibiz→GMDP plain. Versi lama menebak cadangan dari rung
    // TERAKHIR, jadi pindah ke rung TENGAH lolos tanpa peringatan.
    test("tangga 3 rung, pindah ke rung TENGAH → failover true + tunjuk rung yang dipakai", () => {
        const result = _internal.detectFailover([
            { mark: "main", gateway: "10.78.0.3", distance: 1, active: 0, disabled: 0 },
            { mark: "main", gateway: "10.80.51.1", distance: 2, active: 1, disabled: 0 },
            { mark: "main", gateway: "Tunnel-MNI", distance: 3, active: 0, disabled: 0 }
        ]);
        expect(result.main.failover).toBe(true);
        expect(result.main.active_gateway).toBe("10.80.51.1");
        expect(result.main.active_distance).toBe(2);
        expect(result.main.backup_gateway).toBe("10.80.51.1");
        expect(result.main.primary_gateway).toBe("10.78.0.3");
    });

    test("tangga 5 rung, pindah ke rung TENGAH → tidak tertukar dengan rung terakhir", () => {
        const result = _internal.detectFailover([
            { mark: "main", gateway: "10.78.0.3", distance: 1, active: 0, disabled: 0 },
            { mark: "main", gateway: "10.80.51.1", distance: 2, active: 1, disabled: 0 },
            { mark: "main", gateway: "192.168.252.2", distance: 3, active: 0, disabled: 0 },
            { mark: "main", gateway: "192.168.101.123", distance: 4, active: 0, disabled: 0 },
            { mark: "main", gateway: "195.168.62.1", distance: 5, active: 0, disabled: 0 }
        ]);
        expect(result.main.failover).toBe(true);
        expect(result.main.active_gateway).toBe("10.80.51.1");
        expect(result.main.backup_gateway).not.toBe("195.168.62.1");
    });

    test("tangga 5 rung sehat di d=1 → failover false, cadangan pertama yang dilaporkan", () => {
        const result = _internal.detectFailover([
            { mark: "main", gateway: "10.78.0.3", distance: 1, active: 1, disabled: 0 },
            { mark: "main", gateway: "10.80.51.1", distance: 2, active: 0, disabled: 0 },
            { mark: "main", gateway: "192.168.252.2", distance: 3, active: 0, disabled: 0 }
        ]);
        expect(result.main.failover).toBe(false);
        expect(result.main.active_gateway).toBe("10.78.0.3");
        expect(result.main.active_distance).toBe(1);
        expect(result.main.backup_gateway).toBe("10.80.51.1");
        expect(result.main.backup_active).toBe(false);
    });

    test("semua rung mati → failover false (bukan 'backup aktif' palsu)", () => {
        const result = _internal.detectFailover([
            { mark: "main", gateway: "10.78.0.3", distance: 1, active: 0, disabled: 0 },
            { mark: "main", gateway: "10.80.51.1", distance: 2, active: 0, disabled: 0 },
            { mark: "main", gateway: "192.168.252.2", distance: 3, active: 0, disabled: 0 }
        ...
        expect(result.main.failover).toBe(false);
        expect(result.main.active_gateway).toBeNull();
        expect(result.main.active_distance).toBeNull();
    });

    test("rung disabled tidak ikut dihitung sebagai rung utama", () => {
        const result = _internal.detectFailover([
            { mark: "main", gateway: "10.78.0.3", distance: 1, active: 0, disabled: 1 },
            { mark: "main", gateway: "10.80.51.1", distance: 2, active: 1, disabled: 0 },
            { mark: "main", gateway: "192.168.252.2", distance: 3, active: 0, disabled: 0 }
        ]);
        // rung d=1 dinonaktifkan → rung utama efektif adalah d=2 yang sedang aktif
        expect(result.main.failover).toBe(false);
        expect(result.main.primary_gateway).toBe("10.80.51.1");
        expect(result.main.active_gateway).toBe("10.80.51.1");
    });
});

describe("buildStatusReport", () => {
    test("merakit status per jalur (KESEPAKATAN antar target) + failover", async () => {
        const repo = {
            getSummary: async () => [
                // mni: HANYA google yang buruk. Dulu ini cukup membuat jalur divonis GANGGUAN
                // ("worst-of-targets"). Terbukti berbahaya di produksi (#b264): satu target kronis
                // (`meta`, peering Facebook) membuat SELURUH jalur divonis rusak, dan lewat
                // `anyDegraded` setiap pelanggan diberi tahu jaringan kami terganggu padahal sehat.
        ...
        expect(mni.status).toBe("NORMAL"); // 1 dari 2 target buruk = TUJUAN itu bermasalah, bukan jalurnya
    ...
    test("#b264 — DUA target sepakat buruk → jalur benar-benar dilaporkan GANGGUAN", async () => {
        // Pasangan wajib dari tes di atas: menaikkan ambang kesepakatan tak boleh membuat
        // gangguan NYATA ikut hilang.
        const repo = {
            getSummary: async () => [
                { path: "mni", target: "8.8.4.4", target_key: "google", samples: 5, loss_avg: 30, rtt_avg: 90, jitter_avg: 5, last_probed_at: "x", baseline: { rtt_avg: 68 } },
                { path: "mni", target: "1.0.0.1", target_key: "cloudflare", samples: 5, loss_avg: 35, rtt_avg: 95, jitter_avg: 6, last_probed_at: "x", baseline: { rtt_avg: 68 } }
            ],
            getLatestRouteStates: async () => []
        };
        const report = await poller.buildStatusReport({ config: VALID_CFG, repo, nowMs: Date.now() });
        expect(report.paths.find((p) => p.key === "mni").status).toBe("GANGGUAN");
    });

    test("#b264 — beberapa IP satu layanan digabung dari HITUNGAN PAKET", async () => {
        // Tiga alamat `meta`: dua bersih, satu buruk. Digabung per paket, layanan itu tidak
        // otomatis divonis buruk hanya karena satu alamatnya bermasalah.
        const repo = {
            getSummary: async () => [
                { path: "mni", target: "157.240.1.1", target_key: "meta", samples: 5, sent_sum: 25, received_sum: 25, loss_avg: 0, rtt_avg: 260, jitter_avg: 1, last_probed_at: "x", baseline: { rtt_avg: 260 } },
                { path: "mni", target: "157.240.2.2", target_key: "meta", samples: 5, sent_sum: 25, received_sum: 25, loss_avg: 0, rtt_avg: 262, jitter_avg: 1, last_probed_at: "x", baseline: { rtt_avg: 260 } },
                { path: "mni", target: "31.13.79.35", target_key: "meta", samples: 5, sent_sum: 25, received_sum: 10, loss_avg: 60, rtt_avg: 264, jitter_avg: 1, last_probed_at: "x", baseline: { rtt_avg: 260 } }
            ],
            getLatestRouteStates: async () => []
        };
        const report = await poller.buildStatusReport({ config: VALID_CFG, repo, nowMs: Date.now() });
        const mni = report.paths.find((p) => p.key === "mni");
        const meta = mni.targets.find((t) => t.target_key === "meta");
        expect(mni.targets.length).toBe(1);            // tiga baris jadi SATU layanan
        expect(meta.alamat).toHaveLength(3);
        // 75 paket dikirim, 60 kembali -> 20%, bukan rata-rata persentase (0+0+60)/3 = 20 kebetulan
        // sama; yang penting sumbernya PAKET. Nilai dibulatkan 1 desimal oleh perakit.
        expect(meta.loss_avg_pct).toBeCloseTo(20, 0);
    });
});
