...
describe('payment finance service', () => {
  ...
  test('tagihan Rp0 (pelanggan gratis) = LUNAS tanpa pembayaran (bukan "belum bayar")', async () => {
    // Pelanggan gratis: harga efektif 0 (subscription_price 0 + nama paket tak berharga).
    // amountDue diturunkan dari getEffectivePrice(user)=0 — PERSIS jalur read-model (routes/payment-status).
    const freeUser = {
      id: 101, name: 'Customer A', subscription: 'PAKET-GRATIS',
      subscription_price: 0, discount_months: 0, discount_months_used: 0
    };
    const now = new Date();
    const periodMonth = now.getMonth() + 1;
    const periodYear = now.getFullYear();

    const pos = await service.getPaymentPositionForPeriod(freeUser, periodMonth, periodYear);
    expect(pos.amount_due).toBe(0);
    expect(pos.net_paid).toBe(0);
    expect(pos.is_waived).toBe(false);
    expect(pos.outstanding).toBe(0);
    // Inti perbaikan: Rp0 = tak ada yang harus dibayar → is_fully_paid true (dulu false → badge keliru).
    expect(pos.is_fully_paid).toBe(true);

    // Sinkronisasi flag periode berjalan ikut jadi lunas → aman isolir, badge bukan "Belum Bayar".
    const synced = await service.syncUserPaidStatusForPeriod({ user: freeUser, periodMonth, periodYear });
    const dbUser = await get('SELECT paid FROM users WHERE id = 101');
    expect(synced.is_fully_paid).toBe(true);
    expect(dbUser.paid).toBe(1);
  });

  test('tagihan >0 tanpa pembayaran TETAP belum bayar (guard anti-regresi)', async () => {
    const user = global.users[0];
    const now = new Date();
    const periodMonth = now.getMonth() + 1;
    const periodYear = now.getFullYear();
    const pos = await service.getPaymentPositionForPeriod(user, periodMonth, periodYear, { amountDue: 150000 });
    expect(pos.outstanding).toBe(150000);
    expect(pos.is_fully_paid).toBe(false);
  });
});
