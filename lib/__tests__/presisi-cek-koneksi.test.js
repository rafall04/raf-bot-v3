 ...
 *     (#b312 menambah jalan HEALTHY kedua yang SAH: degradasi ringan non-seluruh-jalur
 *     tanpa layanan parah — PUTUS mustahil lolos karena `isPathWideIssue` menandainya
 *     seluruh-jalur mutlak. Guard di bawah kini mematok KEDUA jalan itu.)
...
describe("klaim 'normal' harus berbukti, bukan dari flag config", () => {
    ...
    test("HEALTHY hanya dari jalur NORMAL atau degradasi ringan non-severe (#b312)", () => {
        expect(kodeVerdict).toMatch(/if \(up\.status === 'NORMAL'\) return 'HEALTHY';/);
        // Tepat DUA jalan sah: status NORMAL, dan sisir-akhir cabang status buruk yang hanya
        // dicapai degradasi ringan non-seluruh-jalur tanpa layanan parah. Jalan ketiga berarti
        // klaim sehat tanpa bukti lagi (bentuk bug yang persis ditutup tes ini).
        expect((kodeVerdict.match(/return 'HEALTHY'/g) || []).length).toBe(2);
        // Jalan kedua WAJIB berada SESUDAH vonis nyata di cabangnya — kalau urutannya berubah,
        // jalur pathWide/layanan-parah bisa tertelan menjadi "sehat".
        expect(kodeVerdict).toMatch(
            /if \(up\.pathWide\) return 'UPSTREAM_ISSUE';[\s\S]*?return 'SERVICE_ISSUE';[\s\S]*?return 'HEALTHY';/
        );
    });

    test("PUTUS tak pernah jatuh ke HEALTHY — selalu divonis seluruh-jalur", () => {
        // `isPathWideIssue` menandai PUTUS sebagai seluruh-jalur mutlak, sehingga status PUTUS
        // terhenti di UPSTREAM_ISSUE sebelum menyentuh jalan HEALTHY kedua. Kalau baris ini
        // berubah/hilang, jalur pelanggan yang mati total dilaporkan "sehat".
        expect(baca("lib", "upstream-path-health.js")).toMatch(/entry\.status === 'PUTUS'\) return true/);
    });
