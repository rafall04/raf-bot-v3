...
const { filterTeknisiRecipients, isQuietNow, customerAreaKeys, teknisiCoversArea } = require("../teknisi-recipient-resolver");
...
describe("customerAreaKeys — model kanonik matchRuleTarget", () => {
    test("kumpulkan area/zone/connected_odp_id/odp/odp_id, ternormalisasi & unik", () => {
        expect(customerAreaKeys({ area: "Dusun A", connected_odp_id: "ODP-01" }).sort()).toEqual(["dusun a", "odp-01"]);
        expect(customerAreaKeys({ zone: "  KRAJAN  ", odp: "krajan" })).toEqual(["krajan"]); // dedup lintas-field
        expect(customerAreaKeys({})).toEqual([]); // tak ada field area → kosong (fail-open di pemanggil)
    });
});

describe("teknisiCoversArea — fail-open", () => {
    test("area teknisi kosong → cakup semua; alert tanpa area → dicakup; selain itu irisan", () => {
        expect(teknisiCoversArea([], ["odp-09"])).toBe(true);
        expect(teknisiCoversArea(["ODP-01"], [])).toBe(true);       // area alert tak diketahui
        expect(teknisiCoversArea(["ODP-01"], null)).toBe(true);
        expect(teknisiCoversArea(["ODP-01", "Dusun B"], ["odp-09", "dusun b"])).toBe(true); // salah satu cocok
        expect(teknisiCoversArea(["ODP-01"], ["odp-09"])).toBe(false);
        expect(teknisiCoversArea(["odp-01"], "ODP-01")).toBe(true); // skalar + beda kapital
    });
});
