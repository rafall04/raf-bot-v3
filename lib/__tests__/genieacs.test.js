...
describe('#b347 — vonis queuedOnly di jalur REBOOT (jalur saudara #b254 yang terlewat)', () => {
    const axiosMock = () => require('axios');
    const configAsli = global.config;
    ...
    test('rebootDevice HTTP 202 (modem tak menjawab) → ok:false + DEVICE_UNREACHABLE (bukan "berhasil")', async () => {
        axiosMock().mockResolvedValue({ status: 202, data: { _id: 'task-reboot-q' } });
        const { rebootDevice } = require('../genieacs');
        const hasil = await rebootDevice('DEV-Q');
        expect(hasil.ok).toBe(false);
        expect(hasil.errorCode).toBe('DEVICE_UNREACHABLE');
        expect(hasil.accepted).toBe(true);   // ACS MENERIMA task-nya…
        expect(hasil.applied).toBe(false);   // …modem TIDAK mereboot
    });

    test('rebootDevice HTTP 200 (ACS terima) → ok:true', async () => {
        axiosMock().mockResolvedValue({ status: 200, data: { _id: 'task-reboot-ok' } });
        const { rebootDevice } = require('../genieacs');
        const hasil = await rebootDevice('DEV-OK');
        expect(hasil.ok).toBe(true);
    });

    test('saklar genieacsQueuedOnlyIsFailure=false → 202 kembali ok:true (perilaku lama)', async () => {
        global.config.genieacsQueuedOnlyIsFailure = false;
        axiosMock().mockResolvedValue({ status: 202, data: { _id: 'task-reboot-q' } });
        const { rebootDevice } = require('../genieacs');
        const hasil = await rebootDevice('DEV-Q');
