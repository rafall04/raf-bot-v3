...
describe("finalisasi payroll dua kali tidak boleh mengosongkan komisi", () => {
    ...
    test("#b326 dua finalize KONKUREN (klaim atomik) → HANYA satu sukses, komisi tak ditimpa 0", async () => {
        const draft = await siapkanPayrollBerkomisi();
        // Dua request finalize balapan (double-click). Klaim atomik WHERE status='draft' menang utk SATU.
        const [a, b] = await Promise.all([
            financeService.finalizePayroll(draft.id, { id: 1, name: "Admin" }),
            financeService.finalizePayroll(draft.id, { id: 1, name: "Admin" }),
        ]);
        const sukses = [a, b].filter((x) => x.finalized === true);
        const ditolak = [a, b].filter((x) => x.finalized === false);
        expect(sukses).toHaveLength(1);
        expect(ditolak).toHaveLength(1);
        expect(ditolak[0].reason).toBe("already_finalized");
        const row = await financeService.getPayrollRecord(draft.id);
        expect(row.komisi_collection).toBe(10000); // utuh, bukan ditimpa 0 oleh run kedua
        expect(row.net_amount).toBe(1510000);
    });
