...
describe('technician finance service', () => {
  ...
  test('komisi marketing PSB (Fase 2b) masuk draft + di-settle saat finalize', async () => {
    // Sumber marketing DI-INJECT (fake) — psb-schedule-service asli tak dipakai di unit test ini.
    const settleCalls = [];
    financeService.__setMarketingSourceForTest({
      getUnsettledMarketingForTeknisi: async (teknisiId) => (String(teknisiId) === '77' ? { entries: [{ id: 9 }, { id: 10 }], net_total: 30000 } : { entries: [], net_total: 0 }),
      settleMarketingToPayroll: async ({ teknisiId, payrollId }) => { settleCalls.push({ teknisiId, payrollId }); return { entries: [{ id: 9 }, { id: 10 }], netTotal: 30000 }; }
    });

    const created = await financeService.createPayrollDraft({
      teknisiId: 77, periodMonth: 5, periodYear: 2026, gajiPokok: 500000, bonusManual: 0, potonganKasbon: 0, potonganLain: 0, notes: 'MK'
    }, { id: 1, name: 'Admin' });
    expect(created.created).toBe(true);
    expect(created.draft.komisi_marketing).toBe(30000);
    expect(created.draft.gross_amount).toBe(530000);   // 500k + komisi marketing 30k
    expect(created.draft.net_amount).toBe(530000);

    const finalized = await financeService.finalizePayroll(created.id, { id: 1, name: 'Admin' });
    expect(finalized.finalized).toBe(true);
    expect(finalized.komisiMarketing).toBe(30000);
    expect(settleCalls).toEqual([{ teknisiId: 77, payrollId: created.id }]); // di-settle dgn payroll id benar

    const row = await financeService.getPayrollRecord(created.id);
    expect(row.komisi_marketing).toBe(30000);
    expect(row.gross_amount).toBe(530000);

    const summary = await financeService.getPayrollSummary({ month: 5, year: 2026 });
    expect(summary.total_komisi_marketing).toBe(30000);

    financeService.__setMarketingSourceForTest(null); // bersihkan override
  });

  test('gagal baca marketing TIDAK menjatuhkan payroll (default 0)', async () => {
    financeService.__setMarketingSourceForTest({
      getUnsettledMarketingForTeknisi: async () => { throw new Error('psb db down'); },
      settleMarketingToPayroll: async () => { throw new Error('psb db down'); }
    });
    const created = await financeService.createPayrollDraft({
      teknisiId: 77, periodMonth: 6, periodYear: 2026, gajiPokok: 400000, bonusManual: 0, potonganKasbon: 0, potonganLain: 0
    }, { id: 1, name: 'Admin' });
    expect(created.created).toBe(true);
    expect(created.draft.komisi_marketing).toBe(0);     // defensif: baca gagal → 0
    expect(created.draft.net_amount).toBe(400000);
    const finalized = await financeService.finalizePayroll(created.id, { id: 1, name: 'Admin' });
    expect(finalized.finalized).toBe(true);              // settle gagal → tetap finalize
    expect(finalized.komisiMarketing).toBe(0);
    financeService.__setMarketingSourceForTest(null);
  });
});
