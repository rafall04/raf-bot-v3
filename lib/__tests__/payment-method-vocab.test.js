 ...
 *
 * KENAPA ADA — mapMethodSummary lama menulis `output[normalized] = {...}`, sehingga dua
 * ejaan yang menormal ke ember sama saling membunuh; yang tersisip belakangan MENANG.
 *
 * CAKUPAN YANG JUJUR: produksi Tanjungharjo 2026-08-15 memang memuat "TRANSFER_BANK"
 * (69 transaksi, Rp8.985.000) DAN "transfer" (1 transaksi, Rp10.000), tapi keduanya di
 * TAHUN BERBEDA (2026 vs 2025) sementara laporan SELALU disaring per tahun — jadi
 * tabrakannya BELUM pernah muncul di layar. Yang ditahan tes ini adalah ranjaunya:
 * begitu dua ejaan berbagi satu periode (mudah terjadi, penulis kolom ini banyak dan
 * kosakatanya tak seragam), satu kelompok uang lenyap dari kartu.
 *
...
describe("ringkasan MENJUMLAH ejaan yang bertabrakan, bukan menimpa", () => {
    ...
    test("ejaan produksi Tanjungharjo dalam SATU periode: Rp8.985.000 tak lagi lenyap", () => {
        const hasil = ringkas({
...
describe("labelMetode: yang dibaca PELANGGAN di baris 'Metode:'", () => {
    ...
    test.each([
        ...
        expect(labelMetode(masukan)).toBe(harap);
    });

    test("catatan asal-usul BUKAN metode — jangan dicetak apa adanya", () => {
        // Terukur di produksi Tanjungharjo: dari 35 invoice NYATA, 9 bertuliskan
        // "Manual Send" dan 1 "Updated via Admin Panel" pada baris yang dibaca pelanggan.
        for (const m of ["Manual Send", "Updated via Admin Panel", "system", ""]) {
            expect(labelMetode(m)).toBe("Tidak tercatat");
        }
    });

    test("metode yang benar-benar baru tetap terbaca, tidak disembunyikan", () => {
        expect(labelMetode("GoPay Later")).toBe("GoPay Later");
    });

    test("tak satu pun keluaran berupa kode internal HURUF BESAR bergaris bawah", () => {
        for (const m of ["CASH", "TRANSFER_BANK", "BANK_TRANSFER", "INTERNAL_PAYROLL"]) {
            expect(labelMetode(m)).not.toMatch(/^[A-Z_]+$/);
        }
    });
});
