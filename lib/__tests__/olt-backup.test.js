...
describe('olt-backup', () => {
    ...
    test('runBackupForDevice: menulis file .cfg berisi header + konfigurasi', async () => {
        ...
        const res = await backup.runBackupForDevice(device, { baseDir: tmpDir, capture, sendTelegram: false });
    ...
    test('retensi: file terlama dihapus melebihi keep', async () => {
        ...
        const res = await backup.runBackupForDevice(device, { baseDir: tmpDir, capture, keep: 2, sendTelegram: false });
    ...
    test('listBackups: terurut terbaru dulu + filter device', async () => {
        ...
        await backup.runBackupForDevice(device, { baseDir: tmpDir, capture, sendTelegram: false });
        await backup.runBackupForDevice({ ...device, id: 'olt2', name: 'OLT 2' }, { baseDir: tmpDir, capture, sendTelegram: false });
