...
describe('payment status backfill (ledger from paid flag)', () => {
  ...
  // Jalur default produksi: backfill dipanggil TANPA periode (= periode berjalan), sehingga
  // flag users.paid tetap 1 setelah run pertama. Dulu run paksa kedua menulis baris kedua untuk
  // pelanggan berdiskon, karena diskon sudah "terpakai" pada run pertama → harga efektif naik →
  // ledger tampak belum lunas. Idempotensi kini bersandar pada ledger (created_by), bukan harga.
  test('force re-run on the current period never double-writes, even after a discount was consumed', async () => {
    const { periodMonth, periodYear } = service.getCurrentBillingPeriod();

    const discounted = global.users.find((u) => u.id === 5);
    discounted.discount_months = 1;
    discounted.discount_amount = 25000;
    await run('UPDATE users SET discount_months = 1, discount_amount = 25000 WHERE id = 5');

    const first = await backfill.backfillPaidLedgerFromFlag({ periodMonth, periodYear });
    expect(first.failed).toEqual([]);
    expect(first.created.map((c) => c.id).sort()).toEqual([2, 5]);
    // Diskon dipotong: 125.000 - 25.000, dan diskon tercatat terpakai.
    expect(first.created.find((c) => c.id === 5).amount).toBe(100000);
    expect(discounted.discount_months_used).toBe(1);

    const forced = await backfill.backfillPaidLedgerFromFlag({ periodMonth, periodYear, force: true });
    expect(forced.created).toEqual([]);
    expect(forced.alreadyPaid.map((c) => c.id).sort()).toEqual([2, 5]);

    const rows = await all('SELECT user_id, amount_paid FROM payment_history ORDER BY user_id');
    expect(rows).toHaveLength(2);
    expect(rows[1]).toMatchObject({ user_id: 5, amount_paid: 100000 });
  });
});
