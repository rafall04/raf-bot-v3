...
describe("#b266 — !! kunci config yang dibaca ADMIN wajib diteruskan getMonitorConfig", () => {
    // Ini kelas bug yang sudah pernah didokumentasikan (#b255) dan TETAP terulang saat menulis
    // halaman admin: `getMonitorConfig` MERAKIT ULANG objek config, jadi kunci yang tak disebut
    // di sana hilang. Akibatnya panel menampilkan sakelar MATI padahal di disk MENYALA — dan
    // menyimpan form akan mematikannya diam-diam.
    //
    // Guard ini menaruh nilai NON-BAWAAN di disk, lalu menuntut nilai itu muncul di tampilan
    // admin. Kunci baru yang lupa diteruskan langsung memerahkan tes.
    ...
    const diskCfg = {
        upstreamMonitor: {
            enabled: true, host: "h", user: "u", password: "p",
            paths: [{ key: "main", label: "Utama" }],
            targets: [{ key: "google", label: "Google DNS", address: "8.8.4.4" }],
            // Semua NON-BAWAAN supaya kebocoran passthrough ketahuan.
            stabilitasPelanggan: { enabled: true },
            alarmKestabilan: { enabled: true, consecutiveCycles: 7, cooldownMinutes: 33, paths: ["main"] },
            ambangStabilitas: { lossPeringatanPct: 3, lossBurukPct: 21, jitterPeringatanMs: 6, jitterBurukMs: 44, minSampel: 9 },
            stabilitasWindowMinutes: 17,
            minTargetSepakat: 3,
            traceCount: 15,
            alerts: { enabled: true, paths: ["main"] }
        }
    };

    test("setiap setelan admin memantulkan nilai dari disk, bukan bawaan", () => {
        const asli = global.config;
        global.config = diskCfg;
        try {
            const view = svc.getEditableUpstreamConfig({
                getEffectiveMonitorConfig: () => poller.getMonitorConfig(),
                getEffectiveServiceConfig: () => ({ enabled: false, services: [], paths: [] })
            });
            expect(view.stabilitas.kabariPelanggan).toBe(true);      // pernah GAGAL: terbaca false
            expect(view.stabilitas.alarmAdmin).toBe(true);
            expect(view.stabilitas.siklusBeruntun).toBe(7);
            expect(view.stabilitas.cooldownMinutes).toBe(33);
            expect(view.stabilitas.windowMinutes).toBe(17);
            expect(view.stabilitas.lossBurukPct).toBe(21);
            expect(view.stabilitas.jitterBurukMs).toBe(44);
            expect(view.stabilitas.minSampel).toBe(9);
            expect(view.stabilitas.minTargetSepakat).toBe(3);
            expect(view.stabilitas.traceCount).toBe(15);
            expect(view.alertPaths).toEqual(["main"]);
            expect(view.alarmKestabilanPaths).toEqual(["main"]);
        } finally {
            global.config = asli;
        }
    });
