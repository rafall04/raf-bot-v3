...
const log = require('./logger').logger.child('FAST_WHATSAPP_SENDER');
...
async function fastSend(raf, phoneJid, message) {
    log.info(`[FAST_SEND] Attempting to send to ${phoneJid}`);
    log.info(`[FAST_SEND] WhatsApp state: ${getConnectionState()}`);
    log.info(`[FAST_SEND] RAF exists: ${!!raf}`);

    try {
        const delivery = await sendMessage(phoneJid, { text: message });
        if (delivery.sent) {
            log.info(`[FAST_SEND] Successfully sent to ${phoneJid}`);
        ...
        log.error(`[FAST_SEND] Failed to send to ${phoneJid}:`, error.message);
...
async function fastSendMultiple(raf, phoneNumbers, message, delayMs = 500) {
    ...
    for (let i = 0; i < validRecipients.length; i++) {
        ...
        if (delivery.sent) {
            ...
            log.info(`[FAST_SEND_MULTIPLE] Sent to ${phoneJid} (${i + 1}/${validRecipients.length})`);
        } else {
            ...
            log.error(`[FAST_SEND_MULTIPLE] Failed to send to ${phoneJid}:`, error);
    ...
    log.info(`[FAST_SEND_MULTIPLE] Sent to ${successCount}/${phoneNumbers.length} recipients`);
