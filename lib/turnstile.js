 ...
 */
"use strict";
const log = require('./logger').logger.child('TURNSTILE');
...
async function verifyTurnstile(token, remoteIp) {
    ...
    if (!secret || /^ISI_/i.test(String(secret))) {
        log.error("[TURNSTILE] enabled=true tetapi secretKey belum diisi — menolak (fail-closed).");
    ...
    try {
        ...
        log.warn("[TURNSTILE] Verifikasi gagal:", res && res.data && res.data["error-codes"]);
        return { ok: false, reason: "verify_failed" };
    } catch (err) {
        log.error("[TURNSTILE] Error verifikasi (fail-closed):", err.message);
