 ...
 */
"use strict";
const log = require('../logger').logger.child('TELEGRAM_TEKNISI_CLIENT');
...
function createTelegramClient(botToken, opts = {}) {
    ...
    async function safeSendMessage(chatId, text, options = {}) {
        try {
            ...
            log.error(`[TELEGRAM_TEKNISI] Gagal kirim pesan ke ${chatId}: ${msg}`);
    ...
    async function setMyCommands(commands) {
        try {
            ...
            const msg = (error.response && error.response.data && error.response.data.description) || error.message;
            log.error(`[TELEGRAM_TEKNISI] setMyCommands gagal: ${msg}`);
            return { success: false, error: msg };
        }
    }
