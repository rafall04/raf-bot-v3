 ...
 */
"use strict";
const log = require('../logger').logger.child('TELEGRAM_TEKNISI_BOOTSTRAP');
...
function startTelegramTeknisiBot(opts = {}) {
    if (state.running) {
        log.info("[TELEGRAM_TEKNISI] Sudah berjalan — start diabaikan.");
        return undefined;
    }

    const cfg = readConfig();
    if (!cfg.enabled) {
        log.info("[TELEGRAM_TEKNISI] Nonaktif (set config.telegramTeknisi.enabled=true untuk mengaktifkan).");
        return undefined;
    }
    if (isPlaceholderToken(cfg.botToken)) {
        log.warn("[TELEGRAM_TEKNISI] botToken belum diisi — bot teknisi tidak dijalankan.");
        return undefined;
    }
    ...
    log.info(`[TELEGRAM_TEKNISI] Bot teknisi mulai (long-poll, timeout ${cfg.pollTimeoutSec}s).`);
    ...
    if (client && typeof client.setMyCommands === "function") {
        try {
            Promise.resolve(client.setMyCommands(TEKNISI_COMMANDS)).catch((e) =>
                log.error("[TELEGRAM_TEKNISI] setMyCommands error:", e.message)
            );
        } catch (e) {
            log.error("[TELEGRAM_TEKNISI] setMyCommands error:", e.message);
        }
    ...
    if (opts.autoLoop !== false) {
        ...
        loopDone = pollLoop(client, dispatcher, session).catch((e) => {
            log.error("[TELEGRAM_TEKNISI] Loop poll berhenti tak terduga:", e.message);
...
async function pollOnce(client, dispatcher, session = state) {
    ...
    for (const update of updates) {
        ...
        try {
            ...
            log.error("[TELEGRAM_TEKNISI] handleUpdate error:", e.message);
        }
    }
    if (skippedStale > 0) {
        log.warn(`[TELEGRAM_TEKNISI] Lewati ${skippedStale} pesan basi (antrian sebelum bot start) — anti-replay.`);
    }
    return updates;
}
...
async function pollLoop(client, dispatcher, session = state) {
    while (session.running && !session.stopped) {
        try {
            ...
            log.error("[TELEGRAM_TEKNISI] getUpdates error:", e.message);
            await delay(RETRY_DELAY_MS);
        }
    }
    log.info("[TELEGRAM_TEKNISI] Loop poll selesai.");
}
...
function stopTelegramTeknisiBot() {
    ...
    log.info("[TELEGRAM_TEKNISI] Stop diminta — loop berhenti pada siklus berikutnya.");
}
