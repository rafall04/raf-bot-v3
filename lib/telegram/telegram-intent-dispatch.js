 ...
 */
"use strict";
const log = require('../logger').logger.child('TELEGRAM_INTENT_DISPATCH');
...
function createTelegramDispatcher(deps = {}) {
    ...
    async function handleUpdate(update, ctx = {}) {
        ...
        try {
            ...
            log.error(`[TELEGRAM_TEKNISI] Handler error (${parsed && parsed.command}): ${error.message}`);
    ...
    async function handleCallback(cq, client) {
        ...
        try {
            ...
            log.error(`[TELEGRAM_TEKNISI] callback handler error (${command}): ${error.message}`);
