...
// Predikat "seluruh-jalur" yang SAMA dipakai connection-check-handler untuk vonis kesehatan.
// Dipakai di Kelas 2 supaya balasan app tak membantah note kesehatan (dulu app bilang "jalur ada
// kendala" pada DEGRADASI ringan yang handler nilai HEALTHY → dua kalimat bertolak belakang).
const { isPathWideIssue } = require("./upstream-path-health");

const VERDICT_BAD = ["DOWN", "TERGANGGU", "LAMBAT"];
// Layanan buruk di SEMUA jalur (dgn sampel) = gangguan sisi PLATFORM (mis. outage IG/WA global),
// BUKAN jaringan kami. Cermin upstream-quality-alerter.buildServiceSectionText (badEverywhere):
// hanya DOWN/TERGANGGU yang dihitung "gagal" (LAMBAT di mana-mana bisa jadi hanya lambat, tak putus).
const VERDICT_GAGAL = ["DOWN", "TERGANGGU"];
...
async function buildAppDiagnosis({ addr, appEntity }, deps = {}) {
    try {
            ...
            if (report && Array.isArray(report.services)) {
                ...
                if (cell && cell.samples) {
                    ...
                    if (VERDICT_BAD.includes(cell.verdict)) {
                        // Buruk di SEMUA jalur (dgn sampel) = outage sisi PLATFORM (mis. IG/WA
                        // global), BUKAN jaringan kami. Jangan menyalahkan diri sendiri & berjanji
                        // "menangani" sesuatu yang tak bisa kami sentuh — pelanggan akan salah
                        // kredit/salahkan ISP saat layanan pulih di sisi Meta/dsb.
                        const cellsSvc = Array.isArray(svc.cells) ? svc.cells : [];
                        const badEverywhere = cellsSvc.length > 1
                            && cellsSvc.every((c) => !c.samples || VERDICT_GAGAL.includes(c.verdict));
                        if (badEverywhere) {
                            return render(
                                "conncheck_app_global_outage",
                                `\n\n🎯 *Soal ${namaApp}:* saat ini *${namaApp}* sedang bermasalah dari sisi *${namaApp}* sendiri ` +
                                `(gangguan yang juga dialami pengguna lain di luar jaringan kami) — *bukan* dari jaringan kami ` +
                                `maupun perangkat Anda. Biasanya pulih sendiri setelah sisi ${namaApp} normal kembali. Mohon ditunggu ya 🙏`,
                                { app: namaApp }
                            );
                        }
                        return render(
        ...
        // Vonis "jalur ada kendala" HANYA saat gangguan SELURUH-JALUR — persis kondisi
        // connection-check-handler untuk UPSTREAM_ISSUE: status bermasalah DAN isPathWideIssue.
        // DEGRADASI ringan / spesifik-layanan di sana dinilai HEALTHY ("terpantau normal"); kalau
        // di sini kita bilang "jalur ada kendala", kedua kalimat itu tergabung di satu balasan &
        // saling membantah. (isPathWideIssue sendiri fail-closed=true saat data target tipis, jadi
        // ia harus digerbangi status bermasalah dulu — jalur NORMAL tak boleh lolos.)
        const statusBermasalah = ["DEGRADASI", "GANGGUAN", "PUTUS"].includes(entry.status);
        if (statusBermasalah && isPathWideIssue(entry)) {
            return render(
