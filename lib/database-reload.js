 ...
 */
const log = require('./logger').logger.child('DATABASE_RELOAD');
function reloadUsersFromDatabase() {
    return new Promise((resolve, reject) => {
        ...
        log.info('[DB_RELOAD] Starting database reload...');

        db.all("SELECT * FROM users ORDER BY id ASC", [], (err, rows) => {
            if (err) {
                log.error('[DB_RELOAD] Error loading users:', err);
                return reject(err);
            }
            ...
            log.info(`[DB_RELOAD] Successfully loaded and transformed ${newCount} users into memory.`);
            log.info(`[DB_RELOAD] Changes: ${oldCount} → ${newCount} users, ${oldColumns} → ${newColumns} fields`);
...
function reloadJSONDatabases() {
    try {
        ...
        if (global.packages) {
            ...
            log.info('[DB_RELOAD] Reloaded packages.json');
        }
        ...
        if (global.odps) {
            ...
            log.info('[DB_RELOAD] Reloaded odps.json');
        }
        ...
        if (global.accounts) {
            ...
            log.info('[DB_RELOAD] Reloaded accounts.json');
        }
        
        return true;
    } catch (err) {
        log.error('[DB_RELOAD] Error reloading JSON databases:', err);
        return false;
    }
}
...
async function fullDatabaseReload() {
    try {
        log.info('[DB_RELOAD] Starting full database reload...');
        ...
        log.info('[DB_RELOAD] Full reload completed successfully');
        return result;
    } catch (err) {
        log.error('[DB_RELOAD] Full reload failed:', err);
        throw err;
    }
}
