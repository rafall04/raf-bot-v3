"use strict";
const log = require('./logger').logger.child('ACTIVITY_LOGGER');
...
// Retensi audit log (best practice): simpan 2 tahun lalu bersihkan otomatis.
// Cukup panjang untuk audit/investigasi, tetap wajar untuk privasi (log berisi IP/HP).
    ...
    return new Promise(async (resolve, reject) => {
        try {
            ...
            logsDb = new sqlite3.Database(LOGS_DB_PATH, (err) => {
                if (err) {
                    log.error('[ACTIVITY_LOGGER] Error opening logs database:', err);
                    return reject(err);
                }
                log.info(`[ACTIVITY_LOGGER] Connected to logs database: ${LOGS_DB_PATH}`);
                applySqlitePragmas(logsDb).catch((pragmaErr) => {
                    log.warn(`[ACTIVITY_LOGGER_PRAGMA_WARN] ${pragmaErr.message}`);
                });
            });
            ...
            `, (err) => {
                if (err) {
                    log.error('[ACTIVITY_LOGGER] Error creating login_logs table:', err);
                    return reject(err);
                }
                ...
                `, (err) => {
                    if (err) {
                        log.error('[ACTIVITY_LOGGER] Error creating activity_logs table:', err);
                        return reject(err);
                    }
                    ...
                    // Best-practice retention: jadwalkan cleanup harian (2 tahun).
            ...
            log.error('[ACTIVITY_LOGGER] Error initializing logs database:', error);
            reject(error);
        }
    });
}
...
function logLogin(data) {
    return new Promise((resolve, reject) => {
        if (!logsDb) {
            ...
            log.warn('[ACTIVITY_LOGGER] Logs database not initialized. Attempting to initialize...');
        ...
        , (err) => {
            if (err) {
                log.error(`[AUTH_LOG] ❌ Failed to log ${actionType}: ${username} - ${err.message}`);
            ...
            log.info(`[AUTH_LOG] ✅ ${actionType.toUpperCase()}: ${username} (${role}) - ${timeStr}`);
...
// Kolom JSON korup → kembalikan string mentah (baris tetap terdaftar), bukan melempar
// dan membatalkan seluruh halaman log.
...
function getActivityLogs(options = {}) {
    return new Promise((resolve, reject) => {
        ...
        logsDb.all(query, params, (err, rows) => {
            ...
            // Parse JSON values — satu baris korup tak boleh membatalkan seluruh list.
...
/**
 * Jadwalkan pembersihan log berbasis umur (retensi) — sekali ~1 menit setelah
 * boot, lalu setiap 24 jam. Idempoten (hanya dipasang sekali). Timer di-unref
 * agar tidak menahan proses keluar.
 */
function scheduleRetentionCleanup() {
    ...
    const runCleanup = () => {
        cleanupOldLogs(LOG_RETENTION_DAYS).catch((err) => {
            log.warn('[ACTIVITY_LOGGER] Retention cleanup failed:', err.message);
        });
    };
    const initialTimer = setTimeout(runCleanup, 60 * 1000);
    if (initialTimer.unref) {
        initialTimer.unref();
    }
    retentionTimer = setInterval(runCleanup, 24 * 60 * 60 * 1000);
    if (retentionTimer.unref) {
...
function closeLogsDatabase() {
    return new Promise((resolve, reject) => {
        if (logsDb) {
            logsDb.close((err) => {
                if (err) {
                    log.error('[ACTIVITY_LOGGER] Error closing logs database:', err);
                    return reject(err);
                }
                log.info('[ACTIVITY_LOGGER] Logs database connection closed');
                logsDb = null;
                resolve();
            });
        } else {
