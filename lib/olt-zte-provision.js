...
/**
 * Ekstrak nilai `name` dari blok `show running-config interface gpon-onu_x/y/z:N`.
 * Di VANS nama ONU = username PPPoE (konvensi lapangan). Murni → unit-test.
 * @param {string} text
 * @returns {string|null}
 */
function parseInterfaceName(text) {
    const m = String(text || '').match(/^\s*name\s+(\S+)/im);
    return m ? m[1] : null;
}

/**
 * Daftar SEMUA ONU teregistrasi di SATU port PON (browser "ONU terdaftar per slot") —
 * registrasi (onuId/type/sn) dari `show running-config interface gpon-olt_<port>`, lalu
 * (opsional) diperkaya nama ONU per unit. SEMUA dalam SATU sesi SSH (anti spam koneksi).
 * @param {object} device
 * @param {string} ponPort "1/2/1"
 * @param {object} [opts] {withNames=false} — true: baca `name` tiap ONU (1 perintah/ONU, lebih lambat)
 * @returns {Promise<Array<{onuId, type, sn, name}>>}
 */
async function listPortOnus(device, ponPort, opts = {}) {
    if (!PON_PORT_RE.test(String(ponPort || ''))) throw new Error('Format port PON tidak valid (contoh: 1/2/1)');
    return withHostLock(device && device.host, async () => {
        const session = await openOltShell(device, { commandTimeoutMs: 30000 });
        try {
            const portOut = await session.exec(`show running-config interface gpon-olt_${ponPort}`, { timeoutMs: 30000 });
            const base = parseOnuOccupancy(portOut); // [{onuId, type, sn}]
            if (!opts.withNames) return base.map((o) => ({ ...o, name: null }));
            const out = [];
            for (const o of base) {
                let name = null;
                try {
                    const ifc = await session.exec(`show running-config interface gpon-onu_${ponPort}:${o.onuId}`, { timeoutMs: 20000 });
                    name = parseInterfaceName(ifc);
                } catch (_e) { name = null; } // 1 ONU gagal baca nama ≠ gugurkan seluruh daftar
                out.push({ ...o, name });
            }
            return out;
        } finally {
...
/**
 * Tabel prefix SN → tier vendor (untuk dikirim ke UI: klasifikasi & auto-pilih profil
 * di klien tanpa round-trip per-SN). Sumber kebenaran tunggal = VENDOR_TIERS.
 * @returns {Array<{prefix, vendor, tier, method, oltPushable}>}
 */
function vendorTierTable() {
    return Object.entries(VENDOR_TIERS).map(([prefix, t]) => ({
        prefix, vendor: t.vendor, tier: t.tier, method: t.method, oltPushable: t.tier === 'zte',
    }));
}
