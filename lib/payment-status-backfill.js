 ...
 */
"use strict";
const log = require('./logger').logger.child('PAYMENT_STATUS_BACKFILL');
...
async function runStartupBackfillSafe(options = {}) {
    try {
        ...
        log.error("[PAID_LEDGER_BACKFILL_ERROR]", error.message);
...
if (require.main === module) {
    ...
    initializeDatabase()
        ...
        .catch((error) => {
            log.error("[PAID_LEDGER_BACKFILL_CLI_ERROR]", error);
            process.exit(1);
        });
}
