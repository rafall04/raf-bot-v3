 ...
 */
"use strict";
const log = require('../logger').logger.child('LEDGER');
...
function handleReaderError(error) {
    if (error && typeof error.code === "string" && error.code.startsWith("SQLITE_IOERR")) {
        log.error(`[PAYMENT_FINANCE_READER_IOERR] ${error.message} — koneksi baca dibuang, panggilan berikutnya membuka koneksi segar.`);
...
async function syncUserDiscountSchema() {
    ...
    for (const columnSpec of discountColumnSpecs) {
        if (!existing.has(columnSpec.name)) {
            await dbRun(columnSpec.sql);
            log.info(`[PAYMENT_FINANCE_SCHEMA] Kolom users.${columnSpec.name} ditambahkan (self-heal diskon).`);
        }
    }
}
...
async function syncPaymentLedgerDomains() {
    try {
        ...
        log.error("[PAYMENT_FINANCE_LEDGER_SYNC_ERROR]", error.message);
    }
}
