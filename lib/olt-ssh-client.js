...
'use strict';
const log = require('./logger').logger.child('OLT_SSH_CLIENT');
...
function detectCliError(output) {
    ...
    for (const line of lines) {
        ...
        if (!t) continue;
        if (/^%/.test(t)) {
            // Taksonomi pesan ZXAN (verif live C320 V2.1.0):
            //   %Info NNNNN: ...  → informasi (mis. 20272 saat masuk conf t) — BUKAN error.
            //   %Error NNNNN: ... → error sungguhan.
            //   %Code NNNNN-MOD : → hasil modul; error KECUALI notice hasil-kosong.
            if (/^%\s*(info|warn)/i.test(t)) continue;
            if (/no related information/i.test(t)) continue;
            return t;
        }
...
function stripEchoAndPrompt(raw, command) {
    ...
    if (lines.length && command && lines[0].trim().endsWith(command.trim())) {
        lines.shift();
    } else if (lines.length && command && lines[0].trim().startsWith('$')) {
        // Perintah panjang: ZXAN me-wrap echo dengan penanda '$' + ekor perintah
        // (verif live: "file upload ..." → echo "$n.dat ftp ..."). Buang bila ekornya
        // memang suffix dari perintah yang dikirim.
        const fragment = lines[0].trim().replace(/^\$/, '').trim();
        if (fragment && command.trim().endsWith(fragment)) lines.shift();
    }
...
/**
 * Pecah teks script CLI menjadi daftar perintah yang dikirim: trim, buang baris kosong
 * dan komentar. Baris separator "!" DIKONVERSI menjadi `exit` — pada ZXAN (verif live
 * C320 V2.1.0) konteks (config-if) TIDAK menerima `int gpon-onu_...` langsung; "!"
 * di script config ZTE memang menandai keluar-konteks. Komentar "!teks" tetap dibuang.
 * @param {string} script
 * @returns {string[]}
 */
...
// Pola error fase-transport (sebelum auth): aman untuk diulang. Auth gagal TIDAK
// diulang (kredensial salah permanen + ZXAN punya proteksi brute-force yang justru
// makin mengunci bila ditembak berulang).
const RETRYABLE_CONNECT_RE = /handshake|timed out|ETIMEDOUT|ECONNRESET|ECONNREFUSED|EHOSTUNREACH|ENETUNREACH|Timeout koneksi/i;
...
/**
 * Kebijakan retry fase-koneksi murni (di-unit-test tanpa SSH). Hanya kegagalan transport
 * (handshake/timeout/refused/reset) yang diulang; auth gagal dilempar segera. Backoff
 * "sopan" (default mulai 4 dtk) — backup terjadwal tahan blip TANPA membanjiri OLT
 * yang VTY-nya terbatas. `onceFn` = pembuka sesi satu-percobaan; `sleepFn` injectable.
 *
 * @param {(device, opts) => Promise<any>} onceFn
 * @param {object} device
 * @param {object} [opts]  + {connectRetries=1, connectRetryDelayMs=4000}
 * @param {(ms:number)=>Promise<void>} [sleepFn]
 * @returns {Promise<any>}
 */
async function retryConnect(onceFn, device, opts = {}, sleepFn = sleep) {
    const maxRetries = opts.connectRetries != null ? opts.connectRetries : 1;
    const baseDelay = opts.connectRetryDelayMs || 4000;
    let lastErr = null;
    for (let attempt = 0; attempt <= maxRetries; attempt++) {
        try {
            return await onceFn(device, opts);
        } catch (err) {
            lastErr = err;
            const retryable = RETRYABLE_CONNECT_RE.test(err.message || '');
            if (!retryable || attempt === maxRetries) throw err;
            const delay = baseDelay * (attempt + 1); // 4s, 8s, 12s…
            log.warn(`[OLT-SSH] ${device && device.host}: koneksi gagal (${String(err.message).split('\n')[0]}) — retry ${attempt + 1}/${maxRetries} dalam ${delay}ms`);
            await sleepFn(delay);
        }
    }
    throw lastErr;
...
/**
 * Buka shell OLT dengan retry fase-koneksi berbackoff (pembungkus {@link openOltShellOnce}).
 * @param {object} device
 * @param {object} [opts]  + {connectRetries=1, connectRetryDelayMs=4000}
 * @returns {Promise<{exec, close, prompt, banner}>}
 */
function openOltShell(device, opts = {}) {
    return retryConnect(openOltShellOnce, device, opts);
}

/**
 * Buka koneksi SSH + shell interaktif ke OLT (SATU percobaan), tunggu prompt pertama,
 * dan (opsional) naikkan privilege via `enable` bila prompt masih level user (`>`).
 *
 ...
 */
function openOltShellOnce(device, opts = {}) {
    ...
    return new Promise((resolve, reject) => {
        ...
        conn.on('ready', () => {
            ...
            conn.shell({ term: 'vt100', cols: 400, rows: 200 }, (err, s) => {
                 ...
                 * opts.idleTimeoutMs: mode IDLE — timer di-reset tiap ada data baru; cocok
                 * untuk perintah streaming lama (show running-config C320 bisa belasan menit
                 * tapi output terus mengalir; verif live). timeoutMs tetap jadi pagar total.
                 */
                const waitFor = (predicate, timeoutMs, label, wOpts = {}) => new Promise((res, rej) => {
                    const t0 = Date.now();
                    let lastLen = buffer.length;
                    ...
                    const iv = setInterval(() => {
                        ...
                        if (buffer.length !== lastLen) { lastLen = buffer.length; lastGrowth = Date.now(); }
                        const idleFail = wOpts.idleTimeoutMs && (Date.now() - lastGrowth > wOpts.idleTimeoutMs);
                        if (idleFail || Date.now() - t0 > timeoutMs) {
                            clearInterval(iv);
                            const tail = cleanTerminalOutput(buffer).slice(-400);
                            const why = idleFail
                                ? `output berhenti mengalir ${wOpts.idleTimeoutMs}ms`
                                : `total ${timeoutMs}ms`;
                            rej(new Error(`Timeout menunggu ${label} dari ${host} (${why}). Output terakhir:\n${tail}`));
                        }
                    }, 80);
                });
                ...
                (async () => {
                     ...
                     * @param {object} [o] {timeoutMs, idleTimeoutMs, autoConfirm:false, confirmAnswer:'yes'}
                     *   idleTimeoutMs: pakai untuk perintah streaming lama (capture config) —
                     *   gagal hanya bila output berhenti mengalir selama itu.
                            ...
                            for (;;) {
                                try {
                                    return await waitFor(
                                        (c) => endsWithPrompt(c),
                                        Math.max(500, timeoutMs - (Date.now() - t0)),
                                        `prompt setelah "${command}"`,
        ...
        conn.connect({
            ...
            // Keepalive DIMATIKAN: firmware ZXAN (C320 V2.1.0 diuji live) tidak menjawab
            // global request keepalive ssh2 → koneksi diputus tepat saat perintah lama
            // sunyi output (show running-config "Building configuration..." >30 dtk).
            // Sesi kita pendek dan tiap perintah punya timeout sendiri, keepalive tak perlu.
            keepaliveInterval: 0,
        });
    });
}
