"use strict";
const log = require('./logger').logger.child('TECHNICIAN_SALARY_PLAN');
...
const { createPayrollDraft, getPayrollList, getPayrollRecord, updatePayrollDraft, getKasbonSummary } = require("./technician-finance-service");
...
async function selaraskanDraftPeriodeBerjalan(items, date = new Date()) {
    ...
    for (const item of items) {
        try {
            ...
            log.error("[GAJI_TETAP_SELARAS_ERROR]", item.teknisiId, error.message);
        }
    }
    return hasil;
}
...
async function setAutoDraft({ enabled, draftDay }) {
    // #b345: baca config SEGAR dari disk + tulis ATOMIK (readConfigFresh/saveConfigAtomic,
    // pola #b343). writeFileSync polos ke config.json berisiko torn-write saat SIGKILL/OOM/listrik
    // padam (prod restart 7-13x/hari) → config.json terpotong → bot GAGAL BOOT TOTAL (CONFIG_FATAL)
    // sampai diperbaiki tangan. Read-modify-write dari disk segar juga menjaga subkey penulis lain /
    // hand-edit ops PASCA-BOOT tak terhapus (bahaya merge-key #b336). saveConfigAtomic sekaligus
    // menyinkronkan global.config, jadi sakelar berlaku tanpa restart.
    const { readConfigFresh, saveConfigAtomic } = require("./env-config");
    const isi = readConfigFresh();
...
async function createDraftsForPeriod({ date = new Date(), ignoreDayGate = false, actor = { name: "otomatis" } } = {}) {
    ...
    for (const plan of plans) {
        ...
        // Sisa hutang dibawa ke CATATAN draft. Potongannya tetap 0 (memotong hutang orang
        // adalah keputusan manusia, bukan efek samping cron), tapi angkanya harus ikut ke layar
        // — kalau hanya hidup di halaman kasbon, hutang bisa menganggur bertahun-tahun karena
        // tak satu pun layar di alur penggajian menyebutkannya.
        let catatanHutang = "";
        try {
            const ringkas = await getKasbonSummary({ teknisiId: plan.teknisi_id });
            const sisa = Number(ringkas && ringkas.total_outstanding) || 0;
            if (sisa > 0) {
                catatanHutang = ` — Kasbon aktif Rp${sisa.toLocaleString("id-ID")}, potongan belum diisi`;
            }
        } catch (error) {
            log.error("[GAJI_TETAP_KASBON_READ_ERROR]", plan.teknisi_id, error.message);
        }
        ...
        try {
            const r = await createPayrollDraft(
                {
                    ...
                    potonganKasbon: 0,
                    potonganLain: 0,
                    notes: `Draft otomatis dari Gaji Pokok Tetap${catatanHutang}`
                },
            ...
            log.error("[GAJI_TETAP_DRAFT_ERROR]", plan.teknisi_id, error.message);
        }
    }
...
async function kabarkanDraftBaru({ periode, dibuat, hasil, plans }) {
    try {
        ...
        log.error("[GAJI_TETAP_NOTIF_ERROR]", error.message);
    }
}
