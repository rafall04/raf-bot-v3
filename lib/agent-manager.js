...
/**
 * Header Doc
 * Purpose: Kelola data agen/outlet (`database/agents.json`) untuk topup & penjualan voucher.
 * Caller: lib/agent-transaction-manager, routes/message handler agen.
 * Deps: `lib/json-store` (load/save ATOMIK).
 * MainFuncs: getAgentById, getAllAgents, saveAgents, createAgent, dll.
 * SideEffects: Membaca/menulis `database/agents.json` (ATOMIK).
 */

// #b345: daftar agen (agents.json) lewat json-store — saveJSON ATOMIK + loadJSON KARANTINA
// (bukan JSON.parse telanjang yang, saat berkas rusak, cuma di-log lalu save berikutnya menimpa
// dengan [] → daftar agen hilang permanen). Sibling dari agent-transaction-manager #b345.
...
function initDatabase() {
    const loaded = loadJSON('agents.json');
    agents = Array.isArray(loaded) ? loaded : [];
}
...
function normalizeWhatsappToPhone(whatsappNumber) {
    ...
    // JID @lid membawa identifier numerik internal (bukan nomor HP) — jangan pernah
    // memperlakukannya sebagai telepon; resolve @lid via jalur khusus
    // (agent-transaction-manager → whatsappNumbersMatch / stored mapping).
    if (/@lid$/i.test(String(whatsappNumber))) return null;
