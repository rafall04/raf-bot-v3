/**
 * Header Doc
 * Purpose: Inti adapter MikroTik — config (env/.env/device aktif + cache), retry +
 *   circuit breaker + per-key lock, result helpers, dan DUA transport: bridge PHP
 *   (runPhpMikrotik via spawn) & HTTP site helper (getJsonOverHttp via siteUrl).
 * Caller: lib/mikrotik/{pppoe,netwatch,hotspot,site-http}.js (submodule), facade lib/mikrotik.js.
 *   Dipanggil langsung juga oleh konsumen facade untuk runPhpMikrotik/getMikrotikConfig/dll.
 * Deps: fs/path/http/https/child_process/axios, lib/internal-service-token. Path repo
 *   dihitung dari ../../ (file ini tinggal di lib/mikrotik/).
 * MainFuncs: getMikrotikConfig, invalidateMikrotikConfigCache, isMikrotikSyncEnabled,
 *   withMikrotikRetry, withMikrotikKeyLock, runPhpMikrotik, getJsonOverHttp, getSiteUrl,
 *   createResult, assertMikrotikResult, getMikrotikDiagnostics.
 * SideEffects: Baca .env + database/mikrotik_devices.json; spawn proses `php views/<script>.php`;
 *   HTTP ke siteUrl; console.log/error [MIKROTIK] (tersambung ke logger via jembatan boot).
 */
"use strict";
const log = require('../logger').logger.child('CORE');
...
// Root repo — file ini di lib/mikrotik/, jadi dua tingkat dari sini.
...
function readDotEnvConfig() {
    ...
    try {
        ...
        log.warn('[MIKROTIK_CONFIG_DOTENV_WARN]', error.message);
        return {};
    }
}

function readActiveMikrotikDevice() {
    const devicesPath = path.resolve(REPO_ROOT, 'database', 'mikrotik_devices.json');
    ...
    try {
        ...
        log.warn('[MIKROTIK_CONFIG_DEVICE_WARN]', error.message);
        return null;
    }
}
...
function logResult(result, context = {}) {
    ...
    if (result.ok) {
        log.info('[MIKROTIK]', JSON.stringify(logPayload));
    } else {
        log.error('[MIKROTIK]', JSON.stringify(logPayload));
    }
}
...
function runPhpMikrotik(operation, script, args = [], options = {}) {
    ...
    const scriptPath = path.resolve(REPO_ROOT, 'views', `${script}.php`);
...
module.exports = {
    ...
    _resetMikrotikKeyLocksForTests,
};
