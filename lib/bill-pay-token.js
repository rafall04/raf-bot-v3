 ...
 */
// Base URL portal. Prioritas: opts → config site_url_bot/siteUrl → turunkan dari
// ipaymuCallback (mis. https://host/callback/payment → https://host) supaya tetap jalan
// walau site_url_bot belum diset di config prod.
function isLocalBase(u) {
    return /^https?:\/\/(127\.0\.0\.1|localhost|0\.0\.0\.0|\[::1\])/i.test(String(u || ""));
}

function resolveBaseUrl(opts = {}) {
    const cfg = global.config || {};
    // Link dikirim ke HP pelanggan → WAJIB domain publik. `site_url_bot` di prod adalah alamat
    // INTERNAL (mis. 127.0.0.1, dipakai jembatan Node→PHP/MikroTik) — tak bisa dibuka dari luar.
    // Prioritas: baseUrl eksplisit → `public_url` (field setting khusus) → site_url_bot/siteUrl yang
    // PUBLIK → turunkan dari `ipaymuCallback` (pasti publik karena dipanggil iPaymu).
    let base = [opts.baseUrl, cfg.public_url, cfg.site_url_bot, cfg.siteUrl].find((u) => u && !isLocalBase(u)) || "";
    if (!base && cfg.ipaymuCallback) {
        const m = String(cfg.ipaymuCallback).match(/^(https?:\/\/[^/]+)/i);
        if (m) base = m[1];
    }
    if (!base) base = opts.baseUrl || cfg.site_url_bot || cfg.siteUrl || ""; // fallback dev/lokal
    return String(base).replace(/\/+$/, "");
}

function buildBillPayUrl(user, opts = {}) {
    return `${resolveBaseUrl(opts)}/bayar/${createBillPayToken(user, opts)}`;
}

module.exports = { createBillPayToken, verifyBillPayToken, buildBillPayUrl, resolveBaseUrl };
