...
function resolveBaseUrl(opts = {}) {
    const cfg = global.config || {};
    // Link dikirim ke HP pelanggan → WAJIB domain publik. site_url_bot prod kadang di-set ke
    // 127.0.0.1 (alamat internal) — itu tak bisa dibuka dari luar. Prioritas: baseUrl eksplisit →
    // site_url_bot/siteUrl yang PUBLIK → turunkan dari ipaymuCallback (pasti publik, dipanggil iPaymu).
    let base = [opts.baseUrl, cfg.site_url_bot, cfg.siteUrl].find((u) => u && !isLocalBase(u)) || "";
