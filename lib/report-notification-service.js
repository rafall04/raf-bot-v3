 ...
 * Deps: `./notification-tracker`, `./ticket-workflow`, `./templating`, `./template-service`, `./whatsapp-delivery-service`, `./whatsapp-gateway`, dan `./teknisi-recipient-resolver` (#b355 alert per-area).
 ...
 */
const log = require('./logger').logger.child('REPORT_NOTIFICATION_SERVICE');
const fs = require('fs');
...
// Kunci area pelanggan tiket (untuk penyaringan alert per-area teknisi #b355). Resolve user penuh
// dari global.users via user_id; fallback ke snapshot pelangganDataSystem. Tak ketemu → [] (fail-open).
function ticketAreaKeys(ticket) {
    const uid = ticket && (ticket.user_id != null ? ticket.user_id : ticket.pelangganUserId);
    const full = uid != null ? (global.users || []).find((u) => u && String(u.id) === String(uid)) : null;
    return customerAreaKeys(full || (ticket && ticket.pelangganDataSystem) || {});
}
...
async function safeSendMessage(jid, message, meta = {}) {
    if (!jid) return false;
    if (!canSendWhatsApp()) {
        log.warn('[REPORT_NOTIFICATION_SKIP]', { jid, reason: 'connection_unavailable', ...meta });
        return false;
    }
    if (isNotificationDuplicate(jid, message)) {
        log.warn('[REPORT_NOTIFICATION_SKIP]', { jid, reason: 'duplicate', ...meta });
        return false;
    }
    try {
        const delivery = await sendMessage(jid, message);
        if (!delivery.sent) {
            log.error('[REPORT_NOTIFICATION_ERROR]', { jid, error: delivery.warning || delivery.errorCode || 'SEND_FAILED', ...meta });
            return false;
        }
        markNotificationSent(jid, message);
        return true;
    } catch (error) {
        log.error('[REPORT_NOTIFICATION_ERROR]', { jid, error: error.message, ...meta });
        return false;
    }
}
...
async function notifyNewReport(ticket, options = {}) {
    ...
    // Alert per-area (#b355): saring teknisi menurut preferensi (kelas ticket_new + master + area
    // pelanggan). Gate OFF / belum-setel / area tak diketahui ⇒ lolos semua = perilaku lama.
    const teknisiAccounts = filterTeknisiRecipients({
        recipients: (global.accounts || []).filter((acc) => acc.role === 'teknisi' && acc.phone_number),
        alertClass: 'ticket_new',
        area: ticketAreaKeys(normalized),
    }).recipients;
