...
async function sendTechnicianBulkSummary(teknisiId, items = []) {
    try {
        ...
        console.error(`[BULK_SUMMARY_NOTIF_ERROR] teknisi ${teknisiId}: ${err.message}`);
        return false;
    }
}
...
async function handlePaidStatusChange(user, paymentDetails = {}) {
    ...
    if (perluPulihkan) {
        console.log(`[PAID_STATUS_CHANGE] Profil live = isolir. Memulihkan layanan ${user.name}.`);
        ...
        console.log(`[PAID_STATUS_CHANGE] Tidak memulihkan profil ${user.name}: ${alasanLewat}`);
    ...
    try {
        ...
        console.log(`[PAID_STATUS_CHANGE_NOTIF] ${user.name}: notifEnabled=${isNotifEnabled}, send_invoice=${user.send_invoice}`);
        ...
        if (isReady() && user.phone_number) {
            ...
            console.warn(`[PAID_STATUS_CHANGE_NOTIF_WARN] Skipping (WA not ready) for ${user.name}. hasSocket=${diagnostics.hasSocket}, state=${diagnostics.connectionState}`);
        }
    } catch (error) {
