...
function getDefaultPaths(type) {
    switch (type) {
        ...
        case 'wifiEnable':
            return ['InternetGatewayDevice.LANDevice.1.WLANConfiguration.{index}.Enable', 'Device.WiFi.SSID.{index}.Enable'];
...
async function updatePsbDeviceConfig(deviceId, payload = {}, options = {}) {
    ...
    // Dual-band harus AKTIF, bukan sekadar tertulis: instance WLAN band kedua bisa ada tapi
    // Enable=false (modem bekas yang 5GHz-nya dimatikan pemilik lama, atau bawaan pabrik).
    // Tiap index yang kita konfigurasi dinyalakan sekalian — kalau tidak, PSB "dual-band"
    // tetap memancar satu band dan keluhan "5GHz tidak muncul" berulang.
    if (payload.wifiSSID || payload.wifiPassword) {
        ssidIndices.forEach((ssidIndex) => {
            resolvePathTemplates('wifiEnable', ssidIndex).forEach((path) => {
                parameterValues.push([path, true, 'xsd:boolean']);
            });
        });
    }
    ...
    const result = await setParameterValues(deviceId, parameterValues, {
        ...
        refreshObjects: uniqueValues([
            // Container WLAN UTUH ikut di-refresh, bukan hanya instance yang di-push: instance
            // band lain (mis. 5GHz index 5) yang belum sempat dienumerasi ACS ikut terbaca.
            // Pembacaan ulang kapabilitas band pasca-push (psb.state) bergantung pada ini.
            ...resolvePathTemplates('wifiContainer'),
