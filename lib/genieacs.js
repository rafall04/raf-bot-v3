...
function extractPppoeUsername(device) {
    return extractFirstValue(device, resolvePathTemplates('pppoeUsername'));
}
...
async function genieacsRequest(method, path, options = {}) {
    ...
    try {
        ...
        return createResult(options.operation || 'genieacsRequest', {
            message: options.failureMessage || error.message || 'Permintaan GenieACS gagal.',
            errorCode: mapAxiosError(error),
            details: {
                httpStatus: error.response?.status || null,
                response: sanitizeValue(JSON.stringify(error.response?.data || null)),
            },
            timingMs: Date.now() - startedAt,
        });
    }
...
async function getWifiInfo(deviceId, options = {}) {
    ...
    const refreshObjects = resolvePathTemplates('wifiContainer').concat(['VirtualParameters', 'InternetGatewayDevice.LANDevice.1']);

    if (!skipRefresh) {
        await refreshDeviceObjects(deviceId, refreshObjects, { operation: 'refreshWifiInfo' });
