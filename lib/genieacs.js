...
function recordCircuitFailure(errorCode) {
...
async function genieacsRequest(method, path, options = {}) {
    ...
    recordCircuitFailure(lastErrorCode);
...
async function updatePsbDeviceConfig(deviceId, payload = {}, options = {}) {
    ...
    // ⚠️ UTANG TERBUKA (didokumentasikan supaya tidak hilang): jalur ini memakai
    // `verificationMode: 'accept_task_only'`, jadi hasil "berhasil" di sini berarti TASK DITERIMA
    // ACS — bukan bukti modem sudah menerapkannya. Padahal hasil inilah yang diumumkan wizard
    // sebagai "✅ online!" dan yang memicu pesan selamat datang berisi kredensial WiFi ke pelanggan.
    // Menutupnya butuh SATU fakta yang belum terukur: apakah GenieACS di deployment ini menjawab
    // 202 saat modem tak terjangkau (dan 200 saat benar-benar diterapkan). `submitTask` sudah
    // merekam `details.httpStatus`/`details.queuedOnly` untuk keperluan pengukuran itu.
    // JANGAN mengubah vonis di sini sebelum distribusinya diukur dari produksi — test lama memakai
    // 202 sebagai respons sukses normal, jadi menebak arah bisa membuat pemasangan yang BERHASIL
    // dilaporkan gagal dan menghentikan teknisi di lapangan.
    if (result.ok) {
        result.data = {
            ...(result.data || {}),
            deviceId,
