...
/**
 * Flush data dari file `-wal` ke file `.sqlite` utama sebelum file-level copy.
 * Tanpa step ini, backup file via `fs.copyFileSync` di WAL mode bisa kehilangan
 * transaksi terbaru yang masih nyangkut di `-wal`.
 */
async function checkpointBeforeBackup(sourcePath) {
    if (!fs.existsSync(sourcePath)) return;
    return new Promise((resolve) => {
        const db = new sqlite3.Database(sourcePath, (err) => {
            if (err) {
                console.warn(`[TELEGRAM_BACKUP] Tidak bisa open ${sourcePath} untuk checkpoint: ${err.message}`);
                resolve();
                return;
            }
            walCheckpoint(db)
...
async function performDatabaseBackup() {
    ...
    for (const db of databasesToBackup) {
        ...
        try {
            // Flush WAL ke DB utama supaya backup file berisi state terkini.
            await checkpointBeforeBackup(sourcePath);
