 ...
 */
"use strict";
const log = require('./logger').logger.child('SERVICE_REACHABILITY_PROBER');
...
async function probeCycle(deps = {}) {
    ...
    try {
        ...
        log.error("[SvcProber] siklus error:", err.message);
...
function startServiceProber() {
    if (isRunning) { log.info("[SvcProber] Sudah berjalan"); return; }
    const cfg = getServiceConfig();
    if (!cfg.enabled) { log.info("[SvcProber] Nonaktif (set config.serviceMonitor.enabled=true)"); return; }
    if (!cfg.valid) { log.warn("[SvcProber] Aktif tapi belum ada jalur ber-srcIp — tidak dijalankan."); return; }

    log.info(`[SvcProber] Start (interval ${Math.round(cfg.intervalMs / 1000)}s, ${cfg.services.length} layanan × ${cfg.paths.length} jalur)`);
