 ...
 */
"use strict";
const log = require('./logger').logger.child('UPSTREAM_QUALITY_ALERTER');
...
async function deliver(deps, alertsCfg, text) {
    ...
    if (!recipients.size) {
        log.warn("[UPQ-Alert] TIDAK ADA PENERIMA — isi phone_number akun admin (accounts.json) atau upstreamMonitor.alerts.recipients.");
        return 0;
    }
    let delivered = 0;
    for (const jid of recipients) {
        try {
            ...
            log.warn(`[UPQ-Alert] Gagal kirim ke ${jid}: ${err.message}`);
        }
    }
    log.info(`[UPQ-Alert] terkirim ${delivered}/${recipients.size} penerima`);
    return delivered;
}
...
async function rehydrateFromIncidents(deps) {
    ...
    try {
        ...
        log.warn(`[UPQ-Alert] Rehidrasi state gagal (lanjut state kosong): ${err.message}`);
    }
}
...
/**
 * Kalimat bukti traceroute untuk alert admin.
 *
 * !! MEMBACA `trace.jalur` (hasil analisa), BUKAN `trace.hops` (mentah). Larik mentah memuat
 * BEBERAPA RONDE tertumpuk — memakainya membuat nomor hop meleset dan jumlah hop menggelembung
 * (terukur: 60 baris untuk jalur yang sebenarnya 6 hop).
 *
 * Saat tak ada hop yang bisa dituduh, kalimatnya menyebut SEBAB dari penganalisa, bukan tebakan.
 * Versi lama menulis "indikasi loss intermiten" untuk keadaan apa pun — padahal sebab paling
 * sering justru "jalur SEHAT, tujuan tercapai utuh". Menebak di sini membuat admin mengejar
 * masalah yang tidak ada.
 *
 * Lonjakan RTT ikut disebut karena untuk keluhan GAME loss sering 0% — yang terasa justru
 * tambahan latensi, dan yang berguna adalah tahu di hop mana ia lahir.
 */
function buildTraceText(cfg, pathKey, trace) {
    ...
    const namaTarget = trace.targetKey ? `${target} (${trace.targetKey})` : target;
    // Bila pemanggil mengirim bentuk LAMA (cuma `hops` mentah, mis. insiden tersimpan yang
    // diputar ulang), analisa di sini juga — supaya nomor hop tetap benar dan tak ada jalur
    // pembacaan kedua yang diam-diam memakai larik mentah.
    const { analisaTrace } = require("./traceroute-analyzer");
    const path = Array.isArray(trace.jalur) && trace.jalur.length
        ? trace.jalur
        : analisaTrace(Array.isArray(trace.hops) ? trace.hops : []).jalur;
    const menjawab = path.filter((h) => h && h.menjawab).length;

    const lj = trace.lonjakanRtt;
    const lonjak = lj && lj.deltaMs > 3
        ? `\n   ↳ Latensi bertambah paling banyak antara ${lj.dari} → ${lj.ke} (+${fmt(lj.deltaMs)}ms, ${fmt(lj.porsiPct)}% dari total).`
        : "";

    if (trace.firstBadHop && trace.firstBadHop.address) {
        const cocok = path.find((x) => x.address === trace.firstBadHop.address);
        const nomor = trace.firstBadHop.posisi || (cocok && cocok.posisi) || "?";
        const sebelumnya = path.filter((x) => x.menjawab && x.posisi < nomor).pop();
        const sebelumText = sebelumnya
            ? ` Hop sebelumnya (${sebelumnya.address}) bersih ${fmt(sebelumnya.lossPct)}% loss.`
            : "";
        return `\n🔎 Traceroute via ${jalur} → ${namaTarget}: loss MULAI di hop ${nomor} ` +
            `(${trace.firstBadHop.address}, loss ${fmt(trace.firstBadHop.loss_pct)}%) dan bertahan sampai tujuan.${sebelumText}${lonjak}`;
    }
    if (menjawab) {
        return `\n🔎 Traceroute via ${jalur} → ${namaTarget}: ${menjawab} hop menjawab dari ${path.length} posisi ` +
            `(${trace.ronde || 1} ronde). Tidak ada hop yang bisa dituduh — ${trace.sebab || "sebab tak tercatat"}.${lonjak}`;
    }
    return `\n🔎 Traceroute via ${jalur} → ${namaTarget}: tidak ada hop terbaca (kemungkinan jalur buntu total). Detail di *Insiden*.`;
}
 ...
 */
function buildAffectedLine(cfg, pathKey, affected, opts = {}) {
    const pathCfg = pathCfgOf(cfg, pathKey);
    // Jalur "irisan-layanan" (mis. IH/FREE): trafik WA/game pelanggan MNI di-mangle lewat sini,
    // tapi resolveCustomerPath memetakan mereka ke jalur PRIMER (mni). Jadi hitung per-pelanggan
    // LIVE untuk jalur ini secara STRUKTURAL ~0 walau seluruh pelanggan MNI kena saat IH turun.
    // "0 pelanggan terdampak" di sini menyesatkan admin (deprioritas) — tampilkan anotasi cakupan
    // statis. Opt-in per jalur: config.upstreamMonitor.paths[].serviceSlice=true (default OFF, jadi
    // jalur cadangan yang memang kosong tetap jujur "0 pelanggan").
    const irisanLayanan = !!(pathCfg && pathCfg.serviceSlice);

    if (affected && affected.confidence === "live" && affected.count != null) {
        if (affected.count === 0) {
            if (irisanLayanan && pathCfg.affects) {
                return `\n• Terdampak: ${pathCfg.affects} _(jalur irisan-layanan — tak terhitung per-pelanggan)_`;
            }
            return `\n• Terdampak: *0 pelanggan* aktif di jalur ini saat ini`;
        }
        // Batas jumlah nama diatur dari panel (config.upstreamMonitor.report): report on-demand yang
        // ditarik admin default SEMUA (0), sedangkan notif alert (dorong) dibatasi supaya tak banjir.
        const report = (cfg && cfg.report) || {};
        const max = opts.alert
            ? (Number.isFinite(report.alertAffectedListMax) ? report.alertAffectedListMax : 5)
            : (Number.isFinite(report.affectedListMax) ? report.affectedListMax : 0);
        const allNames = (affected.customers || []).map((c) => c && c.name).filter(Boolean);
        const shown = max > 0 ? allNames.slice(0, max) : allNames; // 0 = tampilkan semua
        const sisa = affected.count - shown.length;
        if (!shown.length) {
            return `\n• Terdampak: *${affected.count} pelanggan* aktif di jalur ini`;
        }
        // Banyak nama → daftar vertikal bernomor (enak dibaca di WA); sedikit → inline (perilaku lama).
        if (shown.length > 5) {
            const lines = shown.map((n, i) => `\n   ${i + 1}. ${n}`).join("");
            const tail = sisa > 0 ? `\n   … +${sisa} lagi` : "";
            return `\n• Terdampak: *${affected.count} pelanggan* aktif di jalur ini:${lines}${tail}`;
        }
        const namesTxt = ` — ${shown.join(", ")}${sisa > 0 ? ` +${sisa} lagi` : ""}`;
...
function buildDirectionText(cfg, pathKey, entry, affected, opts = {}) {
    if (!entry) return "";
    let out = "";
    ...
    out += buildAffectedLine(cfg, pathKey, affected, opts);
    return out;
}
...
async function evaluateAfterCycle(cfg, depsOverride = {}) {
    try {
        ...
        // Penyaring jalur (#b258). TERUKUR 14 hari: menyalakan alerts APA ADANYA di Tanjungharjo
        // menghasilkan ~10,7 alert/hari — dan 7,2 di antaranya dari `vpn`, radio cadangan TERAKHIR
        // yang tak dipakai pelanggan mana pun. Alert sebanyak itu untuk jalur yang memang tak
        // dipakai melatih pemiliknya mengabaikan alert, termasuk yang penting.
        // Kosong = SEMUA jalur (perilaku lama), supaya menambah kunci ini tak pernah MEMATIKAN
        // alert yang sudah dinyalakan seseorang.
        const jalurDiawasi = Array.isArray(alertsCfg.paths) && alertsCfg.paths.length
            ? new Set(alertsCfg.paths.map((x) => String(x)))
            : null;
        ...
        for (const [pathKey, series] of perPath.entries()) {
            if (jalurDiawasi && !jalurDiawasi.has(String(pathKey))) continue;
            ...
            if (state.condition === "HEALTHY") {
                ...
                if (allSick && nowMs - state.lastAlertAt >= cooldownMs) {
                    ...
                    let saran = "";
                    // Notif alert (dorong ke admin) → batasi daftar nama terdampak (alertAffectedListMax).
                    saran += buildDirectionText(cfg, pathKey, entry, affected, { alert: true });
        ...
        log.warn(`[UPQ-Alert] Evaluasi gagal: ${err.message}`);
