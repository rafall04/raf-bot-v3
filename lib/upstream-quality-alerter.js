                ...
                if (allSick && nowMs - state.lastAlertAt >= cooldownMs) {
                    ...
                    if (alertsCfg.includeSteerSuggestion !== false && pathKey === "mni") {
                        ...
                        if (sfLatest && cycleLevel(sfLatest.lossAvg, thresholds) === "NORMAL") {
                            saran += `\n💡 Backup *SF* terpantau sehat (loss ${fmt(sfLatest.lossAvg)}%, RTT ${fmt(sfLatest.rttAvg)}ms). Router TIDAK failover otomatis saat degradasi (hanya saat tunnel mati).`;
                            // Bila fitur switch aktif, tawarkan aksi 1-langkah lewat WA.
                            const swEnabled = global.config && global.config.wanSwitch && global.config.wanSwitch.enabled === true;
                            saran += swEnabled
                                ? `\n➡️ Balas *switch koneksi* untuk mengalihkan MNI→SF sekarang (atau lewat panel /upstream-quality).`
                                : `\nPertimbangkan steer manual MNI→SF bila berlanjut.`;
                        }
                    }
