...
// Entri milik-CCTV DI HOST INI: cocok by netwatchId (paling presisi) ATAU klass CCTV — keduanya
// WAJIB se-host. #b324: dulu id-match mendahului cek host → cocok LINTAS-HOST. Saat IP CCTV berubah,
// device membawa netwatchId LAMA (entri masih di host LAMA), jadi ownedEntriesAt(host BARU, id LAMA)
// keliru mencocokkan entri lama → cabang SET memindahnya ke host baru, lalu blok rehost-remove
// menghapus entri yang sama → coverage HILANG total (monitor tak pernah broadcast DOWN lagi) padahal
// UI bilang sukses. Cek host DULU: lintas-host tak pernah dimaksud.
...
async function syncDevice(device, opts = {}) {
    ...
    if (owned.length > 0) {
        ...
        // PRESERVE entri yang SUDAH conformant (script CCTV valid — buatan tangan/Winbox) — auto-sync
        // TAK PERNAH menulis ulang script yang sudah jalan (menghormati entri yang benar; hindari
        // menormalisasi multi-baris→satu-baris & mengubah nama tampilan diam-diam). Script hanya
        // ditulis bila: (a) entri BELUM conformant (script belum standar → perlu dibuatkan), ATAU
        // (b) `forceScript` — admin memang meng-ganti nama/area lewat Edit, atau menekan "Sinkron
        // ulang" untuk sengaja menerapkan template terbaru (mis. setelah ganti token/pesan Telegram).
        const cls = classifyEntry(target);
        if (telegram && (!cls.conformant || opts.forceScript === true)) {
