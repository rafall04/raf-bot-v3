...
async function syncDevice(device, opts = {}) {
    ...
    if (owned.length > 0) {
        ...
        // Script: hanya tulis bila config valid DAN ISI beda dari router. Perbandingan MENGABAIKAN
        // pemisah baris — entri buatan tangan (Winbox) multi-baris (";\n"), generator WAJIB satu
        // baris (API RouterOS memecah nilai pada "\n"). Isi sama walau beda newline → JANGAN tulis
        // ulang (hormati entri yang sudah jalan). Beda ISI (mis. nama/area di-rename) → tulis (1 baris).
        const norm = (s) => String(s || '').replace(/[\r\n]+/g, '').trim();
        if (telegram && (norm(scripts.upScript) !== norm(target.up_script) || norm(scripts.downScript) !== norm(target.down_script))) {
