...
async function getUnsettledMarketingForTeknisi(teknisiId) {
    await ensureTable();
    ...
    const rows = await all(
        `SELECT * FROM psb_schedule
         WHERE marketing_type = 'teknisi' AND marketing_status = 'pending'
           AND marketing_ref_id = ? AND COALESCE(marketing_fee, 0) > 0
...
async function settleMarketingToPayroll({ teknisiId, payrollId = null, nowIso = null }) {
    await ensureTable();
    if (teknisiId == null || String(teknisiId).trim() === "") return { entries: [], netTotal: 0 };
    const snapshot = await getUnsettledMarketingForTeknisi(teknisiId);
    ...
    await run(
        `UPDATE psb_schedule SET marketing_status = 'settled', marketing_paid_at = ?, marketing_payroll_id = ?, updated_at = ?
         WHERE marketing_type = 'teknisi' AND marketing_status = 'pending' AND marketing_ref_id = ? AND COALESCE(marketing_fee, 0) > 0`,
        [now, payrollId != null ? String(payrollId) : null, now, String(teknisiId)]
    );
