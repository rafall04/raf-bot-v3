...
async function createRequest({ nama, hp, dusun, paket, latitude = null, longitude = null, catatan = "", requestedById = null, requestedByName = null, area = null, nowIso = null }) {
    await ensureTable();
    const now = nowIso || new Date().toISOString();
    const res = await run(
        `INSERT INTO psb_schedule (name, phone_number, dusun, paket, latitude, longitude, catatan, status, area, requested_by_id, requested_by_name, created_at, updated_at)
         VALUES (?, ?, ?, ?, ?, ?, ?, 'menunggu', ?, ?, ?, ?, ?)`,
        [nama, hp || null, dusun || null, paket || null, latitude, longitude, catatan || "", area, requestedById ? String(requestedById) : null, requestedByName || null, now, now]
    );
    const id = res.lastID;
    const ref = `PSB-${id}`;
    await run("UPDATE psb_schedule SET ref = ? WHERE id = ?", [ref, id]);
    return { id, ref, name: nama, phone_number: hp, dusun, paket, status: STATUS.MENUNGGU, area, catatan, latitude, longitude };
}
