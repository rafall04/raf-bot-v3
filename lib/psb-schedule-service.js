 ...
 * MainFuncs: `ensureTable`, `createRequest`, `listSchedules`, `getScheduleById`, `getScheduleSummary`, `setMarketing`, `payMarketingExternal`, `getUnsettledMarketingForTeknisi`, `settleMarketingToPayroll`, `getMarketingReport`, `STATUS`.
 * Note: `payMarketingExternal` bayar komisi LUAR via KAS (Fase 2a, createExpense DI-INJECT); `getUnsettledMarketingForTeknisi`/
 *        `settleMarketingToPayroll` dipakai `technician-finance-service` utk masukkan komisi TEKNISI ke payroll (Fase 2b).
...
// Komisi teknisi (pemberi lead) yang MASIH terutang (pending) — utk dimasukkan ke payroll (Fase 2b).
// PERIODE-AGNOSTIK: ambil SEMUA pending teknisi ini (bukan per bulan install) supaya komisi yang di-harga
// belakangan tak terlanjur "nyangkut" di payroll bulan lama yang sudah tutup. Return {entries, net_total}.
/** Batas periode 'YYYY-MM' dari bulan/tahun payroll; null kalau tak dibatasi. */
function batasPeriode(periodMonth, periodYear) {
...
/**
 * Komisi marketing teknisi yang belum masuk payroll, DIBATASI sampai periode payroll-nya.
 *
 * Tanpa batas ini satu payroll menyapu SELURUH komisi pending lintas bulan: payroll Juli yang
 * difinalisasi setelah lead Agustus terpasang akan menelan komisi Agustus dan membukukannya di
 * Juli. Batasnya "sampai dengan" (bukan "sama dengan") supaya komisi bulan lampau yang tercecer
 * tetap ikut terbayar, bukan menggantung selamanya.
 */
async function getUnsettledMarketingForTeknisi(teknisiId, { periodMonth = null, periodYear = null } = {}) {
    await ensureTable();
    if (teknisiId == null || String(teknisiId).trim() === "") return { entries: [], net_total: 0 };
    const batas = batasPeriode(periodMonth, periodYear);
    const params = [String(teknisiId)];
    let filterPeriode = "";
    if (batas) {
        filterPeriode = " AND substr(COALESCE(installed_at, updated_at, created_at), 1, 7) <= ?";
        params.push(batas);
    }
    const rows = await all(
        `SELECT * FROM psb_schedule
         WHERE marketing_type = 'teknisi' AND marketing_status = 'pending'
           AND marketing_ref_id = ? AND COALESCE(marketing_fee, 0) > 0${filterPeriode}
         ORDER BY datetime(COALESCE(installed_at, updated_at, created_at)) ASC, id ASC`,
        params
    );
    const net = rows.reduce((s, r) => s + (parseInt(r.marketing_fee, 10) || 0), 0);
    return { entries: rows, net_total: net };
}

// SETTLE komisi teknisi ke payroll: kunci semua pending teknisi ini jadi `settled` + tempel payroll_id.
// Dipanggil `technician-finance-service.finalizePayroll` (mirror lockCollectionEntriesToPayroll). netTotal
// dari snapshot baca (pola sama dgn collection). Setelah settled, row terkunci dari setMarketing (kunci).
async function settleMarketingToPayroll({ teknisiId, payrollId = null, nowIso = null, periodMonth = null, periodYear = null }) {
    await ensureTable();
    if (teknisiId == null || String(teknisiId).trim() === "") return { entries: [], netTotal: 0 };
    const snapshot = await getUnsettledMarketingForTeknisi(teknisiId, { periodMonth, periodYear });
    if (!snapshot.entries.length) return { entries: [], netTotal: 0 };
    const now = nowIso || new Date().toISOString();
    // Kunci baris yang PERSIS ada di snapshot, bukan mengulang filternya. Filter yang diulang
    // bisa mengunci baris yang lahir di antara pembacaan dan penulisan — nominalnya lalu tak
    // sama dengan yang tercatat di payroll.
    const ids = snapshot.entries.map((r) => r.id);
    const placeholders = ids.map(() => "?").join(", ");
    await run(
        `UPDATE psb_schedule SET marketing_status = 'settled', marketing_paid_at = ?, marketing_payroll_id = ?, updated_at = ?
         WHERE id IN (${placeholders}) AND marketing_status = 'pending'`,
        [now, payrollId != null ? String(payrollId) : null, now, ...ids]
    );
    return { entries: snapshot.entries, netTotal: snapshot.net_total };
}

/**
 * Melepas komisi marketing yang terkunci ke sebuah payroll, kembali jadi `pending`.
 *
 * Dipakai saat finalisasi payroll DIBATALKAN. Tanpa ini komisi tetap `settled` menempel pada
 * payroll yang tak jadi dibayar — uang teknisi hilang dari perhitungan dan tak bisa masuk
 * payroll mana pun lagi.
 */
async function unsettleMarketingFromPayroll({ payrollId }) {
    await ensureTable();
    if (payrollId == null) return { dilepas: 0 };
    const res = await run(
        `UPDATE psb_schedule
            SET marketing_status = 'pending', marketing_paid_at = NULL, marketing_payroll_id = NULL, updated_at = ?
          WHERE marketing_payroll_id = ? AND marketing_status = 'settled'`,
        [new Date().toISOString(), String(payrollId)]
    );
    return { dilepas: (res && res.changes) || 0 };
}

// Laporan komisi marketing per PEMBERI LEAD (Fase 3). Kelompokkan row psb_schedule (fee>0) per referrer
// (teknisi by ref_id, luar/lainnya by nama) + rincian + total. Filter periode by tgl-aktivitas (install/
// update/create) bila month+year diberi; else all-time. Read-only (dashboard admin). area opsional.
async function getMarketingReport({ month = null, year = null, area = null } = {}) {
    await ensureTable();
    const where = ["COALESCE(marketing_fee, 0) > 0"];
    const params = [];
    if (month && year) {
        where.push("substr(COALESCE(installed_at, updated_at, created_at), 1, 7) = ?");
        params.push(`${year}-${String(month).padStart(2, "0")}`);
    }
    if (area) { where.push("area = ?"); params.push(area); }
    const rows = await all(
        `SELECT id, ref, name, dusun, marketing_type, marketing_ref_id, marketing_ref_name, marketing_ref_phone,
                marketing_fee, marketing_status, installed_at, updated_at, created_at
         FROM psb_schedule WHERE ${where.join(" AND ")}
         ORDER BY datetime(COALESCE(installed_at, updated_at, created_at)) DESC, id DESC`,
        params
    );

    const byRef = new Map();
    const totals = { total_fee: 0, pending_amount: 0, paid_amount: 0, settled_amount: 0, count: 0, teknisi_fee: 0, luar_fee: 0 };
    const entries = [];
    for (const r of rows) {
        const fee = parseInt(r.marketing_fee, 10) || 0;
        const type = r.marketing_type || "lainnya";
        const name = r.marketing_ref_name || (type === "teknisi" ? "(teknisi)" : "(tak diketahui)");
        const key = type === "teknisi" ? `teknisi:${r.marketing_ref_id || name}` : `${type}:${String(name).toLowerCase()}`;
        const status = r.marketing_status || "pending";
        if (!byRef.has(key)) byRef.set(key, { referrer_key: key, type, name, phone: r.marketing_ref_phone || null, count: 0, total_fee: 0, pending_amount: 0, paid_amount: 0, settled_amount: 0 });
        const b = byRef.get(key);
        b.count += 1; b.total_fee += fee;
        totals.count += 1; totals.total_fee += fee;
        if (status === "paid") { b.paid_amount += fee; totals.paid_amount += fee; }
        else if (status === "settled") { b.settled_amount += fee; totals.settled_amount += fee; }
        else { b.pending_amount += fee; totals.pending_amount += fee; }
        if (type === "teknisi") totals.teknisi_fee += fee; else totals.luar_fee += fee;
        entries.push({ ref: r.ref, customer_name: r.name, dusun: r.dusun, type, referrer_name: name, fee, status, date: r.installed_at || r.updated_at || r.created_at });
    }
    const summary = Array.from(byRef.values()).sort((a, b) => b.total_fee - a.total_fee);
...
module.exports = {
    ...
    getUnsettledMarketingForTeknisi,
    settleMarketingToPayroll,
    unsettleMarketingFromPayroll,
    getMarketingReport,
    setDbPathForTest
};
