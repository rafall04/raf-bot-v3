 ...
 * MainFuncs: filterTeknisiRecipients, isQuietNow, isSnoozed.
...
function filterTeknisiRecipients(opsi = {}) {
    ...
    for (const r of recipients) {
        ...
        if (area && Array.isArray(p.areas) && p.areas.length && !p.areas.includes(area)) {
            dropped.push({ recipient: r, reason: "luar-area" }); continue;
        }
        if (!critical) {
...
module.exports = { filterTeknisiRecipients, isQuietNow, isSnoozed, hhmmToMinutes };
