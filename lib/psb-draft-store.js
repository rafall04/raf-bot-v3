 ...
 */
"use strict";
const log = require('./logger').logger.child('PSB_DRAFT_STORE');
...
function loadDrafts(filePath = resolveFilePath()) {
    try {
        ...
        log.warn(`[PSB_DRAFT_STORE] Gagal membaca ${path.basename(filePath)}: ${err.message}`);
        return [];
    }
}
...
function saveDrafts(drafts, filePath = resolveFilePath()) {
    try {
        ...
        log.error(`[PSB_DRAFT_STORE] Gagal menulis ${path.basename(filePath)}: ${err.message}`);
        return false;
    }
}
...
function putDraft(ownerKey, draft, filePath = resolveFilePath(), nowMs = Date.now()) {
    if (!ownerKey || !draft || !draft.data) {
        log.warn("[PSB_DRAFT_STORE] putDraft ditolak: ownerKey & draft.data wajib.");
        return null;
    }
