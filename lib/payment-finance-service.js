/**
 * Header Doc
 * Purpose: Menjadi source of truth ledger pembayaran periodik, sinkronisasi flag `users.paid`, dan helper read-model billing.
 * Caller: Route payment, approval service, billing service, cron, dan diagnostics pembayaran.
 * Deps: `sqlite3`, `./database`, `./env-config`, `./financial-ledger`, `./technician-collection-settlement`, dan runtime app.
 * MainFuncs: `applyPaymentStatusChange`, `getPaymentPositionForPeriod`, `getPaymentStatusReadModel`, `syncUserPaidStatusForCurrentPeriod`, `getPaymentDiagnostics`, `withPaymentReadContext`.
...
function getReaderDb() {
    ...
    // Apply pragmas async — readerGet/readerAll yang dipanggil setelahnya
    // tetap aman karena sqlite3 driver men-queue perintah sampai DB siap.
    const { applySqlitePragmas } = require('./sqlite-pragmas');
    applySqlitePragmas(db).catch((pragmaErr) => {
        console.warn(`[PAYMENT_FINANCE_READER_PRAGMA_WARN] ${pragmaErr.message}`);
    });
    return db;
}
...
async function getPaymentPositionForPeriod(user, periodMonth, periodYear, options = {}) {
    ...
    const { paidRow, reversalRow } = await withReaderContext(async (context) => ({
    ...
    const outstanding = Math.max(0, amountDue - netPaid);
    ...
    return {
        ...
        net_paid: netPaid,
        outstanding,
        is_fully_paid: amountDue > 0 ? netPaid >= amountDue : netPaid > 0
    };
}
