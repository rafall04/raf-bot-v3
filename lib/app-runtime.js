 ...
 */
"use strict";
const log = require('./logger').logger.child('APP_RUNTIME');
        ...
        async initializeBackgroundServices() {
            ...
            this.dbInitPromise.then(async () => {
                runAutoMigration().catch((err) => {
                    log.error("[STARTUP] Auto-migration failed:", err.message);
                });
                ...
                try {
                    ...
                    log.error("[OLT-Scraper] Failed to start:", err.message);
                }
                ...
                try {
                    ...
                    log.error("[OLT-Syslog] Failed to start:", err.message);
                }
                ...
                try {
                    ...
                    log.error("[OLT-EventLog] Prune init gagal:", err.message);
                }

                // Message-log — prune korpus pesan masuk (inbound_messages) yang dulu tumbuh TANPA
                // batas. Never-throw; timer harian ber-unref. Gate config.messageLogging.retentionDays.
                try {
                    const msgLogRepo = require("../repositories/message-log.repository").getMessageLogRepository();
                    const msgLogCfg = (global.config && global.config.messageLogging) || {};
                    const msgRetentionDays = Number.isFinite(msgLogCfg.retentionDays) ? msgLogCfg.retentionDays : 180;
                    msgLogRepo.pruneOld(msgRetentionDays).catch(() => {});
                    const msgPruneTimer = setInterval(() => {
                        msgLogRepo.pruneOld(msgRetentionDays).catch(() => {});
                    }, 24 * 60 * 60 * 1000);
                    if (msgPruneTimer && typeof msgPruneTimer.unref === "function") msgPruneTimer.unref();
                } catch (err) {
                    log.error("[MessageLog] Prune init gagal:", err.message);
                }
                ...
                try {
                    ...
                    log.error("[OLT-State] maintenance init gagal:", err.message);
                }
                ...
                try {
                    ...
                    log.error("[OLT-EventLog] Warm-up resolver GenieACS gagal:", err.message);
                }
                ...
                try {
                    ...
                    log.error("[OLT-rxPoller] Failed to start:", err.message);
                }
                ...
                try {
                    ...
                    log.error("[OLT-SNMP-LOS] Failed to start:", err.message);
                }
                ...
                try {
                    ...
                    log.error("[CCTV] Failed to start:", err.message);
                }
                ...
                try {
                    ...
                    log.error("[UPQ-Poller] Failed to start:", err.message);
                }
                ...
                try {
                    ...
                    log.error("[REBOOTFU] Failed to start:", err.message);
                }

                // Draft laporan gangguan pelanggan yang menunggu foto. Draft hanya hidup di
                // stateStore (memori) dengan promosi lewat setTimeout 15 menit — keduanya
                // lenyap saat pm2 restart (produksi 7-13x/hari), membuang laporan pelanggan
                // TANPA jejak log. Pemindaian ini membacanya dari disk: sekali saat boot
                // (menyelamatkan draft yang restartnya menimpa jendela 15 menit) lalu berkala.
                try {
                    const smartReport = require("../message/handlers/smart-report-handler");
                    if (smartReport && typeof smartReport.pindaiDraftLaporanTertunda === "function") {
                        const jalankanPindai = () => {
                            smartReport
                                .pindaiDraftLaporanTertunda()
                                .catch((e) => log.error("[MATI_DRAFT_SCAN] tick gagal:", e?.message));
                        };
                        // Boot: beri jeda agar DB & koneksi WA sempat siap sebelum membuat tiket.
                        // unref: pemindaian housekeeping — tak layak menahan event loop.
                        const bootScanTimer = setTimeout(jalankanPindai, 60 * 1000);
                        if (typeof bootScanTimer.unref === "function") bootScanTimer.unref();
                        const periodicScanTimer = setInterval(jalankanPindai, 5 * 60 * 1000);
                        if (typeof periodicScanTimer.unref === "function") periodicScanTimer.unref();
                    }
                } catch (err) {
                    log.error("[MATI_DRAFT_SCAN] Failed to start:", err.message);
                }

                // Otorisasi massal di LATAR — permintaan HTTP balik seketika, worker mengerjakan
                // seluruh pending satu per satu dan menulis hasil per pelanggan ke log yang bisa
                // dibaca operator. Gate config.bulkApprovalJob.enabled.
                try {
                    const bulkJob = require("../services/bulk-approval-job.service");
                    if (bulkJob && typeof bulkJob.startBulkApprovalWorker === "function") {
                        bulkJob.startBulkApprovalWorker();
                    }
                } catch (err) {
                    log.error("[OTORISASI_JOB] Failed to start:", err.message);
                }
                ...
                try {
                    ...
                    log.error("[NOTIF_DIGEST] Failed to start:", err.message);
                }
                ...
                try {
                    ...
                    log.error("[SvcProber] Failed to start:", err.message);
                }
                ...
                try {
                    ...
                    log.error("[CustSteer] Failed to start:", err.message);
                }
                ...
                try {
                    ...
                    log.error("[STEER-DRIFT] Failed to start:", err.message);
                }
                ...
                try {
                    ...
                    log.error("[TELEGRAM_TEKNISI] Failed to start:", err.message);
                }
                ...
                try {
                    ...
                    log.error("[CUSTOMER_TRAFFIC] Failed to start collector:", err.message);
                }
            }).catch((err) => {
                log.error("[DATABASE] Failed to initialize database:", err);
            });
        }
    };
