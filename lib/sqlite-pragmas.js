 ...
 */
"use strict";
const log = require('./logger').logger.child('SQLITE_PRAGMAS');
        ...
        db.serialize(() => {
            db.run("PRAGMA journal_mode = WAL", (err) => {
                if (err) {
                    ...
                    log.warn(`[SQLITE_PRAGMA_WARN] Gagal set journal_mode=WAL: ${err.message}`);
                }
            });
            db.run("PRAGMA synchronous = NORMAL", (err) => {
                if (err) {
                    log.warn(`[SQLITE_PRAGMA_WARN] Gagal set synchronous=NORMAL: ${err.message}`);
                }
            });
...
async function hotBackupSqlite(srcPath, destPath) {
    ...
    for (const suffix of ["-wal", "-shm"]) {
        try {
            ...
            log.warn(`[HOT_BACKUP_WARN] Gagal copy companion ${suffix}: ${e.message}`);
        }
    }
    ...
    await new Promise((resolve) => {
        const db = new sqlite3.Database(destPath, (err) => {
            if (err) {
                log.warn(`[HOT_BACKUP_WARN] Tidak bisa open salinan untuk konsolidasi: ${err.message}`);
                resolve();
                return;
            }
...
function walCheckpoint(db) {
    return new Promise((resolve) => {
        ...
        db.run("PRAGMA wal_checkpoint(TRUNCATE)", (err) => {
            if (err) {
                log.warn(`[SQLITE_WAL_CHECKPOINT_WARN] ${err.message}`);
            }
            resolve();
        });
    });
...
function cleanupWalCompanions(dbFilePath) {
    for (const suffix of ["-wal", "-shm"]) {
        ...
        try {
            ...
            log.warn(`[SQLITE_WAL_CLEANUP_WARN] Gagal hapus ${companionPath}: ${e.message}`);
        }
    }
}
