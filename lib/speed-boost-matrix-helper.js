...
const log = require('./logger').logger.child('SPEED_BOOST_MATRIX_HELPER');
const fs = require('fs');
...
function loadSpeedBoostConfig() {
    try {
        ...
        log.error('[SPEED_BOOST_CONFIG_LOAD_ERROR]', error);
    }
...
function getMessageTemplate(type, variables = {}) {
    ...
    return templateService.renderString(template.replace(/\\n/g, '\n'), variables).text;
}
