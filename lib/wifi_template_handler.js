const log = require('./logger').logger.child('WIFI_TEMPLATE_HANDLER');
const fs = require('fs');
...
function loadWifiTemplates() {
    try {
        ...
        // File ini WAJIB array. Deploy pernah menulisnya sebagai object ber-key
        // numerik ("0","1",...) -> wifiTemplates.map melempar TypeError di setiap
        // pesan masuk dan bot berhenti merespons total tanpa crash proses.
        if (!Array.isArray(parsed)) {
            log.error(`[WiFi_Template_Handler] wifi_templates.json bukan array (type: ${typeof parsed}) — cache lama dipertahankan.`);
            return;
        }
        wifiTemplates = parsed;
        // Templates loaded silently
    } catch (error) {
        log.error('[WiFi_Template_Handler] Error loading wifi_templates.json:', error);
        if (!Array.isArray(wifiTemplates)) {
            wifiTemplates = [];
        }
    }
}
...
if (process.env.NODE_ENV !== 'test' && process.env.DISABLE_FILE_WATCHERS !== 'true') {
    fs.watchFile(templatesPath, (_curr, _prev) => {
...
module.exports = {
    ...
    getWifiTemplates: () => wifiTemplates // Katalog perintah live (sumber generator menu owner)
};
