 ...
 *       utilisasi/segmen), config `global.config.upstreamMonitor`.
...
const DEFAULT_PATHS = [
    { key: "gmdp", label: "GMDP (utama)", routingTable: null, iface: "1.VLAN62-GMDP", gatewayTarget: "195.168.62.1" },
    { key: "ih", label: "IndiHome direct", routingTable: "FREE", iface: "2.VLAN502", gatewayTarget: "192.168.102.1" },
    { key: "mni", label: "IH via MNI", routingTable: "MNI", iface: "Tunnel-MNI", tunnelType: "l2tp" },
    { key: "sf", label: "SF (backup MNI)", routingTable: "SF-PROBE", iface: "SF", tunnelType: "sstp" }
];
...
async function runTraceProbe(pathKey, targetAddress = null, deps = {}) {
    try {
        ...
        return { ok: true, firstBadHop, hopCount: hops.length };
    } catch (err) {
        return { ok: false, error: err.message };
    }
}
...
module.exports = {
    ...
    _internal: { verdictForTarget, worstVerdict, detectFailover, runProbeBridge, runBridgeSpec, DEFAULT_PATHS, DEFAULT_TARGETS, DEFAULT_THRESHOLDS, prevLinkCounters }
};
