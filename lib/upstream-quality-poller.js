 ...
 */
"use strict";
const log = require('./logger').logger.child('UPSTREAM_QUALITY_POLLER');
...
function getMonitorConfig() {
    ...
    return {
        ...
        // Gerbang kalimat kestabilan ke PELANGGAN. WAJIB diteruskan di sini — halaman admin
        // membaca setelan lewat getMonitorConfig, dan kunci yang tak disebut akan terbaca MATI
        // walau di disk menyala. Menyimpan form lalu MEMATIKANNYA diam-diam (#b266).
        stabilitasPelanggan: { enabled: false, ...(cfg.stabilitasPelanggan || {}) },
        // Ambang kesepakatan antar target (#b264) & jumlah probe traceroute (#b256).
        minTargetSepakat: Number(cfg.minTargetSepakat) > 0 ? Number(cfg.minTargetSepakat) : 2,
        traceCount: Number(cfg.traceCount) > 0 ? Number(cfg.traceCount) : 10,
        traceLookbackMinutes: Number(cfg.traceLookbackMinutes) > 0 ? Number(cfg.traceLookbackMinutes) : 10,
...
async function pollOnce(deps = {}) {
    ...
    try {
        ...
        log.error("[UPQ-Poller] Siklus probe error:", err.message);
...
function verdictBySepakat(verdicts, minSepakat = 2) {
    ...
    // Sampai sini tak ada tingkat PARAH yang sepakat. "NORMAL" hanya sah bila ADA bukti nyata:
    // minimal satu verdict non-UNKNOWN. Bila SEMUA target UNKNOWN (sampel < minSamples — router
    // timbul-tenggelam saat insiden, DB baru terisi sedikit), ini BUTA, bukan sehat. Dulu jatuh ke
    // "NORMAL" karena UNKNOWN ber-severity 0 tak pernah memicu return di loop → pelanggan yang
    // mengeluh saat gangguan justru dibantah "koneksi terpantau normal" (classifyOnlineVerdict
    // HEALTHY). 'Tak bisa diamati' ≠ 'teramati sehat' — pulangkan UNKNOWN agar hilir → INCONCLUSIVE.
    return daftar.some((v) => v !== "UNKNOWN") ? "NORMAL" : "UNKNOWN";
}
...
async function buildStatusReport(deps = {}) {
    ...
    return {
        ...
        // Dibawa ke laporan supaya halaman admin bisa MENJELASKAN vonisnya (#b267): "1 dari 2
        // target bermasalah" berbeda artinya dari "2 dari 2", dan bedanya menentukan apakah
        // masalahnya di tujuan atau di jalur kita.
        minTargetSepakat: cfg.minTargetSepakat || 2,
        paths,
        failover,
...
function startUpstreamQualityPoller() {
    if (isRunning) {
        log.info("[UPQ-Poller] Sudah berjalan");
        return;
    }
    const cfg = getMonitorConfig();
    if (!cfg.enabled) {
        log.info("[UPQ-Poller] Nonaktif (set config.upstreamMonitor.enabled=true untuk mengaktifkan)");
        return;
    }
    if (!cfg.valid) {
        log.warn("[UPQ-Poller] Aktif tapi konfigurasi tidak lengkap (host/user/password) — poller tidak dijalankan.");
        return;
    }

    log.info(`[UPQ-Poller] Start (interval ${Math.round(cfg.intervalMs / 1000)}s, ${cfg.paths.length} jalur × ${cfg.targets.length} target, router ${cfg.host}:${cfg.port})`);
