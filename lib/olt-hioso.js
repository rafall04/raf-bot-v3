 ...
 */
const log = require('./logger').logger.child('OLT_HIOSO');
...
async function getMultipleOltData(oltDevices) {
    ...
    log.info(`[OLT] Querying ${oltDevices.length} OLT device(s) in parallel...`);
    ...
    const promises = oltDevices.map(async (olt) => {
        try {
            ...
            log.error(`[OLT] Error querying ${olt.name}:`, error.message);
            return {
                oltId: olt.id,
                oltName: olt.name,
                oltHost: olt.host,
    ...
    results.forEach(result => {
        if (result.status === 'success' && result.onus.length > 0) {
            ...
            log.info(`[OLT] [${result.oltName}] Got ${result.onus.length} ONUs`);
        } else if (result.status === 'error') {
            log.error(`[OLT] [${result.oltName}] Error: ${result.message}`);
        }
    });
    
    log.info(`[OLT] Total ONUs from all OLTs: ${allOnus.length}`);
    ...
    if (failedOlts.length > 0) {
        log.warn(`[OLT] Snapshot SEBAGIAN: ${failedOlts.length} dari ${results.length} OLT tidak terbaca`
...
async function getSingleOnuDataWithCache(mac, slotId, onuId) {
    ...
    if (cachedOlt) {
        ...
        log.info(`[OLT] Cache hit for MAC ${normalizedMac}: ${cachedOlt.oltName}`);
        ...
        if (oltDevice) {
            ...
            log.info(`[OLT] Cache hit but query failed/unreadable, falling back to query all OLTs`);
        }
    }
    
    // Cache miss atau query failed - query all OLTs
    log.info(`[OLT] Cache miss for MAC ${normalizedMac}, querying all OLTs...`);
    ...
    const promises = oltDevices.map(async (olt) => {
        ...
        try {
            ...
            if (result.status === 'success' && result.data && result.data.rxPower !== 'N/A') {
                ...
                log.info(`[OLT] Found ONT in ${olt.name}, updating cache`);
                return result;
            }
            return null;
        } catch (error) {
