 ...
 *          `onu.statusKnown=false` saat phaseState tak menyebut ONU itu, `incompleteWalks` di
 *          tingkat OLT, dan status `error` bila SEMUA OLT gagal dibaca (bukan sukses berisi nol
 *          ONU, yang di hilir terbaca sebagai "semua pelanggan offline").
...
async function getOltData(config) {
    ...
    const dataPromise = new Promise(async (resolve) => {
        ...
        try {
            ...
            const getOrCreateOnu = (oid) => {
                ...
                if (!onuMap.has(key)) {
                    onuMap.set(key, {
                        ...
                        // Tiap walk OID di bawah di-catch jadi array kosong (biar satu OID rewel tak
                        // menjatuhkan seluruh fetch); tanpa penanda ini, walk phaseState yang timeout
            ...
            // WALK YANG GAGAL SEBAGIAN dilaporkan, bukan ditelan. Tiap walk OID di-catch jadi []
            // supaya satu OID rewel tak menjatuhkan fetch — konsekuensinya hasil bisa "setengah
            // jadi" dan tampak utuh. Yang paling menyesatkan: rxPower sukses tapi phaseState gagal
            // → redaman terisi, status tak diketahui, dan pembaca menyimpulkan "sinyal bagus".
            ...
            if (incompleteWalks.length > 0) {
                console.warn(`[OLT] Walk tidak lengkap dari ${config.host}: ${incompleteWalks.join(', ')} kosong — data ONU tidak utuh.`);
            }
            ...
            // Semua walk kosong TIDAK dianggap error — itu bisa berarti OLT yang memang belum ada
            // ONU-nya (instalasi baru); kasus itu sudah cukup ditandai `incompleteWalks`.
