...
const LAST_DOWN_CAUSE = {
    NORMAL: '0',
    OFFLINE: '1',        // Generic offline (bisa LOS atau Dying Gasp)
    // Nilai lain mungkin ada tapi belum terverifikasi
};
 ...
 */
function getOidSuffix(oid, baseOid) {
    const normalizedBase = baseOid.startsWith('.') ? baseOid.substring(1) : baseOid;
    const normalizedOid = oid.startsWith('.') ? oid.substring(1) : oid;
    
    if (normalizedOid.startsWith(normalizedBase)) {
        const suffix = normalizedOid.substring(normalizedBase.length);
        return suffix.startsWith('.') ? suffix.substring(1) : suffix;
    }
    
    // Fallback: ambil 3 bagian terakhir
    const parts = oid.split('.');
    if (parts.length >= 3) {
...
async function getSingleOnuDataWithCache(mac, slotId, onuId) {
    ...
    if (cachedOlt) {
        ...
        if (oltDevice) {
            ...
            const result = await getSingleOnuData(config, slotId, onuId);
