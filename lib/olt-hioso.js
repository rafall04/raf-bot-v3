 ...
 *          `failedWalks` di tingkat OLT, dan status `error` bila SEMUA OLT gagal dibaca (bukan
 *          sukses berisi nol ONU, yang di hilir terbaca sebagai "semua pelanggan offline").
...
async function getOltData(config) {
    ...
    const dataPromise = new Promise(async (resolve) => {
        ...
        try {
            ...
            const jalankanWalk = (nama, oid) => performSnmpWalk(session, oid)
                .then((rows) => ({ nama, rows, gagal: false }))
                .catch((err) => ({ nama, rows: [], gagal: true, alasan: err && err.message }));

            const [macW, rxW, phaseW, causeW, nameW] = await Promise.all([
                jalankanWalk('mac', HIOSO_OID.onu.mac),
                jalankanWalk('rxPower', HIOSO_OID.onu.rxPower),
                jalankanWalk('phaseState', HIOSO_OID.onu.phaseState),
                jalankanWalk('lastDownCause', HIOSO_OID.onu.lastDownCause),
                jalankanWalk('name', HIOSO_OID.onu.name),
            ]);

            const semuaWalk = [macW, rxW, phaseW, causeW, nameW];
            const walkGagal = semuaWalk.filter((w) => w.gagal).map((w) => w.nama);

            // SEMUA walk gagal = OLT tak menjawab. Dulu ini lolos jadi "sukses berisi nol ONU" —
            // persis wajah yang sama dengan OLT baru yang belum punya ONU, padahal artinya
            // berlawanan. Terlihat di OLT Dander 192.168.11.2 (perangkat tak menjawab ARP di
            // segmennya sendiri) yang kadang terlaporkan sukses-kosong.
            if (walkGagal.length === semuaWalk.length) {
                console.error(`[OLT] ${config.host}: SEMUA walk gagal (${macW.alasan || 'tak menjawab'}) — pembacaan DITOLAK.`);
                closeSession();
                resolve({
                    status: 'error',
                    message: `OLT tidak menjawab SNMP: ${macW.alasan || 'semua walk gagal'}`,
                    onus: [],
                    systemInfo: {},
                    incompleteWalks: ['mac', 'phaseState', 'rxPower'],
                    failedWalks: walkGagal
                });
                return;
            }

            const onuMacResults = macW.rows;
            const onuRxPowerResults = rxW.rows;
            const onuPhaseStateResults = phaseW.rows;
            const onuLastDownCauseResults = causeW.rows;
            const onuNameResults = nameW.rows;
            ...
            // pembaca menyimpulkan "sinyal bagus".
            const incompleteWalks = [];
            if (onuMacResults.length === 0) incompleteWalks.push('mac');
            if (onuPhaseStateResults.length === 0) incompleteWalks.push('phaseState');
            if (onuRxPowerResults.length === 0) incompleteWalks.push('rxPower');
            oltData.incompleteWalks = incompleteWalks;
            oltData.failedWalks = walkGagal;
            if (incompleteWalks.length > 0) {
                const sebab = walkGagal.length ? ` (gagal: ${walkGagal.join(', ')})` : ' (kosong, bukan gagal)';
                console.warn(`[OLT] Walk tidak lengkap dari ${config.host}: ${incompleteWalks.join(', ')}${sebab} — data ONU tidak utuh.`);
            }

            // WALK MAC KOSONG PADAHAL KOLOM LAIN BERISI = PEMBACAAN GAGAL, bukan "OLT tanpa ONU".
            // MAC adalah kunci identitas ONU di HIOSO, dan filter di atas membuang setiap ONU yang
            ...
            const kolomLainBerisi = onuPhaseStateResults.length > 0
                || onuRxPowerResults.length > 0
                || onuNameResults.length > 0
                || onuLastDownCauseResults.length > 0;
            if (onuMacResults.length === 0 && kolomLainBerisi) {
                console.error(`[OLT] ${config.host}: walk MAC kosong padahal kolom lain berisi — pembacaan DITOLAK (bukan "semua ONU hilang").`);
                closeSession();
                resolve({
                    status: 'error',
                    message: 'Pembacaan OLT tidak utuh: daftar MAC ONU kosong padahal kolom lain terbaca',
                    onus: [],
                    systemInfo: {},
                    incompleteWalks,
                    failedWalks: walkGagal
            ...
            // MAC kosong TANPA kolom lain berisi: sah HANYA bila tak ada walk yang gagal — itu
            // OLT yang memang belum punya ONU (instalasi baru). Bila ada yang gagal, kekosongan itu
            // buah kegagalan, bukan kenyataan lapangan.
            if (onuMacResults.length === 0 && walkGagal.length > 0) {
                console.error(`[OLT] ${config.host}: MAC kosong akibat walk gagal (${walkGagal.join(', ')}) — pembacaan DITOLAK.`);
                closeSession();
                resolve({
                    status: 'error',
                    message: `Pembacaan OLT gagal pada walk: ${walkGagal.join(', ')}`,
                    onus: [],
                    systemInfo: {},
                    incompleteWalks,
                    failedWalks: walkGagal
...
async function getMultipleOltData(oltDevices) {
    ...
    return {
        ...
        onus: allOnus,
        systemInfo: {},
        // Merk/OLT yang walk-nya tak lengkap — dipakai UI untuk menandai data setengah jadi.
        incompleteWalks: results
            .filter((result) => Array.isArray(result.incompleteWalks) && result.incompleteWalks.length > 0)
            .map((result) => ({ oltId: result.oltId, oltName: result.oltName, walks: result.incompleteWalks })),
