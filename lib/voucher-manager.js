...
async function purchaseVoucherWithSaldo(userId, voucherProfileId, saldoManager) {
    try {
        ...
        if (!deducted) {
            ...
            // berubah karena pembelian paralel). Log keras agar operator bisa menindaklanjuti voucher yatim.
            console.error(`[VOUCHER_ORPHAN] Saldo gagal dipotong untuk user ${userId} setelah voucher ${voucherCode.username} dibuat di MikroTik. Perlu ditinjau manual.`);
            return {
                success: false,
