/**
 * Header Doc
 * Purpose: Kelola profil voucher (`database/voucher.json`) + riwayat pembelian voucher
 *          (`database/voucher_purchases.json`, jejak uang) + rekonsiliasi orphan.
 * Caller: routes/message handler voucher, lib/agent-voucher-manager.
 * Deps: `fs` (baca), `path`, `lib/mikrotik`, `lib/financial-ledger`, `lib/voucher-orphan`,
 *       `lib/json-store` (saveJSON ATOMIK).
 * MainFuncs: getVoucherProfiles, saveVoucherProfiles, getPurchaseHistory, savePurchaseHistory.
 * SideEffects: Menulis `database/voucher.json` & `database/voucher_purchases.json` (ATOMIK).
 */
const log = require('./logger').logger.child('VOUCHER_MANAGER');
const fs = require('fs');
...
const { saveJSON } = require('./json-store'); // #b345 tulis ATOMIK (voucher.json multi-writer + riwayat beli)
...
function getVoucherProfiles() {
    try {
        ...
        log.error('Error loading voucher profiles:', error);
        return [];
    }
}

// Save voucher profiles — #b345 ATOMIK (voucher.json juga ditulis lib/voucher.js: multi-writer).
function saveVoucherProfiles(profiles) {
    saveJSON('voucher.json', profiles);
}
...
// Save purchase history — #b345 ATOMIK (riwayat pembelian voucher = jejak uang).
function savePurchaseHistory(purchases) {
    saveJSON('voucher_purchases.json', purchases);
}
...
async function generateVoucherCode(profile, userId) {
    try {
        ...
        log.error('Error generating voucher:', error);
        throw error;
    }
}
...
async function purchaseVoucherWithSaldo(userId, voucherProfileId, saldoManager) {
    try {
        ...
        if (!deducted) {
            ...
            log.error(`[VOUCHER_ORPHAN] Saldo gagal dipotong untuk user ${userId} setelah voucher ${voucherCode.username} dibuat di MikroTik. Dicatat ke worklist orphan.`);
            return {
                success: false,
        ...
        try {
            ...
            log.error('[VOUCHER_LEDGER_SYNC_ERROR]', ledgerSyncError.message);
        }
