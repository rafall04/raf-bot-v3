 ...
 */
"use strict";
const log = require('./logger').logger.child('FINANCIAL_LEDGER');
        ...
        db.all(sql, params, (err, rows) => {
            if (err) {
                if (typeof err.code === "string" && err.code.startsWith("SQLITE_IOERR")) {
                    log.error(`[FINANCIAL_LEDGER_READER_IOERR] ${err.message} — koneksi baca dibuang.`);
...
// Satu file sumber korup tak boleh membatalkan SELURUH sync multi-domain — isolasikan
// per-sumber: log keras, kembalikan [] (idempoten via eventKey, sumber itu ter-skip saja).
function readJsonArray(filePath) {
    ...
    try {
        ...
        log.error(`[FIN_LEDGER] Sumber ${path.basename(filePath)} korup/invalid — domain ini di-skip sync:`, error.message);
        return [];
    }
}

function readVoucherPurchases() {
    return readJsonArray(path.join(__dirname, "../database/voucher_purchases.json"));
}

function readAgentTransactions() {
    return readJsonArray(path.join(__dirname, "../database/agent_transactions.json"));
}

function readSaldoTransactions() {
    return readJsonArray(path.join(__dirname, "../database/saldo_transactions.json"));
}

function readTopupRequests() {
    return readJsonArray(path.join(__dirname, "../database/topup_requests.json"));
}
...
function classifyLedgerCashflow(entry) {
    ...
    case "voucher_purchase":
        // #b310 (T16): voucher berbayar SALDO = uangnya SUDAH dihitung pemasukan saat TOPUP
        // (domain topup_request_approved). Menghitungnya lagi di sini = DOUBLE-COUNT pemasukan untuk
        // rupiah yang sama. Hanya voucher berbayar tunai/gateway (bukan SALDO) = pemasukan usaha nyata.
        return String(entry.payment_method || entry.paymentMethod || "").toUpperCase() === "SALDO"
            ? { incomeDelta: 0, expenseDelta: 0, classification: "internal" }
    ...
    case "tagihan":
        // #b346: KELEBIHAN BAYAR dicatat aftercare (#b329) domain 'tagihan' referenceType
        // 'kelebihan_bayar' — UANG YANG BELUM DIPUTUSKAN (refund vs dipakai periode depan). Selama
        // status 'pending_review' JANGAN dihitung pemasukan: sebelumnya jatuh ke default → dihitung
        // PEMASUKAN FINAL (laba fiktif; bila kelak di-refund tak ada debit lawan) + membanjiri log
        // LEDGER_UNKNOWN_DOMAIN tiap kejadian. Baru cash_in setelah diputuskan (status non-pending);
        // debit (refund manual) = cash_out.
        if (direction === "credit") {
            const st = String(entry.status || "").toLowerCase();
            const belumDiputuskan = st === "pending_review" || st === "pending" || st === "";
            return belumDiputuskan
                ? { incomeDelta: 0, expenseDelta: 0, classification: "internal" }
                : { incomeDelta: amount, expenseDelta: 0, classification: "cash_in" };
        }
        return { incomeDelta: 0, expenseDelta: amount, classification: "cash_out" };
    default:
        // #b308: domain yang TAK dikenal switch ini menyusup ke laba/rugi tanpa peninjauan —
        // kredit langsung dihitung pemasukan, debit langsung pengeluaran. Semua domain yang
        // ditulis financial-ledger saat ini SUDAH terdaftar di atas, jadi cabang ini hanya kena
        // domain BARU yang lupa diklasifikasikan. Suarakan (jangan diam) supaya cepat ketahuan &
        // diklasifikasikan benar; klasifikasi dipertahankan agar angka lama tak berubah.
        log.warn(`[LEDGER_UNKNOWN_DOMAIN] domain "${entry.domain}" belum diklasifikasikan di classifyLedgerCashflow — sementara dihitung ${direction === "credit" ? "PEMASUKAN" : "PENGELUARAN"}. Tambahkan ke switch.`);
        return direction === "credit"
            ? { incomeDelta: amount, expenseDelta: 0, classification: "unknown" }
            : { incomeDelta: 0, expenseDelta: amount, classification: "unknown" };
    }
}
