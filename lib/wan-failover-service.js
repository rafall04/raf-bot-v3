 ...
 */
"use strict";
const log = require('./logger').logger.child('WAN_FAILOVER_SERVICE');
...
function defaultDeps() {
    return {
        ...
        getFailoverConfig,
        // Himpunan pelanggan terdampak LIVE di jalur sakit — supaya usulan failover menampilkan
        // "X pelanggan terdampak" (data keputusan manual owner). Best-effort/fail-closed.
...
async function rehydrateFromIncidents(deps, cfgF) {
    ...
    try {
        ...
        // Insiden 'failover' diambil TERPISAH (filter kind) supaya tak tergusur cap-200 oleh
        // 'flap'/'alert'/'trace' saat instabilitas — justru momen lock & auto-switch terjadi.
        // Fallback: repo lama tanpa filter kind → pakai hasil filter dari daftar umum.
        let failoverIncidents = incidents.filter((inc) => inc.kind === "failover");
        try {
            const filtered = await repo.getIncidents({ limit: 200, kind: "failover" });
            if (Array.isArray(filtered) && filtered.length >= failoverIncidents.length) failoverIncidents = filtered;
        } catch (_e) { /* pakai filter di atas */ }
        const nowMs = deps.nowMs();
        const dayMs = 24 * 60 * 60 * 1000;
        const flapWindowMs = cfgF.flapLockHours * 60 * 60 * 1000;
        const eventWindowMs = Math.max(dayMs, flapWindowMs); // konsisten dgn pruning autoEvents (baris ~341)
        for (const rule of cfgF.rules) {
            const state = getRuleState(rule.switchId);
            // Kejadian auto (apply/restore) → kuota harian + deteksi flapping. Jendela = max(24 jam,
            // flapWindowMs) supaya event yang menyebabkan lock tak jatuh keluar saat flapLockHours > 24.
            for (const inc of failoverIncidents) {
                const d = parseDetail(inc.detail);
                if (!d || d.switchId !== rule.switchId) continue;
                const atMs = Date.parse(inc.created_at);
                if (!Number.isFinite(atMs)) continue;
                if (["auto_apply", "auto_restore"].includes(d.action) && nowMs - atMs < eventWindowMs) {
            ...
            // KUNCI anti-flapping WAJIB tahan restart (prod 7-13x/hari). failoverIncidents DESC →
            // .find = insiden 'lock' TERBARU. Pulihkan lockedUntilMs = waktu-lock + flapWindowMs
            // (honor flapLockHours), buang bila sudah kedaluwarsa. Dulu lock cuma in-memory → reset
            // 0 tiap restart → bot lanjut auto-switch uplink pelanggan SEBELUM jendela lock habis.
            const lastLock = failoverIncidents.find((inc) => {
                const d = parseDetail(inc.detail);
                return d && d.switchId === rule.switchId && d.action === "lock";
            });
            if (lastLock) {
                const lockAtMs = Date.parse(lastLock.created_at);
                if (Number.isFinite(lockAtMs) && lockAtMs + flapWindowMs > nowMs) {
                    state.lockedUntilMs = lockAtMs + flapWindowMs;
                    state.lockNotified = true; // sudah pernah diberitahu → jangan spam saat restart
                }
            }
        ...
        log.warn(`[WAN-Failover] Rehidrasi state gagal (lanjut dengan state kosong): ${err.message}`);
    }
}
...
async function deliver(deps, cfgF, text) {
    ...
    for (const jid of recipients) {
        try {
            ...
            log.warn(`[WAN-Failover] Gagal kirim ke ${jid}: ${err.message}`);
        }
    }
    return delivered;
}
...
/**
 * Baris "Terdampak" LIVE untuk usulan failover — jumlah pelanggan aktif di jalur sakit + beberapa
 * nama. `affected` dari lib/upstream-affected-service.getAffectedSet:
 * live → jumlah+nama; unknown → jujur "tak bisa dipastikan"; null/absen → "" (kompat, tanpa baris).
 */
function buildTerdampakLine(affected, jalurLabel) {
    if (affected && affected.confidence === "live" && affected.count != null) {
        if (affected.count === 0) return `\n👥 Terdampak: *0 pelanggan* aktif di ${jalurLabel} (mungkin sudah dialihkan)`;
        ...
        return `\n👥 Terdampak: *${affected.count} pelanggan* aktif di ${jalurLabel}${namesTxt}`;
    }
...
async function evaluateAfterCycle(monitorCfg, depsOverride = {}) {
    try {
        ...
        for (const rule of cfgF.rules) {
            ...
            if (state.phase === "IDLE") {
                ...
                if (effectiveMode === "propose") {
                    ...
                    // Terdampak LIVE pelanggan di jalur sakit (data keputusan manual owner).
                    let affected = null;
                    try { affected = await deps.getAffectedSet(rule.sickPath); } catch (_e) { affected = null; }
                    dataText.terdampak = buildTerdampakLine(affected, dataText.jalur_sakit);
                    const teks = deps.renderResponseTemplate(
                        ...
                        `sedangkan *${dataText.jalur_target}* sehat (loss ${dataText.loss_target}%).${dataText.terdampak}\n\n` +
        ...
        log.warn(`[WAN-Failover] Evaluasi gagal: ${err.message}`);
...
module.exports = {
    ...
    getRuleStates,
    resetForTest,
    _internal: { levelForLoss, buildSeries, tailAll, normalizeRecipients, buildTerdampakLine, LEVELS, AUTO_ACTOR_LABEL }
};
