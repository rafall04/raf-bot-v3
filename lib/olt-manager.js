...
const log = require('./logger').logger.child('OLT_MANAGER');
const fs = require('fs');
...
function loadMacCache() {
    try {
        if (fs.existsSync(MAC_CACHE_FILE)) {
            ...
            log.info(`[OLT-Manager] Loaded ${macToOltCache.size} MAC mappings from cache`);
        }
    } catch (e) {
        log.error('[OLT-Manager] Error loading MAC cache:', e.message);
    }
}
...
function saveMacCache() {
    try {
        ...
        log.error('[OLT-Manager] Error saving MAC cache:', e.message);
    }
}
...
function getOltDevices() {
    try {
        ...
        log.error('[OLT-Manager] Error loading OLT devices:', e.message);
        return [];
    }
}
...
function getOltGlobalConfig() {
    try {
        ...
        log.error('[OLT-Manager] Error loading global config:', e.message);
        return {
            enabled: false,
            webEnabled: false,
            scrapeInterval: 1,
...
function cleanupMacCache() {
    ...
    if (cleaned > 0) {
        log.info(`[OLT-Manager] Cleaned ${cleaned} old MAC cache entries`);
        saveMacCache();
    }
}
