const log = require('./logger').logger.child('TEMPLATING');
const fs = require('fs');
    ...
    setTimeout(() => {
        try {
            ...
            if (retries > 0) {
                ...
                log.warn(`[Templating] Error reloading ${fileName} (retry ${4 - retries}/3):`, error.message);
                reloadTemplateWithRetry(filePath, cacheKey, retries - 1, delay * 2);
            } else {
                const fileName = path.basename(filePath);
                log.error(`[Templating] Error reloading ${fileName} after 3 retries:`, error.message);
            }
        }
    }, delay);
}
...
function setupTemplateWatcher(filePath, cacheKey, displayName) {
    ...
    fs.watchFile(filePath, { interval: 1000 }, (curr, prev) => {
        ...
        if (currMtime !== prevMtime && currMtime !== lastMtime) {
            lastMtime = currMtime;
            log.info(`[Templating] Template file changed: ${displayName}`);
...
if (process.env.NODE_ENV !== 'test' && process.env.DISABLE_FILE_WATCHERS !== 'true') {
    ...
    log.info(`[Templating] ${templateWatchersCount} template watchers aktif`);
}
...
function renderTemplate(templateName, data = {}) {
    ...
    if (!result.found) {
        log.warn(`[Templating] Warning: Template "${templateName}" not found in notification templates.`);
        return `Error: Template "${templateName}" not found. Please check message_templates.json.`;
    }

    if (result.unresolved.length > 0) {
        log.warn(`[TEMPLATE_SLOT_BASI] Template "${templateName}" punya slot tak terisi:`, result.unresolved.slice(0, 5));
        log.warn(`[Templating] Available data keys:`, Object.keys(fullData));
...
function renderSystemMessage(templateName, data = {}) {
    ...
    if (!result.found) {
        log.error(`[Templating] System template "${templateName}" not found`);
...
function renderReport(reportType, data = {}) {
    ...
    if (!result.found) {
        log.error(`[Templating] Report template "${templateName}" not found`);
...
function renderMenu(menuType, data = {}) {
    ...
    if (!result.found) {
        log.error(`[Templating] Menu template "${templateName}" not found`);
