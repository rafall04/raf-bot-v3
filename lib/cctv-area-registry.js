/**
 * Header Doc
 * Purpose: Registry Area + Koordinator RT (name → {coordinatorName, coordinatorPhone}). Dipakai
 *          cctv-monitor untuk menotifikasi koordinator saat CCTV di area itu mati, dan UI admin
 *          untuk CRUD + dropdown. Dicocokkan ke field `area` tiap CCTV (case-insensitive).
...
function upsert(area) {
    ...
    if (!cleaned.coordinatorPhone) throw new Error('Area: nomor WA koordinator wajib diisi');
