 ...
 */
'use strict';
const log = require('./logger').logger.child('CCTV_AREA_REGISTRY');
...
function save(listv) {
    ...
    catch (e) { log.error('[CCTV-AREA] save error:', e.message); }
}
...
function upsert(area) {
    ...
    // Tanpa id eksplisit: cegah area KEMBAR bernama sama — reuse id area yang sudah ada. Mis. label
    // auto dari form CCTV (routes/cctv.js autoRegisterArea) lalu admin isi koordinatornya = UPDATE,
    // bukan bikin baris kedua. Rename tetap aman: form edit selalu mengirim id → cocok by-id menang.
    // (Registry = SATU sumber kebenaran area; matchByName jadi ambigu kalau ada nama dobel.)
    const prior = area.id ? listv.find((a) => a.id === area.id) : matchByName(listv, area.name);
    const cleaned = {
        id: (prior && prior.id) || area.id || ('area_' + Math.random().toString(36).slice(2, 9)),
        ...
        createdAt: (prior && prior.createdAt) || area.createdAt || new Date().toISOString(),
