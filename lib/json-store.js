 ...
 */
"use strict";
const log = require('./logger').logger.child('JSON_STORE');
...
function loadJSON(filePath) {
    ...
    try {
        ...
        log.warn(`[JSON_LOAD_WARN] File tidak ditemukan: ${fullPath}, membuat file baru.`);
        ...
        log.error(`[JSON_LOAD_ERROR] Gagal memuat atau parse JSON dari ${fullPath}:`, error);
        try {
            if (fs.existsSync(fullPath)) {
            ...
            log.error("[JSON_LOAD_ERROR] Gagal mengkarantina berkas rusak:", errKarantina.message);
        }
        return buildEmptyState(filePath);
    }
}
...
function saveJSON(filePath, data) {
    ...
    try {
        ...
        log.error(`[JSON_SAVE_ERROR] Gagal menyimpan data ke ${fullPath}:`, error);
