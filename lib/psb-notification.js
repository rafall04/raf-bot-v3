...
const log = require('./logger').logger.child('PSB_NOTIFICATION');
...
async function deliverPsbText(phoneJid, message, contextLabel) {
    ...
    log.error(`[PSB_NOTIF] Failed to send ${contextLabel}:`, delivery.warning || delivery.errorCode || 'SEND_FAILED');
    return false;
}
...
async function sendPSBPhase1Notification(user) {
    ...
    if (!isReady()) {
        log.warn('[PSB_NOTIF] WhatsApp not connected, skipping Phase 1 notification');
        return false;
    }
    
    if (!user || !user.phone_number) {
        log.warn('[PSB_NOTIF] User or phone number missing for Phase 1 notification');
        return false;
    }
    ...
    try {
        ...
        if (!phoneJid) {
            log.error(`[PSB_NOTIF] Invalid phone number: ${user.phone_number}`);
            return false;
        }
        
        const sent = await deliverPsbText(phoneJid, message, 'Phase 1 notification');
        if (sent) {
            log.info(`[PSB_NOTIF] Phase 1 notification sent to ${user.phone_number}`);
        }
        return sent;
    } catch (error) {
        log.error(`[PSB_NOTIF] Error in sendPSBPhase1Notification:`, error);
        return false;
    }
}
...
async function sendPSBPhase2Notification(user, config) {
    // PENTING: Cek connection state sesuai rules
    if (!isReady()) {
        log.warn('[PSB_NOTIF] WhatsApp not connected, skipping Phase 2 notification');
        return false;
    }
    
    if (!user || !user.phone_number) {
        log.warn('[PSB_NOTIF] User or phone number missing for Phase 2 notification');
        return false;
    }
    
    if (!config || !config.pppoe_username || !config.wifi_ssid || !config.wifi_password) {
        log.warn('[PSB_NOTIF] Missing config data for Phase 2 notification');
        return false;
    }
    ...
    try {
        ...
        if (sent) {
            log.info(`[PSB_NOTIF] Phase 2 notification sent to ${user.phone_number}`);
        }
        return sent;
    } catch (error) {
        log.error(`[PSB_NOTIF] Error in sendPSBPhase2Notification:`, error);
        return false;
    }
}
...
async function sendPSBTeknisiMeluncurNotification(psbRecord, teknisiInfo) {
    // PENTING: Cek connection state sesuai rules
    if (!isReady()) {
        log.warn('[PSB_NOTIF] WhatsApp not connected, skipping teknisi meluncur notification');
        return false;
    }
    
    if (!psbRecord || !psbRecord.phone_number) {
        log.warn('[PSB_NOTIF] PSB record or phone number missing for teknisi meluncur notification');
        return false;
    }
    
    if (!teknisiInfo || !teknisiInfo.name) {
        log.warn('[PSB_NOTIF] Teknisi info missing for teknisi meluncur notification');
        return false;
    }
    ...
    try {
        ...
        if (!phoneJid) {
            log.error(`[PSB_NOTIF] Invalid phone number: ${psbRecord.phone_number}`);
            return false;
        }
        
        const sent = await deliverPsbText(phoneJid, message, 'teknisi meluncur notification');
        if (sent) {
            log.info(`[PSB_NOTIF] Teknisi meluncur notification sent to ${psbRecord.phone_number}`);
        }
        return sent;
    } catch (error) {
        log.error(`[PSB_NOTIF] Error in sendPSBTeknisiMeluncurNotification:`, error);
        return false;
    }
}
...
async function sendPSBInstallationCompleteNotification(psbRecord) {
    // PENTING: Cek connection state sesuai rules
    if (!isReady()) {
        log.warn('[PSB_NOTIF] WhatsApp not connected, skipping installation complete notification');
        return false;
    }
    
    if (!psbRecord || !psbRecord.phone_number) {
        log.warn('[PSB_NOTIF] PSB record or phone number missing for installation complete notification');
        return false;
    }
    ...
    try {
        ...
        if (sent) {
            log.info(`[PSB_NOTIF] Installation complete notification sent to ${psbRecord.phone_number}`);
        }
        return sent;
    } catch (error) {
        log.error(`[PSB_NOTIF] Error in sendPSBInstallationCompleteNotification:`, error);
        return false;
    }
}
