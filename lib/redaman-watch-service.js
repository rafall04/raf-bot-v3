...
function decideNotify(watch, diag, now, cfg = {}) {
    const changeDb = Number.isFinite(cfg.changeThresholdDb) ? cfg.changeThresholdDb : DEF_CHANGE_DB;
    const heartbeatMs = Number.isFinite(cfg.heartbeatMs) ? cfg.heartbeatMs : DEF_HEARTBEAT_MS;
    const cur = primaryRx(diag);

    // TARGET tercapai: RX masuk band BAIK & belum diumumkan → prioritas tertinggi (tujuan perbaikan).
    if (cur.label === "BAIK" && !watch.targetAnnounced) return { notify: true, kind: "target" };
