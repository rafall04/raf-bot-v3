...
const { body, param, query: _query, validationResult } = require('express-validator');
...
/**
 * Validation rules untuk registrasi pelanggan self-service (site publik anonim).
 */
const publicRegistrationValidation = [
    ...
    body('phone')
        ...
        .matches(/^(\+62|62|0)[0-9]{8,13}$/).withMessage('Format nomor HP tidak valid (contoh: 08xxxxxxxxxx)')
        .customSanitizer(value => {
    ...
    body('address')
        ...
        .isLength({ min: 5, max: 300 }).withMessage('Alamat harus 5-300 karakter')
        .customSanitizer(value => sanitizeText(value, { maxLength: 300 })),
    body('packageInterest')
        .optional({ nullable: true, checkFalsy: true })
        .trim()
        .isLength({ max: 100 }).withMessage('Paket tidak valid')
        .customSanitizer(value => sanitizeString(value, { maxLength: 100 })),
    body('locationUrl')
        .optional({ nullable: true, checkFalsy: true })
        .trim()
        .isLength({ max: 500 }).withMessage('Link lokasi terlalu panjang'),
    body('latitude')
        .optional({ nullable: true, checkFalsy: true })
