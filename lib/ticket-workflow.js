const log = require('./logger').logger.child('TICKET_WORKFLOW');
const { saveReports } = require('./database');
const { generateOTP } = require('./otp-generator');
const { generateTicketId } = require('./ticket-id');

const OTP_TTL_MS = 24 * 60 * 60 * 1000; // 24 jam — selaras dengan jam kerja teknisi 1 hari
const FINAL_STATUSES = new Set(['baru', 'process', 'otw', 'arrived', 'working', 'completed', 'cancelled']);

// !! SATU KOSAKATA, BANYAK EJAAN (#b265). Status tiket pernah ditulis dalam dua bahasa oleh dua
// jalur berbeda: `ticket-workflow` menulis `cancelled`, sedangkan tombol Batalkan di halaman
// admin menulis `dibatalkan`. Karena `dibatalkan` tak dikenal di sini, `ensureFinalStatus`
// memulangkannya ke `baru` — tiket yang SUDAH dibatalkan tampak terbuka lagi dan tombol
// Batalkan muncul kembali. Terbukti di produksi: 2 tiket berstatus `baru` tapi ber-`cancelled_by`.
//
// Semua ejaan yang pernah ditulis SIAPA PUN (rute, bot WhatsApp, data lama) dipetakan di sini —
// satu tempat, supaya pembaca mana pun tak perlu hafal ejaan.
const LEGACY_STATUS_MAP = {
    ...
    open: 'baru',
    new: 'baru',
    // `pending` dulu selamat HANYA karena reset-diam-diam ke `baru`. Begitu reset itu dibuang,
    // ia harus dipetakan eksplisit — kalau tidak, tiket baru hilang dari filter daftar.
    pending: 'baru'
};
...
/**
 * Pastikan status berada di kosakata kanonik.
 *
 * !! DULU: status tak dikenal DIPULANGKAN JADI `'baru'` — diam-diam. Itu memutar tiket TERMINAL
 * kembali jadi terbuka, dan tak meninggalkan satu jejak pun. Terbukti di produksi (#b265): tombol
 * Batalkan admin menulis `dibatalkan`, `ensureTicketShape` mengubahnya jadi `baru`, dan tiket yang
 * sudah dibatalkan muncul lagi lengkap dengan tombol Batalkan-nya.
 *
 * Sekarang status tak dikenal DIPERTAHANKAN apa adanya dan DICATAT. Status aneh yang kelihatan
 * jauh lebih murah daripada tiket selesai/batal yang diam-diam dibuka kembali — dan catatannya
 * membuat ejaan baru ketahuan pada kemunculan pertama, bukan setelah pelanggan mengeluh.
 */
function ensureFinalStatus(status) {
    const normalized = normalizeStatus(status);
    if (!FINAL_STATUSES.has(normalized)) {
        log.warn(
            `[TICKET_STATUS_ASING] "${status}" tidak dikenal kosakata kanonik ` +
            `(${[...FINAL_STATUSES].join(', ')}). Dipertahankan apa adanya — JANGAN dipulangkan ke "baru": ` +
            `itu membuka kembali tiket yang sudah selesai/batal. Tambahkan ejaannya ke LEGACY_STATUS_MAP.`
        );
        return normalized;
    }
    return normalized;
...
function verifyTicketOtp({ ticketId, actor, otp }) {
    ...
    // OTP TTL: tolak kode yang sudah lewat 24 jam sejak diterbitkan.
    // Tiket basi yang ditinggal berhari-hari tidak boleh diverifikasi pakai OTP lama.
    if (ticket.otpIssuedAt && (Date.now() - ticket.otpIssuedAt) > OTP_TTL_MS) {
        const error = new Error('Kode OTP sudah kedaluwarsa. Hubungi admin untuk reset tiket.');
