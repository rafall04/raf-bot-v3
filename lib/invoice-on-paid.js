 ...
 */
"use strict";
const log = require('./logger').logger.child('INVOICE_ON_PAID');
...
async function sendPaidInvoiceOrReceipt(user, opts = {}) {
    ...
    try {
        ...
        if (!invoiceData) {
            log.warn(`[INVOICE_ON_PAID] createInvoice null utk ${user.name} — fallback struk teks (bukan nol pesan).`);
            const sent = await _sendTextToAllPhones(user, messageText, deps);
        ...
        while (retry < maxRetries && !pdfResult) {
            try {
                ...
                retry += 1;
                log.error(`[INVOICE_ON_PAID] generate PDF gagal (attempt ${retry}):`, pdfError.message);
        ...
        log.error(`[INVOICE_ON_PAID_ERROR] invoice gagal utk ${user && user.name}:`, invoiceError && invoiceError.message);
        try {
            const fs = require("fs");
            // Dead-letter via getDatabasePath: prod tulis database/invoice_errors.json, NODE_ENV=test
            // terisolasi ke invoice_errors_test.json (auto-diignore) — jadi test tak mengotori data prod.
            const p = require("./env-config").getDatabasePath("invoice_errors.json");
            let errs = [];
...
/**
 * Wrapper untuk jalur SETTLE (callback gateway / konfirmasi bukti): kirim invoice PDF bila gate
 * `config.invoiceOnSettle.enabled` ON & pelanggan send_invoice ON & pembayaran BERSIH-lunas
 * (isCleanPaid — BUKAN kelebihan-bayar). Return true bila hook benar mengirim (caller SKIP kirim
 * teks-nya sendiri, cegah dobel); false → caller kirim struk teks seperti biasa. NEVER-THROW.
 * @returns {Promise<boolean>}
 */
async function trySendSettleInvoice(user, { messageText, paymentDetails = {}, isCleanPaid = true } = {}) {
    try {
        const gateOn = !!(typeof global !== "undefined" && global.config && global.config.invoiceOnSettle && global.config.invoiceOnSettle.enabled === true);
        if (!gateOn || !isCleanPaid || !user || !isSendInvoiceEnabled(user.send_invoice)) return false;
        const r = await sendPaidInvoiceOrReceipt(user, {
            messageText,
            paymentDetails,
            config: global.config,
            notifEnabled: true,
            deps: {
                deliver: (jid, payload) => require("./whatsapp-delivery-service").sendMessage(jid, payload, { skipDuplicateCheck: true }),
                waitForWhatsAppDelay: require("./wa-timing").waitForWhatsAppDelay,
...
module.exports = { isSendInvoiceEnabled, sendPaidInvoiceOrReceipt, trySendSettleInvoice };
