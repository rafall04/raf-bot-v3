 ...
 */
'use strict';
const log = require('./logger').logger.child('CCTV_MONITOR');
...
function defaultLoadIncidents() {
    ...
    } catch (e) { log.error('[CCTV] read incidents:', e.message); }
    return [];
}
function defaultSaveIncidents(list) {
    ...
    } catch (e) { log.error('[CCTV] write incidents:', e.message); }
}
...
function createCctvMonitor(deps = {}) {
    ...
    function flushBatch() {
        ...
        // #b337: tandai insiden yang DOWN-nya BENAR-BENAR di-flush ke pengiriman. broadcastDown/sweep
        // menandai insiden 'broadcasted' SEBELUM flush (agregasi ≤90s); kalau proses restart di
        // jendela itu, pendingBatch (in-memory) hilang → restore mengira sudah terkirim. `deliveredAt`
        // membedakan "sudah dikirim" dari "diantre lalu hilang restart" → restore bisa kirim ulang.
        const stamp = new Date(now()).toISOString();
        const seenInc = new Set();
        for (const it of items) {
            if (it && it.incidentId && !seenInc.has(it.incidentId)) {
                seenInc.add(it.incidentId);
                updateIncident(it.incidentId, { deliveredAt: stamp });
            }
        }
    }
        ...
        for (const [host, s] of state) {
            ...
            if (s.status === 'down') {
                ...
                if (inc.status === 'pending') {
                    ...
                    if (dev) {
                        const quiet = effectiveQuietNow(dev, conf);
                        const all = recipientsFor(dev);
                        const held = all.filter((r) => quiet && quietAppliesTo(r.role, conf));
                        // #b337 (#6): penerima yg DITAHAN jam tenang saat broadcast hanya tersimpan di
                        // memori (s.deferred) → hilang saat restart, sweep jam-tenang tak menemukan apa
                        // pun → mereka TAK PERNAH dapat "mati". Rekonstruksi dari kondisi jam-tenang kini.
                        if (held.length) s.deferred = { incidentId: inc.incidentId, sinceMs: s.since, recipients: held };
                        // #b337 (#5): DOWN yg diantre agregasi tapi restart SEBELUM flush → `deliveredAt`
                        // kosong = pelanggan tak pernah menerima "mati" (janji harus tahan restart,
                        // prod restart 7-13x/hari). Kirim ulang ke penerima non-tahan. Bila sudah
                        // terkirim (deliveredAt ada), cukup rebuild state (jangan dobel).
                        if (!inc.deliveredAt) {
                            const immediate = all.filter((r) => !(quiet && quietAppliesTo(r.role, conf)));
                            if (immediate.length) enqueueDown(dev, s.since, inc.incidentId, immediate);
                        }
                    }
                    n++;
                }
                ...
                // host sudah pulih saat monitor mati → tutup insiden (akurasi uptime).
                // !! Bila pelanggan SUDAH dikabari "mati" (insiden broadcasted), mereka WAJIB dapat
                // "pulih" juga — janji ke pelanggan harus tahan restart (produksi restart 7-13x/hari).
                // Dulu ditutup diam-diam ("tanpa notif susulan") → pelanggan dapat "mati" tanpa "nyala
                // lagi" bila CCTV pulih persis saat bot restart. Insiden pending/deferred (belum
                // di-broadcast) tetap tak dinotif — pelanggan tak pernah dikabari mati.
                if (inc.status === 'broadcasted' && conf.notifyRecovery !== false) {
                    const dev = devByHost().get(host);
                    if (dev) {
                        const detected = Date.parse(inc.detectedAt);
                        const sinceMs = Number.isFinite(detected) ? detected : (s.since || now());
                        broadcastUp(dev, sinceMs, inc.incidentId, uniqueRoles(recipientsFor(dev))).catch(() => {});
                        logger.log && logger.log('[CCTV] restore: pulih saat bot mati → notif susulan "pulih": ' + dev.name + ' (' + host + ')');
                    }
                }
    ...
    async function pollOnce() {
        ...
        try {
            ...
            // #b337: buang entri state utk host yang device-nya sudah DIHAPUS dari registry.
            // state hanya dibersihkan total di start(); tanpa prune, CCTV yang 'down' saat dihapus
            // meninggalkan entri 'down' PERMANEN → terus dihitung countDown() → salah memicu
            // peredaman gangguan-massal (menahan alert CCTV tunggal yang sah) + getStatus lapor host hantu.
            for (const host of [...state.keys()]) {
                if (!devMap.has(host)) state.delete(host);
            }
