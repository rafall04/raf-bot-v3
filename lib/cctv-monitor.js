...
function createCctvMonitor(deps = {}) {
    ...
    function broadcastDown(host, device, incidentId, sinceMs) {
        ...
        // Jam tenang PER-JENIS: penerima yg jenisnya "kena jam tenang" ditahan (dikirim saat jam tenang
        // berakhir via sweep), sisanya dikirim sekarang. Notif "pulih" nanti hanya ke jenis yg dapat "mati".
        const quiet = isQuietNow(conf);
        const immediate = [];
        const held = [];
        for (const r of all) {
