...
function createCctvMonitor(deps = {}) {
    ...
    // Kirim semua antrean: kelompokkan per nomor → 1 pesan (tunggal/gabungan) per nomor.
    function flushBatch() {
        ...
        for (const it of items) {
            for (const ph of splitPhones(it.device.phone)) {
                if (!byPhone.has(ph)) byPhone.set(ph, []);
                byPhone.get(ph).push(it);
            }
        }
        for (const [phone, list] of byPhone) {
            let text;
            if (list.length === 1) {
                text = renderDownText(list[0].device, list[0].sinceMs, conf);
            } else {
                const listText = list.map((it) => '• ' + it.device.name + ' (sejak ' + fmtLocal(it.sinceMs) + ')').join('\n');
                text = renderTemplate(conf.messageDownMulti || DEFAULTS.messageDownMulti, {
                    customer_name: list[0].device.customerName || 'Pelanggan',
                    count: list.length,
                    list: listText,
                });
            }
            sendDownToPhone(phone, text, list.map((it) => it.incidentId)).catch(() => {});
        }
    }
    // Antre broadcast DOWN; digabung dalam jendela aggregateWindowMs (0 = kirim segera).
    function enqueueDown(device, sinceMs, incidentId) {
        pendingBatch.push({ device, sinceMs, incidentId });
    ...
    function broadcastDown(host, device, incidentId, sinceMs) {
        ...
        enqueueDown(device, sinceMs, incidentId); // kirim (digabung per nomor dalam jendela)
