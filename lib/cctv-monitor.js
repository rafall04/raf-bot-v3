...
function createCctvMonitor(deps = {}) {
    ...
    function recipientsFor(device) {
        ...
        if (coord && coord.enabled !== false) {
            // Grup WA RT (jika diset): koordinator + warga dapat 1 pesan koordinasi di grup.
            if (coord.coordinatorGroupId) {
                out.push({ phone: coord.coordinatorGroupId, role: 'coordinator', area: device.area, coordName: coord.coordinatorName || coord.coordinatorGroupName });
            }
    ...
    function flushBatch() {
        ...
        for (const [phone, ent] of byPhone) {
            const text = ent.role === 'coordinator' ? renderCoordText(ent, conf) : renderCustomerText(ent.list, conf);
