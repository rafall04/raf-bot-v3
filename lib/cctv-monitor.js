 ...
 *
 * Caller: lib/app-runtime.js (start saat boot), lib/process-lifecycle.js (stop), routes/cctv-routes.js (status/getState).
...
const DEFAULTS = {
    ...
    messageDown:
        "Halo {customer_name},\n\n" +
        "CCTV publik *{cctv_name}* terdeteksi *mati* sejak {since_local} ({minutes_down} menit).\n\n" +
        "Mohon dicek apakah listrik / koneksi internet di lokasi sedang bermasalah. " +
        "Bila perlu bantuan, balas pesan ini.\n\nTerima kasih.",
    messageUp:
        "Halo {customer_name},\n\n" +
        "CCTV publik *{cctv_name}* sudah *online kembali* sejak {up_local}.\n\n" +
...
function createCctvMonitor(deps = {}) {
    ...
    let isPolling = false;
    let seeded = false;
    const stats = { started_at: null, poll_count: 0, last_poll_at: null, last_error: null, broadcasts: 0, recoveries: 0 };
    ...
    function buildVars(device, sinceMs, kind) {
        const t = now();
        return {
