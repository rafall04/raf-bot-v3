...
function createCctvMonitor(deps = {}) {
    ...
    function onRecovery(host) {
        ...
        if (s && s.active && device) {
            const activeId = s.active.incidentId;
            // Bila broadcast DOWN masih TERTAHAN di antrean agregasi (belum benar-benar terkirim WA),
            // pulih cepat = anggap flap → buang DOWN dari antrean & JANGAN kirim "pulih". Tanpa ini,
            // UP terkirim duluan lalu DOWN menyusul saat batch flush → notif tak sinkron (UP sebelum DOWN).
            if (pendingBatch.some((it) => it.incidentId === activeId)) {
                pendingBatch = pendingBatch.filter((it) => it.incidentId !== activeId);
                s.active = null;
                updateIncident(activeId, { status: 'recovered_before_send', recoveredAt: new Date(now()).toISOString() });
                logger.log && logger.log('[CCTV] pulih sebelum DOWN sempat terkirim (masih di antrean) → dibatalkan: ' + device.name + ' (' + host + ')');
                return;
            }
            const conf = cfg();
            const sinceMs = s.active.sinceMs; const roles = s.active.roles;
            s.active = null;
            stats.recoveries++;
