 ...
 */
'use strict';
const log = require('./logger').logger.child('STEERING_DRIFT_MONITOR');
...
function saveState(state, statePath = resolveStatePath()) {
    try {
        ...
        log.warn(`[STEER-DRIFT] Gagal menyimpan state: ${err.message}`);
    }
}
...
async function checkOnce(depsOverride = {}) {
    ...
    try {
        ...
        log.warn(`[STEER-DRIFT] computeDrift gagal: ${err.message}`);
        return { drift: [], alerted: false, reason: 'compute-error' };
    }
    if (!drift.length) return { drift, alerted: false, reason: 'aligned' };

    log.warn(`[STEER-DRIFT] ${drift.length} subnet drift: ` + drift.map((d) => `${d.cidr}(peta ${d.poolPath}≠live ${d.livePath})`).join(', '));
    ...
    try {
        ...
        for (const jid of jids) {
            try {
                ...
                log.warn(`[STEER-DRIFT] gagal kirim ke ${jid}: ${err.message}`);
            }
        }
    } catch (err) {
        log.warn(`[STEER-DRIFT] gagal siapkan alert: ${err.message}`);
    }
...
function startSteeringDriftMonitor() {
    if (tickTimer) return;
    if (!isEnabled()) {
        log.info('[STEER-DRIFT] Nonaktif (set config.steeringDriftMonitor.enabled=true untuk mengaktifkan)');
        return;
    }
    log.info('[STEER-DRIFT] Start (cek pertama ~30s setelah boot, lalu harian)');
