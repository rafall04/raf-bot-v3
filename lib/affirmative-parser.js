 ...
 * Deps: Tidak ada.
 * MainFuncs: `isAffirmative`, `isCleanRebootConsent`, `isDecline`, `isResolvedAnswer`, `isUnresolvedAnswer`, `saysAlreadyRestarted`.
...
/**
 * Persetujuan BERSIH untuk aksi berisiko (reboot modem). LEBIH KETAT dari `isAffirmative`:
 * SETIAP kata harus afirmasi / sapaan-pengisi / kata-aksi-reboot, dan tidak boleh ada tanda tanya.
 ...
 * "boleh minta info paket" — TIDAK lagi memicu reboot beneran (mereka punya kata "konten" lain).
...
function isCleanRebootConsent(text) {
    ...
    if (hasAny(tokens, CONSENT_BLOCKERS)) return false; // ada muatan lain → bukan konsen bersih
    return true;
}
