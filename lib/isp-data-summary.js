 ...
 *          24 jam & 7 hari, dan insiden terakhir. Panjang daftar "Terdampak" + on/off tiap seksi
 *          bisa diatur admin lewat `config.upstreamMonitor.report` (panel /upstream-quality, live).
 ...
 *       lazy `lib/upstream-affected-service.getAffectedSet` (baris "Terdampak" LIVE, sama dgn alert),
 ...
 */
"use strict";
const log = require('./logger').logger.child('ISP_DATA_SUMMARY');
...
function defaultDeps() {
    ...
    return {
        ...
        buildDirectionText: (cfg, pathKey, entry, affected) => alerter._internal.buildDirectionText(cfg, pathKey, entry, affected),
        buildConclusionText: (cfg, pathKey, entry) => alerter._internal.buildConclusionText(cfg, pathKey, entry),
        // Himpunan pelanggan terdampak LIVE (sama dengan alert) — supaya baris "Terdampak" di
        // "data <isp>" akurat, bukan fallback statis. Best-effort/fail-closed di dalam service.
...
async function buildIspDataSummary(pathKey, depsOverride = {}) {
    ...
    try {
        ...
        // Toggle seksi report dari panel admin (config.upstreamMonitor.report.sections) — default semua ON.
        const on = (name) => !(cfg.report && cfg.report.sections && cfg.report.sections[name] === false);
        ...
        // Rincian arah + baris "Terdampak" (daftar pelanggan LIVE). getAffectedSet query MikroTik →
        // lewati sekalian bila seksi dimatikan (hemat probe, bukan cuma sembunyikan teks).
        if (on("rincianArah")) {
            let affected = null;
            try { affected = await deps.getAffectedSet(pathKey); } catch (_e) { affected = null; }
            try { out += deps.buildDirectionText(cfg, pathKey, entry, affected); } catch (_e) { /* best-effort */ }
        }
        if (on("layananPopuler")) {
        ...
        try {
            if (probeRows24.length && (on("polaLossPerJam") || on("perArah24jam"))) {
                const { buckets, worst } = buildHourlyBuckets(probeRows24, nowMs, 24);
                if (on("polaLossPerJam")) {
                ...
                if (on("perArah24jam")) {
                    ...
                    if (dir.far.length) {
                        const targetLabelOf = (key, address) => {
                            const t = ((cfg && cfg.targets) || []).find((x) => (x.key || x.address) === key);
                            return (t && t.label) || key || address;
                        };
                        out += `\n• Per arah 24 jam:`;
                        for (const d of dir.far) {
                            out += `\n  - → ${targetLabelOf(d.target_key, d.target)} (${d.target}): loss rata ${fmt(d.loss && d.loss.avg, 1)}% (puncak ${fmt(d.loss && d.loss.max)}%)` +
        ...
        if (on("gangguanTercatat")) out += downtimeLine("24 jam", dt24);
        if (on("layananPopuler")) out += buildService24Line(svcRows24, pathKey);

        // --- 7 HARI ---
        if (on("tujuhHari")) {
            out += `\n\n━━ *7 HARI* ━━` + raporLine("Rapor", isp7) +
                (on("gangguanTercatat") ? downtimeLine("7 hari", dt7) : "");
        }

        // --- Insiden terakhir ---
        try {
            const mine = on("insidenTerakhir") ? (incidents || []).filter((i) => i.path === pathKey).slice(0, 5) : [];
            if (mine.length) {
                const KIND_LABEL = {
        ...
        log.warn(`[ISP-Data] Rangkuman ${pathKey} gagal: ${err.message}`);
    ...
    try {
        ...
        for (const p of cfg.paths || []) {
            ...
            out += `\n\n${STATUS_EMOJI[status] || "⚪"} *${p.label || p.key}* — ${vonis.ok ? "✅" : "🔴"} ${vonis.text}` +
                ...
                (fo && fo.failover ? `\n   ⚠️ BACKUP AKTIF: jalan di ${fo.active_gateway || fo.backup_gateway} (primary ${fo.primary_gateway} tidak jalan)` : "") +
        ...
        log.warn(`[ISP-Data] Overview gagal: ${err.message}`);
