/**
 * Header Doc
 * Purpose: Modul normalisasi JID terpadu — konversi @lid → @s.whatsapp.net + peta identitas
 *          persisten (`database/lid-mappings.json`). Fondasi normalizeJidForSaldo/Message.
 * Caller: message/raf.js, handler saldo/pembayaran, lib/agent-*.
 * Deps: `fs`, `path`, `lib/atomic-file` (writeFileAtomicSync — tulis peta ATOMIK).
 * MainFuncs: normalizeJid, normalizeJidForSaldo, normalizeJidForMessage, loadMappings, saveMappings.
 * SideEffects: Membaca/menulis `database/lid-mappings.json` (ATOMIK).
 */

const log = require('./logger').logger.child('JID_UTILS');
const fs = require('fs');
const path = require('path');
// #b345: peta @lid→telepon (lid-mappings.json) = fondasi normalizeJidForSaldo. Tulis ATOMIK —
// torn-write saat SIGKILL bisa meracuni/mengosongkan peta identitas pelanggan (stakes tinggi).
...
function ensureMappingsFile() {
    try {
        ...
        if (!fs.existsSync(MAPPINGS_FILE)) {
            writeFileAtomicSync(MAPPINGS_FILE, JSON.stringify({ mappings: {} }, null, 2));
        }
    } catch (error) {
        log.error('[JID_UTILS] Failed to ensure mappings file:', error.message);
    }
}
...
function getStoredMappingByLid(lidJid) {
    ...
    if (mappings.mappings && mappings.mappings[lidId]) {
        ...
        try {
            ...
            log.warn(`[JID_UTILS] Error resolving @lid via stored userId:`, error.message);
        }
    }
...
function loadMappings() {
    try {
        ...
        log.warn('[JID_UTILS] Failed to load mappings:', error.message);
    }
    return { mappings: {} };
}
...
function saveMappings(data) {
    try {
        ...
        writeFileAtomicSync(MAPPINGS_FILE, JSON.stringify(data, null, 2));
    } catch (error) {
        log.error('[JID_UTILS] Failed to save mappings:', error.message);
    }
}
...
function logResolutionEvent(level, event, payload = {}) {
    const method = ({ error: 'error', warn: 'warn', debug: 'debug' })[level] || 'info';
    const logger = log[method].bind(log);
...
async function resolveCanonicalCustomerContext({
    ...
    // PRECISION GUARD: angka LID BUKAN nomor telepon. Jangan pernah memperlakukan
    // id LID (atau plainSenderNumber yang ternyata = id LID) sebagai kandidat nomor,
    // karena akan menghasilkan "62<LID>" palsu dan meracuni lid-mappings.json.
    const rawLidId = rawSender.endsWith('@lid') ? rawSender.split('@')[0] : null;
    const safePlainSenderNumber = (plainSenderNumber && plainSenderNumber !== rawLidId)
        ? plainSenderNumber
        : null;
    ...
    const metadataCandidate = normalizePhoneNumber(safePlainSenderNumber || getPreferredPlainSenderNumber(msg, rawSender));
    ...
    // Untuk @lid yang belum terpetakan secara otoritatif, extractPhoneFromJid akan
    // mengembalikan id LID (bukan nomor) — jadi lewati pencarian by-phone agar tidak salah-match.
    const phoneCandidate = safePlainSenderNumber || (rawSender.endsWith('@lid') ? null : extractPhoneFromJid(rawSender));
    const userByPhone = phoneCandidate ? findUserByCanonicalPhone(normalizedUsers, phoneCandidate) : null;
    if (userByPhone) {
...
function extractSenderInfo(msg, isGroup = false) {
    // Guard defensif: msg/msg.key bisa tidak lengkap pada payload aneh. Jangan crash —
    // return shape kosong yang valid supaya caller (resolveAgentContext) tetap jalan.
    if (!msg || !msg.key) {
        return {
            originalSender: null,
            isLid: false,
