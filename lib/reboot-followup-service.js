 ...
 */
"use strict";
const log = require('./logger').logger.child('REBOOT_FOLLOWUP_SERVICE');
...
async function sendToCustomer(jid, text, label) {
    try {
        ...
        log.error(`[REBOOTFU] Gagal kirim ke pelanggan (${label}): ${err.message}`);
        return { ok: false };
    }
}
...
async function notifyAdmins(text) {
    ...
    try {
        ...
        log.error(`[REBOOTFU] Gagal notifikasi admin: ${err.message}`);
    }
}
...
function scheduleFollowupForReboot({ user, jid, reason = "unknown", remoteAddr = null }) {
    ...
    try {
        ...
        if (isLidJid(jid)) {
            log.error(`[REBOOTFU] Menolak menjadwalkan follow-up: JID @lid belum ternormalisasi (${jid})`);
            return null;
        }
    } catch (_e) {
    ...
    if (job) log.info(`[REBOOTFU] Follow-up dijadwalkan ${job.id} (due ${job.dueAt})`);
    return job;
}
...
async function executeRebootAndSchedule({ user, jid, reason = "unknown", remoteAddr = null }) {
    ...
    try {
        ...
        log.error(`[REBOOTFU] rebootRouter melempar: ${err.message}`);
...
async function getPppoeSession(pppoeUsername, routerId) {
    ...
    try {
        ...
        // BEDAKAN "router menjawab" dari "router TAK terjangkau". getActivePPPoEUsers TIDAK
        // melempar saat gagal — ia MEMULANGKAN createResult `{ok:false, data:null, errorCode}`
        // (router unreachable / timeout 12s / breaker MikroTik terbuka). Diteruskan ke destructor
        // di bawah, `{ok:false,data:null}` menjadi list `[]` → `.find` undefined → keliru dibaca
        // "sesi mati", padahal ini BUTA. Persis skenario PTP Tanjungharjo putus yang seluruh modul
        // ini ada untuk menjaganya. Hanya `ok===false` yang dianggap buta; list kosong dari router
        // yang MENJAWAB tetap berarti pelanggan memang offline.
        if (raw && typeof raw === "object" && !Array.isArray(raw) && raw.ok === false) {
...
async function runDeepCheck(job) {
    ...
    // "Hijau" WAJIB berdiri di atas bukti positif, bukan sekadar ketiadaan kabar buruk.
    // `isPppoeOnline` memulangkan null saat MikroTik tak terjangkau (persis kondisi gangguan
    // PTP/uplink) ATAU saat `job.pppoeUsername` kosong — dua-duanya berarti BUTA, bukan sehat.
    // Gerbang lama `if (pppoe === false)` meloloskan null, sehingga pelanggan yang jalurnya
    // sedang putus justru diberi tahu "dari sisi kami semuanya terpantau normal".
    if (pppoe !== true) {
        return {
            verdict: "TAK_TERPANTAU",
            detail: "Status jalur pelanggan tidak dapat dibaca (router tak terjangkau atau data PPPoE kosong)."
        };
    }
...
// Penjaga anti-tumpang-tindih antar-tick. Satu job bisa mendekati/melebihi TICK_INTERVAL
// (retry MikroTik 12s x3 + backoff, + ACS 5s, + tunggu WA 8s), dan pasca-restart beberapa job
// bisa jatuh tempo sekaligus. Tanpa ini, interval berikutnya menjalankan tickOnce KEDUA yang
// membaca job SCHEDULED yang SAMA (updateJob baru dipanggil di akhir processJob) → pesan
// rebootfu & state REBOOTFU_ASK dobel, notifikasi eskalasi admin dobel.
let ticking = false;

/**
 * Satu siklus pemeriksaan pekerjaan jatuh tempo. Dipisah dari timer agar bisa diuji.
 * Serial terhadap dirinya sendiri: pemanggilan yang tumpang-tindih dilewati (skipped).
 */
async function tickOnce(now = Date.now()) {
    if (ticking) return { processed: 0, skipped: true };
    ticking = true;
    try {
        ...
        for (const job of due) {
            try {
                ...
                log.error(`[REBOOTFU] Job ${job.id} gagal diproses: ${err.message}`);
            }
        }
        return { processed };
    } finally {
...
async function processJob(job) {
    ...
    if (modemBack === null) {
        ...
        if (ageMs > conf.maxBlindMs) {
            log.warn(`[REBOOTFU] Job ${job.id} menyerah: pemantauan buta ${Math.round(ageMs / 60000)} menit.`);
            await notifyAdmins(
        ...
        log.info(`[REBOOTFU] Job ${job.id} ditunda: bot buta ke router, tak memvonis.`);
    ...
    if (modemBack) {
        ...
        try {
            ...
            log.error(`[REBOOTFU] Gagal set state REBOOTFU_ASK: ${err.message}`);
        }
...
function startRebootFollowupScheduler({ intervalMs = TICK_INTERVAL_MS } = {}) {
    if (!cfg().enabled) {
        log.info("[REBOOTFU] Nonaktif (set config.rebootAssist.enabled=true untuk mengaktifkan)");
        return null;
    }
    ...
    tickTimer = setInterval(() => {
        tickOnce().catch((err) => log.error(`[REBOOTFU] Tick gagal: ${err.message}`));
    }, intervalMs);
    ...
    log.info(`[REBOOTFU] Scheduler start (tick ${Math.round(intervalMs / 1000)}s)`);
    return tickTimer;
}
