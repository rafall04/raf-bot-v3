...
async function isModemBack(job) {
    ...
    // Sesi hidup. Bila uptime-nya lebih pendek dari waktu sejak reboot, sesi terbentuk ULANG
    // sesudah reboot — bukti lokal yang tak bergantung pada ACS maupun PTP.
    const elapsedSec = Math.max(0, (Date.now() - rebootAt) / 1000);
    if (session.uptimeSeconds !== null && session.uptimeSeconds < elapsedSec) return true;
    ...
    if (lastInform === null) return true;

    // Sesi hidup tapi uptime-nya LEBIH TUA dari reboot dan tak ada inform sesudahnya:
    // reboot tampaknya belum benar-benar terjadi. Tunggu siklus berikutnya.
    return false;
}
