 ...
 *
 * JANGAN pakai "inform < N menit": inform periodik ACS ini ~12 menit, jadi modem sehat pun
 * sering terakhir inform 11 menit lalu. Yang membuktikan modem BOOT ULANG adalah inform yang
 * terjadi SESUDAH `rebootAt`. Sesi PPPoE yang naik adalah bukti sekunder (dan itulah yang
 * benar-benar dirasakan pelanggan). PPPoE yang masih down = internet masih mati → belum kembali.
 */
async function isModemBack(job) {
    const pppoe = await isPppoeOnline(job.pppoeUsername, job.routerId);
    if (pppoe === false) return false;
    ...
    // ACS tak bisa dipastikan, tapi sesi internet pelanggan hidup → cukup untuk bertanya.
    return lastInform === null && pppoe === true;
}
...
module.exports = {
    ...
    notifyAdmins,
    sendToCustomer,
    _internal: { processJob, isModemBack, isPppoeOnline, resolveUpstreamStatus, cfg }
};
