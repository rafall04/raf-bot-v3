 ...
 */
"use strict";
const log = require('./logger').logger.child('OLT_EVENT_LOGGER');
...
function recordOltEventSafe(event) {
    try {
        ...
        const tsMs = Number.isFinite(event.server_time_ms)
            ? event.server_time_ms
            : (event.server_time ? Date.parse(event.server_time) : Date.now());

        // SILANG DUA SUMBER (#b279). LOS adalah KETIADAAN bukti dying-gasp, jadi baris DG yang
        // tak terlihat — paket syslog hilang, atau sudah tergulung dari halaman yang di-scrape —
        // otomatis jadi vonis LOS. Kalau sumber LAIN sempat melihat dying-gasp untuk MAC yang
        // sama, bukti positif itu menang. LOS memanggil teknisi mencari fiber putus; dying-gasp
        // tidak. Setiap LOS palsu = satu perjalanan sia-sia.
        const silang = require("./olt-dg-crosscheck").putuskanJenis({
        ...
        if (silang.dikoreksi) {
            log.info("[OLT-SILANG] " + (event.mac || "?") + " " + event.event_type + " -> "
                + silang.event_type + " (" + silang.alasan + "; sumber laporan ini: "
                + (event.source || "?") + ")");
        }

        const record = {
            ts_ms: tsMs,
            event_type: silang.event_type,
            mac: event.mac || null,
            slot: event.slot,
            onu: event.onu,
