...
function registerHttpSecurity(app, { express, projectRoot, isAllowedPortalOrigin } = {}) {
    ...
    // Logo perusahaan = branding publik (dipakai halaman landing, portal, legal, invoice).
    // Sengaja tetap di sini, sebelum auth, karena memang harus terbuka.
    //
    // AKAR yang diperbaiki: berkasnya DISIMPAN ke `static/uploads/logos/`
    // (routes/invoice.js logoStorage) sementara URL yang dicatat ke config berbunyi
    // `/uploads/logos/<file>` — dan mount ini dulu menunjuk `<root>/uploads/logos` yang
    // SELALU KOSONG. Akibatnya logo yang baru diunggah selalu tampil sebagai gambar rusak
    // di /invoice-settings, portal pelanggan, halaman legal, dan situs publik.
    //
    // Yang diselaraskan adalah MOUNT-nya, bukan nilai `logoPath` yang tersimpan: nilai itu
    // dipakai juga sebagai path FILESYSTEM relatif terhadap `static/`
    // (lib/pdf-invoice-generator.js:129 dan penghapus logo lama di routes/invoice.js).
    // Mengubah prefiksnya jadi `/static/...` akan membuat kedua pembacaan itu menunjuk
    // `static/static/...` dan justru mematikan logo di PDF.
    app.use("/uploads/logos", express.static(`${projectRoot}/static/uploads/logos`));
