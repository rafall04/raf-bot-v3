...
// Loader/penulis JSON terpusat: baca MENGKARANTINA berkas rusak (bukan []), tulis ATOMIK (tmp+rename).
// Store finansial voucher-agent (saldo terpotong + kode voucher + komisi) TAK boleh hilang saat
// restart 7-13x/hari / OOM SIGKILL — sama seperti sibling saldo topup-store (#b321) & invoice (#b330).
...
const { normalizeJidForSaldo } = require('./jid-utils');
const { recordVoucherOrphan } = require('./voucher-orphan');

// Nama store (relatif database/, di-resolve json-store). File fisik:
// database/agent_voucher_{inventory,purchases,sales}.json.
...
// Initialize database.
// Tiap file dimuat TERPISAH lewat json-store (never-throw: berkas rusak DIKARANTINA → [], berkas
// hilang → dibuat). Dulu ketiganya dibungkus SATU try/catch dgn JSON.parse mentah: bila file PERTAMA
// (inventory) rusak, blok berhenti → purchases & sales TAK PERNAH dimuat (tetap []), lalu penulisan
// berikutnya menimpa file valid dgn [] → 1 file korup meng-kaskade jadi kehilangan total 3 domain.
function initDatabase() {
    inventory = loadJSON('agent_voucher_inventory.json');
    purchases = loadJSON('agent_voucher_purchases.json');
    sales = loadJSON('agent_voucher_sales.json');
...
// Save functions — TULIS ATOMIK (tmp+rename via json-store) supaya SIGKILL/OOM di tengah tulis tak
// meninggalkan berkas terpotong (yang lalu di-parse gagal → []-kaskade di init berikutnya).
function saveInventory() {
    saveJSON('agent_voucher_inventory.json', inventory);
}

function savePurchases() {
    saveJSON('agent_voucher_purchases.json', purchases);
}

function saveSales() {
    saveJSON('agent_voucher_sales.json', sales);
}
...
async function purchaseVoucherAsReseller(agentId, voucherProfileId, quantity, paymentMethod = 'saldo', agentName = null) {
    try {
        ...
        // Info rollback: bila saldo sudah dipotong tapi generate voucher gagal, saldo WAJIB
        // dikembalikan agar agent tidak kehilangan uang tanpa menerima voucher.
        let saldoRefundInfo = null;

        if (paymentMethod === 'saldo') {
            // Check and deduct agent saldo
            const saldoManager = require('./saldo-manager');
            
            // Normalisasi nomor agent ke JID kanonik via helper standar (menangani @lid,
            // 0xxx, 62xxx, dan JID penuh). JANGAN hand-roll JID — @lid yang lolos apa adanya
            // akan menargetkan kunci saldo yang salah (lihat invariant JID di CLAUDE.md).
            const agentUserId = await normalizeJidForSaldo(agent.phone, { allowLid: false });
            if (!agentUserId) {
                return {
                    success: false,
                    message: 'Nomor WhatsApp agent tidak valid atau belum dipetakan. Hubungi admin untuk setup.'
                };
            }
            
            // Check agent saldo (getUserSaldo async — WAJIB di-await)
            const agentSaldo = await saldoManager.getUserSaldo(agentUserId);
            ...
            // Deduct saldo (deductSaldo async & atomik — WAJIB di-await agar boolean hasilnya valid)
            const deducted = await saldoManager.deductSaldo(
                agentUserId,
                totalHarga,
            ...
            // Tandai untuk kemungkinan refund jika generate voucher gagal di bawah.
            saldoRefundInfo = { agentUserId, amount: totalHarga };

            logger.info('Agent saldo deducted for voucher purchase', {
                agentId: agentId,
                agentUserId: agentUserId,
                amount: totalHarga,
                remainingSaldo: await saldoManager.getUserSaldo(agentUserId)
            });
        ...
        if (validCodes.length !== qty) {
            // Rollback: generate voucher gagal sebagian/total. (1) Kembalikan saldo yang sudah
            // dipotong (pembayaran saldo) agar agent tidak kehilangan uang. (2) Batalkan
            // transaksi pending (cash/transfer) agar tidak ada transaksi yatim yang bisa
            // dikonfirmasi untuk voucher yang tak pernah jadi.
            if (saldoRefundInfo) {
                try {
                    const saldoManager = require('./saldo-manager');
                    await saldoManager.addSaldo(
                        saldoRefundInfo.agentUserId,
                        saldoRefundInfo.amount,
                        `Refund: gagal generate voucher ${profile.namavc} x${qty}`
                    );
                    logger.info('Saldo agent dikembalikan setelah gagal generate voucher', {
                        agentId,
                        agentUserId: saldoRefundInfo.agentUserId,
                        amount: saldoRefundInfo.amount
                    });
                } catch (refundError) {
                    logger.error('KRITIS: gagal refund saldo agent setelah generate voucher gagal', {
                        agentId,
                        agentUserId: saldoRefundInfo.agentUserId,
                        amount: saldoRefundInfo.amount,
            ...
            if (agentTransactionId) {
                try {
                    require('./agent-transaction-manager').cancelTransaction(
                        agentTransactionId,
                        'Pembelian voucher dibatalkan: gagal generate voucher code'
                    );
                } catch (cancelError) {
                    logger.error('Gagal membatalkan transaksi agent setelah generate voucher gagal', {
                        agentTransactionId,
                        error: cancelError.message
                    });
                }
            }
            // #b325: voucher yang TERLANJUR dibuat di MikroTik (validCodes) TIDAK boleh dibuang senyap —
            // mereka jadi hotspot user aktif tak terlacak (bocor). Catat sebagai orphan supaya admin bisa
            // hapus/tinjau manual (revoke otomatis butuh jalur hapus hotspot yang aman; orphan dulu).
            for (const vc of validCodes) {
                try {
                    recordVoucherOrphan({
                        sender: `agent_${agentId}`,
                        voucherCode: vc.username,
                        profile: profile.prof,
                        price: hargaReseller,
                        reason: 'agent_partial_rollback'
                    });
                } catch (_e) { /* never-throw: pencatatan orphan tak boleh menggagalkan rollback */ }
            }
            if (validCodes.length > 0) {
                logger.warn(`[VOUCHER_ORPHAN] ${validCodes.length} voucher agent terlanjur dibuat saat gagal parsial — dicatat ke worklist orphan (hapus manual).`, { agentId });
            }
            return {
                success: false,
                message: `Gagal generate ${qty - validCodes.length} voucher code. ${saldoRefundInfo ? 'Saldo Anda telah dikembalikan. ' : ''}Silakan coba lagi.`
            };
        }
        ...
        // Tambah stok HANYA untuk pembelian yang langsung lunas (saldo). Untuk cash/transfer
        // status masih 'pending' — stok BARU ditambahkan setelah pembayaran diverifikasi via
        // activatePurchaseByTransaction() (dipanggil dari confirmTransaction). Tanpa ini, stok
        // "hantu" muncul tapi tidak bisa dijual karena sellVoucherToCustomer hanya mengambil
        // kode dari purchase berstatus 'completed'.
...
/**
 * Aktifkan pembelian voucher reseller yang masih 'pending' (cash/transfer) SETELAH
 * pembayaran diverifikasi: set purchase jadi 'completed' dan tambahkan stoknya ke
 * inventory agent. Dipanggil dari agent-transaction-manager.confirmTransaction saat
 * transaksi bertipe voucher_purchase dikonfirmasi. Idempoten: bila tidak ada purchase
 * pending yang cocok (mis. sudah pernah diaktifkan), tidak melakukan apa-apa.
 *
 * @param {string} agentTransactionId - ID transaksi agent yang baru dikonfirmasi
 * @returns {{success: boolean, message?: string, purchase?: object, inventory?: object}}
 */
function activatePurchaseByTransaction(agentTransactionId) {
    if (!agentTransactionId) {
        return { success: false, message: 'agentTransactionId wajib diisi' };
    }

    const purchase = purchases.find(
        p => p.agentTransactionId === agentTransactionId && p.status === 'pending'
    );

    if (!purchase) {
        // Tidak ada yang perlu diaktifkan (bukan pembelian voucher, atau sudah aktif).
        return { success: false, message: 'Pembelian voucher pending tidak ditemukan untuk transaksi ini' };
    }
    ...
    const currentInventory = getAgentInventory(purchase.agentId);
    const currentStok = currentInventory.inventory.find(
        item => item.voucherProfileId === purchase.voucherProfileId
    )?.stok || 0;

    updateInventoryItem(purchase.agentId, purchase.voucherProfileId, {
        agentName: purchase.agentName,
        stok: currentStok + purchase.quantity
    });

    logger.info('Stok voucher agent diaktifkan setelah verifikasi pembayaran', {
        purchaseId: purchase.id,
        agentTransactionId,
        agentId: purchase.agentId,
        voucherProfileId: purchase.voucherProfileId,
        quantity: purchase.quantity
    });

    return { success: true, purchase, inventory: getAgentInventory(purchase.agentId) };
}
