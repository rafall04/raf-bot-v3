/**
 * Header Doc
 * Purpose: OLT LOS Broadcaster — mengubah stream event OLT jadi alarm fiber-putus yang presisi
 *          (konfirmasi, klaster, dedup, verifikasi anti mati-listrik), lalu menutup alarmnya saat
 *          ONU pulih. Rinciannya di bawah.
 * Caller: `lib/olt-syslog-receiver.js` / `lib/olt-log-scraper.js` / `lib/olt-snmp-los-poller.js`
 *         (via `handleOltEvent`), halaman `/los-broadcast` (listIncidents/getLosState).
 * Deps: `./whatsapp-critical-delivery` (sendCritical), `./whatsapp-gateway` + `./whatsapp-delivery-service`
 *       (post grup), `./olt-los-verifier` (scrape web log OLT), `./olt-customer-resolver`,
 *       `./los-ticket-service`, `./account-classification`, `./teknisi-recipient-resolver` (#b355 filter
 *       LOS per kelas alert teknisi), `./post-repair-verification` (laporan
 *       verifikasi setelah pulih), file `database/los-incidents.json`.
 * MainFuncs: `createLosBroadcaster`, `handleOltEvent`, `listLosIncidents`, `getLosState`.
 * SideEffects: Kirim WhatsApp (grup + japri teknisi + pelanggan), tulis `los-incidents.json`,
 *              buat/batalkan tiket LOS. Gate `config.oltLosBroadcast.enabled` (default OFF).
 *
 ...
 */
"use strict";
const log = require('./logger').logger.child('OLT_LOS_BROADCASTER');
...
// Penjaga data internal untuk teks yang dibaca PELANGGAN. Sudah lama ada, tapi dulu hanya
// dipasang di broadcast admin — jalur notifikasi pelanggan LOS ini tak pernah dijaganya.
...
// Kalimat "siapa yang sedang mengerjakan" — DUA versi, dipilih dari jam kerja teknisi.
//
// !! Alur LOS ini otomatis dan meletus jam berapa saja; kabel putus tengah malam dulu tetap
// dijanjikan "sedang menangani" padahal jam kerja teknisi 08:00-17:00. Janji yang tak bisa
// ditepati itu justru memancing telepon jam 3 pagi dan mengikis percaya.
const DEFAULT_PENANGANAN_DALAM_JAM =
    "Tim teknisi sudah dapat infonya dan sedang menangani. Kami *pasti kabari lagi* begitu sudah normal.";
const DEFAULT_PENANGANAN_LUAR_JAM =
    "Tim teknisi sudah dapat infonya 🙏 Karena sekarang di luar jam kerja, penanganan " +
    "dikerjakan {jam_kerja}. Kami dahulukan gangguan yang paling luas dulu, dan pasti " +
    "kami kabari begitu koneksi Kakak sudah normal.";
// Dipakai saat jam kerja tak bisa dipastikan waktunya (nextWorkingTime kosong) — tetap jujur
// tanpa menyebut jam yang tak kami ketahui.
const DEFAULT_PENANGANAN_LUAR_JAM_TANPA_WAKTU =
    "Tim teknisi sudah dapat infonya 🙏 Karena sekarang di luar jam kerja, penanganan " +
    "dikerjakan pada jam kerja berikutnya. Kami dahulukan gangguan yang paling luas dulu, " +
    "dan pasti kami kabari begitu koneksi Kakak sudah normal.";

const DEFAULT_CUSTOMER_TEMPLATE =
    "Halo Kak {customer_name} 👋\n\n" +
    "Kami lihat koneksi internet di lokasi Kakak lagi terganggu nih.\n\n" +
    "{penanganan}\n\n" +
...
// Catatan GANGGUAN AREA — disisipkan lewat slot {area_note}, kosong bila bukan gangguan area.
//
// TERUKUR: 21 dari 25 notifikasi pelanggan (84%) adalah gangguan AREA, bukan sambungan pribadi.
// Bagi pelanggan itu fakta paling menenangkan yang bisa disampaikan — sekaligus yang paling
// mengurangi telepon masuk, karena menjawab lebih dulu dua pertanyaan pertama mereka:
// "apa cuma saya?" dan "apa gara-gara alat saya?".
//
// !! Sengaja TIDAK menyebut jumlah pelanggan terdampak, nama ODP/ODC, atau lokasi teknis apa pun
// (aturan customer-facing-no-internal-ids). Cukup "di area Kakak".
const DEFAULT_AREA_NOTE =
    "Gangguan ini terjadi di *area Kakak* — beberapa pelanggan lain juga terdampak, jadi ini bukan dari perangkat Kakak ya.";
...
const DEFAULTS = {
    ...
    notifyCustomer: {
        ...
        areaNoteTemplate: DEFAULT_AREA_NOTE,
        // Kalimat penanganan sadar-jam-kerja (#b269) — dua versi, admin boleh mengedit.
        penangananTemplate: DEFAULT_PENANGANAN_DALAM_JAM,
        penangananLuarJamTemplate: DEFAULT_PENANGANAN_LUAR_JAM,
        penangananLuarJamTanpaWaktuTemplate: DEFAULT_PENANGANAN_LUAR_JAM_TANPA_WAKTU,
    },
};
...
function defaultLoadIncidents() {
    try {
        ...
        log.error("[LOS-BROADCAST] Gagal baca incidents:", err.message);
    }
    return [];
}
...
function defaultSaveIncidents(list) {
    try {
        ...
        log.error("[LOS-BROADCAST] Gagal tulis incidents:", err.message);
    }
}

// Penerima broadcast LOS. Disaring per-preferensi teknisi (#b355): HANYA kelas `los` + master-ON.
// LOS = fiber putus (paling kritis) → `critical:true` (tembus jam-diam/snooze) dan SENGAJA TANPA
// filter-area (insiden bisa lintas banyak ODP; area-scoping fiber-cut berisiko tak ada yang respons —
// pengarahan per-area untuk LOS terjadi di penugasan TIKET, bukan di broadcast ini). Gate OFF /
// belum-setel prefs ⇒ resolver lolos apa adanya = perilaku lama. Tetap balikan STRING telepon.
function defaultTeknisiRecipients() {
    const { filterTeknisiRecipients } = require("./teknisi-recipient-resolver");
    const candidates = (global.accounts || [])
        .filter((acc) => acc && acc.role === "teknisi" && acc.phone_number)
        .map((acc) => ({ accountId: acc.id, phone: String(acc.phone_number).split("|")[0].trim() }))
        .filter((c) => c.phone);
    const { recipients } = filterTeknisiRecipients({ recipients: candidates, alertClass: "los", critical: true });
    return recipients.map((c) => c.phone);
}
...
async function defaultPostToGroup(groupId, text) {
    try {
        ...
        log.error("[LOS-BROADCAST] postToGroup error:", e && e.message);
...
function createLosBroadcaster(deps = {}) {
    ...
    // Satu-satunya pemilik format durasi untuk teks pelanggan (#b260) — dulu ada dua salinan
    // dengan perilaku berbeda, dan keduanya sama-sama sampai ke pelanggan.
    const fmtDuration = (ms) => require("./format-duration").formatDurasi(ms);
    ...
    async function sendRecovery(oltKey, recs) {
        ...
        if (ncfg.enabled && ncfg.notifyOnRecovery !== false) {
            for (const rec of recs) {
                try {
                    ...
                    if (!phone) continue;
                    // Template PULIH juga bisa diedit admin — dijaga sama seperti pesan gangguan.
                    const custMsg = amankanTeksPelanggan(buildCustomerRecoveryMessage(a.customer, a, ncfg), a.customer, { isRecovery: true, ncfg });
        ...
        // LAPORAN VERIFIKASI PASCA-PERBAIKAN (gate sendiri di modulnya, default OFF).
        // Pesan pulih di atas hanya menyebut yang KEMBALI ONLINE; yang masih mati tak terlihat sama
        // sekali, padahal justru itu yang menentukan boleh-tidaknya teknisi meninggalkan lokasi.
        // Karena itu insiden yang masih aktif di OLT yang sama ikut disertakan. NEVER-THROW.
        try {
            const stillDown = [...activeIncidents.values()]
                .filter((a) => a && (a.oltId || "default") === oltKey)
                .map((a) => ({
                    mac: a.mac,
                    slot: a.slot,
                    onu: a.onu,
                    name: a.customer ? a.customer.name : null,
                    customerId: a.customer ? a.customer.id : null,
                    downAtMs: a.downAtMs || null,
                }));
            require("./post-repair-verification").reportAfterRecovery({
                oltKey,
                recovered: recs.map((rec) => ({
                    mac: rec.active.mac,
                    slot: rec.active.slot,
                    onu: rec.active.onu,
                    name: rec.active.customer ? rec.active.customer.name : null,
                    // Dipakai laporan verifikasi untuk mengambil redaman versi GenieACS.
                    customerId: rec.active.customer ? rec.active.customer.id : null,
                    downAtMs: rec.active.downAtMs || null,
                })),
                stillDown,
            });
        } catch (e) {
            logger.error && logger.error("[LOS-BROADCAST] Jadwal verifikasi pasca-perbaikan gagal:", e && e.message);
        }
    }
    ...
    /**
     * Pesan gangguan untuk PELANGGAN.
     *
     * Slot `{mac}` / `{slot}` / `{onu}` SENGAJA TIDAK LAGI DISEDIAKAN. Ketiganya adalah alamat
     * perangkat di OLT — tak berguna bagi pelanggan dan membocorkan topologi jaringan. Dulu
     * ketiganya diisi di sini DAN diiklankan halaman /los-broadcast ("Placeholder: … {mac},
     * {slot}, {onu} …"), padahal jalur notifikasi pelanggan sudah HIDUP di salah satu bot
     * produksi — jadi kebocoran hanya berjarak satu edit admin, edit yang justru disarankan
     * halaman itu sendiri. Dan karena config produksi di-merge-key (tak pernah ditimpa deploy),
     * template yang terlanjur memuatnya akan bertahan selamanya.
     *
     * Dua lapis sekarang: (1) slotnya dikosongkan, bukan diisi; (2) penjaga
     * `lib/customer-text-guard` memeriksa hasil akhirnya sebelum dikirim (lihat pemanggil).
     */
    /**
     * Kalimat penanganan yang JUJUR menurut jam kerja teknisi (#b269).
     * Memulangkan "" saat dalam jam kerja + template memang sudah bicara sendiri.
     */
    function buildPenangananNote(ncfg) {
        const { dalamJamKerja, teks, teksRentang } = waktuMulaiKerjaBerikutnya();
        if (dalamJamKerja) {
            return String(ncfg.penangananTemplate || DEFAULT_PENANGANAN_DALAM_JAM);
        }
        if (!teks) {
            return String(ncfg.penangananLuarJamTanpaWaktuTemplate || DEFAULT_PENANGANAN_LUAR_JAM_TANPA_WAKTU);
        }
        // {jam_kerja} = RENTANG jam kerja (dipakai kalimat bawaan). {waktu_mulai} tetap
        // didukung supaya template admin yang terlanjur memakainya tak mengirim slot mentah.
        return String(ncfg.penangananLuarJamTemplate || DEFAULT_PENANGANAN_LUAR_JAM)
            .split('{jam_kerja}').join(teksRentang || teks)
            .split('{waktu_mulai}').join(teks);
    }

    /**
     * Jaring untuk template LAMA yang tak punya slot {penanganan}.
     *
     * !! Kedua bot produksi menyimpan messageTemplate sendiri di config.json (merge-key,
     * tak pernah ditimpa deploy). Kalau perbaikan ini hanya mengandalkan slot, template
     * tersimpan itu akan menjanjikan "sedang menangani" jam 2 pagi SELAMANYA. Jadi di luar
     * jam kerja catatannya disisipkan sendiri — tepat di atas baris tanda tangan.
     */
    function sisipkanCatatanJamKerja(teksPesan, catatan) {
        if (!catatan) return teksPesan;
        const baris = String(teksPesan).split("\n");
        let tandaTangan = -1;
        for (let i = baris.length - 1; i >= 0; i--) {
            const t = baris[i].trim();
            if (!t) continue;
            if (t.startsWith("—") || t.startsWith("--")) tandaTangan = i;
            break;
        }
        if (tandaTangan === -1) return `${teksPesan}\n\n${catatan}`;
        baris.splice(tandaTangan, 0, catatan, "");
        return baris.join("\n");
    }
    ...
    function buildCustomerMessage(customer, incident, ncfg) {
        ...
        const areaNote = incident && incident.areaOutage
            ? String(ncfg.areaNoteTemplate || DEFAULT_AREA_NOTE)
            : "";
        const penanganan = buildPenangananNote(ncfg);
        const punyaSlot = /\{penanganan\}/.test(String(tpl));
        const { dalamJamKerja } = waktuMulaiKerjaBerikutnya();
        let hasil = String(tpl)
            .replace(/\{penanganan\}/g, penanganan)
            .replace(/\{area_note\}/g, areaNote)
            .replace(/\{customer_name\}/g, customer.name || "Pelanggan")
            .replace(/\{address\}/g, customer.address || "-")
            // Dikosongkan, BUKAN diisi: template lama di server yang masih memakainya tidak
            // menampilkan `{mac}` mentah ke pelanggan, dan tidak pula membocorkan nilainya.
            .replace(/\{mac\}/g, "")
            .replace(/\{slot\}/g, "")
            .replace(/\{onu\}/g, "")
            .replace(/\{company_name\}/g, (global.config && global.config.companyName) || "")
            // {area_note} kosong saat bukan gangguan area — rapikan baris kosong yang ditinggalkannya
            // supaya pesan tak pernah terkirim dengan celah menganga di tengah.
            .replace(new RegExp("[ \t]+\\n", "g"), "\n")
            .replace(new RegExp("\\n{3,}", "g"), "\n\n")
            .trim();
        // Template LAMA tanpa slot: sisipkan catatannya sendiri, tapi HANYA di luar jam kerja
        // (di dalam jam kerja teks lama sudah benar apa adanya — jangan tambah kebisingan).
        if (!punyaSlot && !dalamJamKerja) hasil = sisipkanCatatanJamKerja(hasil, penanganan);
        return hasil;
    }

    /**
     * Jaring terakhir sebelum teks sampai ke pelanggan. Kalau template hasil editan admin tetap
     * memuat data internal (mis. ODP diketik manual sebagai teks biasa, bukan slot), pesannya
     * DIGANTI template bawaan yang sudah pasti bersih — pelanggan tetap dapat kabar, tapi tanpa
     * data internal. Tak pernah melempar: gagal-kirim tak boleh menjatuhkan jalur gangguan.
     */
    function amankanTeksPelanggan(message, customer, opts = {}) {
        try {
            const bocor = findCustomerTextLeaks(message);
            if (!bocor || !bocor.length) return message;
            logger.warn && logger.warn(
                `[LOS-BROADCAST] Template pesan pelanggan memuat data internal (${describeLeaks(bocor)}) — dipakai template bawaan yang bersih. Perbaiki di halaman /los-broadcast.`
            );
            const nama = (customer && customer.name) || "Pelanggan";
            const alamat = (customer && customer.address) || "-";
            const perusahaan = (global.config && global.config.companyName) || "";
            // Pakai template bersih yang SESUAI jenis pesan. Kalau tidak, pesan PULIH yang bocor malah
            // diganti template GANGGUAN → pelanggan dikira masih down (padahal sudah normal).
            if (opts.isRecovery) {
                return String(DEFAULT_CUSTOMER_RECOVERY_TEMPLATE)
                    .replace(/\{customer_name\}/g, nama)
                    .replace(/\{address\}/g, alamat)
                    .replace(/\{company_name\}/g, perusahaan);
            }
            // Gangguan: WAJIB isi {penanganan} juga (slot ADA di DEFAULT_CUSTOMER_TEMPLATE) — kalau
            // tidak, "{penanganan}" mentah ikut terkirim ke pelanggan.
            return String(DEFAULT_CUSTOMER_TEMPLATE)
                .replace(/\{customer_name\}/g, nama)
                .replace(/\{address\}/g, alamat)
                .replace(/\{penanganan\}/g, buildPenangananNote(opts.ncfg || {}))
                .replace(/\{company_name\}/g, perusahaan);
        } catch (_e) {
            return message;
        }
    ...
    async function onCustomerNotify(mac) {
        ...
        const message = amankanTeksPelanggan(buildCustomerMessage(customer, a, ncfg), customer, { ncfg });
    ...
    async function broadcastGroup(oltKey, items) {
        ...
        for (const it of items) {
            ...
            const active = {
                ...
                // Dibawa ke insiden AKTIF (bukan cuma yang tersimpan) supaya pesan pelanggan bisa
                // menyebutnya: TERUKUR 21 dari 25 notifikasi (84%) adalah gangguan AREA, dan
                // "ini terjadi di area Kakak, bukan hanya di rumah Kakak" adalah fakta paling
                // menenangkan yang bisa kita sampaikan sekaligus paling mengurangi telepon masuk.
                areaOutage: isAreaOutage,
            };
