...
async function evaluasiKestabilan(cfg = {}, deps = {}) {
    ...
    try {
        ...
        for (const p of jalur) {
            ...
            const adminFallback = Array.isArray(jids) ? jids : [];
            // Routing kategori 'network_quality': ke grup bila diarahkan, jika tidak FAIL-OPEN ke
            // DM admin (perilaku lama). Kirim TETAP sendCritical (alarm jaringan harus durable).
            let daftar = adminFallback;
            try {
                const { recipientsFor } = require("./notif-router");
                daftar = recipientsFor("network_quality", { adminFallback }).recipients || adminFallback;
            } catch (_e) { daftar = adminFallback; }
            if (!daftar.length) {
            ...
            for (const jid of daftar) {
                try {
                    // !! Dulu memanggil sendNotification dari whatsapp-notification-wrapper
                    // yang TIDAK mengekspornya — setiap alarm melempar "kirim is not a
                    // function" dan terbukti di log produksi: alarm ini tak pernah sampai.
                    const kirim = deps.kirim
                        || ((to, text) => require("./whatsapp-critical-delivery").sendCritical(to, { text }, { label: "alarm_kestabilan", waitForReadyMs: 8000 }));
                    const r = await kirim(jid, teks);
                    if (r !== false && !(r && r.delivered === false)) terkirim += 1;
                } catch (e) {
