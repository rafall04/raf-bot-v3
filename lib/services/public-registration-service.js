 ...
 */
"use strict";
const log = require('../logger').logger.child('PUBLIC_REGISTRATION_SERVICE');
...
async function notifyAdmins(text) {
    ...
    if (!jids.length) {
        log.error("[PUBLIC_REG] Tidak ada JID admin valid untuk notifikasi lead.");
        return;
    }
    for (const jid of jids) {
        try {
            ...
            log.error(`[PUBLIC_REG] Gagal kirim notif lead ke ${jid}:`, e.message);
        }
    }
}
...
async function submitPublicRegistration({
    ...
    try {
        ...
        log.error("[PUBLIC_REG] Gagal alokasi/simpan lead PSB:", err && err.message);
    ...
    try {
        ...
        log.error("[PUBLIC_REG] Gagal update snapshot psbRecords (lead tetap tersimpan di DB):", err.message);
    }
    ...
    try {
        ...
        if (isTemplateError(adminText)) {
            log.warn("[PUBLIC_REG] Template registrasi_lead_admin hilang/rusak — pakai fallback plain.");
            adminText = `📥 Lead pendaftaran online baru\nNama: ${record.name}\nHP: ${record.phone_number}\nAlamat: ${record.address}\nPaket: ${record.psb_data.package_interest || "-"}\nID: ${customerId}`;
        }
        await notifyAdmins(adminText);
    } catch (err) {
        log.error("[PUBLIC_REG] Gagal render/kirim notif admin:", err.message);
    }
    ...
    try {
        if (hasAuthenticatedSession()) {
            ...
            if (jid) {
                ...
                if (isTemplateError(custText)) {
                    log.warn("[PUBLIC_REG] Template registrasi_lead_pelanggan hilang/rusak — notif pelanggan dilewati.");
                } else {
        ...
        log.error("[PUBLIC_REG] Gagal kirim notif calon pelanggan (diabaikan):", err.message);
    }
