 ...
 */
"use strict";
const log = require('../logger').logger.child('KAS_GROUP_NOTIFIER');
...
async function kabarkanKeGrupKas(templateKey, opsi = {}) {
    ...
    try {
        ...
        if (!teks || /^Error: Template/.test(String(teks))) {
            log.error("[KAS_GRUP_NOTIF] Teks kosong/gagal render untuk", templateKey);
        ...
        log.error("[KAS_GRUP_NOTIF]", e && e.message);
