...
const TEMPLATE_KEY_KELEBIHAN = "tagihan_kelebihan_bayar";

/**
 * Pesan untuk pembayaran yang MASUK tapi periodenya SUDAH lunas sebelumnya.
 *
 * KENAPA TERPISAH dari struk lunas: pembayaran kedua TIDAK menambah status lunas apa pun.
 * Mengirim struk "PEMBAYARAN BERHASIL — tagihan Anda sudah LUNAS" untuk uang yang tak
 * teralokasi membuat pelanggan mengira ia sudah membayar bulan berikutnya, dan
 * menyembunyikan dari pemilik bahwa ada uang masuk yang belum diputuskan nasibnya.
 *
 * Nada pesannya sengaja TIDAK menuduh dan TIDAK menjanjikan refund otomatis — hanya
 * menyatakan apa adanya bahwa uangnya diterima, periodenya memang sudah lunas, dan admin
 * akan menghubungi.
 */
function buildOverpaymentText({ user, amount, periodMonth, periodYear, method, paidAt, refId } = {}) {
    const account = user || {};
    const data = {
        nama_pelanggan: account.name || "Pelanggan",
        nama_paket: account.subscription || "-",
    ...
    const teks = renderTemplate(TEMPLATE_KEY_KELEBIHAN, data);

    // PENJAGA yang sama dengan struk cicilan — `renderTemplate` TIDAK memulangkan string kosong
    // saat key tak ada, melainkan kalimat `Error: Template "..." not found.`. Produksi memakai
    // `message_templates.json` merge-key, jadi key baru belum tentu sudah mendarat di sana saat
    // kode ini jalan. Pelanggan yang BARU menyerahkan uang tak boleh menerima teks error mentah.
    if (!teks || /^Error: Template/i.test(String(teks))) {
        return [
            "ℹ️ *PEMBAYARAN DITERIMA*",
            "",
            ...
            `• No. Ref : ${data.no_ref}`,
            "",
            `Namun tagihan periode ${data.periode} tercatat *sudah lunas sebelumnya*, jadi pembayaran ini belum kami alokasikan.`,
            "Admin kami akan menghubungi Anda untuk menindaklanjuti. Mohon simpan bukti transfer ini ya Kak."
        ].join("\n");
    }
...
module.exports = {
    ...
    buildOverpaymentText,
    TEMPLATE_KEY,
    TEMPLATE_KEY_KELEBIHAN
};
