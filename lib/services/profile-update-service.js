...
const log = require('../logger').logger.child('PROFILE_UPDATE_SERVICE');
...
class ProfileUpdateService {
    ...
    static async applyProfileUpdate(user, newProfile, options = {}) {
        ...
        if (!newProfile) {
            ...
            if (notification.enabled && user.phone_number && user.phone_number.trim() !== '') {
                try {
                    ...
                    log.warn(`[${context}_WARN] [User: ${user.name || user.pppoe_username}] ${errMsg}`);
        ...
        try {
            ...
            log.info(`[${context}] Updating profile untuk ${user.pppoe_username} ke ${newProfile}`);
            ...
            log.error(`[${context}_ERROR] [User: ${user.name || user.pppoe_username}] ${errMsg}`);
