 ...
 */
"use strict";
const log = require('../logger').logger.child('REACTIVATION_OUTCOME');
...
async function alertReaktivasiGagal({ user, refId }, deps = {}) {
    try {
        ...
        if (!jids.length) {
            log.error("[REAKTIVASI_GAGAL] Tak ada JID admin untuk alarm reaktivasi gagal.");
            return false;
        }
        let sent = false;
        for (const jid of jids) {
            try {
                ...
                log.error("[REAKTIVASI_GAGAL] Gagal kirim ke", jid, e && e.message);
            }
        }
        return sent;
    } catch (e) {
        log.error("[REAKTIVASI_GAGAL] Alarm reaktivasi gagal total:", e && e.message);
        return false;
    }
}
