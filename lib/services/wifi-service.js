...
const log = require('../logger').logger.child('WIFI_SERVICE');
...
async function writeCustomerPortalWifiLog({ customer, ssidIndex, changeType, changes, req, fallbackReason = '' }) {
    try {
        ...
        log.error('[WIFI_LOGGING][web_customer] Gagal menulis audit log:', logError.message);
    }
}
        ...
        try {
            ...
            if (customer.bulk && Array.isArray(customer.bulk) && customer.bulk.length > 0) {
                ...
                if (allowedSSIDIds.length > 0) {
                    ...
                    log.info(`[WifiService] Filtered SSIDs: ${filteredSSIDs.length} dari ${wifiInfo.ssid.length} total. Allowed: [${allowedSSIDIds.join(', ')}]`);
                }
            } else {
                ...
                log.info(`[WifiService] Customer ${customer.id} tidak memiliki bulk, hanya return SSID 1`);
            }
        ...
        try {
            ...
            if (customer.bulk && Array.isArray(customer.bulk) && customer.bulk.length > 0) {
                ...
                if (allowedSSIDIds.length > 0) {
                    ...
                    log.info(`[WifiService] Filtered SSIDs for connected devices: ${filteredSSIDs.length} dari ${wifiInfo.ssid.length} total. Allowed: [${allowedSSIDIds.join(', ')}]`);
                }
            } else {
                // Jika customer tidak memiliki bulk, hanya return SSID 1 (default)
                filteredSSIDs = filteredSSIDs.filter(ssid => {
                ...
                log.info(`[WifiService] Customer ${customer.id} tidak memiliki bulk, hanya return SSID 1 untuk connected devices`);
            }
