...
async function buildMoneySummary(opsi = {}) {
    ...
    return {
        ...
        akanTerlewat,
        akanPerKategori,
        // Sisa sekarang & sesudah tagihan rutin. Keduanya null bila bahannya tak terbaca —
        // proyeksi dari angka buta lebih buruk daripada tak ada proyeksi.
        sisa: pemasukan && keluar != null ? bulat(pemasukan.netPaid) - keluar : null,
        proyeksiSisa:
            pemasukan && keluar != null && akanKeluar != null
                ? bulat(pemasukan.netPaid) - keluar - akanKeluar
                : null,
        gagal
    };
}
...
function buildMoneySummaryText(data, opsi = {}) {
    ...
    return renderResponseTemplate(
            ...
            "• Sekarang       : ${sisa_bersih}${proyeksi_info}${catatan}",
        {
            ...
            tren,
            // Pengeluaran & sisa memakai angka KAS bulan berjalan (expense-manager), sumber
            // yang sama dengan halaman — bukan `arus.totalExpense` dari buku besar yang
            // cakupannya berbeda. Dua sumber untuk satu label adalah cara angka mulai berselisih.
            total_pengeluaran: nominalAtau(data.keluar),
