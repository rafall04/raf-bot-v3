...
class SpeedRequestService extends BaseService {
    ...
    static async cancelRequest(user, requestId, req = null) {
        ...
        global.speed_requests?.findIndex(r => r.id === requestId) ?? -1;
    ...
    static async getAvailableSpeedBoosts(user) {
        ...
        return availablePackages.map(pkg => {
            ...
            return {
                name: pkg.name,
                // HANYA label tampilan — pola yang sama dengan `routes/packages.js` yang sudah
                // dikeraskan lebih dulu. JANGAN fallback ke `pkg.profile`: itu nama profil
                // MikroTik, dan angkanya bukan yang dibeli pelanggan (PAKET-110K dijual
                // "Up To 10Mbps", profilnya `12Mbps`). Kosong lebih baik daripada bocor.
    ...
    static async getSpeedBoostPackages() {
        ...
        return availableBoosts.map(pkg => {
            ...
            return {
                name: pkg.name,
                price: pkg.price,
                // Endpoint ini TANPA LOGIN (`/api/speed-boost/packages` terdaftar publik di
                // `lib/http-auth-bootstrap.js`), jadi `pkg.profile` di sini berarti siapa pun
                // di internet bisa mengambil peta profil jaringan. Sama seperti di atas:
                // label tampilan saja, tanpa fallback.
                profile: pkg.displayProfile || '',
