...
function barisPendapatan(row) {
    ...
    const marketing = Number.parseInt(row.komisi_marketing, 10) || 0;
    // SATU sumber, bukan penjumlahan. `bonus` dan `bonus_manual` selalu berisi nilai yang SAMA
    // (formatPayrollRow menyetel keduanya dari `bonus_manual || bonus`, dan tiap jalur tulis
    // mengisi keduanya sekaligus) — menjumlahkannya membuat teknisi membaca bonus DUA KALI
    // LIPAT, dan rincian struk tak lagi menjumlah ke gaji bersih di bawahnya.
    const bonus = (Number.parseInt(row.bonus_manual, 10) || 0) || (Number.parseInt(row.bonus, 10) || 0);
    if (komisi > 0) baris.push(`• Komisi penarikan: ${rupiah(komisi)}`);
    // Komisi marketing IKUT menambah gaji kotor, jadi kalau tak dicetak, rincian yang dibaca
    // teknisi lebih kecil daripada gaji bersihnya tanpa penjelasan.
    if (marketing > 0) baris.push(`• Komisi marketing: ${rupiah(marketing)}`);
