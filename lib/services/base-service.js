...
const log = require('../logger').logger.child('BASE_SERVICE');
...
class BaseService {
    ...
    static logOperation(serviceName, operation, context = {}) {
        log.info(`[${serviceName}] ${operation}`, context);
    }
    ...
    static logError(serviceName, operation, error, context = {}) {
        log.error(`[${serviceName}_ERROR] ${operation}`, {
            error: error.message,
            stack: error.stack,
            ...context
        });
    ...
    static logDataAccess(serviceName, operation, userId, resourceId = null, success = true, req = null) {
        ...
        if (shouldLog) {
            ...
            if (success) {
                log.info('[DATA_ACCESS]', logData);
            } else {
                log.warn('[DATA_ACCESS_FAILED]', logData);
            }
        }
    }
}
