...
const log = require('./logger').logger.child('ROUTING_SERVICE');
...
const getRoutingConfig = () => {
    try {
        ...
        log.error('[ROUTING_SERVICE] Error loading config:', error);
        return {
            apiKey: null,
            enabled: false,
            baseUrl: 'https://api.openrouteservice.org/v2/directions',
...
function cleanupExpiredCache() {
    ...
    if (cleanedCount > 0) {
        log.info(`[ROUTING_SERVICE] Cleaned up ${cleanedCount} expired cache entries.`);
    }
}
...
async function getRoute(startLat, startLng, endLat, endLng, profile = 'driving-car') {
    ...
    if (typeof startLat !== 'number' || typeof startLng !== 'number' || 
        ...
        log.warn('[ROUTING_SERVICE] Invalid coordinates, returning straight line');
    ...
    if (startLat < -90 || startLat > 90 || startLng < -180 || startLng > 180 ||
        ...
        log.warn('[ROUTING_SERVICE] Coordinates out of valid range, returning straight line');
        return [[startLat, startLng], [endLat, endLng]];
    }
    ...
    if (!config.enabled || !config.apiKey) {
        ...
        if (!routeCache.has('_routing_disabled_warning')) {
            log.warn('[ROUTING_SERVICE] Routing disabled atau tidak ada API key. Garis akan menggunakan straight line.');
            log.warn('[ROUTING_SERVICE] Untuk mengaktifkan routing, set OPENROUTE_SERVICE_API_KEY di .env atau enable di config.json');
    ...
    try {
        ...
        log.warn('[ROUTING_SERVICE] Invalid response format, returning straight line');
        return [[startLat, startLng], [endLat, endLng]];
        ...
        if (error.response) {
            ...
            log.error('[ROUTING_SERVICE] API error:', {
            ...
            log.error('[ROUTING_SERVICE] Network error or timeout:', error.message);
        } else {
            // Other error
            log.error('[ROUTING_SERVICE] Error:', error.message);
        }
...
function clearCache() {
    ...
    routeCache.clear();
    log.info(`[ROUTING_SERVICE] Cache cleared. Removed ${count} entries.`);
}
