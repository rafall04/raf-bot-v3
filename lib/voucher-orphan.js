 ...
 */
"use strict";
const log = require('./logger').logger.child('VOUCHER_ORPHAN');
...
function recordVoucherOrphan(entry) {
    try {
        ...
        log.error("[VOUCHER_ORPHAN] Gagal mencatat orphan:", err.message);
    }
}
...
function resolveVoucherOrphan(id, { action, note, resolvedBy, voucherCode } = {}) {
    try {
        ...
        log.error("[VOUCHER_ORPHAN] Gagal menandai resolved:", err.message);
        return null;
    }
}
