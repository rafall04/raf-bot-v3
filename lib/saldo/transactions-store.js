 ...
 */
"use strict";
const log = require('../logger').logger.child('TRANSACTIONS_STORE');
...
function loadTransactions() {
    try {
        ...
        log.error('[SALDO-MANAGER] Error loading transactions (berkas mungkin rusak):', error);
        try {
            if (fs.existsSync(TRANSACTIONS_DB)) {
                const cap = new Date().toISOString().replace(/[:.]/g, '-');
                const karantina = `${TRANSACTIONS_DB}.rusak-${cap}`;
                fs.renameSync(TRANSACTIONS_DB, karantina);
                log.error(`[SALDO-MANAGER] saldo_transactions.json rusak DIKARANTINA ke ${karantina} — jangan hapus sebelum diperiksa.`);
            }
        } catch (errK) {
            log.error('[SALDO-MANAGER] Gagal karantina saldo_transactions.json:', errK.message);
        }
        transactions = [];
    }
}
...
function saveTransactions() {
    try {
        ...
        log.error('[SALDO-MANAGER] Gagal menyimpan saldo_transactions.json:', error);
...
function reloadTransactions() {
    try {
        if (fs.existsSync(TRANSACTIONS_DB)) {
            ...
            log.info(`[SALDO-MANAGER] Reloaded ${transactions.length} transactions from database`);
        }
    } catch (error) {
        log.error('[SALDO-MANAGER] Error reloading transactions:', error);
    }
}
...
function addTransaction(userId, type, amount, description, balance, topupRequestId = null) {
    ...
    log.info(`[TRANSACTION] ${type.toUpperCase()} ${amountFormatted} → ${shortUserId} | Saldo: ${balanceFormatted}`);
