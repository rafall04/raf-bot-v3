 ...
 */
"use strict";
const log = require('../logger').logger.child('TOPUP_STORE');
...
function loadTopupRequests() {
    try {
        ...
        log.error('[SALDO-MANAGER] Error loading topup requests (berkas mungkin rusak):', error);
        try {
            if (fs.existsSync(TOPUP_REQUESTS_DB)) {
                ...
                log.error(`[SALDO-MANAGER] topup_requests.json rusak DIKARANTINA ke ${karantina} — jangan hapus sebelum diperiksa.`);
            }
        } catch (errK) {
            log.error('[SALDO-MANAGER] Gagal karantina topup_requests.json:', errK.message);
        }
        topupRequests = [];
    }
}
...
function saveTopupRequests() {
    try {
        ...
        log.error('[SALDO-MANAGER] Gagal menyimpan topup_requests.json:', error);
...
function reloadTopupRequests() {
    try {
        if (fs.existsSync(TOPUP_REQUESTS_DB)) {
            ...
            log.info(`[SALDO-MANAGER] Reloaded ${topupRequests.length} topup requests from database`);
        }
    } catch (error) {
        log.error('[SALDO-MANAGER] Error reloading topup requests:', error);
    }
}
    ...
    if (agentId) {
        try {
            ...
            if (agent) {
                ...
                log.info('[SALDO-MANAGER] Agent transaction created:', agentTransaction.id);
            }
        } catch (error) {
            log.error('[SALDO-MANAGER] Failed to create agent transaction:', error);
        }
    }
...
async function verifyTopupRequest(requestId, adminId, approved = true, notes = null) {
    log.info('[VERIFY_TOPUP] Starting verification:', { requestId, adminId, approved });

    const requestIndex = topupRequests.findIndex(r => r.id === requestId);
    if (requestIndex === -1) {
        log.info('[VERIFY_TOPUP] Request not found:', requestId);
        return false;
    }

    const request = topupRequests[requestIndex];
    log.info('[VERIFY_TOPUP] Found request:', { id: request.id, status: request.status, amount: request.amount });

    if (request.status !== 'pending') {
        log.info('[VERIFY_TOPUP] Request status not pending:', request.status);
        return false;
    }
    ...
    if (!approved) {
        ...
        request.notes = notes;
        saveTopupRequests();
        log.info('[VERIFY_TOPUP] Request rejected');
        return request;
    }

    // Kredit DULU baru tandai verified: kalau addSaldo gagal/@lid tak ter-resolve,
    // request tetap 'pending' sehingga admin/agen bisa retry — menandai verified
    // sebelum kredit menabrak guard idempoten dan mengunci topup tanpa saldo.
    log.info('[VERIFY_TOPUP] Calling addSaldo with requestId:', requestId);
    try {
        // Lazy-require untuk hindari circular dep dengan balance-operations
        const { addSaldo } = require('./balance-operations');
        const credited = await addSaldo(request.userId, request.amount, `Topup verified - ${request.paymentMethod}`, null, requestId);
        if (!credited) {
            log.error('[VERIFY_TOPUP] addSaldo returned falsy (JID @lid tak ter-resolve?) — request tetap pending:', requestId);
            return false;
        }
        log.info('[VERIFY_TOPUP] addSaldo completed');
    } catch (error) {
        log.error('[VERIFY_TOPUP] Error adding saldo:', error);
        return false;
    }
    ...
    saveTopupRequests();
    log.info('[VERIFY_TOPUP] Request updated in database');
...
async function processAgentConfirmation(agentTransactionId) {
    log.info('[SALDO-MANAGER] Processing agent confirmation:', agentTransactionId);
    ...
    try {
        ...
        if (topupRequest.status !== 'pending') {
            log.warn(`[SALDO-MANAGER] Topup ${topupRequest.id} sudah '${topupRequest.status}' — lewati kredit ulang (idempoten).`);
            const done = agentTransactionManager.completeTransaction(agentTransactionId);
            if (!done) log.warn('[SALDO-MANAGER] Gagal complete agent transaction (topup sudah diproses sebelumnya).');
            return {
                success: true,
        ...
        // Kredit DULU baru tandai verified: menandai 'verified' sebelum addSaldo
        // membuat kegagalan kredit tak bisa di-retry (guard idempoten di atas
        // menolak request non-pending) → topup nyangkut tanpa saldo.
        try {
            ...
            log.error('[SALDO-MANAGER] Error adding saldo:', error);
            return {
                success: false,
                message: 'Failed to add saldo to customer',
                error: error.message
        ...
        log.info('[SALDO-MANAGER] Saldo added to customer:', agentTransaction.customerId);
        ...
        log.info('[SALDO-MANAGER] Topup request verified:', topupRequest.id);

        const completed = agentTransactionManager.completeTransaction(agentTransactionId);

        if (!completed) {
            log.warn('[SALDO-MANAGER] Failed to complete agent transaction, but saldo already added');
        }
        ...
        log.error('[SALDO-MANAGER] Error processing agent confirmation:', error);
        return {
            success: false,
