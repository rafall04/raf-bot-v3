    ...
    if (typeof normalizedUserId === 'string' && normalizedUserId.endsWith('@lid')) {
        try {
            ...
            if (m && m.pnJid && m.pnJid.endsWith('@s.whatsapp.net')) {
                console.warn(`[SALDO-MANAGER] @lid ter-resolve ke ${m.pnJid} via peta identitas (defense-in-depth).`);
                normalizedUserId = m.pnJid;
            }
        } catch (e) {
            console.warn('[SALDO-MANAGER] resolve @lid gagal (biarkan hilir fail-closed):', e && e.message);
        }
    }
