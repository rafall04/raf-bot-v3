 ...
 */
"use strict";
const log = require('../logger').logger.child('SHARED');
...
function initSaldoDatabase() {
    return new Promise((resolve, reject) => {
        ...
        saldoDb = new sqlite3.Database(SALDO_DB_PATH, (err) => {
            if (err) {
                log.error('[SALDO-MANAGER] Error opening saldo database:', err);
                log.error('[SALDO-MANAGER] Database path:', SALDO_DB_PATH);
                return reject(err);
            }
            ...
            applySqlitePragmas(saldoDb, { busyTimeout: 5000 }).catch((pragmaErr) => {
                log.warn(`[SALDO_DB_PRAGMA_WARN] ${pragmaErr.message}`);
            });
            ...
            `, (err) => {
                if (err) {
                    log.error('[SALDO-MANAGER] Error creating user_saldo table:', err);
                    log.error('[SALDO-MANAGER] Error details:', err.message, err.stack);
                    return reject(err);
                }
                ...
                ensureSaldoCheckConstraint(saldoDb, SALDO_DB_PATH).then(() => {
                    ...
                    saldoDb.run("CREATE INDEX IF NOT EXISTS idx_user_saldo_user_id ON user_saldo(user_id)", (err) => {
                        if (err) {
                            log.warn('[SALDO] Index warning:', err.message);
                        }
...
function ensureSaldoCheckConstraint(db, dbPath) {
    ...
    return (async () => {
        ...
        try {
            ...
            log.warn('[SALDO_MIGRATION] Gagal baca skema user_saldo (skip migrasi):', e.message);
            return;
        }
        ...
        log.info('[SALDO_MIGRATION] user_saldo belum punya CHECK(saldo>=0); menjalankan migrasi rebuild...');
        ...
        if (dbPath) {
            ...
            try {
                ...
                log.info(`[SALDO_MIGRATION] Backup dibuat: ${backupPath}`);
            } catch (e) {
                log.warn('[SALDO_MIGRATION] Backup VACUUM INTO gagal (lanjut — migrasi tetap atomik via transaksi):', e.message);
            }
        }
        ...
        try {
            ...
            if (negRows && negRows.length) {
                log.warn(`[SALDO_MIGRATION] ${negRows.length} baris saldo NEGATIF (invalid) di-clamp ke 0 saat migrasi:`, negRows);
            }
            ...
            log.info('[SALDO_MIGRATION] ✅ CHECK(saldo>=0) berhasil diterapkan ke user_saldo.');
        } catch (e) {
            log.error('[SALDO_MIGRATION] Migrasi GAGAL — rollback, tabel tidak berubah:', e.message);
...
function normalizeUserJid(userId) {
    ...
    // DEFENSE-IN-DEPTH @lid: dulu @lid LOLOS apa adanya di sini (fn ini sync, resolusi @lid kanonik
    // ada di normalizeJidForSaldo yang ASYNC di caller). Kalau caller lupa menormalkan, @lid nyampai
    // ke sini lalu ditolak fail-closed di addSaldo/deductSaldo/transfer — aman tapi user dgn @lid
    // ter-peta pun ikut tertolak. Kini coba resolve SINKRON via peta tersimpan (lid-mappings +
    // global.users). Ter-resolve → JID kanonik (baca/tulis benar). TIDAK ter-resolve → biarkan @lid
    // (guard fail-closed hilir tetap menolak → uang tak pernah nyasar ke key palsu).
    if (typeof normalizedUserId === 'string' && normalizedUserId.endsWith('@lid')) {
        try {
            const { getStoredMappingByLid } = require('../jid-utils');
            const m = getStoredMappingByLid(normalizedUserId);
            if (m && m.pnJid && m.pnJid.endsWith('@s.whatsapp.net')) {
                log.warn(`[SALDO-MANAGER] @lid ter-resolve ke ${m.pnJid} via peta identitas (defense-in-depth).`);
                normalizedUserId = m.pnJid;
            }
        } catch (e) {
            log.warn('[SALDO-MANAGER] resolve @lid gagal (biarkan hilir fail-closed):', e && e.message);
        }
    }
