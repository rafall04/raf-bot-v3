 ...
 */
"use strict";
const log = require('../logger').logger.child('TRANSFER_OPERATIONS');
...
// H3: wrapper publik yang men-serialize transfer lewat withSaldoWriteLock (lihat shared.js).
// Transfer = dual-update (debit pengirim + credit penerima) dalam satu BEGIN/COMMIT. Lock global
// menjamin transaksi ini berjalan eksklusif di koneksi singleton — tidak ada add/deduct/transfer
// lain yang BEGIN bersamaan, dan tidak ada statement single yang tersapu ke transaksi ini.
function transferSaldo(...args) {
    return withSaldoWriteLock(() => transferSaldoImpl(...args));
}

async function transferSaldoImpl(fromUserId, toUserId, amount, description = 'Transfer saldo') {
    return new Promise(async (resolve, _reject) => {
        try {
            // PENTING: Validasi amount - jangan izinkan amount 0 atau undefined
            if (!amount || amount === 0 || isNaN(amount) || parseInt(amount) <= 0) {
                log.error('[SALDO-MANAGER] Invalid amount for transferSaldo:', amount);
            ...
            if ((normalizedFromUserId && normalizedFromUserId.endsWith('@lid')) ||
                ...
                log.warn(`[SALDO-MANAGER] transferSaldo called with @lid format: from=${fromUserId}, to=${toUserId}. Normalize JID before calling this function.`);
                return resolve(false);
            }
            ...
            if (!saldoDb) {
                log.error('[SALDO-MANAGER] Database not initialized');
                return resolve(false);
            }
                ...
                saldoDb.run("BEGIN TRANSACTION", (beginErr) => {
                    ...
                    saldoDb.get("SELECT saldo FROM user_saldo WHERE user_id = ?", [normalizedFromUserId], (err, senderRow) => {
                        if (err) {
                            ...
                            log.error('[SALDO-MANAGER] Error checking sender saldo:', {
                                fromUserId: normalizedFromUserId,
                                toUserId: normalizedToUserId,
                                amount,
                                errorCode: err.code,
                        ...
                        if (!senderRow || senderRow.saldo < amount) {
                            saldoDb.run("ROLLBACK");
                            log.warn(`[SALDO-MANAGER] Insufficient saldo for transfer: sender=${normalizedFromUserId}, current=${senderRow?.saldo || 0}, requested=${amount}`);
                            return resolve(false);
                        }
                        ...
                        if (senderNewSaldo < 0) {
                            saldoDb.run("ROLLBACK");
                            log.error(`[SALDO-MANAGER] Validation failed: senderNewSaldo would be negative (${senderNewSaldo}) for user ${normalizedFromUserId}`);
                            return resolve(false);
                        }
                        ...
                        saldoDb.run(
                            ...
                            function (updateErr) {
                                if (updateErr) {
                                    ...
                                    log.error('[SALDO-MANAGER] Error updating sender saldo:', {
                                        fromUserId: normalizedFromUserId,
                                        toUserId: normalizedToUserId,
                                        amount,
                                        senderNewSaldo,
                                    ...
                                    () => {
                                        ...
                                        saldoDb.get("SELECT saldo FROM user_saldo WHERE user_id = ?", [normalizedToUserId], (err, recipientRow) => {
                                            if (err) {
                                                ...
                                                log.error('[SALDO-MANAGER] Error checking recipient saldo:', {
                                                    fromUserId: normalizedFromUserId,
                                                    toUserId: normalizedToUserId,
                                                    amount,
                                                    errorCode: err.code,
                                            ...
                                            if (recipientNewSaldo > Number.MAX_SAFE_INTEGER) {
                                                saldoDb.run("ROLLBACK");
                                                log.error(`[SALDO-MANAGER] Validation failed: recipientNewSaldo would exceed MAX_SAFE_INTEGER for user ${normalizedToUserId}`);
                                                return resolve(false);
                                            }
                                            ...
                                            saldoDb.run(
                                                ...
                                                function (updateErr2) {
                                                    if (updateErr2) {
                                                        ...
                                                        log.error('[SALDO-MANAGER] Error updating recipient saldo:', {
                                                            fromUserId: normalizedFromUserId,
                                                            toUserId: normalizedToUserId,
                                                            amount,
                                                            recipientNewSaldo,
                                                    ...
                                                    saldoDb.run("COMMIT", (commitErr) => {
                                                        if (commitErr) {
                                                            log.error('[SALDO-MANAGER] Error committing transaction:', commitErr);
                                                            return resolve(false);
                                                        }
                                                        ...
                                                        try {
                                                            ...
                                                            log.error('[SALDO-MANAGER] Error recording transactions (saldo already updated):', transactionErr);
                                                            log.warn(`[SALDO-MANAGER] WARNING: Transfer completed but transaction history not recorded: ${normalizedFromUserId} -> ${normalizedToUserId}, amount ${amount}`);
                                                        }
            ...
            log.error('[SALDO-MANAGER] Error transferring saldo:', error);
            resolve(false);
        }
    });
}
