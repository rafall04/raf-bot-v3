 ...
 */
"use strict";
const log = require('../logger').logger.child('BALANCE_OPERATIONS');


const { initSaldoDatabase, getSaldoDb, normalizeUserJid, withSaldoWriteLock } = require('./shared');
...
// PERINGATAN: fungsi ini MENGEMBALIKAN PROMISE meski tak ditandai `async`. Komentar lamanya
// berbunyi "synchronous untuk backward compatibility" — itu KELIRU dan sempat menyesatkan:
// `lib/saldo/topup-store.js` memakainya tanpa `await`, sehingga pesan "TOPUP BERHASIL" ke
// pelanggan berbunyi "Saldo baru: Rp [object Promise]". PEMANGGIL WAJIB `await`.
// Memakai singleton saldoDb untuk mencegah koneksi ganda & SQLITE_BUSY.
function getUserSaldo(userId) {
    try {
        ...
        return initSaldoDatabase().then(() => {
            ...
            log.error('[SALDO-MANAGER] Error getting user saldo:', err);
            return 0;
        });
    } catch (error) {
        log.error('[SALDO-MANAGER] Error getting user saldo:', error);
...
async function getUserSaldoData(userId) {
    try {
        ...
        log.error('[SALDO-MANAGER] Error getting user saldo data:', error);
        return null;
    }
}
    ...
    try {
        ...
        return new Promise((resolve) => {
            ...
            saldoDb.run(
                ...
                function (err) {
                    if (err) {
                        log.error('[SALDO-MANAGER] Error creating user saldo:', err);
                        resolve(false);
                    } else {
        ...
        log.error('[SALDO-MANAGER] Error creating user saldo:', error);
        return false;
    }
}

// H3: wrapper publik yang men-serialize addSaldo lewat withSaldoWriteLock (lihat shared.js).
// Implementasi DB tetap di addSaldoImpl agar logic teruji tidak berubah; lock hanya menjamin
// tidak ada mutasi saldo lain berjalan bersamaan di koneksi singleton.
function addSaldo(...args) {
    return withSaldoWriteLock(() => addSaldoImpl(...args));
}

function addSaldoImpl(userId, amount, description = 'Topup saldo', pushname = null, topupRequestId = null) {
    ...
    if (!amount || amount === 0 || isNaN(amount) || parseInt(amount) <= 0) {
        ...
        log.error('[SALDO-MANAGER]', error.message);
        return Promise.reject(error);
    }

    log.info(`[SALDO-MANAGER] Adding saldo: userId=${userId}, amount=${amount}, description=${description}, topupRequestId=${topupRequestId}`);
    ...
    return new Promise(async (resolve, reject) => {
        ...
        if (typeof userId === 'string' && userId.endsWith('@lid')) {
            log.error('[SALDO-MANAGER] TOLAK addSaldo: userId masih @lid (belum kanonik), saldo TIDAK ditambahkan', { userId, amount });
        ...
        initSaldoDatabase().then(() => {
            ...
            saldoDb.serialize(() => {
                ...
                saldoDb.run("BEGIN TRANSACTION", (beginErr) => {
                    if (beginErr) {
                        ...
                        log.error('[SALDO-MANAGER] Error beginning transaction:', {
                            userId,
                            amount,
                    ...
                    saldoDb.get("SELECT saldo FROM user_saldo WHERE user_id = ?", [userId], (err, row) => {
                        if (err) {
                            ...
                            log.error('[SALDO-MANAGER] Error checking user:', {
                                userId,
                                errorCode: err.code,
                            ...
                            saldoDb.run(
                                ...
                                function (insertErr) {
                                    if (insertErr) {
                                        ...
                                        log.error('[SALDO-MANAGER] Error creating user:', {
                                            userId,
                                            amount,
                                    ...
                                    if (newSaldo > Number.MAX_SAFE_INTEGER) {
                                        saldoDb.run("ROLLBACK", () => { });
                                        log.error(`[SALDO-MANAGER] Validation failed: newSaldo would exceed MAX_SAFE_INTEGER for user ${userId}`);
                                    ...
                                    saldoDb.run("COMMIT", (commitErr) => {
                                        if (commitErr) {
                                            ...
                                            log.error('[SALDO-MANAGER] Error committing transaction:', {
                                                userId,
                                                amount,
                                                newSaldo,
                                        ...
                                        log.info(`[SALDO-MANAGER] Saldo updated: 0 -> ${newSaldo}`);
                                        ...
                                        try {
                                            ...
                                            log.error('[SALDO-MANAGER] Error recording transaction (saldo already updated):', transactionErr);
                                            log.warn(`[SALDO-MANAGER] WARNING: Saldo updated but transaction history not recorded for user ${userId}, amount ${amount}`);
                                        }
                            ...
                            saldoDb.run(
                                ...
                                function (updateErr) {
                                    if (updateErr) {
                                        ...
                                        log.error('[SALDO-MANAGER] Error updating saldo:', {
                                            userId,
                                            oldSaldo,
                                            newSaldo,
                                            amount,
                                    ...
                                    saldoDb.run("COMMIT", (commitErr) => {
                                        ...
                                        log.info(`[SALDO-MANAGER] Saldo updated: ${oldSaldo} -> ${newSaldo}`);
            ...
            log.error('[SALDO-MANAGER] Error initializing database:', error);
            reject(error);
        });
    });
}

// H3: wrapper publik yang men-serialize deductSaldo lewat withSaldoWriteLock (lihat shared.js).
// Serialisasi global memastikan read-modify-write (SELECT saldo → cek → UPDATE) tidak pernah
// interleave dengan mutasi saldo lain → tidak ada double-spend, tidak ada BEGIN bersarang.
function deductSaldo(...args) {
    return withSaldoWriteLock(() => deductSaldoImpl(...args));
}

async function deductSaldoImpl(userId, amount, description = 'Pembelian') {
    try {
        // PENTING: Validasi amount - jangan izinkan amount 0 atau undefined
        if (!amount || amount === 0 || isNaN(amount) || parseInt(amount) <= 0) {
            log.error('[SALDO-MANAGER] Invalid amount for deductSaldo:', amount);
            return false;
        }
        ...
        if (typeof normalizedUserId === 'string' && normalizedUserId.endsWith('@lid')) {
            log.error('[SALDO-MANAGER] TOLAK deductSaldo: userId masih @lid (belum kanonik)', { userId, amount });
            return false;
        }
        ...
        return new Promise((resolve) => {
            saldoDb.serialize(() => {
                saldoDb.run("BEGIN TRANSACTION", (beginErr) => {
                    if (beginErr) {
                        log.error('[SALDO-MANAGER] Error beginning transaction:', beginErr);
                        return resolve(false);
                    }
                    ...
                    saldoDb.get("SELECT saldo FROM user_saldo WHERE user_id = ?", [normalizedUserId], (err, row) => {
                        if (err) {
                            ...
                            log.error('[SALDO-MANAGER] Error checking saldo:', {
                        ...
                        if (!row || row.saldo < amount) {
                            saldoDb.run("ROLLBACK", () => { });
                            log.warn(`[SALDO-MANAGER] Insufficient saldo for user ${normalizedUserId}: current=${row?.saldo || 0}, requested=${amount}`);
                            return resolve(false);
                        }
                        ...
                        if (newSaldo < 0) {
                            saldoDb.run("ROLLBACK", () => { });
                            log.error(`[SALDO-MANAGER] Validation failed: newSaldo would be negative (${newSaldo}) for user ${normalizedUserId}`);
                            return resolve(false);
                        }
                            ...
                            function (updateErr) {
                                ...
                                saldoDb.run("COMMIT", (commitErr) => {
                                    ...
                                    try {
                                        ...
                                        // Log error tapi jangan reject - saldo sudah valid
                                        log.error('[SALDO-MANAGER] Error recording transaction (saldo already updated):', transactionErr);
                                        log.warn(`[SALDO-MANAGER] WARNING: Saldo updated but transaction history not recorded for user ${normalizedUserId}, amount ${amount}`);
                                    }
        ...
        log.error('[SALDO-MANAGER] Error deducting saldo:', error);
        return false;
    }
}
    ...
    try {
        ...
        return new Promise((resolve) => {
            ...
            saldoDb.run(
                ...
                function (err) {
                    if (err) {
                        log.error('[SALDO-MANAGER] Error updating pushname:', err);
                        resolve(false);
                    } else {
                        resolve(this.changes > 0);
                    }
        ...
        log.error('[SALDO-MANAGER] Error updating pushname:', error);
        return false;
    }
}
...
async function getAllSaldoData() {
    try {
        ...
        if (error.code === 'SQLITE_ERROR' && error.message.includes('no such table')) {
            log.warn('[SALDO-MANAGER] Table belum ada, returning empty array');
            return [];
        }
        log.error('[SALDO-MANAGER] Error getting all saldo data:', error.message);
        return [];
    }
}
