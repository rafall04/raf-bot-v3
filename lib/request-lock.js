...
const log = require('./logger').logger.child('REQUEST_LOCK');
const locks = new Map();
...
async function acquireLock(resourceId, timeout = 5000) {
    ...
    while (locks.has(resourceId)) {
        if (Date.now() - startTime > timeout) {
            log.info(`[LOCK_TIMEOUT] Failed to acquire lock for ${resourceId} after ${timeout}ms`);
            return false;
        }
    ...
    log.info(`[LOCK_ACQUIRED] Lock acquired for ${resourceId}`);
    return true;
}
...
function releaseLock(resourceId) {
    if (locks.has(resourceId)) {
        ...
        log.info(`[LOCK_RELEASED] Lock released for ${resourceId}`);
    }
}
...
function cleanupStaleLocks() {
    ...
    for (const [resourceId, lockInfo] of locks.entries()) {
        if (now - lockInfo.acquiredAt > maxAge) {
            log.info(`[LOCK_CLEANUP] Removing stale lock for ${resourceId}`);
            locks.delete(resourceId);
        }
    }
}
