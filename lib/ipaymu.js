 ...
 * - payDirect(props): transaksi direct channel apa pun (qris/va/cstore), balikan ternormalisasi.
 * - payHosted(props): buat SESI hosted (endpoint /payment) → return Url untuk redirect buyer ke
 *   halaman iPaymu (iPaymu yang menyajikan pilih channel). TransactionId via callback (trx_id).
 * - getPaymentChannels(): daftar channel aktif (GET /payment-channels).
...
/**
 * Buat SESI pembayaran HOSTED (redirect) — iPaymu yang menyajikan halaman pilih channel +
 * pembayaran (bukan halaman sendiri). Endpoint `/payment` (beda dari `/payment/direct`):
 * kirim product/qty/price + returnUrl/cancelUrl/notifyUrl, balikan `Url` untuk redirect buyer.
 * TransactionId BELUM ada di sini (baru muncul saat buyer bayar) → datang via callback (`trx_id`).
 * Dipakai mode bayar-tagihan hosted (mis. instance yang pilih halaman iPaymu, bukan portal HTML sendiri).
 *
 * @returns {Promise<{url, sessionId, reffId, sandbox, gateway}>}
 */
async function payHosted(props) {
    const cfg = getConfig();
    let { amount, comment, reffId, name, phone, email, sandbox, returnUrl, cancelUrl } = props || {};
    amount = parseInt(amount, 10);
    sandbox = sandbox === true;

    if (isNaN(amount) || !comment || !reffId || !name || !phone || !email) {
        throw new Error("payHosted: amount, comment, reffId, name, phone, email wajib diisi.");
    }
    if (!cfg.ipaymuCallback) {
        throw new Error("Callback iPaymu belum di-set.");
    }
    ...
    const body = {
        ...
        price: [amount],
        description: [comment],
        returnUrl: returnUrl || cfg.ipaymuCallback,
        cancelUrl: cancelUrl || returnUrl || cfg.ipaymuCallback,
        notifyUrl: cfg.ipaymuCallback,
        referenceId: reffId,
        buyerName: name,
        buyerPhone: phone,
    ...
    const res = await ipaymuRequest("/payment", body, { sandbox });
    if (!res || res.Success !== true) {
        throw new Error(res?.Message || "Sesi pembayaran iPaymu gagal dibuat.");
    }
    const d = res.Data || {};
    const url = d.Url || d.url;
    if (!url) {
        throw new Error("iPaymu tidak mengembalikan URL pembayaran.");
    }
    return {
        url,
        sessionId: d.SessionID || d.SessionId || null,
        reffId,
        sandbox,
        gateway: "ipaymu",
    };
...
module.exports.payHosted = payHosted;
