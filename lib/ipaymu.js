...
async function getPaymentChannels(options = {}) {
    const res = await ipaymuRequest("/payment-channels", {}, { method: "GET", ...options });
    if (!res || res.Success !== true) {
...
async function payDirect(props) {
    const cfg = getConfig();
    let { amount, comment, reffId, name, phone, email, paymentMethod, paymentChannel } = props || {};
    amount = parseInt(amount, 10);
    ...
    const res = await ipaymuRequest("/payment/direct", body);
    if (!res || res.Success !== true) {
