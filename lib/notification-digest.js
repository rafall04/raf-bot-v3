 ...
 */
"use strict";
const log = require('./logger').logger.child('NOTIFICATION_DIGEST');
...
function loadBuckets(storePath = resolveStorePath()) {
    try {
        ...
        log.warn(`[NOTIF_DIGEST] Gagal membaca store: ${err.message}`);
        return {};
    }
}
...
function saveBuckets(buckets, storePath = resolveStorePath()) {
    try {
        ...
        log.error(`[NOTIF_DIGEST] Gagal menulis store: ${err.message}`);
        return false;
    }
}
...
async function deliver(recipient, text) {
    try {
        ...
        log.error(`[NOTIF_DIGEST] Gagal kirim ke ${recipient}: ${err.message}`);
        return { ok: false };
    }
}
...
async function enqueueOrSendFirst({ recipient, kind, detailText, summaryItem, windowMs = DEFAULT_WINDOW_MS, now = Date.now() }) {
    if (!recipient || String(recipient).endsWith("@lid")) {
        ...
        log.error(`[NOTIF_DIGEST] Penerima tidak sah (kosong/@lid): ${recipient}`);
        return { sent: "none" };
    }
...
async function tickDigests(now = Date.now()) {
    ...
    for (const [key, bucket] of due) {
        try {
            ...
            log.error(`[NOTIF_DIGEST] Flush ${key} gagal: ${err.message}`);
        } finally {
            delete buckets[key];
        }
    }
...
function startDigestScheduler({ intervalMs = TICK_INTERVAL_MS } = {}) {
    ...
    tickTimer = setInterval(() => {
        tickDigests().catch((err) => log.error(`[NOTIF_DIGEST] Tick gagal: ${err.message}`));
    }, intervalMs);
    if (typeof tickTimer.unref === "function") tickTimer.unref();
    log.info(`[NOTIF_DIGEST] Scheduler start (tick ${Math.round(intervalMs / 1000)}s)`);
    return tickTimer;
}
