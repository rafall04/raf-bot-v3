...
'use strict';
const log = require('./logger').logger.child('OLT_PROVISION_STORE');
...
function readStore() {
    try {
        ...
        log.error('[OLT-PROVISION-STORE] Gagal baca store, pakai default:', e.message);
    }
...
function ensureSeeded() {
    if (!fs.existsSync(STORE_FILE)) {
        ...
        log.info('[OLT-PROVISION-STORE] Seed profil tipe modem default →', STORE_FILE);
    }
}
