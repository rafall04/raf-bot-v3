 ...
 */
"use strict";
const log = require('./logger').logger.child('HTTP_SOCKET_BOOTSTRAP');
    ...
    function cleanupOldPendingRequests() {
        try {
            ...
            if (cleanedCount > 0) {
                ...
                log.info(`[CLEANUP] Total ${cleanedCount} pending requests dibersihkan.`);
            } else {
                log.info("[CLEANUP] Tidak ada pending requests yang perlu dibersihkan.");
            }
        } catch (error) {
            log.error("[CLEANUP_ERROR] Error cleaning up old requests:", error);
        }
    }
    ...
    function startHttpServer(connect) {
        server.listen(port, async () => {
            ...
            if (require("fs").existsSync(sessionPath)) {
                ...
                log.info("[WA] No session found - scan QR code to connect");
