 ...
 */
"use strict";
const log = require('./logger').logger.child('JSON_COLLECTIONS_LOADER');
...
function loadCollectionIntoGlobal(filePath, globalKey, fallbackValue = []) {
    try {
        ...
        log.info(`[DB_INIT] File ${path.relative(path.join(__dirname, ".."), filePath)} dibuat.`);
        return global[globalKey];
    } catch (error) {
        log.error(`[DB_LOAD_ERROR] Gagal memuat atau membuat ${path.relative(path.join(__dirname, ".."), filePath)}:`, error);
...
function setupAnnouncementsAndNewsWatchers() {
    ...
    function reloadFileWithRetry(fileName, globalKey, retries = 3, delay = 100) {
        setTimeout(() => {
            try {
                ...
                log.info(`[Database] Reloaded ${Array.isArray(data) ? data.length : 0} ${globalKey}.`);
            } catch (error) {
                if (retries > 0) {
                    log.warn(`[Database] Error reloading ${globalKey}.json (retry ${4 - retries}/3):`, error.message);
                    reloadFileWithRetry(fileName, globalKey, retries - 1, delay * 2);
                } else {
                    log.error(`[Database] Error reloading ${globalKey}.json after 3 retries:`, error.message);
                }
            }
        }, delay);
    }
    ...
    watchedFiles.forEach(({ filePath, globalKey }) => {
        ...
        fs.watchFile(filePath, { interval: 1000 }, (curr, prev) => {
            ...
            if (currMtime !== prevMtime && currMtime !== lastMtime) {
                lastMtime = currMtime;
                log.info(`[Database] ${path.basename(filePath)} changed`);
    ...
    if (watcherCount > 0) {
        log.info(`[Database] ${watcherCount} file watcher(s) aktif`);
    }
}
