...
const CONTEXT_TOKENS = new Set([
    "wifi", "wifine", "wifinya", "wifiku", "wify", "wifii", "wifian", "wivi",
    "internet", "internetnya", "internete", "internetku", "inet", "inetnya", "internetan",
    "jaringan", "jaringannya", "jaringane", "jaringanku",
    "koneksi", "koneksine", "koneksinya", "konek", "koneknya", "connect", "conect", "koneksian",
...
// Termasuk dialek Jawa/informal yang TIDAK bertabrakan dengan nama orang/kata umum
// (mis. "medhot"=putus, "mandek"=macet, "alon"=pelan). SENGAJA tak memasukkan "angel" (nama
// "Angel") atau "lot" (English "a lot") untuk menghindari salah-tangkap.
const SYMPTOM_TOKENS = new Set([
    "lemot", "lemotnya", "lemod", "lambat", "lambet", "lelet", "leled", "lola", "pelan", "alon",
    "lag", "lagg", "ngelag", "ngadat", "buffering", "ngebuffer", "ngebufer", "bufring", "buffring",
    "loding", "loading", "louding", "lowding", "muter", "muser", "mubeng", "rto",
    "mati", "putus", "putusnya", "medhot", "medot", "pedhot", "pedot", "kepedot",
    "mandek", "mandeg", "mandheg", "nyendat", "macet",
    "eror", "error", "eroran", "erorr", "err", "ngeror", "gangguan", "ganguan", "trouble",
    "hilang", "ilang", "lost", "los", "merah",
    "down", "drop", "ngedrop", "patah"
]);
...
const SYMPTOM_PHRASES = [
    ...
    "muter muter", "muser muser", "putus putus", "patah patah",
    "ndak ada", "ndk ada", "gk ada", "kg ada", "ra ono", "ora ono",
    // Dialek Jawa & frasa dua-kata (aman dari tabrakan karena butuh ≥2 kata)
    "ora mlaku", "ra mlaku", "ora mlebu", "ra mlebu", "ora konek", "ra konek", "ora iso mlebu",
    "gak kenek", "ga kenek", "ora kenek", "ra kenek", "gk kenek", "ra nyambung", "ora nyambung",
    "ping tinggi", "ping gede", "ping naik", "ping gedhe",
    "gada sinyal", "ga ada sinyal", "gak ada sinyal", "sinyal ilang", "sinyal hilang", "ora ono sinyal",
    "sinyal lemah", "sinyale lemah", "sinyal jelek", "sinyale jelek", "sinyal kurang", "sinyal naik turun", "sinyale mudun",
    "susah konek", "angel konek", "angel mlebu", "lampu merah", "lampune merah", "lampu abang", "lampune abang"
];

// Frasa yang HANYA sah sebagai keluhan koneksi bila DIGABUNG kata konteks jaringan (dipakai di
// Priority 3, TIDAK di fallback tanpa-konteks). Menangkap "internet nya belum ada", "wifi durung
// ono" tanpa membuat "uang belum ada" salah tangkap. Sengaja bukan token tunggal.
const CONTEXT_GATED_PHRASES = [
    "belum ada", "belum konek", "belum nyala", "belum masuk", "belum jalan", "belum bisa",
    "durung ono", "durung ana", "durung metu", "durung iso", "ora metu"
];
...
// Tanya-bermasalah: interogatif yang, DIGABUNG kata konteks jaringan, menyiratkan ada yang tidak
// beres ("Wifi-nya kenapa lagi ini mas", "internet kok gini", "jaringane piye iki"). Sengaja
// TIDAK memasukkan "berapa"/"kapan"/"bisa" supaya pertanyaan tagihan/info ("Wifi bulan Juli ya
// kak", "wifi berapa bulan") tidak salah tangkap sebagai keluhan koneksi.
const TROUBLE_QUESTION_TOKENS = new Set([
    "kenapa", "knp", "knpa", "kenopo", "kenap", "napa", "ngapa", "ngopo", "kok",
    "gimana", "gmn", "gimna", "piye", "pie", "kepiye", "keno", "kena", "error"
]);
const TROUBLE_QUESTION_PHRASES = [
    "ada apa", "ono opo", "kenapa ya", "kok bisa", "kok gini", "kok gitu", "ini kenapa", "napa ini"
];
...
function getLooseIntentFromKeywords(message) {
    ...
    // Prioritas 2.7 — pelanggan MELAPOR sudah mencabut/merestart modemnya sendiri
    // ("Ini baru dicoba cabut", "sudah berkali kali mas masih tetap sama"). Tanpa aturan ini
    // kalimatnya tak punya kata konteks jaringan → intent undefined → bot DIAM, padahal
    // pelanggan sedang menunggu kabar. Diarahkan ke CEK_KONEKSI supaya bot memverifikasi
    // pemulihan dan menindaklanjuti (lihat lib/reboot-followup-service).
    try {
        const { saysAlreadyRestarted } = require("./affirmative-parser");
        if (saysAlreadyRestarted(normalized)) {
            return { intent: "CEK_KONEKSI", matchedKeyword: "loose:restart-sendiri", loose: true };
        }
    } catch (_e) { /* parser opsional — jangan jatuhkan matcher */ }
    ...
    if (hasContext) {
        ...
        // Tanya-bermasalah + konteks ("Wifi-nya kenapa lagi ini mas") = keluhan implisit.
        const troubleToken = tokens.find((token) => TROUBLE_QUESTION_TOKENS.has(token)) || null;
        const troublePhrase = TROUBLE_QUESTION_PHRASES.find((p) => ` ${normalized} `.includes(` ${p} `)) || null;
        // "belum ada/konek/nyala" dsb — hanya keluhan bila ADA konteks (aman dari "uang belum ada").
        const gatedPhrase = CONTEXT_GATED_PHRASES.find((p) => ` ${normalized} `.includes(` ${p} `)) || null;
        const matched = symptomToken || symptomPhrase || checkToken || troubleToken || troublePhrase || gatedPhrase;
        if (matched) {
