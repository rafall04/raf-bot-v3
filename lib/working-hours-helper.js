...
/**
 * Ambil "YYYY-MM-DD" kalender Jakarta dari Date wall-shifted (`jakartaTime`): field lokalnya
 * memang memuat jam/dinding Jakarta (lihat konstruksi `toLocaleString(..., Asia/Jakarta)`),
 * jadi BACA field lokal — `toISOString()` mengembalikan tanggal UTC yang sehari lebih awal
 * pada pagi WIB (00:00–06:59).
 */
function jakartaWallDateStr(dateObj) {
...
function isWithinWorkingHours() {
    ...
    const todayStr = jakartaWallDateStr(jakartaTime); // YYYY-MM-DD kalender Jakarta
...
function getNextWorkingDay(fromDate) {
    ...
    for (let i = 0; i < 7; i++) {
        ...
        const dateStr = jakartaWallDateStr(nextDay);
...
/**
 * Jadwal (start/end) untuk satu tanggal — mendukung struktur config baru (days.<hari>)
 * maupun lama (weekdays/saturday/sunday). Memulangkan null bila tak bisa dipastikan.
 */
function jadwalHari(tanggal) {
    try {
        const config = global.config.teknisiWorkingHours;
        if (!config) return null;
        const namaHari = ['sunday', 'monday', 'tuesday', 'wednesday', 'thursday', 'friday', 'saturday'][tanggal.getDay()];
        let jadwal = null;
        if (config.days && config.days[namaHari]) jadwal = config.days[namaHari];
        else if (tanggal.getDay() === 0) jadwal = config.sunday;
        else if (tanggal.getDay() === 6) jadwal = config.saturday;
        else jadwal = config.weekdays;
        if (!jadwal || !jadwal.start || !jadwal.end) return null;
        return { start: String(jadwal.start), end: String(jadwal.end) };
    } catch (_e) {
        return null;
    }
}
/**
 * Frasa "kapan penanganan lapangan dimulai" untuk teks PELANGGAN.
 *
 * Satu-satunya pemilik frasa ini. Sebelumnya `smart-report-text-menu` merakit sendiri
 * nama hari + jam, dan alur LOS otomatis tidak merakit apa pun — itulah sebabnya kabel
 * putus jam 2 pagi tetap dijanjikan "sedang ditangani" (jam kerja teknisi 08:00-17:00).
 *
 * Ketika fitur jam kerja dimatikan, isWithinWorkingHours() memulangkan isWithinHours:true
 * (layanan 24/7) sehingga fungsi ini memulangkan teks null — pemanggil otomatis diam.
 *
 * @returns {{dalamJamKerja: boolean, teks: string|null}} teks mis. "besok pukul 08:00 WIB"
 */
function waktuMulaiKerjaBerikutnya() {
    try {
        // Rentang, bukan jam mulai. Menyebut "pukul 08:00" membuat pelanggan menunggu di jam
        // itu persis, lalu kecewa saat teknisi mengerjakan gangguan yang lebih parah dulu.
        // Rentang jam kerja tidak menjanjikan apa pun yang tak bisa ditepati.
        const susun = (hari, jamMulai, tanggal) => {
            const jadwal = jadwalHari(tanggal);
            const rentang = jadwal
                ? `${hari} pada jam kerja (${jadwal.start}–${jadwal.end} WIB)`
                : `${hari} pada jam kerja`;   // jam selesai tak terbaca -> JANGAN mengarang
            return { dalamJamKerja: false, teks: `${hari} pukul ${jamMulai}`, teksRentang: rentang };
        };
        const status = isWithinWorkingHours();
        if (!status || status.isWithinHours) return { dalamJamKerja: true, teks: null, teksRentang: null };
        const next = status.nextWorkingTime;
        if (!next) return { dalamJamKerja: false, teks: null, teksRentang: null };

        const jam = `${String(next.getHours()).padStart(2, '0')}:${String(next.getMinutes()).padStart(2, '0')} WIB`;
        const hariIni = new Date();
        const bedaHari = Math.round(
            (new Date(next.getFullYear(), next.getMonth(), next.getDate()) -
                new Date(hariIni.getFullYear(), hariIni.getMonth(), hariIni.getDate())) / 86400000
        );
        if (bedaHari <= 0) return susun('hari ini', jam, next);
        if (bedaHari === 1) return susun('besok', jam, next);
        const namaHari = ['Minggu', 'Senin', 'Selasa', 'Rabu', 'Kamis', 'Jumat', 'Sabtu'];
        return susun(namaHari[next.getDay()], jam, next);
    } catch (_e) {
        // Teks pelanggan tak boleh gagal gara-gara jam kerja. Diam lebih baik daripada salah.
        return { dalamJamKerja: true, teks: null };
    }
}
