...
function waktuMulaiKerjaBerikutnya() {
    try {
        ...
        if (!status || status.isWithinHours) return { dalamJamKerja: true, teks: null };
        const next = status.nextWorkingTime;
        if (!next) return { dalamJamKerja: false, teks: null };
        ...
        if (bedaHari <= 0) return { dalamJamKerja: false, teks: `hari ini pukul ${jam}` };
        if (bedaHari === 1) return { dalamJamKerja: false, teks: `besok pukul ${jam}` };
        const namaHari = ['Minggu', 'Senin', 'Selasa', 'Rabu', 'Kamis', 'Jumat', 'Sabtu'];
        return { dalamJamKerja: false, teks: `${namaHari[next.getDay()]} pukul ${jam}` };
    } catch (e) {
