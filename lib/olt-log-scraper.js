...
const log = require('./logger').logger.child('OLT_LOG_SCRAPER');
...
// #b324: alarm PUSH saat SEMUA OLT tak terbaca (throttled) + notif PULIH — supaya OLT mati total tak
// lagi "tak seorang pun tahu sampai buka halaman". Never-throw; opt-out config.oltMonitor.alertAllDown=false.
let _oltAllDownSince = 0;
let _oltAllDownLastAlertAt = 0;
const OLT_ALLDOWN_THROTTLE_MS = 30 * 60 * 1000;
// P0 housekeeping: alarm PARSIAL — OLT mati SEBAGIAN (mis. 1 dari 2) dulu TAK beralarm karena
// alertOltAllDown hanya berbunyi saat SEMUA mati. Padahal insiden nyata = OLT besar mati (~78%
// pelanggan) sementara OLT kecil hidup → deteksi LOS BUTA tanpa alarm. Ini menutup kasus itu:
// alarm PER-DEVICE saat sebuah OLT unreachable, throttled per-OLT + notif pulih. olt.id -> lastAlertAt.
const _deviceAlarmAt = new Map();

async function _kirimAlarmOltAdmin(teks) {
    try {
        const { getAdminJids } = require('./admin-recipients');
        const { sendCritical } = require('./whatsapp-critical-delivery');
        const { recipientsFor } = require('./notif-router');
        // Penerima lewat choke-point routing kategori 'los_alarm': ke grup bila diarahkan, kalau
        // tidak FAIL-OPEN ke DM admin (getAdminJids) — persis perilaku lama saat notifRouting OFF.
        // Kirimnya TETAP sendCritical + waitForReadyMs (OLT bisa alarm saat WA baru pulih dari blip).
        const { recipients } = recipientsFor('los_alarm', { adminFallback: (await getAdminJids()) || [] });
        for (const jid of recipients) {
            try { await sendCritical(jid, { text: teks }, { label: 'olt-alldown', waitForReadyMs: 8000 }); } catch (_e) { /* satu gagal tak stop */ }
        }
    } catch (_e) { /* never-throw */ }
}

async function alertOltAllDown({ successCount, unreachableCount, degradedCount, oltDevices }) {
    const cfg = (global.config && global.config.oltMonitor) || {};
    if (cfg.alertAllDown === false) return; // opt-out
    // PULIH: sebelumnya all-down, kini ada yang sukses → kabari admin + reset state.
    if (successCount > 0 && _oltAllDownSince) {
        const durasiMnt = Math.round((Date.now() - _oltAllDownSince) / 60000);
        _oltAllDownSince = 0; _oltAllDownLastAlertAt = 0;
        await _kirimAlarmOltAdmin(`✅ *OLT PULIH* — pembacaan log OLT kembali normal setelah ~${durasiMnt} menit buta total. Deteksi LOS aktif lagi.`);
        return;
    }
    const adaDevice = Array.isArray(oltDevices) && oltDevices.length > 0;
    const semuaMati = adaDevice && successCount === 0 && (unreachableCount > 0 || degradedCount > 0);
    if (!semuaMati) return;
    const now = Date.now();
    if (!_oltAllDownSince) _oltAllDownSince = now;
    if (now - _oltAllDownLastAlertAt < OLT_ALLDOWN_THROTTLE_MS) return; // throttle 30 mnt
    _oltAllDownLastAlertAt = now;
    await _kirimAlarmOltAdmin(
        `🚨 *SEMUA OLT TAK TERBACA* (${unreachableCount} unreachable, ${degradedCount} degraded dari ${oltDevices.length}).\n\n` +
        'Selama ini deteksi LOS/gangguan pelanggan BUTA — tak ada broadcast gangguan yang bisa dikirim. ' +
        'Cek koneksi ke OLT / jaringan manajemen.'
    );
}

/**
 * P0: alarm OLT mati SEBAGIAN (per-device). Melengkapi alertOltAllDown yang hanya menutup blackout
 * TOTAL. Bila ADA OLT sukses tapi OLT LAIN unreachable, kirim alarm PER-OLT (throttled per-OLT) +
 * notif PULIH — supaya OLT besar mati sementara OLT kecil hidup TAK lagi lolos tanpa alarm. Never-throw;
 * opt-out config.oltMonitor.alertPartialDown === false.
 */
async function alertOltPartialDown({ successCount }) {
    const cfg = (global.config && global.config.oltMonitor) || {};
    if (cfg.alertPartialDown === false) return;
    // successCount===0 = SEMUA mati → sudah ditangani alertOltAllDown; jangan dobel.
    if (!(successCount > 0)) return;
    const now = Date.now();
    for (const s of getDeviceStatuses()) {
        const id = s.olt_id;
        if (s.status === 'unreachable') {
            const last = _deviceAlarmAt.get(id) || 0;
            if (now - last < OLT_ALLDOWN_THROTTLE_MS) continue; // throttle per-OLT 30 mnt
            _deviceAlarmAt.set(id, now);
            await _kirimAlarmOltAdmin(
                `🚨 *OLT ${s.olt_name || id} TAK TERBACA* (unreachable).\n\n` +
                'Deteksi LOS/gangguan pelanggan di OLT INI sedang BUTA — broadcast gangguan area ini tak bisa dikirim. ' +
                'OLT lain masih normal. Cek koneksi/jaringan manajemen ke OLT ini.'
            );
        } else if (s.status === 'healthy' && _deviceAlarmAt.has(id)) {
            _deviceAlarmAt.delete(id);
            await _kirimAlarmOltAdmin(`✅ *OLT ${s.olt_name || id} PULIH* — pembacaan log kembali normal. Deteksi LOS aktif lagi.`);
        }
    }
}
...
function loadEvents() {
    try {
        ...
        log.error('[OLT-Scraper] Error loading events:', e.message);
    }
    return {};
}
...
function saveEvents(events) {
    try {
        ...
        log.error('[OLT-Scraper] Error saving events:', e.message);
    }
}
...
function loadHwm() {
    try {
        ...
        log.error('[OLT-Scraper] Error loading HWM:', e.message);
    }
    return {};
}
...
function saveHwm(hwm) {
    try {
        ...
        log.error('[OLT-Scraper] Error saving HWM:', e.message);
    }
}
...
function cleanupOldEvents(events) {
    ...
    if (cleaned > 0) {
        log.info(`[OLT-Scraper] Cleaned ${cleaned} old events`);
    }
...
function parseLogLine(line) {
    ...
    if (parsed && process.env.DEBUG_OLT_SCRAPER === 'true') {
        log.info(`[OLT-Scraper DEBUG] Parsed ${parsed.type}:`, parsed);
    }
    return parsed;
}
...
async function fetchOltLog(host, username, password, maxPages = 3, timeWindowMinutes = 10, hwmMs = null, port = 80) {
    ...
    for (let page = 0; page < cap; page++) {
        try {
            ...
            if (lines.length === 0) {
                ...
                log.info(`[OLT-Scraper] No more logs at page ${page}, stopping`);
                break;
            }
            
            allLines.push(...lines);
            halamanTerbaca = page + 1;
            log.info(`[OLT-Scraper] Page ${page}: ${lines.length} log lines`);
            ...
            log.error(`[OLT-Scraper] Error fetching page ${page}:`, error.message);
            if (page === 0) {
    ...
    if (halamanTerbaca >= cap) {
        ...
        log.warn(`[OLT-Scraper] !! Pembacaan TERPOTONG di cap ${cap} halaman (${allLines.length} baris). `
    ...
    log.info(`[OLT-Scraper] Fetched ${allLines.length} log lines (adaptif, cap ${cap} halaman, terbaca ${halamanTerbaca}, berhenti: ${alasanBerhenti})`);
    return allLines;
}
...
function extractLogLines(html) {
    ...
    if (process.env.DEBUG_OLT_SCRAPER === 'true') {
        ...
        try {
            ...
            log.info(`[OLT-Scraper DEBUG] HTML saved to ${debugPath}`);
        } catch (e) {
            log.error('[OLT-Scraper DEBUG] Failed to save HTML:', e.message);
        }
    }
    ...
    log.info(`[OLT-Scraper] Found ${timestamps.length} timestamps in HTML`);
    ...
    if (lines.length > 0) {
        log.info(`[OLT-Scraper] Extracted ${lines.length} log lines`);
        if (process.env.DEBUG_OLT_SCRAPER === 'true') {
            log.info('[OLT-Scraper DEBUG] All extracted lines:');
            lines.forEach((line, idx) => log.info(`  [${idx}]`, line));
        }
    } else {
        log.info('[OLT-Scraper] No log lines extracted - HTML format might be different');
        // Try to find any EPON-related text
        const eponMatches = html.match(/EPON/gi);
        if (eponMatches) {
            log.info(`[OLT-Scraper] Found ${eponMatches.length} "EPON" occurrences in HTML`);
            ...
            if (eponIndex !== -1) {
                ...
                log.info(`[OLT-Scraper] Sample HTML around EPON:`, sample);
            }
        }
    }
...
function processLog(logLines, events, timeWindowMinutes = 10, options = {}) {
    ...
    // MAC yang MELIHAT baris Discovery siklus ini (ONT online kembali). Dikumpulkan supaya caller
    // bisa menghapusnya dari store PERSISTEN (allEvents): LOS yang tercatat di siklus SEBELUMNYA
    // tak ada di `events` fresh ini, jadi delete di cabang Discovery no-op — tanpa daftar ini,
    // insiden LOS tak pernah ditutup s/d cleanup 2 hari (recovery scraper mati untuk bot syslog-less).
    const recoveredMacs = new Set();
    ...
    if (latestLogTime) {
        ...
        log.info(`[OLT-Scraper] Time offset: ${offsetMinutes} minutes (OLT ${offsetMinutes > 0 ? 'ahead' : 'behind'} server)`);
        log.info(`[OLT-Scraper] Time window: ${timeWindowMinutes} minutes`);
    }
    ...
    if (effectiveHwm !== null && latestLogTime &&
        ...
        log.warn('[OLT-Scraper] Jam OLT tampak reset mundur — abaikan HWM siklus ini, pakai jendela-waktu');
        effectiveHwm = null;
    }
    ...
    logLines.forEach(line => {
        ...
        if (!parsed) {
            unparsedCount++;
            if (process.env.DEBUG_OLT_SCRAPER === 'true') {
                log.info('[OLT-Scraper DEBUG] Unparsed line:', line);
            }
            return;
        }
        ...
        if (effectiveHwm !== null) {
            ...
            if (timeSinceLog > TIME_WINDOW_MS) {
                skippedOldCount++;
                if (process.env.DEBUG_OLT_SCRAPER === 'true') {
                    log.info(`[OLT-Scraper DEBUG] Skipped old log (${Math.round(timeSinceLog/60000)} min ago):`, line.substring(0, 50));
                }
                return;
            }
        }
        ...
        if (parsed.type === 'dying-gasp') {
            ...
            log.info(`[OLT-Scraper] Found dying-gasp: ${parsed.mac} at ${fullTimestamp}`);
            ...
            log.info(`[OLT-Scraper] ${parsed.mac}: ${eventType.toUpperCase()} at ${fullTimestamp}`);
            
        } else if (parsed.type === 'discovery') {
            discoveryCount++;
            // ONT online kembali - hapus event offline siklus ini (bila ada)...
            if (events[parsed.mac]) {
                log.info(`[OLT-Scraper] ${parsed.mac}: Online (Discovery)`);
                delete events[parsed.mac];
            }
            // ...DAN catat pulih supaya caller hapus dari allEvents persisten (LOS siklus lalu).
            recoveredMacs.add(parsed.mac);
        }
    });
    
    log.info(`[OLT-Scraper] Parsed: ${dyingGaspCount} dying-gasp, ${lostCount} lost, ${discoveryCount} discovery, ${unparsedCount} unparsed, ${skippedOldCount} skipped (old/seen)`);
    ...
    if (report) {
        ...
        report.recoveredMacs = [...recoveredMacs];
    }
...
function parseTimestamp(logTimestamp, year) {
    ...
    if (match) {
        ...
        const time = match[3];
        // Year-wrap: log OLT (RFC3164) tak menyimpan tahun. Di batas Des/Jan, baris Desember yang
        // masih ada di buffer OLT akan ter-stamp tahun BARU (≈11 bulan di masa depan) → urutan
        // kronologis terbalik: dying-gasp Des-31 sortir SESUDAH Lost Jan-1 → korelasi DG↔Lost
        // meleset → LOS palsu ("fiber putus") saat mati listrik tahun baru + HWM teracuni ke masa
        // depan (memaksa mode jendela-waktu berbulan-bulan). Sama seperti olt-event-classifier:
        // bila tanggal jatuh > now + 1 hari, pakai tahun sebelumnya.
        let y = year;
        const asMs = Date.parse(`${y}-${month}-${day}T${time}`);
        if (Number.isFinite(asMs) && asMs > Date.now() + 86_400_000) {
            y = year - 1;
        }
        return `${y}-${month}-${day}T${time}`;
    }
...
async function scrapeOltLog() {
    ...
    if (isScraping) {
        log.info('[OLT-Scraper] Siklus sebelumnya belum selesai — skip tick ini (anti-overlap)');
        return;
    }
    isScraping = true;
    try {
        ...
        if (!globalConfig.enabled || !globalConfig.webEnabled) {
            log.info('[OLT-Scraper] Web scraping disabled in config');
            return;
        }
        
        if (oltDevices.length === 0) {
            log.info('[OLT-Scraper] No OLT devices configured');
            return;
        }
        
        log.info(`[OLT-Scraper] Scraping ${oltDevices.length} OLT device(s)...`);
        ...
        const scrapePromises = oltDevices.map(async (olt) => {
            ...
            try {
                ...
                if (stateChanged) {
                    log.info(`[OLT-Scraper] [${olt.name}] recovered and is healthy again`);
                }

                return { oltId: olt.id, oltName: olt.name, events: oltEvents, logLines: logLines.length, status: 'healthy', newHwmMs: report.newHwmMs, recoveredMacs: report.recoveredMacs || [] };

            } catch (error) {
                const stateChanged = markDeviceFailure(olt, error.message);
                const state = getDeviceState(olt);
                if (stateChanged) {
                    log.warn(`[OLT-Scraper] [${olt.name}] ${state.status}: ${error.message}`);
                }
                return {
                    oltId: olt.id,
                    oltName: olt.name,
        ...
        results.forEach(result => {
            ...
            // RECOVERY PERSISTEN: hapus dari allEvents MAC yang melihat Discovery siklus ini, KECUALI
            // yang justru turun lagi di siklus yang sama (masih ada di result.events → jangan hapus,
            // Object.assign sudah menaruh state barunya). Tanpa ini, Object.assign hanya MENAMBAH —
            // LOS lama tak pernah dibuang, insiden tak ditutup s/d cleanup 2 hari.
            for (const mac of (result.recoveredMacs || [])) {
                if (!(result.events && result.events[mac])) delete allEvents[mac];
            }
        ...
        try {
            ...
            log.error('[OLT-Scraper] LOS broadcaster dispatch error:', broadcastErr.message);
        }
        ...
        // #b324: lastSuccessAt HANYA disegarkan bila BENAR-BENAR ada OLT yang berhasil dibaca. Dulu
        // di-set tanpa syarat walau successCount=0 (SEMUA OLT unreachable), karena kegagalan per-device
        // ditangkap di map & tak melempar → blok luar selalu selesai. Akibatnya surface /api/olt/
        // scraper-status tampak "sehat / sukses baru saja" padahal deteksi LOS/DG/cek-koneksi BUTA total
        // (blindspot terukur: Dander OLT mati berminggu-minggu ~78% pelanggan, 0 alarm). "tak bisa
        // observasi" != "observasi baik".
        if (successCount > 0) {
            lastError = null;
            lastSuccessAt = new Date();
        } else if (unreachableCount > 0 || degradedCount > 0) {
            lastError = `Tidak ada OLT berhasil dibaca (unreachable=${unreachableCount}, degraded=${degradedCount}, skipped=${skippedCount}). Deteksi LOS BUTA sampai pulih.`;
            log.error('[OLT-Scraper] ⚠️ ' + lastError);
        }
        // Alarm PUSH ke admin saat SEMUA OLT tak terbaca (dan pulih) — jangan andalkan admin membuka
        // halaman. Throttled + never-throw; opt-out via config.oltMonitor.alertAllDown === false.
        try { await alertOltAllDown({ successCount, unreachableCount, degradedCount, oltDevices }); } catch (_e) { /* never-throw */ }
        // P0: alarm OLT mati SEBAGIAN (per-device) — menutup kasus 1-dari-N OLT unreachable yang lolos alertOltAllDown.
        try { await alertOltPartialDown({ successCount }); } catch (_e) { /* never-throw */ }

        const totalEvents = Object.keys(allEvents).length;
        log.info(`[OLT-Scraper] ✓ Completed scraping ${oltDevices.length} OLT(s), ${totalEvents} active events | healthy=${successCount} degraded=${degradedCount} unreachable=${unreachableCount} skipped=${skippedCount}`);
        
    } catch (error) {
        lastError = error.message;
        log.error('[OLT-Scraper] ✗ Error:', error.message);
    } finally {
        isScraping = false;
    }
}
...
function startScraper() {
    if (isRunning) {
        log.info('[OLT-Scraper] Already running');
        return;
    }
    ...
    try {
        ...
        if (!globalConfig.enabled || !globalConfig.webEnabled) {
            log.info('[OLT-Scraper] Web scraping disabled');
            return;
        }
        ...
        log.info(`[OLT-Scraper] Starting scraper (interval: ${intervalMinutes} minute${intervalMinutes > 1 ? 's' : ''})`);
        ...
        log.error('[OLT-Scraper] Error starting:', e.message);
    }
}
...
module.exports = {
    ...
    __testHooks: {
        ...
        resetDeviceStatuses,
        alertOltPartialDown,
        _resetDeviceAlarms: () => _deviceAlarmAt.clear(),
        processLog,
        parseTimestamp,
        ...
        fetchLogForStatus,
        alertOltAllDown,
        _resetOltAllDownState: () => { _oltAllDownSince = 0; _oltAllDownLastAlertAt = 0; }
    }
};
