...
const DEFAULT_LOG_PAGES = 20;
const MIN_LOG_PAGES = 15;
const MAX_LOG_PAGES_HARD = 40;
 ...
 */
async function fetchOltLog(host, username, password, maxPages = 3, timeWindowMinutes = 10, hwmMs = null) {
    ...
    for (let page = 0; page < cap; page++) {
        try {
            const { body: html } = await fetchOltLogPage(host, username, password, page);
    ...
    console.log(`[OLT-Scraper] Fetched ${allLines.length} log lines (adaptif, cap ${cap} halaman)`);
    return allLines;
}
 ...
 */
async function fetchLogForStatus(host, username, password, maxPages = 12) {
    ...
    for (let page = 0; page < cap; page++) {
        let res;
        try {
            res = await fetchOltLogPage(host, username, password, page);
        } catch (e) {
            if (page === 0) throw e;
...
async function getOnuStatusMap(oltDevice, opts = {}) {
    ...
    const { lines, offsetMs, year } = await fetchLogForStatus(oltDevice.host, oltDevice.webUsername, oltDevice.webPassword, maxPages);
    ...
    try {
        ...
        const scrapePromises = oltDevices.map(async (olt) => {
            ...
            try {
                ...
                const logLines = await fetchOltLog(olt.host, olt.webUsername, olt.webPassword, globalConfig.maxLogPages, globalConfig.timeWindow, deviceHwm);
...
async function testWebConnection(host, username, password) {
    try {
        ...
        const logLines = await fetchOltLog(host, username, password);
        return {
            success: true,
