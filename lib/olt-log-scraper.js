 ...
 */
async function fetchOltLog(host, username, password, maxPages = 3, timeWindowMinutes = 10) {
    // Floor cap: nilai stale (mis. 3) jangan melumpuhkan baca saat burst — stop ADAPTIF
    // (jendela waktu, di akhir loop) yang jadi penentu utama kedalaman.
    ...
    for (let page = 0; page < cap; page++) {
        try {
            ...
            if (tsList.length > 0) {
                ...
                if (pageOldest < mostRecentMs - windowMs - slackMs) break;
            }
    ...
    try {
        ...
        const scrapePromises = oltDevices.map(async (olt) => {
            ...
            try {
                ...
                const logLines = await fetchOltLog(olt.host, olt.webUsername, olt.webPassword, globalConfig.maxLogPages, globalConfig.timeWindow);
