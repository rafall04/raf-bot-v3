const log = require('./logger').logger.child('AUTH_CACHE');
...
class AuthCache {
    ...
    startCleanupInterval() {
        ...
        // Cleanup setiap 1 menit. unref timer agar tidak menahan event loop di
        // test/CLI script — produksi tetap jalan normal karena ada HTTP listener aktif.
    ...
    getJWTVerification(token, verifyFunction) {
        ...
        // #b328 KEAMANAN (P0): kunci cache HARUS dari token PENUH (sha256), BUKAN 50 char pertama.
        // Prefix 50 char = header JWT konstan + awal payload (untuk admin ~ {id,u...} yang bisa ditebak,
        // id admin kecil & berurutan). Dengan prefix-key, cache HIT memulangkan payload TER-DECODE tanpa
        // pernah memanggil verifyFunction() (jwt.verify) → penyerang cukup kirim Bearer <prefix sama> +
        // sampah + tanda tangan INVALID dan lolos sebagai admin selama ada 1 request admin sah dalam
        // TTL (praktis selalu, polling dashboard/socket). sha256 token penuh → token beda = cache MISS →
        // jwt.verify dipanggil → forge ditolak. (HMAC verify murah; cache hanya dedup token identik.)
        const cacheKey = crypto.createHash('sha256').update(String(token)).digest('hex');
    ...
    async reloadAccounts() {
        ...
        try {
            ...
            log.error(`[AUTH_CACHE] Failed to reload accounts.json:`, err);
            this.reloadInProgress = false;
