...
const SNMP_DOWN_COOLDOWN_MS = 300000; // 5 menit: lewati SNMP setelah gagal (cegah dead-wait berulang)
const snmpDownUntil = new Map(); // host -> epoch ms (berhenti coba SNMP sampai waktu ini)
 ...
 */
async function getSnmpHealth(device, opts = {}) {
    const host = device && device.host;
    // Fast-skip: bila SNMP host ini baru saja gagal (mis. UDP 161 tak di-forward), jangan
    // dead-wait lagi — langsung null agar caller fallback SSH seketika.
    if (host && (snmpDownUntil.get(host) || 0) > Date.now()) return null;
    const markDown = () => {
        if (host) snmpDownUntil.set(host, Date.now() + SNMP_DOWN_COOLDOWN_MS);
    };
    const session = snmp.createSession(device.host, device.snmpCommunity || 'public', {
        port: parseInt(device.snmpPort, 10) || 161,
        version: snmp.Version2c,
        // INTERAKTIF (dashboard): timeout PENDEK + tanpa retry → gagal cepat. Sengaja BUKAN
        // device.snmpTimeout (30s, untuk poller background) yang bikin health dead-wait ~90s.
        timeout: opts.timeoutMs || 3000,
        retries: opts.retries != null ? opts.retries : 0
    });
    try {
        ...
        if (!Object.keys(scalars).length) {
            markDown();
            return null; // SNMP mati/community salah/UDP161 tak di-forward → tandai down, fallback SSH
        }
        ...
        if (host) snmpDownUntil.delete(host); // SNMP pulih → boleh dicoba lagi normal
