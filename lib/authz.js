...
function buatGerbangTeknisi(daftarIzin, opsi = {}) {
    ...
    // !! WAJIB: predikat "jalur ini memang MILIK router tempat gerbang dipasang".
    // Router admin dipasang di ROOT (`app.use("/", adminApiRouter)`), jadi middleware di dalamnya
    // mencegat SETIAP permintaan aplikasi — termasuk halaman PHP dan router lain yang dipasang
    // sesudahnya. Tanpa predikat ini, gerbang akan menolak `/teknisi-pelanggan`, `/api/users`,
    // dan seluruh jalur yang bukan urusannya. Terbukti saat uji lokal sebelum ditegakkan.
    // Absen (undefined) hanya boleh di unit test yang memang menguji gerbangnya sendiri.
    const milikRouter = typeof opsi.milikRouter === "function" ? opsi.milikRouter : null;
    ...
    return function gerbangTeknisi(req, res, next) {
        ...
        // Bukan jalur milik router ini → bukan urusan gerbang ini.
        if (milikRouter && !milikRouter(req.method, jalur)) return next();
