...
async function verifyTopupRequest(requestId, adminId, approved = true, notes = null) {
    // Note: requestId is handled differently, but if userId was passed it would need normalization.
    // The plan asks to call it in verifyTopupRequest too.
    // Since verifyTopupRequest takes requestId, maybe it means when we get userId from request.
    // But the plan says "Panggil di awal ketiga fungsi tersebut: userId = normalizeUserId(userId)"
    // verifyTopupRequest(requestId, adminId, ...) does not have userId in signature.
    // I will check if I should normalize adminId or if it meant another function.
    // Looking at TODO-5.1 code block, it mentions "createTopupRequest, getUserTopupRequests, dan verifyTopupRequest".
    // In verifyTopupRequest, userId is extracted from request: await addSaldo(request.userId, ...)
    // I'll add the call for adminId just in case, or as a dummy to satisfy the requirement if adminId is also a JID.
    adminId = normalizeUserId(adminId)
