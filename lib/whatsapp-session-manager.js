...
const log = require('./logger').logger.child('WHATSAPP_SESSION_MANAGER');
const fs = require('fs');
...
class WhatsAppSessionManager {
    ...
    async cleanSession(phoneJid) {
        try {
            ...
            log.info(`[SESSION_CLEANUP] Cleaning session for ${phoneNumber}`);
            ...
            for (const file of sessionFiles) {
                ...
                if (fs.existsSync(filePath)) {
                    ...
                    log.info(`[SESSION_CLEANUP] Removed ${file}`);
                    cleaned = true;
                }
            }
            
            if (cleaned) {
                log.info(`[SESSION_CLEANUP] Session cleaned for ${phoneNumber}`);
            } else {
                log.info(`[SESSION_CLEANUP] No session files found for ${phoneNumber}`);
            }
            
            return cleaned;
        } catch (error) {
            log.error('[SESSION_CLEANUP] Error cleaning session:', error);
            return false;
        }
    }
    ...
    async sendMessageWithRecovery(socketSource, phoneJid, message, maxRetries = 3) {
        ...
        if (retries >= maxRetries) {
            log.error(`[SESSION_RECOVERY] Max retries reached for ${phoneJid}`);
        ...
        try {
            ...
            if (retries > 0) {
                log.info(`[SESSION_RECOVERY] Retry attempt ${retries} for ${phoneJid}`);
            ...
            this.badMacRetries.delete(retryKey);
            log.info(`[SESSION_RECOVERY] Message sent successfully to ${phoneJid}`);
            ...
            if (errorMsg.includes('Bad MAC') || errorMsg.includes('decrypt')) {
                log.error(`[SESSION_RECOVERY] Bad MAC error for ${phoneJid}, attempting recovery...`);
                ...
                log.error(`[SESSION_RECOVERY] Timeout error for ${phoneJid}`);
    ...
    clearRetryCounters() {
        ...
        log.info('[SESSION_RECOVERY] Retry counters cleared');
    }
