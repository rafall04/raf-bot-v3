 ...
 *          upstream (resolve IP PPPoE → path via lib/upstream-path-resolver), dedup per
 ...
 * Deps: `lib/upstream-path-resolver.resolvePathForIp`, lazy `lib/mikrotik.getActivePPPoEUsers`
...
function defaultDeps() {
    return {
        ...
        resolvePathForIp: (ip, cfg) => require("./upstream-path-resolver").resolvePathForIp(ip, cfg),
...
async function resolvePathForUser(user, addr, deps) {
    ...
    try {
        ...
        const resolved = deps.resolvePathForIp(ip, upCfg);
        return (resolved && resolved.path) || "unknown";
    } catch (_e) {
        return "unknown";
    }
}
 ...
 * @param {{user:object, source:string, addr?:string, lineStatus?:string}} payload
 */
async function recordComplaint(payload = {}, depsOverride = {}) {
    try {
        ...
        const daftar = [...distinct.values()]
            .slice(-8)
            .map((s) => `  - ${s.name} (${s.source === "tiket_lemot" ? "tiket lemot" : "cek koneksi"})`)
            .join("\n");
        const kondisi = await buildPathConditionText(pathKey, deps);
