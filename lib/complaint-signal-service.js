 ...
 */
"use strict";
const log = require('./logger').logger.child('COMPLAINT_SIGNAL_SERVICE');
...
async function deliver(deps, cfg, text) {
    ...
    for (const jid of recipients) {
        try {
            ...
            log.warn(`[Complaint-Signal] Gagal kirim ke ${jid}: ${err.message}`);
        }
    }
    return delivered;
}
...
async function recordComplaint(payload = {}, depsOverride = {}) {
    try {
        ...
        log.warn(`[Complaint-Signal] recordComplaint gagal: ${err.message}`);
