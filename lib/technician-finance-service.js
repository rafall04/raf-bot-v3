...
async function getCollectionPayableEntries({ teknisiId, periodMonth, periodYear }) {
    await ensureFinanceTables();
    ...
    return dbAll(
        `SELECT *
         ...
         WHERE teknisi_id = ?
           ...
           AND period_year = ?
         ORDER BY datetime(created_at) ASC, id ASC`,
        [String(teknisiId), parseInt(periodMonth, 10), parseInt(periodYear, 10)]
    );
}
...
async function buildPayrollPayload({
    ...
    const normalizedPotonganKasbon = Math.max(0, Math.min(toInteger(potonganKasbon), summary.total_outstanding));
    ...
    const grossAmount = normalizedGajiPokok + normalizedBonusManual + komisiCollection;
...
async function createPayrollDraft(payload, actor = {}) {
    ...
    const result = await dbRun(
        `INSERT INTO technician_gaji (
            ...
            gaji_pokok, bonus, bonus_manual, komisi_collection,
...
function formatPayrollRow(row) {
    ...
    return {
        ...
        gross_amount: toInteger(row.gross_amount || (toInteger(row.gaji_pokok) + toInteger(row.bonus_manual || row.bonus) + toInteger(row.komisi_collection))),
...
async function updatePayrollDraft(id, payload) {
    ...
    await dbRun(
        `UPDATE technician_gaji
         SET gaji_pokok = ?, bonus = ?, bonus_manual = ?, komisi_collection = ?,
             potongan_kasbon = ?, potongan_lain = ?, potongan_lain_keterangan = ?,
             gross_amount = ?, total_potongan = ?, total_gaji = ?, net_amount = ?,
             notes = ?, updated_at = ?
         WHERE id = ?`,
...
async function finalizePayroll(id, actor = {}) {
    ...
    const grossAmount = refreshed.gaji_pokok + refreshed.bonus_manual + komisiCollection;
    const totalPotongan = refreshed.potongan_kasbon + refreshed.potongan_lain;
    const netAmount = grossAmount - totalPotongan;

    await dbRun(
        `UPDATE technician_gaji
         SET bonus = ?, bonus_manual = ?, komisi_collection = ?, gross_amount = ?, total_potongan = ?,
    ...
    return { finalized: true, collectionEntries: lockResult.entries, netAmount, komisiCollection };
}
