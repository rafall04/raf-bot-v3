 ...
 */
"use strict";
const log = require('./logger').logger.child('TECHNICIAN_FINANCE_SERVICE');
...
async function insertKasbonLedgerEntry({
    ...
    try {
        ...
        try {
            ...
            log.error('[TECHNICIAN_KASBON_LEDGER_SYNC_ERROR]', ledgerSyncError.message);
        }
...
async function ensureFinanceTables() {
    if (!initializationPromise) {
        initializationPromise = (async () => {
            ...
            // Status pengiriman struk gaji. Gagal kirim yang hanya muncul di log PM2 sama saja
            // dengan tidak tercatat — tak bisa dilihat, tak bisa dikirim ulang.
            await ensureColumn("technician_gaji", "struk_status", "TEXT");
            await ensureColumn("technician_gaji", "struk_at", "TEXT");
            await ensureColumn("technician_gaji", "struk_error", "TEXT");
...
async function getMarketingPayableSummary({ teknisiId, periodMonth = null, periodYear = null }) {
    try {
        // Periode WAJIB diteruskan: tanpa itu satu payroll menyapu komisi marketing lintas
        // bulan dan membukukan komisi bulan depan di bulan ini.
        const mk = await marketingSource().getUnsettledMarketingForTeknisi(teknisiId, { periodMonth, periodYear });
        return { entries: (mk && mk.entries) || [], net_total: toInteger(mk && mk.net_total) };
    } catch (error) {
        log.error("[TECHNICIAN_MARKETING_READ_ERROR]", error.message);
        return { entries: [], net_total: 0 };
    }
}

async function lockMarketingToPayroll({ payrollId, teknisiId, periodMonth = null, periodYear = null }) {
    try {
        const r = await marketingSource().settleMarketingToPayroll({ teknisiId, payrollId, periodMonth, periodYear });
        return { entries: (r && r.entries) || [], netTotal: toInteger(r && r.netTotal) };
    } catch (error) {
        log.error("[TECHNICIAN_MARKETING_SETTLE_ERROR]", error.message);
...
/**
 * Membatalkan finalisasi: payroll kembali ke `draft` dan SEMUA kunci komisinya dilepas.
 *
 * Melepas kuncinya adalah bagian yang tak boleh dilupakan. Finalisasi menempelkan `payroll_id`
 * ke baris komisi penarikan dan menandai komisi marketing `settled`; kalau statusnya dikembalikan
 * ke draft tanpa melepas itu, komisinya menggantung pada payroll yang tak jadi dibayar — hilang
 * dari perhitungan dan tak bisa masuk payroll mana pun lagi.
 *
 * Payroll yang SUDAH DIBAYAR tak bisa dibatalkan dari sini: uangnya sudah berpindah tangan dan
 * membatalkannya adalah keputusan lain yang butuh jalannya sendiri.
 */
async function unfinalizePayroll(id, actor = {}) {
    const existing = await getPayrollRecord(id);
    if (!existing) return { ok: false, reason: "not_found" };
    if (existing.status === "paid") return { ok: false, reason: "sudah_dibayar" };
    if (existing.status !== "finalized") return { ok: false, reason: "bukan_finalized" };

    const lepasCollection = await dbRun(
        `UPDATE technician_collection_ledger
            SET payroll_id = NULL, payroll_locked_at = NULL
          WHERE payroll_id = ?`,
        [toInteger(id)]
    );

    let lepasMarketing = 0;
    try {
        const src = marketingSource();
        if (src && typeof src.unsettleMarketingFromPayroll === "function") {
            const r = await src.unsettleMarketingFromPayroll({ payrollId: toInteger(id) });
            lepasMarketing = (r && r.dilepas) || 0;
        }
    } catch (error) {
        log.error("[PAYROLL_UNFINALIZE_MARKETING_ERROR]", error.message);
    }

    await dbRun(
        `UPDATE technician_gaji
            SET status = 'draft', finalized_at = NULL, finalized_by = NULL, finalized_by_name = NULL, updated_at = ?
          WHERE id = ?`,
        [nowIso(), toInteger(id)]
    );
    ...
    return {
        ok: true,
        komisiDilepas: (lepasCollection && lepasCollection.changes) || 0,
        marketingDilepas: lepasMarketing,
        oleh: actor.name || actor.username || "admin"
    };
}

/** Mencatat hasil pengiriman struk pada baris payroll-nya (terkirim / gagal + alasannya). */
async function setPayrollReceiptStatus(payrollId, { status, error = "" }) {
    await ensureFinanceTables();
    await dbRun(
        "UPDATE technician_gaji SET struk_status = ?, struk_at = ?, struk_error = ? WHERE id = ?",
        [String(status || ""), nowIso(), String(error || ""), toInteger(payrollId)]
    );
    return { ok: true };
}
...
async function buildPayrollPayload({
    ...
    const marketing = await getMarketingPayableSummary({ teknisiId, periodMonth, periodYear });
...
async function finalizePayroll(id, actor = {}) {
    ...
    // #b326: KLAIM finalisasi secara ATOMIK sebelum settle. Cek status di atas hanya menangkap
    // double-run SEKUENSIAL; dua finalize yang sama-sama membaca 'draft' SEBELUM salah satu menulis
    // 'finalized' (check-then-act lintas banyak await) lolos → kaki MARKETING run kedua membaca 0 (row
    // sudah di-'settled' run pertama) lalu menimpa komisi_marketing jadi 0 → komisi teknisi HILANG
    // permanen (payroll lalu 'paid', tak bisa di-unfinalize). UPDATE bersyarat WHERE status='draft'
    // hanya sukses untuk SATU pemanggil (changes===1); yang kalah balik 'already_finalized'.
    const klaim = await dbRun("UPDATE technician_gaji SET status = 'finalized' WHERE id = ? AND status = 'draft'", [existing.id]);
    if (!klaim || klaim.changes !== 1) {
        return { finalized: false, reason: "already_finalized" };
    }
    ...
    const lockMarketing = await lockMarketingToPayroll({ payrollId: existing.id, teknisiId: existing.teknisi_id, periodMonth: existing.period_month, periodYear: existing.period_year });
...
async function payPayroll(id, actor = {}, notes = "") {
    ...
    try {
        ...
        log.error('[TECHNICIAN_PAYROLL_LEDGER_SYNC_ERROR]', ledgerSyncError.message);
    }
