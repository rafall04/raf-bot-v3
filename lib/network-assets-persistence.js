 ...
 */
"use strict";
const log = require('./logger').logger.child('NETWORK_ASSETS_PERSISTENCE');
    ...
    try {
        if (fs.existsSync(networkAssetsDbPath)) {
            ...
            if (!Array.isArray(jsonData)) {
                log.error("[ASSET_LOAD_ERROR] Data di network_assets.json bukan array. Membuat backup dan file baru.");
        ...
        log.error("[ASSET_LOAD_FATAL_ERROR] Gagal menangani file network_assets.json:", error);
        return [];
    }
}
...
function saveNetworkAssets(assets, globalScope = global) {
    ...
    try {
        ...
        log.error("[ASSET_SAVE_ERROR] Gagal menyimpan data aset jaringan:", error);
...
async function updateNetworkAssetsWithLock(updateFunction, globalScope = global) {
    ...
    try {
        ...
        log.error("[LOCK_ERROR] Failed to update network assets with lock:", error);
        throw error;
    } finally {
        if (release) {
            try {
                ...
                log.error("[LOCK_RELEASE_ERROR]", releaseError);
            }
        }
    }
}
