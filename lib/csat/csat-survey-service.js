 ...
 */
"use strict";
const log = require('../logger').logger.child('CSAT_SURVEY_SERVICE');
...
async function safeReply(reply, text) {
    ...
    try {
        ...
        log.error("[CSAT_REPLY_SEND_ERROR]", e && e.message);
    }
}
...
async function notifyOwnerDetractor(text, label) {
    try {
        ...
        if (!jids || !jids.length) { log.error("[CSAT_DETRACTOR] tak ada JID admin/owner — alert HILANG (cek accounts.json role admin)"); return; }
        let terkirim = 0;
        for (const jid of jids) {
            try { const r = await sendCritical(jid, { text }, { label: label || "csat-detractor" }); if (r && r.delivered !== false) terkirim += 1; }
            catch (e) { log.error(`[CSAT_DETRACTOR] gagal kirim ke ${jid}:`, e && e.message); }
        }
        // Breadcrumb SUKSES: dulu jalur sukses TAK log apa pun → mengira 'mati' padahal terkirim.
        log.info(`[CSAT_DETRACTOR] alert terkirim ke ${terkirim}/${jids.length} admin.`);
    } catch (e) {
        log.error("[CSAT_DETRACTOR_ERROR]", e && e.message);
    }
}
    ...
    try {
        ...
        if (active.status === "sent") {
            if (parsed.kind === "optout") {
                ...
                await repository.setSurveyOptout({ user_id: userId, canonical_jid: active.canonical_jid, reason: "balas STOP" });
                ...
                logger.log(`[CSAT_REPLY] optout PERMANEN (survei) dari ${displayName} (survey#${active.id})`);
                return true;
            }
            ...
            // unclear → bisa jadi (a) afirmasi/ucapan singkat ("siap","iya","makasih") atau
            // (b) pesan sungguhan ("Info mas", "cek koneksi"). Untuk (a) JANGAN dibiarkan jatuh ke
            // pipeline normal: di sana "siap"/"iya" bisa disalahartikan sbg konfirmasi reboot dsb
            // (akar insiden "ketik siap malah reboot"). Nudge sekali ke rating lalu telan; balasan
            // ack berikutnya ditelan diam. Untuk (b) tetap balik false → mengalir normal.
            if (isBareAck(raw)) {
                if (!active.reminded_at) {
                    await repository.noteReminded(active.id, nowIso);
                    await safeReply(reply, renderCustomerText("rating_reprompt", { nama_pelanggan: displayName },
                        "Makasih Kak 🙏 Boleh dibantu kasih *nilai 1-5* untuk layanan internetnya? (1 = sangat kurang, 5 = sangat puas)"));
                }
                logger.log(`[CSAT_REPLY] ack non-rating dari ${displayName} (survey#${active.id}) — reprompt/swallow`);
                return true;
            }
            // pesan sungguhan → biarkan lanjut ke pipeline normal (jangan membajak).
            return false;
        }
        ...
        if (active.status === "rated") {
            ...
            // !! KELUHAN DITERUSKAN BERDASARKAN ISINYA, BUKAN SKORNYA (#b263).
            //
            // Dulu komentar hanya diteruskan bila skor <= detractorMaxScore (2). Terukur pada
            // korpus CSAT produksi: **67-75% keluhan "lemot" datang dari pemberi skor 3, 4, dan 5**
            // — termasuk yang menulis "jaringan nya agak lemot kak MOHON SEGERA DIPERBAIKI" dengan
            // skor 5. Permintaan paling tegas yang mungkin ditulis pelanggan, dari rating
            // tertinggi, dan tak membangunkan siapa pun.
            //
            // Orang di sini memberi skor tinggi karena sungkan, lalu menuliskan keluhannya di
            // kolom komentar. Menyaring dengan skor berarti membuang justru bagian yang berisi.
            const isiKeluhan = adaKeluhanDiKomentar(raw);
            const alasanTeruskan = isDetractorAlert(cfg, active.score)
                ? `skor rendah (${active.score})`
                : (isiKeluhan.keluhan ? `isi komentar: ${isiKeluhan.sebab}` : null);

            if (alasanTeruskan) {
                const tandaSkorTinggi = !isDetractorAlert(cfg, active.score)
                    ? "\n_(skor tinggi, tapi komentarnya memuat keluhan)_"
                    : "";
                await alertFn(`💬 *Keluhan pelanggan* — *${displayName}*${active.phone ? ` (${active.phone})` : ""} — ${active.score}⭐:\n"${raw.slice(0, 300)}"${tandaSkorTinggi}`, "csat-detractor-comment");
            }
            logger.log(`[CSAT_KOMENTAR] skor=${active.score} keluhan=${isiKeluhan.keluhan} (${isiKeluhan.sebab}) -> ${alasanTeruskan ? "diteruskan (" + alasanTeruskan + ")" : "tidak diteruskan"}`);
            await safeReply(reply, renderCustomerText("rating_thanks", { nama_pelanggan: displayName },
        ...
        log.error("[CSAT_REPLY_ERROR]", err && err.message);
