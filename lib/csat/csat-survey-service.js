 ...
 */
async function handleInboundReply({ user, text, reply, logger = console, repo, config, now } = {}) {
    try {
...
async function buildOwnerDigest(period, { repo, namaWifi } = {}) {
    ...
    if (detractors.length) {
        ...
        detractors.slice(0, 15).forEach((r) => {
            parts.push(`• ${r.name || r.phone || "-"} (${r.score}⭐)${r.comment ? `: ${String(r.comment).slice(0, 120)}` : ""}`);
        });
    }
    ...
    if (withComments.length) {
        ...
        withComments.slice(0, 20).forEach((c) => {
            parts.push(`• ${c.name || c.phone || "-"} (${c.score ? `${c.score}⭐` : "?"}): ${String(c.comment).slice(0, 140)}`);
        });
    }
...
module.exports = {
    ...
    resetForTest,
    // diekspor untuk test
    _internal: { renderCustomerText, safeReply },
};
