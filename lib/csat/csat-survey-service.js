...
async function notifyOwnerDetractor(text, label) {
    try {
        ...
        if (!jids || !jids.length) { console.error("[CSAT_DETRACTOR] tak ada JID admin/owner — alert HILANG (cek accounts.json role admin)"); return; }
        let terkirim = 0;
        ...
        console.log(`[CSAT_DETRACTOR] alert terkirim ke ${terkirim}/${jids.length} admin.`);
    } catch (e) {
