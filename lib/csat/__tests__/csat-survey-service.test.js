...
describe("handleInboundReply", () => {
    ...
    test("ack singkat ('siap') saat survei aktif TIDAK bocor — ditangani + reprompt sekali", async () => {
        // Akar insiden: "siap" (balasan ke admin) jatuh ke pipeline & dikira konfirmasi reboot.
        await seedSent(repo, 30);
        const h1 = await service.handleInboundReply({
            user: { id: 30, name: "Andi" }, text: "siap", reply, repo, config: cfg,
            now: () => "2026-07-22 09:35:00", logger: { log() {} },
        });
        expect(h1).toBe(true); // ditangani → TIDAK diteruskan ke pipeline reboot
        expect(replies).toHaveLength(1); // di-reprompt sekali
        const row = await repo.getActiveByUserId(30, "2026-07-22 09:40:00");
        expect(row.status).toBe("sent"); // masih menunggu rating (belum dinilai)
        expect(row.reminded_at).toBeTruthy();

        // ack kedua → tetap ditangani (tak bocor) TAPI tak reprompt lagi (anti-spam)
        const h2 = await service.handleInboundReply({
            user: { id: 30, name: "Andi" }, text: "iya", reply, repo, config: cfg,
            now: () => "2026-07-22 09:36:00", logger: { log() {} },
        });
        expect(h2).toBe(true);
        expect(replies).toHaveLength(1); // tidak bertambah
    });

    test("ack diikuti rating beneran tetap tercatat", async () => {
        await seedSent(repo, 31);
        await service.handleInboundReply({ user: { id: 31 }, text: "siap", reply, repo, config: cfg, now: () => "2026-07-22 09:35:00", logger: { log() {} } });
        const h = await service.handleInboundReply({ user: { id: 31 }, text: "5", reply, repo, config: cfg, now: () => "2026-07-22 09:37:00", logger: { log() {} } });
        expect(h).toBe(true);
        const row = await repo.getActiveByUserId(31, "2026-07-22 09:40:00");
        expect(row.status).toBe("rated");
