 ...
 */
"use strict";
const log = require('./logger').logger.child('REDAMAN_ALERT_COOLDOWN_STORE');
...
function muat(filePath = resolveFilePath()) {
    try {
        ...
        log.warn(`[REDAMAN_COOLDOWN] Gagal membaca ${path.basename(filePath)}: ${err.message}`);
        return {};
    }
}
...
function simpan(peta, filePath = resolveFilePath()) {
    try {
        ...
        log.error(`[REDAMAN_COOLDOWN] Gagal menulis ${path.basename(filePath)}: ${err.message}`);
        return false;
    }
}
