 ...
 *   ('ipaymu' default | 'tripay' | 'mayar') dan menyeragamkan ke interface tipis yang dipakai alur
 *   bayar tagihan: `chargeRedirect()` (buat transaksi → URL redirect ke halaman gateway auto-settle)
 *   + `verify()` (S2S cek status, dipakai callback). Adapter membungkus klien gateway masing-masing.
 * Caller: routes/bill-payment.js (GET /bayar/:token mode redirect + callback Tripay/Mayar).
 * Deps: ./ipaymu (payHosted, checkTransaction), ./tripay (createCharge, checkTransaction), ./mayar (createInvoice, checkTransaction).
...
const ADAPTERS = {
    ...
    mayar: {
        name: "mayar",
        async chargeRedirect(p) {
            // Mayar = invoice hosted (semua metode: VA/e-wallet/QRIS/retail dalam 1 halaman).
            // returnUrl → redirectUrl Mayar. reference = id invoice (dipakai verify + korelasi callback).
            const r = await mayar.createInvoice(p);
            return { url: r.url, reference: r.reference, gateway: "mayar" };
        },
        verify: (ref, opts) => mayar.checkTransaction(ref, opts),
    },
};
