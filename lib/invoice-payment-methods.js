...
// Teks contekan dari config.example.json yang tak boleh pernah dianggap data sungguhan.
//
// SENGAJA HANYA POLA YANG TAK MUNGKIN JADI DATA ASLI. Nomor karangan seperti
// '1234567890' TIDAK disaring di sini: menebak "ini pasti dummy" dari isinya bisa
// menolak rekening pelanggan yang sah, dan pertahanan yang benar terhadap nomor
// karangan adalah MENGHAPUS fallback yang mengarangnya (sudah dilakukan di
// lib/invoice-generator.js dan message/handlers/steps/saldo-steps.js), bukan
// mencocokkan string di hilir.
const POLA_PLACEHOLDER = [
    /^isi[_\s-]/i,          // ISI_BANKNAME, ISI_ACCOUNTNUMBER, ISI_ACCOUNTNAME
    /^bank default$/i,
];
...
function kosong(nilai) {
    ...
    if (teks === "") return true;
    return POLA_PLACEHOLDER.some((p) => p.test(teks));
}
