...
async function getOltSnapshot(opts = {}) {
    ...
    // SATU-SATUNYA tempat sumber optik dipilih (#b275). Bawaannya WEB.
    //
    // !! SNMP MEMBUAT OLT HANG. Pendukungnya terukur: poller rxPower menembak SNMP tiap 60
    // detik (1.440 walk/hari) dan hanya menyala di Dander — OLT Dander-lah yang kini tak
    // terjangkau, sementara Tanjungharjo (poller mati) sehat.
    //
    // Yang HILANG saat pindah ke web: tidak ada. SNMP Hioso tak pernah bisa membedakan LOS
    // dari dying-gasp (kodenya sendiri mencatat itu dan selalu memulangkan isDyingGasp=false);
    // pembeda sebenarnya datang dari log web lewat `olt-log-scraper`.
    const sumber = String(
        (global.config && global.config.olt && global.config.olt.sumberOptik) || "web"
    ).toLowerCase();
    const getOltData = opts.getOltData
        || (sumber === "snmp"
            ? (devices) => lazyOltHioso().getMultipleOltData(devices)
            : () => require("./olt-web-optical").getWebOpticalSnapshot({ getDevices }));
