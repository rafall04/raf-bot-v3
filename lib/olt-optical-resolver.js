...
/**
 * Mana yang layak menang bila dua ONU memperebutkan kunci indeks yang sama.
 *
 * !! Satu modem bisa punya DUA catatan di OLT: yang HIDUP di PON sekarang, dan catatan BASI
 * dari PON/OLT lamanya yang tak pernah dihapus setelah modem dipindah. Terukur di produksi
 * Dander: 5 pelanggan, dan EMPAT catatan basinya bertanda **LOS**.
 *
 * Dulu indeks ini memakai `=` polos, jadi yang menang cuma soal urutan array — dan terukur
 * catatan basi menang 5 dari 5. Akibatnya pelanggan yang sehat (rx -13,64 s/d -21,55)
 * dipulangkan sebagai `Offline`/`LOS` dengan `rxPower` kosong ke SEMUA pemakai indeks:
 * cron alert redaman (jadi buta untuk mereka), verifikasi pasca-perbaikan, detail pelanggan,
 * dan perintah Telegram teknisi.
 *
 * @param {object|null} lama kandidat yang sudah menempati kunci
 * @param {object} baru kandidat berikutnya
 * @returns {object} yang layak dipakai
 */
function pilihOnuTerbaik(lama, baru) {
    if (!lama) return baru;
    const hidup = (o) => o && o.status === "Online";
    if (hidup(baru) !== hidup(lama)) return hidup(baru) ? baru : lama;
    // Sama-sama hidup / sama-sama tidak: yang punya angka redaman lebih berguna.
    const punyaRx = (o) => o && o.rxPower !== null && o.rxPower !== undefined && o.rxPower !== "" && o.rxPower !== "N/A";
    if (punyaRx(baru) !== punyaRx(lama)) return punyaRx(baru) ? baru : lama;
    // Benar-benar setara → pertahankan yang pertama supaya hasilnya tidak bergantung urutan.
    return lama;
}

/**
 * Bangun indeks pencarian ONU: MAC-prefix(10 heksa pertama; 2 digit terakhir sengaja
 * diabaikan karena MAC di OLT berbeda di oktet itu), deskripsi(PPPoE, harus mengandung '@')
 * dan serial. Semua key lower-case.
 *
 * Bila dua ONU memperebutkan kunci yang sama, pemenangnya ditentukan `pilihOnuTerbaik`,
 * BUKAN urutan array.
...
function buildOnuIndex(onus, opts = {}) {
    ...
    (Array.isArray(onus) ? onus : []).forEach((onu) => {
        ...
        if (normalizedMac && normalizedMac.length >= 10) {
            const k = normalizedMac.substring(0, 10);
            oltByMac[k] = pilihOnuTerbaik(oltByMac[k], onu);
        }
        if (onu.description && String(onu.description).includes("@")) {
            const k = String(onu.description).trim().toLowerCase();
            oltByPppoe[k] = pilihOnuTerbaik(oltByPppoe[k], onu);
        }
        if (onu.serial) {
            const k = String(onu.serial).trim().toLowerCase();
            oltBySerial[k] = pilihOnuTerbaik(oltBySerial[k], onu);
        }
    });
