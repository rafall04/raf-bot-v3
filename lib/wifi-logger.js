const logger = require('./logger').logger.child('WIFI_LOGGER');
...
const WIFI_CHANGE_SOURCES = new Set(['web_admin', 'web_technician', 'web_customer', 'wa_bot', 'api']);
    ...
    try {
        ...
        try {
            ...
            if (error.code !== 'ENOENT') {
                logger.warn('[WiFi Logger] Error reading existing logs:', error.message);
            }
            logs = [];
        }

        logs.push(logEntry);
        // Retensi: SIMPAN SELAMANYA — log perubahan WiFi tidak pernah dibuang.
        // (Dulu di-cap 1000 entri via slice; itu membuang riwayat lama diam-diam.)

        await fs.writeFile(WIFI_LOGS_FILE, JSON.stringify(logs, null, 2));

        logger.info(`[WiFi Logger] Logged WiFi change for user ${normalizedLogData.userId}, device ${normalizedLogData.deviceId}, type: ${normalizedLogData.changeType}`);
        return logEntry;
    } catch (error) {
        logger.error('[WiFi Logger] Error logging WiFi change:', error);
        throw error;
    }
}
...
async function getWifiChangeLogs(filters = {}) {
    try {
        ...
        try {
            ...
            if (error.code !== 'ENOENT') {
                logger.warn('[WiFi Logger] Error reading logs:', error.message);
            }
            return { logs: [], total: 0 };
        }
        ...
        logger.error('[WiFi Logger] Error getting WiFi logs:', error);
        throw error;
    }
}
...
async function getWifiChangeStats() {
    try {
        ...
        logger.error('[WiFi Logger] Error getting WiFi stats:', error);
        throw error;
    }
}
