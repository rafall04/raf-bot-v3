...
function buildCctvConfigPatch(current, body) {
    ...
    // Jam tenang (tunda WA pelanggan).
    if (b.quietHoursEnabled !== undefined) next.quietHoursEnabled = toBool(b.quietHoursEnabled);
    if (b.quietStart !== undefined) next.quietStart = String(b.quietStart).trim() || '22:00';
    if (b.quietEnd !== undefined) next.quietEnd = String(b.quietEnd).trim() || '06:00';
    // Jam tenang berlaku per-jenis penerima (default ya).
    if (b.quietApplyCustomer !== undefined) next.quietApplyCustomer = toBool(b.quietApplyCustomer);
    if (b.quietApplyCoordinator !== undefined) next.quietApplyCoordinator = toBool(b.quietApplyCoordinator);
    if (b.quietApplyGroup !== undefined) next.quietApplyGroup = toBool(b.quietApplyGroup);
    if (b.aggregateWindowMs !== undefined) {
        const n = parseInt(b.aggregateWindowMs, 10);
        next.aggregateWindowMs = Number.isFinite(n) && n >= 0 ? Math.min(n, 600000) : DEFAULTS.aggregateWindowMs;
    }
    ...
    if (b.messageDownMulti !== undefined) next.messageDownMulti = String(b.messageDownMulti);
    if (b.messageCoordDown !== undefined) next.messageCoordDown = String(b.messageCoordDown);
    if (b.messageGroupDown !== undefined) next.messageGroupDown = String(b.messageGroupDown);
    if (b.messageGroupUp !== undefined) next.messageGroupUp = String(b.messageGroupUp);
...
function toPublicView(cfg) {
    ...
    return {
        ...
        quietHoursEnabled: c.quietHoursEnabled === true,
        quietStart: c.quietStart || '22:00',
        quietEnd: c.quietEnd || '06:00',
        quietApplyCustomer: c.quietApplyCustomer !== false,
        quietApplyCoordinator: c.quietApplyCoordinator !== false,
        quietApplyGroup: c.quietApplyGroup !== false,
        aggregateWindowMs: Number.isFinite(+c.aggregateWindowMs) ? +c.aggregateWindowMs : DEFAULTS.aggregateWindowMs,
        messageDown: effectiveMsg(raw.messageDown, DEFAULTS.messageDown),
        messageUp: effectiveMsg(raw.messageUp, DEFAULTS.messageUp),
        messageDownMulti: effectiveMsg(raw.messageDownMulti, DEFAULTS.messageDownMulti),
        messageCoordDown: effectiveMsg(raw.messageCoordDown, DEFAULTS.messageCoordDown),
        messageGroupDown: effectiveMsg(raw.messageGroupDown, DEFAULTS.messageGroupDown),
        messageGroupUp: effectiveMsg(raw.messageGroupUp, DEFAULTS.messageGroupUp),
