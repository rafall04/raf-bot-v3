...
// PROFIL yang menandakan kredensial ini bukan pelanggan sama sekali: VPN operator & akun
// monitoring. Terukur di produksi — `laptop-aldi` (profil "vpn monitor") kebetulan sedang tak
// tersambung sehingga lolos sebagai "tidak ada sesi", dan tombol hapusnya menyala untuk VPN
// operator sendiri. Status sesi saja tidak cukup: ia hanya bercerita tentang SEKARANG, sedangkan
// profil bercerita tentang PERUNTUKAN. Keduanya dipakai bersama.
const POLA_PROFIL_BUKAN_PELANGGAN = /vpn|monitor/i;

function profilBukanPelanggan(profile) {
    return POLA_PROFIL_BUKAN_PELANGGAN.test(String(profile || ""));
}
...
async function listOrphanSecrets(deps = {}) {
    ...
    const yatim = secrets
        ...
        .map((s) => {
            ...
            const bukanPelanggan = profilBukanPelanggan(s.profile);
            return {
                username: s.name,
                ...
                sedangAktif,
                bukanPelanggan,
                // Saran, BUKAN vonis. Dua syarat harus terpenuhi: tak ada sesi hidup (soal SEKARANG)
                // DAN profilnya bukan VPN/monitoring (soal PERUNTUKAN). Mengandalkan salah satunya
                // saja pernah membuat VPN operator tampil dengan tombol hapus menyala.
                aman_dihapus: sedangAktif === false && !bukanPelanggan,
                catatan: bukanPelanggan
                    ? "BUKAN kredensial pelanggan — profilnya VPN/monitoring. Jangan dihapus dari sini."
                    : (sedangAktif === true
                        ...
                        : (sedangAktif === null
                            ...
                            : "Tidak ada sesi hidup dan profilnya profil pelanggan. Kemungkinan besar memang sisa."))
            };
        })
...
async function removeOrphanSecret(username, deps = {}) {
    ...
    if (!baris.aman_dihapus) {
        return {
            ok: false,
            message: baris.bukanPelanggan
                ? `Tidak jadi dihapus: \`${username}\` berprofil \`${baris.profile}\` — itu VPN/monitoring, bukan kredensial pelanggan. Hapus manual di router kalau memang sudah tak dipakai.`
                : (baris.sedangAktif === true
                    ? `Tidak jadi dihapus: \`${username}\` SEDANG dipakai (ada sesi hidup). Pastikan dulu ini milik apa.`
