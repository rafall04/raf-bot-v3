...
const { getSnmpHealth } = require('./olt-snmp-health');
...
function deriveAlerts(snapshot, thresholds = DEFAULT_THRESHOLDS) {
    ...
    const pe = snapshot.powerEnv;
    if (pe && Array.isArray(pe.activeAlarms)) {
        for (const a of pe.activeAlarms) add('critical', `Alarm power/lingkungan OLT: ${a}`);
    }
    return alerts;
}
...
/**
 * Ambil uplink/L3/VLAN (+ fallback bulk) via SATU sesi SSH read-only. Kembalikan null bila
 * koneksi SSH gagal (mis. ZXAN mengunci port 22 / SSH mati) — caller tetap pakai data SNMP.
 */
async function fetchSshHealth(device) {
    let session = null;
    try {
    ...
    try {
        ...
        const sysRaw = await safeExec(CORE_COMMANDS.system);
        const procRaw = await safeExec(CORE_COMMANDS.processor);
        const fanRaw = await safeExec(CORE_COMMANDS.fan);
        const ipRaw = await safeExec(CORE_COMMANDS.ipIntf);
        const vlanRaw = await safeExec(CORE_COMMANDS.vlan);
...
/**
 * Snapshot kesehatan OLT, di-cache + serial per host. SNMP (bebas-lockout) menjadi sumber utama
 * bulk (identitas/suhu/kipas/kartu/CPU/mem/power-env); SSH melengkapi uplink/L3/VLAN dan jadi
 * fallback bulk bila SNMP mati. Salah satu sumber boleh gagal — snapshot tetap dibangun.
 * @param {object} device {host, snmp*, sshPort, sshUsername, sshPassword}
 * @param {object} [opts] {force=false, ttlMs}
 */
const refreshingHealth = new Set(); // host yang sedang background-refresh (SWR)

function refreshHealth(device, opts = {}) {
    ...
    return withHostLock(host, async () => {
        ...
        // !! SNMP MEMBUAT OLT HANG (#b275). OID di `olt-snmp-health` adalah ZTE enterprise —
        // menembakkannya ke OLT HIOSO tidak pernah menghasilkan apa pun, hanya beban. Karena
        // itu jalur ini dikunci ke merk ZTE saja; merk lain langsung memakai SSH.
        // `brand: "auto"` berarti HIOSO (lihat olt-hioso), jadi TIDAK termasuk.
        const merk = String((device && device.brand) || "").toLowerCase();
        let snmpData = null;
        if (merk === "zte") {
            try {
                snmpData = await getSnmpHealth(device);
            } catch (_e) {
                snmpData = null;
            }
        }
        const ssh = await fetchSshHealth(device);

        if (!snmpData && !ssh) {
            return {
                ok: false,
                fetchedAt: new Date().toISOString(),
                error: 'OLT tak terjangkau via SNMP maupun SSH',
                alerts: []
            };
        }
        ...
        const snapshot = {
            ok: true,
            fetchedAt: new Date().toISOString(),
            source: [snmpData ? 'snmp' : null, ssh ? 'ssh' : null].filter(Boolean).join('+'),
            identity: { ...(sh.identity || {}), ...(sn.identity || {}) },
            temperature: { ...(sh.temperature || {}), ...(sn.temperature || {}) },
            cards: pick(sn.cards, sh.cards) || [],
            processors: pick(sn.processors, sh.processors) || [],
            uplinks: sh.uplinks || [],
            l3: sh.l3 || [],
            vlans: sh.vlans || {},
            powerEnv: sn.powerEnv || null,
...
/**
 * Snapshot kesehatan dengan strategi stale-while-revalidate: kalau ada cache (walau kedaluwarsa)
 * → kembalikan SEKETIKA + refresh di belakang (tak blok UI). `force` / belum-ada-cache → blocking.
 */
function getHealthSnapshot(device, opts = {}) {
    const host = device && device.host;
    const ttl = opts.ttlMs || CACHE_TTL_MS;
    const cached = host && cache.get(host);
    if (opts.force || !cached) return refreshHealth(device, opts);
    const stale = Date.now() - cached.at >= ttl;
    if (stale && host && !refreshingHealth.has(host)) {
        refreshingHealth.add(host);
        Promise.resolve()
            .then(() => refreshHealth(device, opts))
            .catch(() => {})
            .finally(() => refreshingHealth.delete(host));
    }
    return Promise.resolve({ ...cached.snapshot, cached: true, stale });
}
