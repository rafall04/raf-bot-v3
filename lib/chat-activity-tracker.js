 ...
 *          (cooldown anti-spam), (3) kapan pelanggan terakhir mengeluh koneksi (konteks multi-turn
 *          — supaya sebutan app polos susulan "Shopee kak"/"TikTok" dikenali sebagai lanjutan
 *          komplain, pola korpus prod). Murni Map di memori — hilang saat restart itu aman.
 * Caller: `message/raf.js` (noteAdminOutbound pada event fromMe) dan
 *         `message/handlers/customer-fallback-handler.js` (cek umur + catat cooldown + konteks komplain).
...
/** Catat bahwa chat ini baru saja mengeluh koneksi (konteks multi-turn untuk sebutan app susulan). */
function noteConnectivityComplaint(chatJid) {
    note(complaintByChat, chatJid);
}
