...
const FEATURE_FLAGS = [
    ...
    { key: "invoiceOnSettle", path: ["invoiceOnSettle", "enabled"], kategori: "Pembayaran", label: "Invoice PDF di Jalur Settle", desc: "Kirim invoice PDF (bila pelanggan send_invoice ON) saat lunas via konfirmasi bukti foto WA / bayar online — dulu cuma struk teks." },
    ...
    // defaultEnabled:true = gate DEFAULT-AKTIF (nyala walau key absen di config.json prod merge-key).
    // Dulu dibaca cek truthy → 404-senyap saat key tak ada; kini punya toggle web + default aman.
    { key: "voucherSalesDashboard", path: ["voucherSalesDashboard", "enabled"], kategori: "Voucher", label: "Dashboard Penjualan Voucher", desc: "Halaman statistik voucher online terjual (/voucher-sales).", defaultEnabled: true },
    { key: "voucherOrphanWorklist", path: ["voucherOrphanWorklist", "enabled"], kategori: "Voucher", label: "Worklist Voucher Orphan", desc: "Halaman rekonsiliasi voucher gagal terbit / terbit-tanpa-bayar (/voucher-orphans).", defaultEnabled: true },
];
...
function getFlagEnabled(config, flag) {
    ...
    for (const seg of flag.path) {
        if (cur == null || typeof cur !== "object") return flag.defaultEnabled === true;
        cur = cur[seg];
    }
    // Eksplisit true/false menang; ABSEN → default per-flag (deploy-gelap = false; default-aktif = true).
    // Ini yang membunuh kelas bug "404/mati-senyap karena key belum di-merge di config.json prod".
...
/**
 * Cek gate fitur dari registry (SATU cara baca kanonik). Menggantikan 3 idiom tersebar
 * (`===true` / `!==false` / truthy) yang salah-default = bug. Key tak dikenal → lempar (paksa
 * daftar di registry) kecuali `fallback` diberikan eksplisit.
 * @param {string} key
 * @param {{config?:object, fallback?:boolean}} [opts]
 */
function isFeatureEnabled(key, { config = (typeof global !== "undefined" ? global.config : {}), fallback } = {}) {
    const flag = flagByKey(key);
    if (!flag) {
        if (fallback === undefined) throw new Error(`Feature flag tak dikenal: ${key}`);
        return fallback === true;
    }
    return getFlagEnabled(config || {}, flag);
}
...
module.exports = { FEATURE_FLAGS, flagByKey, getFlagEnabled, isFeatureEnabled, readFlags, applyFlag, resyncWorkerForFlag };
