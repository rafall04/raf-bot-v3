...
const log = require('./logger').logger.child('SALDO');
...
function loadAtmFromJson() {
    try {
        ...
        log.error('[SALDO] Error loading atm.json:', error);
        atm = [];
    }
}
...
// unref timer agar tidak menahan event loop di test/CLI script — produksi tetap
// jalan normal karena ada HTTP listener aktif.
const _startupSyncTimer = setTimeout(() => {
    syncAtmFromSqlite().catch(() => {
        // Silent fail - akan di-sync nanti saat database ready
    });
}, 2000);
if (typeof _startupSyncTimer.unref === 'function') {
    _startupSyncTimer.unref();
}
...
const addKoinUser = async (userId, amount) => {
    ...
    if (!amount || amount === 0 || isNaN(amount) || parseInt(amount) <= 0) {
        log.warn(`[SALDO] addKoinUser called with invalid amount: ${amount} for userId: ${userId}. Skipping.`);
        return false;
    }

    // Normalisasi JID ke JID kanonik (resolve @lid via mapping). allowLid:false → kembalikan
    // null bila tak bisa di-resolve, supaya saldo TIDAK nyangkut di key @lid palsu (uang hilang).
    // Lazy-require untuk hindari kemungkinan circular dependency saat load module.
    const { normalizeJidForSaldo } = require('./jid-utils');
    const normalizedUserId = await normalizeJidForSaldo(userId, { allowLid: false });
    if (!normalizedUserId) {
        log.error('[SALDO] addKoinUser: JID tidak bisa di-resolve ke kanonik, saldo TIDAK ditambahkan', { userId, amount });
        return false;
    }
    ...
    const success = await saldoManager.addSaldo(normalizedUserId, amount, 'Topup via payment callback').catch(err => {
        log.error('[SALDO] Error adding saldo:', err);
        return false;
    });
...
const checkATMuser = async (userId) => {
    ...
    try {
        ...
        log.error(`[SALDO] Error:`, error.message);
        return 0;
    }
};
...
const checkRegisteredATM = async (userId) => {
    ...
    try {
        ...
        log.error('[SALDO] Error checkRegisteredATM:', error.message);
        return false;
    }
};
...
const delSaldo = async (nomer) => {
    ...
    try {
        ...
        log.error('[SALDO] Error delSaldo:', error.message);
        return false;
    }
