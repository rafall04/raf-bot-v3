...
// Dicocokkan (substring, case-insensitive) ke: serial number, device id, dan username PPPoE yang
// SEDANG tertulis di modem. Yang terakhir itu kunci modem copotan: modem bekas masih membawa
// username pemilik lama, jadi mengetik "wimpi" menemukannya walau pelanggannya sudah dihapus.
...
async function findPsbCandidatesByHint({ hint, limit = 10, timeoutMs = 30000 } = {}) {
    ...
    const matched = devices.filter((d) => {
        ...
        const pppoe = String(d.currentPPPUsername || "").toLowerCase();
        return sn.includes(q) || id.includes(q) || pppoe.includes(q);
    });
