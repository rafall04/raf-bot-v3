...
async function cancelExpense(id, actor = {}, notes = "") {
    ...
    if (current.source_recurring_id) {
        try {
            await dbRun(
                "UPDATE recurring_expenses SET last_settled_period=NULL, last_reminded_period=NULL, last_action='dibatalkan', updated_at=datetime('now') WHERE id=?",
                [toInteger(current.source_recurring_id)]
            );
        } catch (err) {
