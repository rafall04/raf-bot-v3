 ...
 * Deps: `global.db` (sqlite utama), `./financial-ledger.recordFinancialLedgerEntry`, `./jakarta-date` (filter kalender WIB).
 ...
 */
"use strict";
const log = require('./logger').logger.child('EXPENSE_MANAGER');


const { recordFinancialLedgerEntry } = require("./financial-ledger");
const { jakartaDayRangeIso, jakartaMonthRangeIso } = require("./jakarta-date");
...
async function cancelExpense(id, actor = {}, notes = "") {
    ...
    if (current.source_recurring_id) {
        try {
            ...
            log.warn("[EXPENSE_CANCEL_RECURRING_RESET]", err && err.message ? err.message : err);
        }
    }
...
async function listExpenses(filters = {}) {
    ...
    // expense_date tersimpan ISO UTC; strftime/date() SQLite membaca kalender UTC
    // sementara derivePeriod & UI berpikir dalam WIB → pengeluaran WIB 00:00–06:59
    // terbucket sehari/bulan lebih awal. Bandingkan sebagai instant: konversi batas
    // kalender Jakarta ke ISO UTC di sini.
    if (filters.month && filters.year) {
        const [startIso, endIso] = jakartaMonthRangeIso(toInteger(filters.year), toInteger(filters.month));
        where.push("expense_date >= ? AND expense_date < ?");
        params.push(startIso, endIso);
    } else if (filters.year) {
        const [startIso] = jakartaMonthRangeIso(toInteger(filters.year), 1);
        const [, endIso] = jakartaMonthRangeIso(toInteger(filters.year) + 1, 1);
        where.push("expense_date >= ? AND expense_date < ?");
        params.push(startIso, endIso);
    }
    ...
    if (filters.dateFrom) {
        const [startIso] = jakartaDayRangeIso(String(filters.dateFrom));
        where.push("expense_date >= ?");
        params.push(startIso);
    }
    if (filters.dateTo) {
        const [, endIso] = jakartaDayRangeIso(String(filters.dateTo));
        where.push("expense_date < ?");
        params.push(endIso);
    }
    if (filters.category) {
