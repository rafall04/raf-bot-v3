...
const log = require('./logger').logger.child('WHATSAPP_NOTIFICATION_WRAPPER');
const { 
...
function initializeWrapper(raf) {
    if (!raf || !raf.sendMessage) {
        log.warn('[NOTIF] Cannot initialize - raf.sendMessage not available');
        return false;
    }
    ...
    raf.sendMessage = async function(jid, message, options) {
        try {
            ...
            if (!skipDuplicateCheck && isNotificationDuplicate(jid, message)) {
                ...
                log.info(`[NOTIF_DUPLICATE_BLOCKED] #${blockedCount} to ${normalized}`);
                log.info(`[NOTIF_DUPLICATE_BLOCKED] Message: ${message.text?.substring(0, 50)}...`);
            ...
            if (!isReady()) {
                ...
                log.warn(`[NOTIF_WRAPPER] WhatsApp not connected (state: ${runtimeState}), skipping send to ${jid}`);
                throw new Error(`WhatsApp connection not ready (state: ${runtimeState})`);
            }
            
            // Penanda "sudah dikirim" dipasang SETELAH pengiriman benar-benar berhasil (lihat
            // di bawah). Dulu dipasang di sini — SEBELUM kirim — sehingga bila pengiriman gagal
            // transient lalu pemanggil mencoba ulang dalam TTL cache (10 dtk), percobaan kedua
            // justru dikenali sebagai DUPLIKAT dan dijawab sukses-palsu: pesan tak pernah keluar,
            // tapi pemanggil yakin sudah terkirim.
            ...
            for (let attempt = 0; attempt <= maxRetries; attempt++) {
                try {
                    if (attempt > 0) {
                        ...
                        log.info(`[NOTIF_WRAPPER] Retry attempt ${attempt} after ${delay}ms delay`);
                        await new Promise(resolve => setTimeout(resolve, delay));
                    }
                    
                    const sendResult = await originalSendMessage(jid, message, options);
                    // Baru sekarang ditandai terkirim — bukti kirim sudah ada di tangan.
                    ...
                    if (errorMsg.includes('attrs') || errorMsg.includes('USyncQuery') || errorMsg.includes('parseUSyncQueryResult')) {
                        log.warn(`[NOTIF_WRAPPER] USync error on attempt ${attempt + 1}/${maxRetries + 1}:`, errorMsg);
                        
                        // If last attempt, don't retry
                        if (attempt >= maxRetries) {
                            log.error('[NOTIF_WRAPPER] Max retries reached for USync error, giving up');
                            break;
                        }
            ...
            if (errorMsg.includes('connection not ready') || 
                ...
                log.error('[NOTIF_WRAPPER] Non-retryable error:', errorMsg);
            ...
            log.error('[NOTIF_WRAPPER] Error in wrapped sendMessage:', error);
            throw error;
        }
    };
...
function removeWrapper(raf) {
    if (!wrapperActive || !originalSendMessage) {
        log.info('[NOTIF_WRAPPER] Not active, nothing to remove');
        return false;
    }
    ...
    log.info('[NOTIF_WRAPPER] Removed wrapper, using original sendMessage');
    return true;
}
