...
function createBroadcastLedger(overrides = {}) {
    ...
    async function ensureSchema(db) {
        ...
        await run(db, `CREATE TABLE IF NOT EXISTS broadcast_ledger (
            ...
            blocked_reason TEXT,
            updated_at INTEGER)`);
        // Cache status onWhatsApp (idempoten — abaikan error 'duplicate column' bila tabel sudah ada).
        for (const col of ["wa_registered INTEGER", "wa_checked_at INTEGER"]) {
            try { await run(db, `ALTER TABLE broadcast_ledger ADD COLUMN ${col}`); } catch (_e) { /* kolom sudah ada */ }
        }
    }
    ...
    return {
        ...
        // ── Cache status onWhatsApp (hemat query berulang; nomor terdaftar tetap terdaftar) ──
        async getWaStatus(jid) {
            if (!jid) return null;
            try {
                const db = await getDb();
                const row = await get(db, "SELECT wa_registered, wa_checked_at FROM broadcast_ledger WHERE jid=?", [String(jid)]);
                if (!row || row.wa_registered === null || row.wa_registered === undefined) return null;
                return { registered: row.wa_registered === 1, checkedAt: row.wa_checked_at || 0 };
            } catch (_e) { return null; }
        },

        async setWaStatus(jid, registered) {
            if (!jid) return;
            try {
                const db = await getDb();
                const ts = now();
                await run(db,
                    `INSERT INTO broadcast_ledger (jid, wa_registered, wa_checked_at, send_count, updated_at)
                     VALUES (?,?,?,0,?)
                     ON CONFLICT(jid) DO UPDATE SET wa_registered=excluded.wa_registered, wa_checked_at=excluded.wa_checked_at, updated_at=excluded.updated_at`,
                    [String(jid), registered ? 1 : 0, ts, ts]);
            } catch (_e) { /* fail-open */ }
        },
