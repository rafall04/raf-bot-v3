 ...
 */
"use strict";
const log = require('./logger').logger.child('CUSTOMER_STEERING_SERVICE');
...
function readIntents() {
    try {
        ...
        log.warn(`[CustSteer] Gagal baca intent: ${err.message}`);
    }
    return [];
}
...
function writeIntents(list) {
    try {
        ...
        log.warn(`[CustSteer] Gagal tulis intent: ${err.message}`);
    }
}
...
async function reconcileOnce(depsOverride = {}) {
    ...
    try {
        ...
        log.warn(`[CustSteer] Rekonsiliasi gagal: ${err.message}`);
...
function startCustomerSteeringReconciler() {
    const cfg = getSteeringConfig();
    if (!cfg.enabled) {
        log.info("[CustSteer] Nonaktif (set config.customerSteering.enabled=true untuk mengaktifkan)");
        return;
    }
    if (!cfg.valid) {
        log.warn("[CustSteer] Aktif tapi kredensial router kosong — reconciler tidak dijalankan.");
        return;
    }
    if (reconcilerTimer) return;
    log.info(`[CustSteer] Reconciler start (interval ${Math.round(cfg.reconcileIntervalMs / 1000)}s)`);
