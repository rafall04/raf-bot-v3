...
const log = require('./logger').logger.child('UPLOAD_HELPER');
const fs = require('fs');
...
function getUploadDir(category, date = new Date()) {
    ...
    if (!fs.existsSync(uploadDir)) {
        ...
        log.info(`[UPLOAD] Created directory: uploads/${category}/${yearMonth}/`);
    }
...
function initializeUploadDirs() {
    ...
    log.info('[UPLOAD] Upload directories initialized');
}
        ...
        months.forEach(month => {
            ...
            if (month < cutoffYearMonth) {
                ...
                try {
                    ...
                    log.info(`[UPLOAD] Deleted old uploads: ${category}/${month}/ (${files.length} files)`);
                } catch (err) {
                    log.error(`[UPLOAD] Error deleting ${month}:`, err.message);
                    errors++;
                }
            }
        });
    } catch (err) {
        log.error(`[UPLOAD] Error cleaning up ${category}:`, err.message);
...
function getUploadStats(category = null) {
    ...
    categories.forEach(cat => {
        ...
        try {
            ...
            log.error(`[UPLOAD] Error getting stats for ${cat}:`, err.message);
        }
