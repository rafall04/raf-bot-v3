 ...
 */
const log = require('./logger').logger.child('PHONE_VALIDATOR_INTERNATIONAL');
...
function detectCountry(phone) {
    ...
    if (matches.length > 1) {
        ...
        log.warn(`[PHONE_VALIDATOR] Ambiguous phone number detected: ${cleaned}. Multiple countries matched: ${matches.map(m => m.country).join(', ')}. Selected: ${matches[0].country}`);
        ...
        if (defaultConfig) {
            ...
            if (matchesDefault) {
                ...
                if (alsoMatchesDetected && country !== defaultCountry && country !== 'INTERNATIONAL') {
                    ...
                    log.warn(`[PHONE_VALIDATOR] Phone ${cleaned} matches both ${country} and ${defaultCountry}. Using default: ${defaultCountry}`);
...
async function checkPhoneExists(db, phoneNumber, excludeUserId = null) {
    return new Promise((resolve, reject) => {
        ...
        db.get(query, params, (err, row) => {
            if (err) {
                log.error('[PHONE_VALIDATOR] Error checking phone:', err);
                reject(err);
            } else {
                resolve(row || null);
            }
...
async function validatePhoneNumbers(db, phoneNumbers, excludeUserId = null, defaultCountry = 'ID') {
    ...
    for (const phone of phones) {
        ...
        if (expectedCountryCode && countryCode !== expectedCountryCode && !warning) {
            ...
            log.warn(`[PHONE_VALIDATOR] Phone ${phone} detected as ${validation.country} but normalized to ${countryCode} (expected ${expectedCountryCode})`);
        }
    ...
    for (const phoneDetail of phoneDetails) {
        try {
            ...
            log.error('[PHONE_VALIDATOR] Error validating phone:', phoneDetail.original, error);
            return {
                valid: false,
