    ...
    return new Promise((resolve, reject) => {
        ...
        psbDb = new sqlite3.Database(psbDbPath, (err) => {
            ...
            applySqlitePragmas(psbDb).catch((pragmaErr) => {
                console.warn(`[PSB_DB_PRAGMA_WARN] ${pragmaErr.message}`);
            });
    ...
    return new Promise((resolve, reject) => {
        ...
        global.db.all('SELECT id FROM users ORDER BY id ASC', [], (err, dbRows) => {
            ...
            collectHistoricalUserIds((histIds) => {
                ...
                if (DEBUG) {
                    console.log(`[GET_NEXT_ID] users:${dbIds.length} mem:${memoryIds.length} finansial:${histIds.length}`);
                }
                if (allIds.length === 0) {
                    if (DEBUG) console.log('[GET_NEXT_ID] Tak ada id sama sekali, mulai dari 1');
                    resolve(1);
                    return;
                }
                // MAX+1 lintas semua sumber (users + memori + jejak finansial) — tak pernah dipakai ulang.
                const nextId = Math.max(...allIds) + 1;
                if (DEBUG) console.log(`[GET_NEXT_ID] MAX+1 (anti reuse) → ${nextId}`);
                resolve(nextId);
            });
        });
    });
