...
const log = require('./logger').logger.child('PSB_DATABASE');
...
function initializePSBDatabase() {
    return new Promise((resolve, reject) => {
        ...
        if (!psbDbPath.includes(path.sep + 'database' + path.sep) && !psbDbPath.includes('/database/')) {
            log.warn(`[PSB_DB_INIT] WARNING: PSB database path is not in database/ folder: ${psbDbPath}`);
        }
        
        psbDb = new sqlite3.Database(psbDbPath, (err) => {
            if (err) {
                log.error('[PSB_DB_INIT] Error opening PSB database:', err.message);
                return reject(err);
            }

            // PSB database connected (silent)
            applySqlitePragmas(psbDb).catch((pragmaErr) => {
                log.warn(`[PSB_DB_PRAGMA_WARN] ${pragmaErr.message}`);
            });
            ...
            psbDb.run(createTableSql, (err) => {
                if (err) {
                    log.error('[PSB_DB_INIT] Error creating psb_records table:', err.message);
                    return reject(err);
                }
...
function loadPSBRecords() {
    return new Promise((resolve, reject) => {
        ...
        psbDb.all('SELECT * FROM psb_records ORDER BY id ASC', [], (err, rows) => {
            if (err) {
                log.error('[PSB_DB_LOAD] Error loading PSB records:', err.message);
                return reject(err);
            }
            ...
            const transformedRecords = rows.map(record => ({
                ...
                psb_data: (() => {
                    try {
                        ...
                        log.error(`[PSB_DB_LOAD] Failed to parse psb_data for record ${record.id}:`, e.message);
                        return null;
                    }
                })()
            }));
...
function insertPSBRecord(record) {
    return new Promise((resolve, reject) => {
        ...
        psbDb.run(insertSql, [
            ...
            if (err) {
                log.error('[PSB_DB_INSERT] Error inserting PSB record:', err.message);
                return reject(err);
            }
            
            if (DEBUG) log.info(`[PSB_DB_INSERT] PSB record ${record.id} inserted successfully`);
            resolve(this.lastID);
        });
    });
}
...
function updatePSBRecord(id, updates) {
    return new Promise((resolve, reject) => {
        ...
        psbDb.run(updateSql, updateValues, function(err) {
            if (err) {
                log.error('[PSB_DB_UPDATE] Error updating PSB record:', err.message);
                return reject(err);
            }
            
            if (DEBUG) log.info(`[PSB_DB_UPDATE] PSB record ${id} updated successfully`);
    ...
    return new Promise((resolve, reject) => {
        ...
        psbDb.get('SELECT * FROM psb_records WHERE id = ?', [id], (err, row) => {
            ...
            try {
                ...
                log.error(`[PSB_DB_GET] Failed to parse psb_data for record ${id}:`, e.message);
                row.psb_data = null;
            }
        ...
        psbDb.all('SELECT * FROM psb_records WHERE psb_status = ? ORDER BY id DESC', [status], (err, rows) => {
            ...
            const transformedRecords = rows.map(record => {
                try {
                    ...
                    log.error(`[PSB_DB_GET_STATUS] Failed to parse psb_data for record ${record.id}:`, e.message);
...
function deletePSBRecord(id) {
    return new Promise((resolve, reject) => {
        ...
        psbDb.run('DELETE FROM psb_records WHERE id = ?', [id], function(err) {
            ...
            if (DEBUG) log.info(`[PSB_DB_DELETE] PSB record ${id} deleted`);
            resolve(this.changes);
        });
    });
}
...
function getNextAvailableUserId() {
    return new Promise((resolve, reject) => {
        ...
        global.db.all('SELECT id FROM users ORDER BY id ASC', [], (err, dbRows) => {
            if (err) {
                log.error('[GET_NEXT_ID_ERROR] Database query failed:', err);
                reject(err);
                return;
            }
            ...
            // #b327: JANGAN pakai-ulang id yang masih punya JEJAK FINANSIAL. deleteUserById hanya
            // menghapus baris users (tak membersihkan payment_history/reversals/waivers ber-key user_id),
            // jadi id yang di-isi-gap mewarisi status lunas/tunggakan pelanggan LAMA → ISP berhenti
            // menagih / salah tagih + invoice pelanggan lama bocor ke pelanggan baru. Kumpulkan id
            // historis dari tabel finansial, lalu alokasikan MAX+1 (bukan isi-gap) supaya satu id tak
            // pernah menunjuk dua identitas berbeda.
            collectHistoricalUserIds((histIds) => {
                const allIds = [...new Set([...dbIds, ...memoryIds, ...histIds])];
                if (DEBUG) {
                    log.info(`[GET_NEXT_ID] users:${dbIds.length} mem:${memoryIds.length} finansial:${histIds.length}`);
                }
                if (allIds.length === 0) {
                    if (DEBUG) log.info('[GET_NEXT_ID] Tak ada id sama sekali, mulai dari 1');
                    resolve(1);
                    return;
                }
                // MAX+1 lintas semua sumber (users + memori + jejak finansial) — tak pernah dipakai ulang.
                const nextId = Math.max(...allIds) + 1;
                if (DEBUG) log.info(`[GET_NEXT_ID] MAX+1 (anti reuse) → ${nextId}`);
                resolve(nextId);
            });
        });
    });
...
// #b327: kumpulkan user_id yang PERNAH punya jejak finansial (payment_history/reversals/waivers) supaya
// getNextAvailableUserId tak pernah mengalokasikan ulang id yang masih mewarisi data pelanggan lama.
// Tabel dari allowlist tetap (aman dari injeksi); sebagian bisa belum ada (fresh install) → pindai
// sqlite_master dulu. Error apa pun → [] (fail-safe: minimal tetap MAX+1 atas users+memori).
function collectHistoricalUserIds(cb) {
    if (!global.db) return cb([]);
    const kandidat = ['payment_history', 'payment_reversals', 'payment_waivers'];
    global.db.all(
        `SELECT name FROM sqlite_master WHERE type='table' AND name IN (${kandidat.map(() => '?').join(',')})`,
        kandidat,
        (e, rows) => {
            if (e || !rows || rows.length === 0) return cb([]);
            const tabel = rows.map((r) => r.name);
            const unionSql = tabel.map((t) => `SELECT DISTINCT user_id AS id FROM ${t} WHERE user_id IS NOT NULL`).join(' UNION ');
            global.db.all(unionSql, [], (e2, idRows) => {
                if (e2 || !idRows) return cb([]);
                cb(idRows.map((r) => parseInt(r.id)).filter((id) => !isNaN(id)));
            });
        }
    );
}
...
function getNextAvailablePSBId() {
    return new Promise((resolve, reject) => {
        ...
        psbDb.all('SELECT id FROM psb_records ORDER BY id ASC', [], (err, psbDbRows) => {
            if (err) {
                log.error('[GET_NEXT_PSB_ID_ERROR] PSB database query failed:', err);
                return reject(err);
            }
            ...
            global.db.all('SELECT id FROM users ORDER BY id ASC', [], (err, usersDbRows) => {
                if (err) {
                    log.error('[GET_NEXT_PSB_ID_ERROR] Users database query failed:', err);
                    return reject(err);
                }
                ...
                if (DEBUG) {
                    log.info(`[GET_NEXT_PSB_ID] Found ${psbDbIds.length} IDs in PSB database, ${psbMemoryIds.length} in PSB memory`);
                    log.info(`[GET_NEXT_PSB_ID] Found ${usersDbIds.length} IDs in users database, ${usersMemoryIds.length} in users memory`);
                    log.info(`[GET_NEXT_PSB_ID] All existing IDs:`, allIds.length > 0 ? `[${allIds.slice(0, 10).join(', ')}${allIds.length > 10 ? '...' : ''}]` : '[]');
                }
                
                // If no records at all, start with ID 1
                if (allIds.length === 0) {
                    if (DEBUG) log.info('[GET_NEXT_PSB_ID] No existing records, starting with ID 1');
                    resolve(1);
                    return;
                }
                ...
                for (const id of allIds) {
                    if (id > expectedId) {
                        ...
                        if (DEBUG) log.info(`[GET_NEXT_PSB_ID] Found gap at ID ${expectedId} (before ${id})`);
                        resolve(expectedId);
                        return;
                    }
                    expectedId = id + 1;
                ...
                const nextId = Math.max(...allIds) + 1;
                if (DEBUG) log.info(`[GET_NEXT_PSB_ID] No gaps found, using next sequential ID: ${nextId}`);
                resolve(nextId);
            });
        });
    });
        ...
        try {
            ...
            const idExists = await new Promise((resolveCheck, rejectCheck) => {
                global.db.get('SELECT id FROM users WHERE id = ?', [newUserId], (err, row) => {
                    if (err) {
                        log.error('[PSB_DB_MOVE] Error checking ID existence:', err);
                        rejectCheck(err);
                        return;
                    }
                    resolveCheck(!!row);
            ...
            if (idExists) {
                log.warn(`[PSB_DB_MOVE] ID ${newUserId} already exists in database! Getting new ID...`);
                ...
                newUserId = await new Promise((resolveMax, rejectMax) => {
                    global.db.get('SELECT MAX(id) as maxId FROM users', [], (err, row) => {
                        ...
                        if (DEBUG) log.info(`[PSB_DB_MOVE] Using fallback MAX+1 ID: ${nextId}`);
                        resolveMax(nextId);
                    });
                });
            }
            
            if (DEBUG) log.info(`[PSB_DB_MOVE] PSB record ID ${psbRecord.id} (temporary) will be assigned new user ID ${newUserId} (final)`);
            ...
            try {
                ...
                log.warn(`[PSB_DB_MOVE] Failed to parse SSID indices from psb_data: ${e.message}`);
            }
            ...
            global.db.run(insertUserSql, [
                ...
                if (insertErr) {
                    ...
                    if (insertErr.message && insertErr.message.includes('UNIQUE constraint failed')) {
                        log.error(`[PSB_DB_MOVE] DUPLICATE ID ERROR: ID ${newUserId} already exists! This should not happen.`);
                        log.error('[PSB_DB_MOVE] Error details:', insertErr.message);
                        return reject(new Error(`ID ${newUserId} sudah ada di database. Silakan coba lagi.`));
                    }
                    log.error('[PSB_DB_MOVE] Error inserting user:', insertErr.message);
                    return reject(insertErr);
                }
                
                if (DEBUG) log.info(`[PSB_DB_MOVE] PSB record ${psbRecord.id} (temporary) moved to users table with new ID ${newUserId} (final)`);
                
                // Return the new user ID
                resolve(newUserId);
            });
        } catch (error) {
            log.error('[PSB_DB_MOVE] Error getting next available ID:', error);
            reject(error);
        }
    });
}
