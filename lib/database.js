 ...
 */
"use strict";
const log = require('./logger').logger.child('DATABASE');
...
function setupAnnouncementsAndNewsWatchers() {
        ...
        setTimeout(() => {
            try {
                ...
                if (globalVarName === 'announcements') {
                    ...
                    log.info(`[Database] ✅ Reloaded ${Array.isArray(data) ? data.length : 0} announcements.`);
                } else if (globalVarName === 'news') {
                    global.news = data;
                    log.info(`[Database] ✅ Reloaded ${Array.isArray(data) ? data.length : 0} news items.`);
                }
            } catch (error) {
                // Jika error dan masih ada retry, coba lagi
                if (retries > 0) {
                    log.warn(`[Database] ⚠️ Error reloading ${globalVarName}.json (retry ${4 - retries}/3):`, error.message);
                    reloadFileWithRetry(filePath, globalVarName, retries - 1, delay * 2); // Exponential backoff
                } else {
                    log.error(`[Database] ❌ Error reloading ${globalVarName}.json after 3 retries:`, error.message);
                }
            }
        }, delay);
    }
    ...
    if (fs.existsSync(announcementsPath)) {
        ...
        fs.watchFile(announcementsPath, { interval: 1000 }, (curr, prev) => {
            ...
            if (currMtime !== prevMtime && currMtime !== lastMtime) {
                lastMtime = currMtime;
                log.info('[Database] 🔄 announcements.json changed');
    ...
    if (fs.existsSync(newsPath)) {
        ...
        fs.watchFile(newsPath, { interval: 1000 }, (curr, prev) => {
            ...
            if (currMtime !== prevMtime && currMtime !== lastMtime) {
                lastMtime = currMtime;
                log.info('[Database] 🔄 news.json changed');
    ...
    if (dbWatchersCount > 0) {
        log.info(`[Database] ✅ ${dbWatchersCount} file watcher(s) aktif`);
    }
}
...
function seedDefaultAdmin() {
    ...
    log.warn(line);
    log.warn('[SEED_ADMIN] Tidak ada akun -> akun default dibuat:');
    log.warn('[SEED_ADMIN]   username: admin');
    log.warn('[SEED_ADMIN]   password: admin123');
    log.warn('[SEED_ADMIN] >> GANTI password ini segera setelah login pertama!');
    log.warn(line);
}
...
function initializeDatabase() {
    return new Promise((resolve, reject) => {
        ...
        if (fs.existsSync(oldDbPath) && !fs.existsSync(dbPath)) {
            log.warn('[DB_INIT] Old database.sqlite found. Please run migration script to migrate to users.sqlite');
            log.warn(`[DB_INIT] Old path: ${oldDbPath}`);
            log.warn(`[DB_INIT] New path: ${dbPath}`);
        }
        
        const dbDir = path.dirname(dbPath);
        if (!fs.existsSync(dbDir)) {
            fs.mkdirSync(dbDir, { recursive: true });
            log.info("[DB_INIT] Created database directory:", dbDir);
        }
        
        if (!dbPath.includes(path.sep + 'database' + path.sep) && !dbPath.includes('/database/')) {
            log.warn(`[DB_INIT] WARNING: Database path is not in database/ folder: ${dbPath}`);
        }
        ...
        const db = new sqlite3.Database(dbPath, (err) => {
            ...
            applySqlitePragmas(db).catch((pragmaErr) => {
                log.warn(`[DB_INIT_PRAGMA_WARN] ${pragmaErr.message}`);
            });

            // SATU SUMBER trigger jaring-pengaman timestamp users. WAJIB dipasang di DUA jalur:
            // (a) saat tabel baru dibuat, DAN (b) saat tabel SUDAH ADA (kasus PROD) — karena blok
            // CREATE TABLE di bawah hanya berjalan bila tabel belum ada (`if (!row)` lalu `return`).
            // Tanpa (b), DB prod lama tak akan pernah dapat trigger-nya.
            const USERS_TIMESTAMP_TRIGGER_SQL = `CREATE TRIGGER IF NOT EXISTS trg_users_stamp_timestamps
                AFTER INSERT ON users
                FOR EACH ROW
                WHEN (NEW.created_at IS NULL OR NEW.created_at = '')
                   OR (NEW.updated_at IS NULL OR NEW.updated_at = '')
                BEGIN
                   UPDATE users
            ...
            db.serialize(() => {
                db.get("SELECT name FROM sqlite_master WHERE type='table' AND name='users'", (err, row) => {
                    ...
                    if (!row) {
                        log.info('[DB] Creating users table...');
                        ...
                        const createIndexesSql = [
                            ...
                            // Jaring pengaman timestamp (definisi tunggal di atas) — jalur "tabel baru".
                            USERS_TIMESTAMP_TRIGGER_SQL
                        ];
                        
                        db.run(createTableSql, (createErr) => {
                            if (createErr) {
                                log.error('[DB_ERROR] Gagal membuat tabel users:', createErr.message);
                                return reject(createErr);
                            }
                            
                            log.info('[DB_INIT] Tabel users berhasil dibuat.');
                            
                            let currentIndex = 0;
                            
                            function createNextIndex() {
                                if (currentIndex >= createIndexesSql.length) {
                                    log.info(`[DB_INIT] Semua index berhasil dibuat.`);
                                    loadUsersData();
                                    return;
                                }
                                ...
                                db.run(indexSql, (indexErr) => {
                                    if (indexErr) {
                                        log.warn(`[DB_WARN] Gagal membuat index ${currentIndex + 1}:`, indexErr.message);
                                    } else {
                                        log.info(`[DB_INIT] Index ${currentIndex + 1}/${createIndexesSql.length} berhasil dibuat.`);
                                    }
                    ...
                    // Tabel users SUDAH ADA (kasus PROD) — blok CREATE TABLE di atas dilewati, jadi
                    // trigger jaring-pengaman dipasang DI SINI. Idempoten (IF NOT EXISTS) → aman tiap
                    // startup. Gagal pasang TIDAK boleh menjatuhkan boot (log saja, lanjut load users).
                    db.run(USERS_TIMESTAMP_TRIGGER_SQL, (triggerErr) => {
                        if (triggerErr) {
                            log.warn('[DB_WARN] Gagal memasang trigger timestamp users:', triggerErr.message);
                        } else {
                            log.info('[DB_INIT] Trigger timestamp users siap (jaring pengaman created_at).');
                        }
                        loadUsersData();
                    });
                });
                ...
                function loadUsersData() {
                    // Jamin kolom `billing_cycle` ada SEBELUM SELECT (siklus tagihan akhir-bulan,
                    // opt-in per pelanggan). DB prod dibuat sebelum kolom ini ada; CREATE TABLE
                    // IF NOT EXISTS tak menambah kolom ke tabel lama, jadi ALTER idempoten di sini.
                    // Gagal ALTER TIDAK boleh menjatuhkan boot (log saja) — sama seperti trigger
                    // timestamp di atas. Guard drift skema di api-users.repository me-SKIP kolom
                    // yang belum ada, jadi tanpa ALTER ini nilai `billing_cycle` tak akan persist.
                    db.all("PRAGMA table_info(users)", [], (pragmaErr, cols) => {
                        const punyaKolom = Array.isArray(cols) && cols.some((c) => c.name === 'billing_cycle');
                        const lanjut = () => db.all('SELECT * FROM users', [], onUsersLoaded);
                        if (pragmaErr || punyaKolom) {
                            if (pragmaErr) log.warn('[DB_WARN] PRAGMA table_info(users) gagal, lewati ensure billing_cycle:', pragmaErr.message);
                            return lanjut();
                        }
                        db.run("ALTER TABLE users ADD COLUMN billing_cycle TEXT DEFAULT 'awal_bulan'", (alterErr) => {
                            if (alterErr) log.warn('[DB_WARN] Gagal menambah kolom users.billing_cycle:', alterErr.message);
                            else log.info("[DB_INIT] Kolom users.billing_cycle siap (siklus tagihan akhir-bulan).");
                            lanjut();
                        });
                    });

                    function onUsersLoaded(err, rows) {
                        if (err) {
                            log.error('[DB_ERROR] Error loading users from database:', err.message);
                            log.error('[DB_ERROR] Error stack:', err.stack);
                            return reject(err);
                        }
                        ...
                        if (rows && rows.length > 0) {
                            rows.forEach((user, index) => {
                                try {
                                            ...
                                            try {
                                                ...
                                                if (typeof user.bulk === 'string') {
                                                    ...
                                                    if (trimmed === '[object Object]' || trimmed.startsWith('[object')) {
                                                        log.warn(`[DB_WARNING] Corrupted bulk data for user ${user.id}: "${trimmed}", resetting to default`);
                                                        return [];
                                                    }
                                                ...
                                                log.warn(`[DB_WARNING] Failed to parse bulk for user ${user.id}:`, e.message);
                                                return [];
                                            }
                                        })(),
                                    ...
                                    transformErrorCount++;
                                    log.error(`[DB_ERROR] Failed to transform user at index ${index} (ID: ${user.id}, Name: ${user.name}):`, transformErr.message);
                                    log.error(`[DB_ERROR] Raw user data:`, JSON.stringify(user, null, 2));
                                    try {
                                        ...
                                        log.info(`[DB_WARNING] Added user ${user.id} with minimal transformation after error`);
                                    } catch (minimalErr) {
                                        log.error(`[DB_ERROR] CRITICAL: Cannot add user ${user.id} even with minimal transformation:`, minimalErr.message);
                                    }
                                }
                            });
                        }
                        ...
                        if (transformErrorCount > 0 || global.users.length !== (rows ? rows.length : 0)) {
                            log.info(`[DB] Loaded ${global.users.length} users (${transformErrorCount} errors)`);
                        }
                        
                        if (transformErrorCount > 0) {
                            log.warn(`[DB_WARNING] ${transformErrorCount} user(s) had transformation errors but were still added to memory`);
                        }
                        
                        if (global.users.length === 0 && rows && rows.length > 0) {
                            log.error('[DB_ERROR] CRITICAL: Rows found in database but transformation resulted in 0 users!');
                            log.error('[DB_ERROR] Sample row:', JSON.stringify(rows[0], null, 2));
                        }
                        
                        if (global.users.length < rows.length) {
                            const missingCount = rows.length - global.users.length;
                            log.error(`[DB_ERROR] CRITICAL: ${missingCount} user(s) from database were NOT transformed!`);
                            log.error(`[DB_ERROR] Database has ${rows.length} rows but only ${global.users.length} users in memory`);
                            const loadedIds = new Set(global.users.map(u => String(u.id)));
                            const missingUsers = rows.filter(u => !loadedIds.has(String(u.id)));
                            if (missingUsers.length > 0) {
                                log.error(`[DB_ERROR] Sample missing users:`, missingUsers.slice(0, 5).map(u => ({ id: u.id, name: u.name, phone_number: u.phone_number })));
                            }
                        }
                        ...
                        try {
                            const backfillPromise = require('./payment-status-backfill').runStartupBackfillSafe();
                            // Setelah backfill migrasi (idempoten via marker), SAMAKAN flag users.paid ke
                            // ledger periode berjalan. Ini menyembuhkan flag basi bila reset bulanan
                            // (cron set-unpaid, tgl 1) terlewat karena bot mati saat itu — akar bug
                            // "Broadcast Tagihan/reminder/isolir pakai status bulan lalu". Best-effort,
                            // tak memblokir boot, READ-ONLY terhadap ledger (hanya set flag).
                            Promise.resolve(backfillPromise).finally(() => {
                                try {
                                    require('./paid-flag-reconcile').runStartupPaidReconcileSafe();
                                } catch (reconcileErr) {
                                    log.error('[PAID_RECONCILE_HOOK_ERROR]', reconcileErr.message);
                                }
                            });
                        } catch (backfillErr) {
                            log.error('[PAID_LEDGER_BACKFILL_HOOK_ERROR]', backfillErr.message);
                        }

                        const { initializeActivityLogTables } = require('./activity-logger');
                        initializeActivityLogTables().catch(err => {
                            log.error('[DB] Activity logging init failed:', err.message);
                        });

                        // Pemakaian port DITURUNKAN dari data (ODP=jumlah pelanggan, ODC=jumlah ODP anak),
                        // tak pernah di-increment. Satu implementasi dipakai boot & seluruh jalur mutasi
                        // → angka menyembuhkan diri sendiri. Lihat lib/network-assets-service.js.
                        require('./network-assets-service').recomputePortUsage(global.networkAssets, global.users);
                        ...
                        initializePSBDatabase()
                            ...
                            .catch(err => {
                                log.error("[DB] PSB/Waypoints init error:", err.message);
                                resolve(db);
                            });
                    }
                }
