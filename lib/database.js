            ...
            db.serialize(() => {
                db.get("SELECT name FROM sqlite_master WHERE type='table' AND name='users'", (err, row) => {
                    ...
                    db.run(USERS_TIMESTAMP_TRIGGER_SQL, (triggerErr) => {
                        if (triggerErr) {
                            console.warn('[DB_WARN] Gagal memasang trigger timestamp users:', triggerErr.message);
                        } else {
                            console.log('[DB_INIT] Trigger timestamp users siap (jaring pengaman created_at).');
                        }
                        loadUsersData();
                    });
                });
                ...
                function loadUsersData() {
                    ...
                    db.all("PRAGMA table_info(users)", [], (pragmaErr, cols) => {
                        ...
                        if (pragmaErr || punyaKolom) {
                            if (pragmaErr) console.warn('[DB_WARN] PRAGMA table_info(users) gagal, lewati ensure billing_cycle:', pragmaErr.message);
                            return lanjut();
                        }
                        db.run("ALTER TABLE users ADD COLUMN billing_cycle TEXT DEFAULT 'awal_bulan'", (alterErr) => {
                            if (alterErr) console.warn('[DB_WARN] Gagal menambah kolom users.billing_cycle:', alterErr.message);
                            else console.log("[DB_INIT] Kolom users.billing_cycle siap (siklus tagihan akhir-bulan).");
                            lanjut();
                        });
                    });
                        ...
                        try {
                            ...
                            Promise.resolve(backfillPromise).finally(() => {
                                try {
                                    ...
                                    console.error('[PAID_RECONCILE_HOOK_ERROR]', reconcileErr.message);
                                }
                            });
                        } catch (backfillErr) {
