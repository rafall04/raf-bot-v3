 ...
 */
"use strict";
const log = require('./logger').logger.child('LAPORAN_DRAFT_STORE');
...
function loadDrafts(filePath = resolveFilePath()) {
    try {
        ...
        log.error("[LAPORAN_DRAFT] Gagal membaca store, dianggap kosong:", error.message);
        return [];
    }
}
...
function saveDrafts(drafts, filePath = resolveFilePath()) {
    try {
        ...
        log.error("[LAPORAN_DRAFT] Gagal menulis store:", error.message);
        return false;
    }
}
