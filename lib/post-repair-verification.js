...
const DEFAULTS = {
    ...
    // "web" (bawaan, tanpa SNMP) atau "snmp". SNMP membuat OLT hang.
    sumber: "web",
    groupId: "",
};
...
function createPostRepairVerifier(deps = {}) {
    ...
    // SUMBER REDAMAN: web (bawaan) atau snmp.
    //
    // !! SNMP membuat OLT HANG — itu sebabnya bawaannya WEB. Jalur web terbukti setara:
    // ONU pembanding 0/1/2:56 terbaca -19,87 dBm lewat web vs -19,91 lewat SNMP (selisih
    // 0,04 dB = jitter pengukuran), 99 ONU dalam 778 ms. Ubah ke 'snmp' hanya bila memang
    // diinginkan dan pahami risikonya.
    const getOltSnapshot = deps.getOltSnapshot || ((opts) => {
        const sumber = String((getConfig() || {}).sumber || "web").toLowerCase();
        if (sumber === "snmp") return require("./olt-optical-resolver").getOltSnapshot(opts);
        return require("./olt-web-optical").getWebOpticalSnapshot({});
    });
