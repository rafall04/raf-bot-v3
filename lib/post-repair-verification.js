...
const DEFAULTS = {
    ...
    // Jangan bikin laporan untuk kejadian kecil — 1-2 ONU cukup ditangani notif biasa.
    minAffected: 3,
...
function createPostRepairVerifier(deps = {}) {
    ...
    const getOltSnapshot = deps.getOltSnapshot || ((opts) => require("./olt-optical-resolver").getOltSnapshot(opts));
    ...
    async function buildReport({ oltKey = "default", affected = [] } = {}) {
        ...
        const rows = affected.map((item) => {
            ...
            const status = onu.status || null;
            const valid = isRxPowerValid(onu, status);
            const rx = Number(parseFloat(onu.rxPower));
            const row = {
                ...base,
            ...
            if (onu.statusKnown === false) return { ...row, verdict: "TIDAK_TERBACA" };
    ...
    function buildMessage(report, { oltKey = "default", waktu = null } = {}) {
        ...
        return kepala + badan + ekor + catatan;
    }
    ...
    function reportAfterRecovery({ oltKey = "default", recovered = [], stillDown = [] } = {}) {
        try {
            ...
            const affected = [...recovered, ...stillDown].filter(Boolean);
            const minAffected = numOr(conf.minAffected, DEFAULTS.minAffected);
            if (affected.length < minAffected) return { scheduled: false, reason: "below_min_affected" };

            const delay = numOr(conf.settleDelayMs, DEFAULTS.settleDelayMs);
            const timer = setTimer(
                () => verifyAndReport({ oltKey, affected }).catch(() => { /* never-throw */ }),
                delay
            );
            if (timer && typeof timer.unref === "function") timer.unref();
            logger.log &&
                logger.log(`[POST-REPAIR] Verifikasi ${oltKey} dijadwalkan ${Math.round(delay / 1000)}s lagi untuk ${affected.length} pelanggan.`);
            return { scheduled: true, affected: affected.length, delayMs: delay };
        } catch (e) {
