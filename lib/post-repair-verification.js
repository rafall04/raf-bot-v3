 ...
 */
"use strict";
const log = require('./logger').logger.child('POST_REPAIR_VERIFICATION');
...
async function defaultPostToGroup(groupId, text) {
    try {
        ...
        log.error("[POST-REPAIR] postToGroup error:", e && e.message);
...
function createPostRepairVerifier(deps = {}) {
    ...
    // Redaman versi GenieACS (dilaporkan MODEM) untuk pelanggan-pelanggan terdampak.
    //
    // Ditampilkan BERDAMPINGAN dengan angka OLT, tidak disinkronkan — keputusan pemilik:
    // memaksa keduanya jadi satu angka membuang informasi. Teknisi yang memutuskan boleh
    // tidaknya meninggalkan lokasi berhak melihat kedua sudut pandang.
    // SATU query untuk semua yang terdampak (biasanya 1-3 orang). NEVER-THROW: gagal =
    // laporan tetap terkirim dengan angka OLT saja.
    const bacaAcsRedaman = deps.bacaAcsRedaman || (async (customerIds) => {
        const peta = new Map();
        try {
            const ids = [...new Set((customerIds || []).filter((v) => v != null).map(String))];
            if (!ids.length) return peta;
            const users = (global.users || []).filter((u) => u && ids.includes(String(u.id)) && u.device_id);
            if (!users.length) return peta;
            const deviceIds = [];
            const idByDevice = new Map();
            for (const u of users) {
                for (const potongan of String(u.device_id).split("|")) {
                    const dev = potongan.trim();
                    if (!dev) continue;
                    deviceIds.push(dev);
                    if (!idByDevice.has(dev)) idByDevice.set(dev, String(u.id));
                }
            }
            if (!deviceIds.length) return peta;
            const { REDAMAN_PATHS } = require("./wifi");
            const hasil = await require("./genieacs").queryDevices({
                query: { _id: { $in: deviceIds } },
                projection: ["_id", ...REDAMAN_PATHS].join(","),
                timeoutMs: 20000,
                operation: "postRepair.acsRedaman",
            });
            const { bacaNilaiRedaman } = require("./redaman-alert-scope");
            for (const d of (hasil && hasil.data) || []) {
                const baca = bacaNilaiRedaman(d, REDAMAN_PATHS);
                const id = idByDevice.get(d._id);
                if (baca && id && Number.isFinite(Number(baca.angka))) peta.set(id, Number(baca.angka));
            }
        } catch (_e) {
            // diam — laporan tetap jalan dengan angka OLT
        }
        return peta;
    });
    ...
    async function buildReport({ oltKey = "default", affected = [] } = {}) {
        ...
        // Satu query untuk semua yang terdampak — bukan per baris.
        let acsPerPelanggan = new Map();
        try { acsPerPelanggan = (await bacaAcsRedaman(affected.map((it) => it && it.customerId))) || new Map(); }
        catch (_e) { acsPerPelanggan = new Map(); }   // never-throw: laporan tak boleh jatuh
        ...
        const rows = await Promise.all(affected.map(async (item) => {
            ...
            const base = {
                ...
                // Angka dari GenieACS — ditampilkan berdampingan, tidak disinkronkan.
                rxAcs: item && item.customerId != null && acsPerPelanggan.has(String(item.customerId))
                    ? acsPerPelanggan.get(String(item.customerId))
                    : null,
                status: null,
    ...
    function line(row, index) {
        ...
        // DUA sumber apa adanya — tidak disinkronkan, tidak ada yang dibuang. Teknisi yang
        // memutuskan boleh-tidaknya meninggalkan lokasi berhak melihat keduanya.
        const dua = `GenieACS ${fmtRx(row.rxAcs)} · OLT ${fmtRx(row.rxPower)}`;
        return `${index + 1}. ${row.name}${port} — ${dua}${banding}`;
    }
