...
function registerRoutes(app, runtime) {
    ...
    const { ensureAuthenticatedStaff: ensureAuthenticatedStaffApi } = require("../routes/api-route-helpers");
    ...
    const teknisiSettingsRouter = require("../routes/teknisi-settings-api"); // #b354 self-service prefs teknisi
    ...
    // #b354: self-service prefs teknisi — prefix sendiri (lolos gerbang admin-router; gate ensureAuthenticatedStaff di router).
    app.use("/api/teknisi", teknisiSettingsRouter);
    // Digerbangi di TITIK MOUNT, bukan per-endpoint: routes/monitoring-api.js tak punya satu pun
    // rujukan ke `req.user`, sehingga setiap endpoint baru di sana lahir tanpa gerbang. Prefiksnya
    // sudah dicabut dari PUBLIC_PATHS sehingga anonim tertolak, TAPI gerbang global meloloskan
    // `req.user || req.customer` — jadi sesi PELANGGAN tetap bisa membaca daftar seluruh sesi
    // PPPoE aktif (username, IP, MAC, uptime milik semua pelanggan) dan memicu 18 perintah
    // RouterOS (~11 detik terukur) berulang kali ke router produksi.
    // Satu-satunya konsumennya halaman dashboard admin (views/sb-admin/index.php).
    app.use("/api/monitoring", ensureAuthenticatedStaffApi, monitoringApiRouter);
