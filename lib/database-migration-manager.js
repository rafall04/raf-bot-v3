...
const log = require('./logger').logger.child('DATABASE_MIGRATION_MANAGER');
const fs = require('fs');
...
class DatabaseMigrationManager {
    ...
    async createBackup(dbPath) {
        ...
        try {
            // Backup HOT AMAN tanpa membuka koneksi ke DB LIVE — checkpoint via koneksi
            // TERPISAH ke DB live me-WEDGE global.db (SQLITE_IOERR senyap sampai restart,
            // akar bug fatal 2026-07-06). Lihat lib/sqlite-pragmas.hotBackupSqlite.
            const { hotBackupSqlite } = require('./sqlite-pragmas');
            await hotBackupSqlite(dbPath, backupPath);
            log.info(`[MIGRATION] Backup created: ${backupName}`);
            return backupPath;
        } catch (error) {
            log.error(`[MIGRATION] Failed to create backup: ${error.message}`);
            throw error;
        }
    }
    ...
    async migrateDatabase(dbName, targetVersion = null) {
        ...
        if (!fs.existsSync(dbPath)) {
            log.info(`[MIGRATION] Database ${dbName} not found, will be created on first use`);
        ...
        try {
            ...
            log.info(`[MIGRATION] Current version for ${dbName}: ${currentVersion}`);

            // Get expected schema version (dari code)
            const expectedVersion = targetVersion || this.getExpectedVersion();

            if (currentVersion >= expectedVersion) {
                log.info(`[MIGRATION] ${dbName} already at version ${currentVersion}, no migration needed`);
            ...
            for (let version = currentVersion + 1; version <= expectedVersion; version++) {
                ...
                if (migrationResult.success) {
                    ...
                    migrated = true;
                    log.info(`[MIGRATION] ${dbName} migrated to version ${version}`);
                } else {
            ...
            log.error(`[MIGRATION] Migration failed for ${dbName}:`, error.message);
            log.error(`[MIGRATION] Backup available at: ${backupPath}`);
            return {
                success: false,
                error: error.message,
                backupPath
    ...
    async runMigrationStep(dbPath, dbName, version) {
        ...
        const migrations = {
            ...
            5: async (dbPath, dbName, tableName) => {
                // Migration version 5: tandai akun infrastruktur (mis. modem CCTV/monitoring)
                // supaya bisa dipisah dari data pelanggan tapi tetap terbaca di monitor OLT.
                // Default 'pelanggan' agar semua user existing tetap dianggap pelanggan biasa.
                if (dbName !== 'users.sqlite' || !tableName) {
                    return { description: `Skip account_type column for ${dbName}` };
                }
                await this.addColumn(dbPath, tableName, 'account_type', 'TEXT', "'pelanggan'");
                return { description: "Add account_type column to users (default 'pelanggan')" };
            },
            6: async (dbPath, dbName, tableName) => {
                // Migration version 6: tandai agen penagih yang ditugaskan ke pelanggan.
                // Default NULL = pelanggan belum ditugaskan ke agen mana pun. Dipakai oleh
                // fitur role 'agen' (penagih pembayaran berbasis fee) agar agen hanya
                // menagih pelanggan miliknya.
                if (dbName !== 'users.sqlite' || !tableName) {
                    return { description: `Skip assigned_agen_id column for ${dbName}` };
                }
                await this.addColumn(dbPath, tableName, 'assigned_agen_id', 'INTEGER', null);
                return { description: 'Add assigned_agen_id column to users (default NULL = belum ditugaskan)' };
            },
            7: async (dbPath, dbName, tableName) => {
                // Migration version 7: dusun sebagai KOLOM tersendiri. Sebelumnya dusun hanya "hidup"
                // di dalam string username PPPoE (`<nama>-<dusun>@realm`) hasil wizard PSB — tak bisa
                // dipakai mengelompokkan (broadcast per dusun, gangguan area, pemetaan ODP), dan
                // pelanggan lama tak berpola itu sama sekali. Default NULL = belum diisi.
                if (dbName !== 'users.sqlite' || !tableName) {
                    return { description: `Skip dusun column for ${dbName}` };
                }
                await this.addColumn(dbPath, tableName, 'dusun', 'TEXT', null);
                return { description: 'Add dusun column to users (default NULL = belum diisi)' };
            },
            8: async (dbPath, dbName, tableName) => {
                // Migration version 8: JEJAK ASAL koordinat pelanggan.
                // Tanpa ini, titik hasil stempel otomatis panel (koordinat default saat izin GPS
                // ditolak) tak bisa dibedakan dari titik asli hasil share-lokasi — 92 pelanggan
                // sempat menumpuk di satu titik dan terpaksa dibersihkan buta. Dengan kolom ini,
                // tiap titik bisa ditelusuri sumber & waktunya.
                if (dbName !== 'users.sqlite' || !tableName) {
                    return { description: `Skip location_source columns for ${dbName}` };
                }
                await this.addColumn(dbPath, tableName, 'location_source', 'TEXT', null);
                await this.addColumn(dbPath, tableName, 'location_updated_at', 'TEXT', null);
                return { description: 'Add location_source + location_updated_at columns to users' };
            }
    ...
    getExpectedVersion() {
        ...
        return 8; // Update ini ke version terbaru
    }
