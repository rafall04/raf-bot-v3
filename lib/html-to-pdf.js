 ...
 */
"use strict";
const log = require('./logger').logger.child('HTML_TO_PDF');


const fs = require("fs");

// puppeteer@25 adalah paket ESM-murni — require() ditangguhkan sampai render diminta agar
// pemanggil non-PDF (dan runner test CJS) tak ikut memuat dependency browser berat ini.
...
async function renderHtmlToPdf(html, options = {}) {
    ...
    if (process.platform === "linux") {
        ...
        if (terpasang) {
            ...
            log.info(`[HTML_TO_PDF] Memakai Chromium sistem: ${terpasang}`);
        } else {
            ...
            log.info(`[HTML_TO_PDF] Chromium sistem tak ada; memakai browser bawaan puppeteer: ${bawaan}`);
        }
    }
