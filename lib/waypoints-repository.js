 ...
 */
"use strict";
const log = require('./logger').logger.child('WAYPOINTS_REPOSITORY');
...
function withWaypointsDatabase(handler) {
    return new Promise((resolve, reject) => {
        const db = new sqlite3.Database(getWaypointsDatabasePath(), (err) => {
            if (err) {
                log.error("[WAYPOINTS_DB] Error opening database:", err.message);
                reject(err);
                return;
            }
            ...
            applySqlitePragmas(db)
                .catch((pragmaErr) => {
                    log.warn(`[WAYPOINTS_DB_PRAGMA_WARN] ${pragmaErr.message}`);
                })
    ...
    return withWaypointsDatabase((db) => new Promise((resolve, reject) => {
        ...
        db.serialize(() => {
            db.run(createTableSql, (err) => {
                if (err) {
                    log.error("[WAYPOINTS_DB] Error creating table:", err.message);
                    reject(err);
                    return;
                }
                ...
                createIndexesSql.forEach((sql) => {
                    db.run(sql, (indexError) => {
                        if (indexError) {
                            log.warn("[WAYPOINTS_DB] Error creating index:", indexError.message);
                        }
                        indexCount += 1;
...
function getConnectionWaypoints(connectionType, sourceId, targetId) {
    return withWaypointsDatabase((db) => new Promise((resolve, reject) => {
        db.get(
            ...
            (err, row) => {
                if (err) {
                    log.error("[WAYPOINTS_DB] Error getting waypoints:", err.message);
                    reject(err);
                    return;
                }
                ...
                try {
                    ...
                    log.error("[WAYPOINTS_DB] Error parsing waypoints JSON:", parseError);
                    resolve(null);
                }
            }
        );
...
function saveConnectionWaypoints(connectionType, sourceId, targetId, waypoints, userId = null) {
    return withWaypointsDatabase((db) => new Promise((resolve, reject) => {
        db.run(
            ...
            (err) => {
                if (err) {
                    log.error("[WAYPOINTS_DB] Error saving waypoints:", err.message);
                    reject(err);
                    return;
                }
                resolve(true);
...
function deleteConnectionWaypoints(connectionType, sourceId, targetId) {
    return withWaypointsDatabase((db) => new Promise((resolve, reject) => {
        db.run(
            ...
            (err) => {
                if (err) {
                    log.error("[WAYPOINTS_DB] Error deleting waypoints:", err.message);
                    reject(err);
                    return;
                }
                resolve(true);
...
function getAllConnectionWaypoints() {
    return withWaypointsDatabase((db) => new Promise((resolve, reject) => {
        db.all(
            ...
            (err, rows) => {
                if (err) {
                    log.error("[WAYPOINTS_DB] Error getting all waypoints:", err.message);
                    reject(err);
                    return;
                }
