...
const log = require('./logger').logger.child('STATE_MANAGER');
...
function isProcessing(sender) {
    ...
    if (Date.now() - lock.timestamp > LOCK_TIMEOUT) {
        if (DEBUG) {
            log.info(`[STATE_MANAGER] Lock expired for ${sender}, clearing`);
        }
    ...
    if (DEBUG) {
        log.info(`[STATE_MANAGER] ${sender} is already being processed`);
    }
    return true;
}
...
function setProcessing(sender) {
    ...
    if (DEBUG) {
        log.info(`[STATE_MANAGER] Lock acquired for ${sender}`);
    }
    ...
    const cleanupTimer = setTimeout(() => {
        if (stateLocks.has(sender)) {
            ...
            if (Date.now() - lock.timestamp >= LOCK_TIMEOUT) {
                if (DEBUG) {
                    log.info(`[STATE_MANAGER] Auto-clearing expired lock for ${sender}`);
                }
                stateLocks.delete(sender);
            }
        }
...
function clearProcessing(sender) {
    ...
    if (stateLocks.has(sender)) {
        if (DEBUG) {
            ...
            log.info(`[STATE_MANAGER] Lock cleared for ${sender} after ${duration}ms`);
        }
        stateLocks.delete(sender);
    }
}
...
function clearAll() {
    ...
    stateLocks.clear();
    if (DEBUG) {
        log.info(`[STATE_MANAGER] Cleared ${size} locks`);
    }
}
