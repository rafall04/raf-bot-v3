 ...
 */
"use strict";
const log = require('./logger').logger.child('CONFIG_WRITER');
...
function saveConfigGate(mutate, opts = {}) {
    ...
    if (opts.runtime && typeof opts.runtime.setConfig === "function") {
        try { opts.runtime.setConfig(global.config); } catch (e) { log.error("[CONFIG_WRITER] setConfig runtime gagal:", e && e.message); }
    }
    if (opts.reinitCron) {
        try { require("./cron").initializeAllCronTasks(); } catch (e) { log.error("[CONFIG_WRITER] reinit cron gagal:", e && e.message); }
    }
    if (opts.resyncWorkerKey) {
        try { require("./feature-flags").resyncWorkerForFlag(opts.resyncWorkerKey); } catch (e) { log.error("[CONFIG_WRITER] resync worker gagal:", e && e.message); }
    }
