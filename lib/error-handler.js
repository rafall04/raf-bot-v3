 ...
 */
const log = require('./logger').logger.child('ERROR_HANDLER');
...
function errorHandler(err, req, res, _next) {
    ...
    log.error(`[ERROR_HANDLER] ${new Date().toISOString()}`, {
        error: err.message,
        stack: err.stack,
        url: req.url,
        method: req.method,
    ...
    return new Promise((resolve, reject) => {
        try {
            operation((err, result) => {
                if (err) {
                    log.error(`[DB_ERROR] ${errorMessage}:`, err);
                    reject(createError(
            ...
            log.error(`[DB_OPERATION_ERROR] ${errorMessage}:`, error);
            reject(createError(
                ErrorTypes.DATABASE_ERROR,
                errorMessage,
                500,
...
async function whatsappOperation(operation, errorMessage = 'WhatsApp operation failed') {
    try {
        ...
        log.error(`[WHATSAPP_ERROR] ${errorMessage}:`, error);
...
async function externalApiOperation(operation, errorMessage = 'External API operation failed') {
    try {
        ...
        log.error(`[EXTERNAL_API_ERROR] ${errorMessage}:`, error);
