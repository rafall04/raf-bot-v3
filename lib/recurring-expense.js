...
async function simpan(data = {}) {
    ...
    const perkiraan = Math.round(Number(data.perkiraan));
...
async function konfirmasi(id, { nominal, actor = "WhatsApp", date = new Date() } = {}) {
    ...
    const jumlah = Number.isFinite(Number(nominal)) && Number(nominal) > 0 ? Math.round(Number(nominal)) : Number(item.perkiraan);
