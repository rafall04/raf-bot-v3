...
/**
 * Item yang perlu diingatkan SEKARANG: jatuh tempo pada/ sebelum hari ini di periode berjalan,
 * dan belum diingatkan + belum dituntaskan periode ini.
 *
 * JENDELA `tgl <= hariIni`, bukan `=== hariIni` (#b308). Cron pengingat hanya menandai
 * `last_reminded_period` saat kirim SUKSES; kalau tanggal-D itu WhatsApp kebetulan putus / bot
 * baru restart (prod restart 7-13x/hari, node-cron tak mengejar tick terlewat), pengingat gagal
 * dan `=== hariIni` membuatnya HILANG sebulan penuh (esok tgl sudah lewat). Dengan jendela ini,
 * item yang terlewat tetap muncul esok harinya sampai berhasil diingatkan — idempotensi tetap
 * dijaga oleh penanda `last_reminded_period` (sekali terkirim, berhenti). Tanggal 29-31 pada
 * bulan pendek tetap jatuh ke hari terakhir bulan.
 */
 ...
 *
 * #b310 (T7): KLAIM periode ATOMIK sebelum createExpense (anti-TOCTOU). Dulu urutannya
 * baca→cek→createExpense→tandai, sehingga DUA konfirmasi bersamaan (balas `ok` di grup WA +
 * klik "catat" di /kas-usaha, atau dua owner hampir bersamaan — sender beda, guard isProcessing
 * per-sender tak menyerialkan) sama-sama lolos cek lalu sama-sama membuat expense → biaya rutin
 * tercatat DOBEL di expense_entries & buku besar (event_key ledger tak menolong karena id expense
 * berbeda). Sekarang: `UPDATE ... WHERE last_settled_period belum periode ini` — hanya SATU yang
 * mendapat `changes===1`, sisanya gagal klaim. Kalau createExpense gagal, klaim DILEPAS lagi supaya
 * periode tetap bisa dicoba ulang (mempertahankan sifat "gagal ⇒ masih tertunda" yang lama).
 */
async function konfirmasi(id, { nominal, actor = "WhatsApp", date = new Date() } = {}) {
    ...
    // Klaim periode ATOMIK — pemenang balapan saja yang lanjut.
    const klaim = await run(
        `UPDATE recurring_expenses SET last_settled_period=?, last_action='dicatat', updated_at=datetime('now')
          WHERE id=? AND (last_settled_period IS NULL OR last_settled_period != ?)`,
        [periode, Number(id), periode]
    );
    if (!klaim.changes) throw new Error(`"${item.nama}" sudah dituntaskan periode ini`);

    const { createExpense } = require("./expense-manager");
    let dibuat;
    try {
        dibuat = await createExpense(
            {
                title: item.nama,
                ...
                // #b310 (T18): tautkan expense ke biaya rutin sumbernya, supaya cancelExpense bisa
                // MELEPAS penanda periode ini bila entri dibatalkan (lihat expense-manager.cancelExpense).
                source_recurring_id: item.id
            },
            { name: actor }
        );
    } catch (e) {
        // Pencatatan gagal → LEPAS klaim supaya periode ini bisa dicoba lagi (jangan tertinggal
        // 'settled' tanpa entri apa pun). Kembalikan ke nilai sebelumnya.
        await run(
            "UPDATE recurring_expenses SET last_settled_period=?, last_action=?, updated_at=datetime('now') WHERE id=?",
            [item.last_settled_period, item.last_action, Number(id)]
        );
        throw e;
    }
