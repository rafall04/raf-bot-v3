...
function handleMessage(msg, rinfo) {
    ...
    // dari syslog header yang bisa drift). Caller bisa override kalau perlu.
    const event = correlator.ingest(parsed, Date.now());
    if (!event) {
    ...
    console.log(`[OLT-Syslog] ${event.event_type.toUpperCase()} ${event.mac} from ${rinfo.address} slot=${event.slot} onu=${event.onu}`);
}
