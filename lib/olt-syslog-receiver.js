...
function flushPending() {
    ...
    try {
        ...
        console.error('[OLT-Syslog] Flush error:', err.message);
...
function emitEvent(event) {
    ...
    console.log(`[OLT-Syslog] ${event.event_type.toUpperCase()} ${event.mac}${from} slot=${event.slot} onu=${event.onu}${confLabel}`);
    ...
    try {
        ...
        console.error('[OLT-Syslog] LOS broadcaster error:', broadcastErr.message);
    }
}
...
function startSyslogReceiver() {
    ...
    server.on('listening', () => {
        ...
        try {
            ...
            console.warn('[OLT-Syslog] Gagal set recv buffer:', bufErr.message);
        }
        try {
            stats.recv_buffer_bytes = server.getRecvBufferSize();
        } catch (__e) { /* noop */ }
        console.log(`[OLT-Syslog] Listening on ${addr.address}:${addr.port} (recvbuf=${stats.recv_buffer_bytes || '?'}B)`);
