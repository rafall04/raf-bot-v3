...
const log = require('./logger').logger.child('OLT_SYSLOG_RECEIVER');
...
const stats = {
    ...
    last_event_at: null,
    last_error: null,
    // Persist batching (hardening burst mass-outage).
    flush_count: 0,
    last_flush_at: null,
    last_flush_size: 0,
    pending_peak: 0,
...
// === Persist batching ===
// Mass-outage: ratusan paket DG+Lost tiba serempak (<1–2 dtk). Menulis olt-events.json
// (load+save JSON PENUH) PER PAKET memblok event loop → UDP recv buffer penuh → kernel
// DROP paket → paket dying-gasp hilang → salah vonis LOS justru saat paling kritis.
// Solusi: stage event di memori, flush ter-debounce → 1 read+write per ~250 ms / per batch,
// bukan per paket. Kerja per-paket jadi murni in-memory (Map.set) → socket cepat terkuras.
const pendingPersist = new Map(); // mac -> event (terbaru menang)
let flushTimer = null;
const FLUSH_DEBOUNCE_MS = 250;
const FLUSH_MAX_BATCH = 200; // flush segera bila menumpuk (burst besar) — jangan tunggu timer
...
function loadEvents() {
    try {
        ...
        log.error('[OLT-Syslog] Error loading events file:', err.message);
    }
    return {};
}
...
function saveEvents(events) {
    try {
        ...
        log.error('[OLT-Syslog] Error saving events file:', err.message);
    }
}
...
function pruneOldEvents(events) {
    ...
    if (removed > 0) {
        log.info(`[OLT-Syslog] Pruned ${removed} old events`);
    }
    return events;
}
 ...
 */
/**
 * Stage event untuk persist (batched). Terbaru menang per-MAC. Flush ter-debounce,
 * atau SEGERA bila batch besar (burst). TIDAK lakukan I/O di sini → murah saat burst.
 */
function stagePersist(event) {
    const cur = pendingPersist.get(event.mac);
    ...
    if (!cur || new Date(event.timestamp).getTime() >= new Date(cur.timestamp).getTime()) {
        pendingPersist.set(event.mac, event);
    }
    if (pendingPersist.size > stats.pending_peak) stats.pending_peak = pendingPersist.size;

    if (pendingPersist.size >= FLUSH_MAX_BATCH) {
        flushPending();
        return;
    }
    if (!flushTimer) {
...
/**
 * Tulis semua event yang ter-stage ke olt-events.json dalam SATU read+write.
 * Prioritas event terbaru per-MAC (semantik sama dengan persistEvent lama).
 */
function flushPending() {
    ...
    try {
        ...
        saveEvents(events);
        stats.flush_count += 1;
        stats.last_flush_at = new Date().toISOString();
        stats.last_flush_size = batch.length;
    } catch (err) {
        log.error('[OLT-Syslog] Flush error:', err.message);
        // Kembalikan batch ke pending (yang lebih baru menang) & jadwalkan ulang.
        for (const event of batch) {
            const cur = pendingPersist.get(event.mac);
            if (!cur || new Date(event.timestamp).getTime() >= new Date(cur.timestamp).getTime()) {
                pendingPersist.set(event.mac, event);
...
function handleMessage(msg, rinfo) {
    ...
    if (!syslog) {
        if (process.env.DEBUG_OLT_SYSLOG === 'true') {
            log.info('[OLT-Syslog DEBUG] Unparseable packet from', rinfo.address, '→', raw.slice(0, 120));
        }
        return;
    }
    ...
    // meta menempel ke event (termasuk yang ter-defer lewat grace window di korelator).
    const meta = { received_from: rinfo.address, raw_line: syslog.message };
    const event = correlator.ingest(parsed, eventServerTimeMs, meta);
    if (event) {
        // Keputusan langsung (DG ter-correlate / mode sinkron). Untuk LOS yang
        // di-defer, korelator akan panggil emitEvent lewat callback onEvent.
        emitEvent(event);
    }
    // null → DG menunggu Lost, ATAU Lost ditahan grace window (emit nanti via callback).
}

/**
 * Finalisasi sebuah event terklasifikasi: enrich rxPower + confidence, persist, log.
 * Dipanggil dari handleMessage (keputusan sinkron) DAN dari korelator onEvent
 * (keputusan ter-defer setelah grace window).
 */
function emitEvent(event) {
    // Phase 2: perkuat klasifikasi dengan riwayat rxPower (sinyal optik) — backup
    // saja, karena pembeda utama (kehadiran dying-gasp) sudah ditentukan korelator.
    if (event.event_type === 'dying-gasp' || event.event_type === 'los') {
        let extraSignals = [];
        try {
            const eventMs = Date.parse(event.server_time) || Date.now();
            const rxSignal = rxHistory.analyzeOfflineEvent(event.mac, eventMs);
            if (rxSignal && rxSignal.available) {
                extraSignals = [rxSignal];
            }
        } catch (rxErr) {
            log.error('[OLT-Syslog] rxPower analysis error:', rxErr.message);
        }
    ...
    const confLabel = event.classification_confidence !== undefined
        ...
        : '';
    const from = event.received_from ? ` from ${event.received_from}` : '';
    log.info(`[OLT-Syslog] ${event.event_type.toUpperCase()} ${event.mac}${from} slot=${event.slot} onu=${event.onu}${confLabel}`);

    // Log durable + enrich pelanggan (SEMUA tipe: DG/LOS/discovery). Never-throw, fire-and-forget.
    oltEventLog.recordOltEventSafe(event);

    // Auto-broadcast ke teknisi untuk LOS (fiber putus). Non-fatal: kegagalan di sini
    // TIDAK boleh mengganggu persist/log event. Broadcaster sendiri yang memfilter
    // event_type (los/discovery), confidence, dedup, dan confirmation window.
    try {
        losBroadcaster.handleOltEvent(event);
    } catch (broadcastErr) {
        log.error('[OLT-Syslog] LOS broadcaster error:', broadcastErr.message);
    }
}

const DEFAULT_LOST_GRACE_MS = 4000; // tahan vonis LOS ~4 detik untuk tunggu dying-gasp menyusul
// UDP recv buffer default 4MB: serap burst mass-outage. Kernel meng-cap ke net.core.rmem_max
// (default Linux ~208KB) → WAJIB naikkan sysctl rmem_max di host (lihat docs) agar efektif.
const DEFAULT_RECV_BUFFER_BYTES = 4 * 1024 * 1024;
...
function getSyslogConfig() {
    ...
    return {
        ...
        // Grace window: DG & Lost simultan, paket UDP bisa reorder. Tahan LOS sejenak.
        lostGraceMs: Number.isInteger(cfg.lostGraceMs) ? cfg.lostGraceMs : DEFAULT_LOST_GRACE_MS,
        recvBufferBytes: Number.isInteger(cfg.recvBufferBytes) && cfg.recvBufferBytes > 0
            ? cfg.recvBufferBytes
...
function startSyslogReceiver() {
    if (isRunning) {
        log.info('[OLT-Syslog] Already running');
        return;
    }
    const config = getSyslogConfig();
    if (!config.enabled) {
        log.info('[OLT-Syslog] Disabled (set config.oltSyslog.enabled=true to enable)');
        return;
    }
    ...
    correlator = createEventCorrelator({
        ...
        // Grace window: tahan LOS, emit lewat callback kalau DG tidak menyusul.
    ...
    server.on('error', (err) => {
        log.error('[OLT-Syslog] Server error:', err.message);
        ...
        try { server.close(); } catch (__e) { /* noop */ }
        server = null;
        isRunning = false;
    });
    ...
    server.on('message', (msg, rinfo) => {
        try {
            ...
            log.error('[OLT-Syslog] Handler error:', handlerErr.message);
    ...
    server.on('listening', () => {
        ...
        // Perbesar UDP recv buffer supaya burst mass-outage diantri, bukan di-drop.
        // Kernel meng-cap ke net.core.rmem_max → set juga sysctl di host (lihat docs).
        try {
            server.setRecvBufferSize(config.recvBufferBytes);
        } catch (bufErr) {
            log.warn('[OLT-Syslog] Gagal set recv buffer:', bufErr.message);
        }
        try {
            stats.recv_buffer_bytes = server.getRecvBufferSize();
        } catch (__e) { /* noop */ }
        log.info(`[OLT-Syslog] Listening on ${addr.address}:${addr.port} (recvbuf=${stats.recv_buffer_bytes || '?'}B)`);
    ...
    try {
        ...
        log.error('[OLT-Syslog] Bind failed:', bindErr.message);
...
function stopSyslogReceiver() {
    flushPending(); // jangan kehilangan event yang masih ter-stage di memori.
    if (server) {
        try { server.close(); } catch (__e) { /* noop */ }
        server = null;
    }
