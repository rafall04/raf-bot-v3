...
const log = require('./logger').logger.child('OLT_RXPOWER_POLLER');
...
async function pollOnce() {
    ...
    try {
        ...
        if (Array.isArray(result.failedOlts) && result.failedOlts.length) {
            log.warn('[OLT-rxPoller] ' + result.failedOlts.length + ' OLT tak terbaca siklus ini: '
                + result.failedOlts.map((o) => o.oltName || o.oltHost).join(', '));
        }

        // Siklus PERTAMA yang berhasil dilaporkan sekali — fitur yang tak pernah terlihat
        // gampang jadi fitur yang diam-diam mati (dan riwayat ini memang pernah mati).
        if (stats.poll_count === 0 && sampleCount > 0) {
            log.info('[OLT-rxPoller] Siklus pertama OK: ' + sampleCount + ' ONU terekam dalam '
                + (Date.now() - startedAt) + ' ms (sumber: ' + ((global.config && global.config.olt && global.config.olt.sumberOptik) || 'web') + ').');
        }
        stats.poll_count += 1;
        ...
        log.error('[OLT-rxPoller] Poll cycle error:', err.message);
    } finally {
        isPolling = false;
    }
}
...
function startRxPowerPoller() {
    if (isRunning) {
        log.info('[OLT-rxPoller] Already running');
        return;
    }
    const config = getPollerConfig();
    if (!config.enabled) {
        log.info('[OLT-rxPoller] Disabled (set config.oltRxPowerHistory.enabled=true to enable)');
        return;
    }

    log.info(`[OLT-rxPoller] Starting (interval: ${Math.round(config.intervalMs / 1000)}s)`);
