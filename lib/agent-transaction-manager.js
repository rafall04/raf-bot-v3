...
/**
 * Header Doc
 * Purpose: Kelola transaksi antara pelanggan dan agen reseller (topup, payment, voucher) +
 *          kredensial PIN agen. Ledger uang: dipakai rekonsiliasi financial-ledger domain 'agent'.
 * Caller: routes/agent*, message/handlers agen, lib/agent-voucher-manager (aktivasi stok).
 * Deps: `bcrypt`, `lib/logger`, `lib/id-generator`, `lib/json-store` (load/save ATOMIK), `lib/request-lock`.
 * MainFuncs: createAgentTransaction, confirmTransaction, cancelTransaction, completeTransaction,
 *          registerAgentCredentials, verifyAgentCredentials, saveAgentTransactions, saveAgentCredentials.
 * SideEffects: Membaca/menulis `database/agent_transactions.json` & `agent_credentials.json` (atomik).
 */

const bcrypt = require('bcrypt');
const { logger } = require('./logger');
const { generateAgentTransactionId } = require('./id-generator');
// #b345: store ledger transaksi & kredensial agen (uang reseller) lewat json-store — saveJSON
// tulis ATOMIK (tmp+rename) dan loadJSON MENGKARANTINA berkas rusak (bukan sekadar log lalu []).
// Sebelumnya: writeFileSync polos + initDatabase JSON.parse dalam SATU try/catch yang cuma log →
// torn-write saat SIGKILL → agentTransactions tetap [] (tanpa karantina) → save berikutnya
// MENIMPA berkas rusak dengan [] → riwayat uang agen HILANG PERMANEN & senyap. Sibling #b341
// (agent-voucher-manager) sudah difix; ini berkas terpisah yang terlewat.
...
function initDatabase() {
    // loadJSON per-file: satu berkas rusak dikarantina ke .rusak-<ts> lalu [] — TIDAK menular ke
    // berkas lain, dan berkas asli tetap bisa dipulihkan tangan. Tidak ada JSON.parse telanjang.
    const loadedTx = loadJSON('agent_transactions.json');
    agentTransactions = Array.isArray(loadedTx) ? loadedTx : [];
    const loadedCred = loadJSON('agent_credentials.json');
    agentCredentials = Array.isArray(loadedCred) ? loadedCred : [];
...
function saveAgentTransactions() {
    saveJSON('agent_transactions.json', agentTransactions);
}

function saveAgentCredentials() {
    saveJSON('agent_credentials.json', agentCredentials);
}
