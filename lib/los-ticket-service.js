 ...
 *       `lib/account-classification`, `lib/teknisi-recipient-resolver` (#b355 assign sadar-area),
 *       `global.config.oltLosBroadcast.autoTicket`, `global.users`, `global.accounts`, `global.reports`.
 ...
 */
"use strict";
const log = require('./logger').logger.child('LOS_TICKET_SERVICE');
...
// Teknisi dgn tiket terbuka PALING SEDIKIT dari sebuah daftar.
function leastLoaded(teknisi) {
...
// Pilih teknisi tujuan: forced (config) → SADAR-AREA (#b355: prioritaskan teknisi yang area-nya
// mencakup pelanggan + kelas ticket_new ON) → least-loaded. FAIL-OPEN: bila fitur off / tak ada
// teknisi yang cocok area → jatuh ke least-loaded SELURUH teknisi (LOS jangan pernah tak ter-assign).
function pickTeknisi(user) {
    ...
    let pool = teknisi;
    try {
        const { filterTeknisiRecipients, customerAreaKeys } = require("./teknisi-recipient-resolver");
        const cocok = filterTeknisiRecipients({
            recipients: teknisi,
            alertClass: "ticket_new",
            area: customerAreaKeys(user || {}),
            critical: true, // kepemilikan tiket abaikan jam-diam/snooze (itu penindasan NOTIF, ditangani saat kirim)
        }).recipients;
        if (cocok.length) pool = cocok;                 // ada yg cocok area/kelas → utamakan; else fail-open ke semua
    } catch (_e) { /* fail-open: pakai seluruh teknisi */ }
...
function maybeCreateLosTicket(incident) {
    try {
        ...
        const teknisi = pickTeknisi(user);
        if (teknisi) {
            try {
                ...
                log.error("[LOS-TICKET] auto-assign gagal:", assignErr && assignErr.message);
            }
        }
        ...
        try {
            require("./report-notification-service").notifyNewReport(ticket, { notifyAdmins: true }).catch((e) => {
                log.error("[LOS-TICKET] notifyNewReport error:", e && e.message);
            });
        } catch (e) {
            log.error("[LOS-TICKET] notifyNewReport gagal dispatch:", e && e.message);
        }

        log.info(`[LOS-TICKET] Tiket ${ticket.ticketId} dibuat dari LOS ${incident.mac} → ${user.name}${teknisi ? ` (assign ${teknisi.username})` : " (pool)"}.`);
        return ticket.ticketId;
    } catch (err) {
        log.error("[LOS-TICKET] maybeCreateLosTicket error:", err && err.message);
        return null;
    }
}
...
function maybeCancelLosTicket(ticketId, reason) {
    try {
        ...
        log.info(`[LOS-TICKET] Tiket ${ticketId} auto-batal — ONU pulih sebelum ditangani.`);
        return true;
    } catch (err) {
        log.error("[LOS-TICKET] maybeCancelLosTicket error:", err && err.message);
        return false;
    }
}
