 ...
 */
"use strict";
const log = require('./logger').logger.child('REPAIR_GROUP_NOTIFIER');
...
async function postToRepairGroup(text, ticketId) {
    ...
    try {
        ...
        log.error("[REPAIR_GROUP_NOTIF_ERROR]", { ticketId, error: e && e.message });
...
async function notifyRepairGroupNewTicket(ticket) {
    try {
        ...
        log.error("[REPAIR_GROUP_NOTIF_NEW_ERROR]", e && e.message);
    }
}
...
async function notifyRepairGroupCompleted(ticket, meta = {}) {
    try {
        ...
        log.error("[REPAIR_GROUP_NOTIF_DONE_ERROR]", e && e.message);
    }
}
