 ...
 */
"use strict";
const log = require('./logger').logger.child('INVOICE_GENERATOR');
...
function getRuntimeConfig() {
    ...
    try {
        ...
        log.error('[INVOICE_GENERATOR] Failed to load config:', error.message);
        return {
            invoice: {
                prefix: 'INV',
                enableTax: false,
...
function generateInvoiceNumber() {
    ...
    if (invoiceCounter.date !== dateStr) {
        ...
        try {
            ...
            log.error('[INVOICE_GENERATOR] Gagal menyeed nomor dari invoices.json:', error && error.message);
        }
...
function loadPackages() {
    ...
    try {
        ...
        log.warn('[INVOICE] Could not load packages database:', error.message);
    }
...
function getPackagePrice(subscription) {
    ...
    log.warn(`[INVOICE] Package "${subscription}" not found in database, trying to extract from name`);
...
function saveInvoice(invoiceData) {
    ...
    try {
        ...
        log.error('[INVOICE_SAVE_ERROR] Gagal memuat invoices.json — penyimpanan dibatalkan agar tak menimpa:', error && error.message);
        return false;
    }
    ...
    try {
        ...
        log.info(`[INVOICE_SAVED] Invoice ${invoiceData.invoiceNumber} saved successfully`);
        return true;
    } catch (error) {
        log.error('[INVOICE_SAVE_ERROR] Error saving invoice:', error);
...
function createInvoice(user, paymentData = {}) {
    try {
        if (!shouldGenerateInvoice(user)) {
            log.info(`[INVOICE_SKIP] User ${user.id} does not have send_invoice enabled`);
            return null;
        }
        ...
        if (saved) {
            log.info(`[INVOICE_CREATED] Invoice ${invoiceData.invoiceNumber} created for user ${user.id}`);
            return invoiceData;
        }

        log.error(`[INVOICE_ERROR] Failed to save invoice for user ${user.id}`);
        return null;
    } catch (error) {
        log.error('[INVOICE_CREATE_ERROR] Error creating invoice:', error);
        return null;
    }
}
...
function getUserInvoices(userId) {
    ...
    try {
        ...
        log.error('[INVOICE_GET_ERROR] Error reading invoices:', error);
        return [];
    }
}
