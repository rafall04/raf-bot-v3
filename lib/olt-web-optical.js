...
const STRIDE = 13;
const IDX = { onuId: 0, name: 1, mac: 2, status: 3, txPower: 10, rxPower: 11, jarak: 12 };
...
function parseOnuList(html) {
    ...
    for (let i = 0; i + STRIDE <= medan.length; i += STRIDE) {
        ...
        keluar.push({
            ...
            txPower: naik && Number.isFinite(parseFloat(f[IDX.txPower])) ? parseFloat(f[IDX.txPower]) : null,
            rxMentah,
            slot,
            onu,
        });
...
/**
 * Baca SATU ONU dari halaman detailnya — nilai PALING SEGAR yang tersedia.
 *
 * !! Halaman daftar (`onuConfigOnuList.asp`) di-cache OLT dan tidak realtime (informasi
 * pemilik, TERUKUR: 6 dari 8 ONU berbeda dari halaman detail, selisih s/d 0,23 dB). Kecil,
 * tapi cukup untuk membalik vonis di ambang batas. Karena itu:
 *   - daftar  → untuk TAMPILAN massal (99 ONU dalam 2 permintaan)
 *   - per-ONU → untuk KEPUTUSAN (alarm, verifikasi pasca-perbaikan, cek satu pelanggan)
 *
 * Halaman detail menyediakan `onuOpmInfo`, dengan label dari halamannya sendiri:
 *   index 4 = "Optic Module Tx Power", index 5 = "Optic Module Rx Power".
 *
 * @returns {Promise<{ok:boolean, rxPower:number|null, txPower:number|null, err?:string}>}
 */
async function bacaOnuSegar(device, pon, onuId, deps = {}) {
    const ambil = deps.fetchPage || fetchPage;
    const hal = await ambil(
        device,
        `/onuConfig.asp?onuno=${encodeURIComponent(onuId)}&oltponno=${encodeURIComponent(pon)}`,
        deps
    );
    if (!hal.ok) return { ok: false, rxPower: null, txPower: null, err: hal.err || `HTTP ${hal.code}` };
    const medan = ambilArrayJs(hal.body, "onuOpmInfo");
    if (!medan || medan.length < 6) return { ok: false, rxPower: null, txPower: null, err: "halaman detail tak dikenali" };

    const angka = (v) => {
        const s = String(v == null ? "" : v).trim();
        if (RX_KOSONG.has(s.toLowerCase())) return null;
        const n = parseFloat(s);
        return Number.isFinite(n) ? n : null;
    };
    return { ok: true, onuId: String(medan[0] || onuId), txPower: angka(medan[4]), rxPower: angka(medan[5]) };
}
