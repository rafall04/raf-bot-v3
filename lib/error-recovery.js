...
const log = require('./logger').logger.child('ERROR_RECOVERY');
...
class ErrorRecovery {
    ...
    async handleError(error, context = {}) {
        ...
        if (action) {
            ...
            if (recovered) {
                log.info(`[ERROR_RECOVERY] Successfully recovered from ${error.code} using ${action}`);
        ...
        if (shouldRetry && context.retryable !== false) {
            ...
            log.info(`[ERROR_RECOVERY] Will retry in ${delay}ms (attempt ${retryCount}/${this.maxRetries})`);
    ...
    async executeRecovery(action, context) {
        log.info(`[ERROR_RECOVERY] Executing recovery action: ${action}`);
        ...
        try {
            switch(action) {
                ...
                default:
                    log.info(`[ERROR_RECOVERY] Unknown action: ${action}`);
                    return false;
            }
        } catch (recoveryError) {
            log.error(`[ERROR_RECOVERY] Recovery action failed:`, recoveryError);
            return false;
        }
    }
    ...
    async reconnectWhatsApp() {
        ...
        if (now - this.lastReconnectAttempt < this.reconnectCooldown) {
            log.info('[ERROR_RECOVERY] Reconnect cooldown active');
            return false;
        }
        ...
        try {
            log.info('[ERROR_RECOVERY] Attempting WhatsApp reconnection...');
            ...
            log.info('[ERROR_RECOVERY] WhatsApp reconnection initiated');
            return true;
        } catch (error) {
            log.error('[ERROR_RECOVERY] Reconnection failed:', error);
            return false;
        }
    }
    ...
    async resetDatabase() {
        try {
            if (global.db) {
                log.info('[ERROR_RECOVERY] Resetting database connection...');
                
                // Close existing connection
                if (global.db.close) {
                    await new Promise((resolve) => {
                        global.db.close((err) => {
                            if (err) log.error('[ERROR_RECOVERY] DB close error:', err);
                            resolve();
                        });
                    });
                }
                ...
                global.db = new sqlite3.Database(dbPath);
                {
                    ...
                    applySqlitePragmas(global.db).catch((pragmaErr) => {
                        log.warn(`[ERROR_RECOVERY_PRAGMA_WARN] ${pragmaErr.message}`);
                    });
                }

                log.info('[ERROR_RECOVERY] Database connection reset');
                return true;
            }
            return false;
        } catch (error) {
            log.error('[ERROR_RECOVERY] Database reset failed:', error);
            return false;
        }
    }
    ...
    async restoreDatabase() {
        try {
            ...
            if (fs.existsSync(backupPath)) {
                log.info('[ERROR_RECOVERY] Restoring database from backup...');
                ...
                log.info('[ERROR_RECOVERY] Database restored from backup');
                return true;
            }
            
            log.info('[ERROR_RECOVERY] No backup available');
            return false;
        } catch (error) {
            log.error('[ERROR_RECOVERY] Database restore failed:', error);
            return false;
        }
    }
    ...
    async clearQueue(queueName) {
        try {
            if (global.messageQueue && queueName) {
                log.info(`[ERROR_RECOVERY] Clearing queue: ${queueName}`);
            ...
            if (global.messageQueue) {
                ...
                log.info('[ERROR_RECOVERY] All queues cleared');
                return true;
            }
            
            return false;
        } catch (error) {
            log.error('[ERROR_RECOVERY] Queue clear failed:', error);
            return false;
        }
    }
    ...
    async resetQueue(queueName) {
        try {
            ...
            log.info('[ERROR_RECOVERY] Queue reset completed');
            return true;
        } catch (error) {
            log.error('[ERROR_RECOVERY] Queue reset failed:', error);
            return false;
        }
    }
    ...
    async clearMemory() {
        try {
            ...
            if (global.gc) {
                global.gc();
                log.info('[ERROR_RECOVERY] Garbage collection executed');
            }
            ...
            log.info('[ERROR_RECOVERY] Memory cleanup completed');
            return true;
        } catch (error) {
            log.error('[ERROR_RECOVERY] Memory clear failed:', error);
            return false;
        }
    }
    ...
    async closeExcessFiles() {
        try {
            ...
            log.info('[ERROR_RECOVERY] File handle limit reached - consider increasing ulimit');
            ...
            log.error('[ERROR_RECOVERY] File close failed:', error);
            return false;
        }
    }
    ...
    async restartService(serviceName) {
        try {
            log.info(`[ERROR_RECOVERY] Restarting service: ${serviceName}`);
            ...
            switch(serviceName) {
                ...
                default:
                    log.info(`[ERROR_RECOVERY] Unknown service: ${serviceName}`);
            }
            
            return false;
        } catch (error) {
            log.error('[ERROR_RECOVERY] Service restart failed:', error);
            return false;
        }
    }
    ...
    async alertAdmin(error, context, retryCount) {
        try {
            ...
            if (admins.length === 0 || !hasAuthenticatedSession()) {
                log.info('[ERROR_RECOVERY] No admin numbers configured or WhatsApp not connected');
                return;
            }
            ...
            for (const adminNumber of admins) {
                ...
                if (isReady()) {
                    try {
                        ...
                        log.info(`[ERROR_RECOVERY] Alert sent to admin: ${adminNumber}`);
                    } catch (sendError) {
                        log.error('[SEND_MESSAGE_ERROR]', {
                            jid,
                            error: sendError.message
                        });
                        log.error(`[ERROR_RECOVERY] Failed to alert admin ${adminNumber}:`, sendError);
                        // Continue to next admin
                    }
                } else {
                    log.warn('[SEND_MESSAGE_SKIP] WhatsApp not connected, skipping send to admin', adminNumber);
                }
            }
        } catch (alertError) {
            log.error('[ERROR_RECOVERY] Alert system error:', alertError);
        }
    }
    ...
    logError(errorInfo) {
        ...
        try {
            ...
            log.error('[ERROR_RECOVERY] Failed to write error log:', writeError);
        }
    }
