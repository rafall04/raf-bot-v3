...
- **`config.json` and `database/*.json` are merge-key, never overwrite.** Production customises templates and config; a blanket copy destroys that. Use `scripts/merge-data-json.js` (`merge` / `verify-shapes`) — it aborts on shape change instead of corrupting. **Hand-rolled merges must never assume dict shape:** `wifi_templates.json` is an *array* of `{intent, keywords}` — a `{...json}` merge silently rewrites it as `{"0":{...}}`, `JSON.parse` still succeeds, and every inbound WA message then dies on `wifiTemplates.map` (b403 outage, 21 Sep 2026 — both bots mute ~2 days while PM2 stayed "online").
