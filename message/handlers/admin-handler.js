...
async function handlePppStats() {
    try {
        const pppStatsResult = await getPppStats({ caller: 'admin-handler.statusppp' });
...
async function handleHotspotStats() {
    try {
        const hotspotStatsResult = await getHotspotStats({ caller: 'admin-handler.statushotspot' });
