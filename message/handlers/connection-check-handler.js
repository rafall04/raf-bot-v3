...
// `isGatewayHealthy` / `isPathWideIssue` / GATEWAY_LOSS_UNHEALTHY_PCT diangkat ke modul bersama
// `lib/upstream-path-health.js` agar `lib/app-aware-diagnosis.js` memakai predikat yang SAMA PERSIS
// (dulu app-aware memvonis "jalur ada kendala" pada DEGRADASI ringan yang di sini justru HEALTHY →
// balasan saling membantah). Nama lokal dipertahankan supaya seluruh pemakai di bawah tak berubah.
const { isGatewayHealthy, isPathWideIssue } = require('../../lib/upstream-path-health');
