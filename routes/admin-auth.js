 ...
 */
"use strict";
const log = require('../lib/logger').logger.child('ADMIN_AUTH');


function ensureAuthenticatedStaff(req, res, next) {
    if (!req.user) {
        log.info(`[AUTH_DEBUG] ensureAuthenticatedStaff: req.user is null/undefined. Path: ${req.path}, Method: ${req.method}`);
        log.info("[AUTH_DEBUG] Cookies:", req.cookies);
        log.info("[AUTH_DEBUG] Headers:", req.headers.authorization ? "Authorization header present" : "No Authorization header");
    ...
    if (!["admin", "owner", "superadmin", "teknisi"].includes(req.user.role)) {
        log.info(`[AUTH_DEBUG] ensureAuthenticatedStaff: Invalid role. User: ${req.user.username}, Role: ${req.user.role}, Path: ${req.path}`);
