...
function defaultDeps() {
    return {
        ...
        ensureJid: optionalRequire("../lib/whatsapp-delivery-service", "ensureJid", (value) => value),
        ...
        setUserState: optionalRequire("../message/handlers/conversation-handler", "setUserState", null),
        deleteUserState: optionalRequire("../message/handlers/conversation-handler", "deleteUserState", null),
        now: () => new Date()
    };
}
...
function getStateTarget(deps, user = {}) {
    const phone = getPhoneTarget(user);
    if (!phone) return null;
    return deps.ensureJid ? deps.ensureJid(phone) : phone;
}
...
function createAutoOutageConversationService(overrides = {}) {
    ...
    async function startConversation({ user = {}, state = {}, rule = {} } = {}) {
        ...
        const stateTarget = getStateTarget(deps, user);
        if (stateTarget && deps.setUserState) {
            deps.setUserState(stateTarget, {
                step: "AUTO_OUTAGE_TRIAGE",
                user_id: String(user.id),
                pppoe_username: user.pppoe_username || state.pppoe_username || null,
                conversation_id: conversation.id
            }, { scope: "auto-outage" });
    ...
    async function finalizeTicketDecision({ user = {}, conversation = {}, text = "" } = {}) {
        if (wantsTicket(text)) {
            const ticket = deps.createCustomerReportTicket
                ? await deps.createCustomerReportTicket({
                    user,
                    category: conversation.triage_category || "internet_mati",
                    description: conversation.description || "Gangguan terkonfirmasi dari auto outage PPPoE.",
                    source: "auto_outage",
                    pppoe_username: conversation.pppoe_username || user.pppoe_username || null,
                    sender: getPhoneTarget(user) || String(user.id),
                    laporanText: conversation.description || "Gangguan terkonfirmasi dari auto outage PPPoE.",
                    issueType: conversation.triage_category || "internet_mati",
                    priority: "normal",
                    createdBy: "auto_outage",
                    createdByRole: "system",
                    customerPhotos: conversation.media_json || [],
                    additionalFields: {
                        source: "auto_outage",
                        pppoe_username: conversation.pppoe_username || user.pppoe_username || null
                    }
                ...
                : { ticketId: null };
            const ticketId = ticket.ticketId || ticket.id || ticket.ticket_id || null;
            ...
            const stateTarget = getStateTarget(deps, user);
            if (stateTarget && deps.deleteUserState) deps.deleteUserState(stateTarget);
        ...
        if (declinesTicket(text)) {
            ...
            const stateTarget = getStateTarget(deps, user);
            if (stateTarget && deps.deleteUserState) deps.deleteUserState(stateTarget);
            const message = render(deps, "auto_outage_ticket_declined", "Baik Kak, belum kami buatkan tiket. Jika butuh bantuan, silakan balas lagi.", {});
            await sendText(user, message);
