...
- [Fix 2026-09-07 (RONDE 4 P2/P3 cron/ACS — auto-outage re-entrancy guard (anti dobel-DM) + rebootDevice 202→DEVICE_UNREACHABLE (queuedOnly #b254) + billing-akhir-bulan dedup profil LIVE (anti re-isolir/reboot) + refreshDeviceObjects purge task-202 (anti queue leak #b260) + speed/compensation revert transien-retry (anti tertahan boost) + redaman-check pppoeActive MAC live #b340)](docs/boundary-log.md#b347)
...
- [Fitur 2026-09-07 (RONDE 6 Fase A — self-service teknisi FONDASI: store prefs per-teknisi teknisi-prefs.repository (keyed account.id, atomik) + API self-scoped /api/teknisi/prefs + halaman "Pengaturan Saya" + WA `setelan saya` + resolver bersama teknisi-recipient-resolver (default=all, dipasang Fase B); gated config.teknisiPrefs default OFF)](docs/boundary-log.md#b354)
- [Fitur 2026-09-08 (RONDE 6 Fase B — alert per-area: resolver teknisi-recipient-resolver DIPASANG ke 3 choke-point fan-out (olt-los-broadcaster LOS kelas+critical tanpa-area, los-ticket-service pickTeknisi sadar-area fail-open, report-notification-service notifyNewReport filter area+kelas) via model area kanonik matchRuleTarget + WA `alert on/off/kelas/area/kanal`; gated config.teknisiPrefs default OFF)](docs/boundary-log.md#b355)
- [Fitur 2026-09-08 (RONDE 6 Fase C — pantau pribadi (prefs.pantau override interval/target/ambang di redaman-watch decideNotify+store) + quiet-hours/snooze non-critical (resolver, terpasang di notif tiket-baru) + `tiket saya` sadar-staf (TIKET_SAYA diperluas: teknisi lihat tiket ditugaskan by account.id, keyword dedup dari CEK_TIKET) + filter web "Tugas saya"; gated config.teknisiPrefs default OFF)](docs/boundary-log.md#b356)
- [Fitur 2026-09-08 (RONDE 6 Fase D — self-profil + hubungkan WA: lib/teknisi-account-link menutup gap account.lid (kode CSPRNG TTL sekali-pakai dari web + tebus WA `hubungkan <kode>` → tulis lid/phone via withLock+saveAccounts+invalidasi authCache) + WA HUBUNGKAN_WA + web kartu Profil&WA self-scoped; gated config.teknisiPrefs)](docs/boundary-log.md#b357)
- [Fitur 2026-09-09 (Cetak Voucher 36/lembar Mikhmon mono + auto-kirim PDF ke WhatsApp: render.js mode grid 4×9 + token index/login_url/durasi_raw + layout mikhmon36; lib/html-to-pdf helper Chromium; service renderPdf/renderPdfAndSend Opsi A getAdminJids; routes /voucher/print/pdf + /send-wa; gated config.voucherPrint default OFF)](docs/boundary-log.md#b358)
- [Fitur 2026-09-10 (Cetak Voucher: engine template SADAR-LOGIKA {{#if}}/{{#ifeq}}/{{#unless}}/{{else}} tanpa PHP-eval (render.js renderLogic/renderTemplateContent) + paritas variabel Mikhmon via alias (user/password/hotspotname/validity/datalimit/type…) + importer v2 konversi if($type)/datalimit jadi blok aman + preset mikhmon-pro; mikhmon-import.js convertConditionals)](docs/boundary-log.md#b359)
- [Fitur 2026-09-10 (Cetak Voucher pengerasan skala 360: kunci in-flight per-instance cegah provision GANDA generate + render Chromium ganda (tolak BUSY 409) + timeout render eksplisit anti-hang (lib/html-to-pdf timeoutMs 90s); services/voucher-print.service withInFlight)](docs/boundary-log.md#b360)
- [Fitur 2026-09-10 (Cetak Voucher registri batch server-side: repository voucher_print_batches.json (saveBatch/getBatch/prune 200) + generateBatch simpan snapshot ter-enrich otomatis + route GET /voucher/print/batches[/:id] + UI "Riwayat Batch" tombol Muat → cetak-ulang/kirim-ulang TANPA provision MikroTik; menutup R2 #b360)](docs/boundary-log.md#b361)
- [Fitur 2026-09-10 (BAGIAN 1 — ganti paket TERTUNDA opsi B: harga & kecepatan mulai siklus berikutnya via lib/package-change-scheduler + package-change-apply + cron package-change-rollover; approve/ubah-paket → scheduled+effective_date, cron apply awal bulan depan; tutup kebocoran tagihan-berjalan + bypass panel; gated config.packageChangeDeferred)](docs/boundary-log.md#b362)
- [Fitur 2026-09-10 (BAGIAN 2 — otorisasi pembayaran teknisi/agen via WA: handler+state PAYREQ_ (setujui/tolak RPQ-<id>|N, quote-reply, setujui semua) reuse payment-approval.service + bulk-approval-job (batch tanpa batas 20); error→WA admin via beritahuAdminGagal; token RPQ-<id> + fix link /pembayaran/otorisasi; gated config.paymentRequestWa default OFF)](docs/boundary-log.md#b363)
...
- [Fix 2026-09-11 (P1 — ketangguhan Otorisasi: /api/requests enrichment Map O(R+U) (dulu .find per baris O(R×U)); bulk-approval-job isJobStale/staleThresholdMs + GET /bulk-approve/log ekspos workerAktif/stale/heartbeat + banner FE worker-mati; activity-log untuk otorisasi massal (latar+sinkron))](docs/boundary-log.md#b371)
- [Fitur 2026-09-11 (P1 — routing notifikasi per-kategori ke GRUP [1/3 mesin inti]: lib/notif-categories registri 6 kategori + lib/notif-router choke-point recipientsFor/dispatch (gate OFF→DM admin legacy, ON+grup→grup, ON tanpa grup→FAIL-OPEN admin; @g.us-only tolak @lid; severity critical→sendCritical/info→sendMessage; never-throw) + config.notifRouting default OFF; belum ada call-site)](docs/boundary-log.md#b372)
- [Fitur 2026-09-11 (P1 — routing notif ke grup [2/3 halaman]: lib/notif-routing-config readRouting/setEnabled/setRoute + GET/PUT/POST /api/notif-routing (master, per-kategori grup+severity, verifikasi anggota getGroups=unknownGroups, kirim uji) admin-only fail-closed atomik + halaman notif-routing.php/js multi-select + link navbar Sistem)](docs/boundary-log.md#b373)
- [Fitur 2026-09-11 (P1 — routing notif ke grup [3/3 migrasi]: beritahuAdminGagal→dispatch("otorisasi_gagal"), olt-log-scraper._kirimAlarmOltAdmin→recipientsFor("los_alarm")+sendCritical(waitForReadyMs) dipertahankan; behavior-preserving saat notifRouting OFF; sisa payment_request/network_quality/voucher/billing belum dimigrasi)](docs/boundary-log.md#b374)
- [Fitur 2026-09-11 (P1 — routing notif migrasi billing_isolir+network_quality: admin-alarm.sendAdminAlarm +opsi category→recipientsFor (isolir/set-unpaid pass billing_isolir, kirim tetap sendCritical durable); quality-alarm→recipientsFor("network_quality"); behavior-preserving saat OFF)](docs/boundary-log.md#b375)
- [Fitur 2026-09-11 (P1 — routing notif migrasi voucher_sale+payment_request, 6/6 kategori tuntas: public.alertAdmins _kategoriDariTag (voucher*→voucher_sale, reaktivasi/tagihan→billing_isolir); requests.getPaymentRequestRecipients→recipientsFor("payment_request") di broadcast+digest; notifRouting tetap OFF)](docs/boundary-log.md#b376)
- [Fix 2026-09-11 (P1 — payload-pagination /api/requests: lib/request-window applyRequestWindow (pending selalu ikut, resolved N bln terakhir) + countByStatus; GET / terima ?sinceMonths=N opsional + meta.counts (tanpa param=semua, halaman teknisi/agen tak berubah); otorisasi.js ajax sinceMonths=6 + kartu pakai meta.counts; halaman admin tak lagi unduh seluruh arsip)](docs/boundary-log.md#b377)
