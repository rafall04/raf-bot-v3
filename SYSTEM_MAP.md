...
- Draft laporan gangguan pelanggan: `database/laporan-drafts.json` (`laporan-drafts_test.json` saat `NODE_ENV=test`) — draft `{userId,step,ticketData,targetUser,deviceStatus,createdAtMs}` yang sudah lengkap tapi masih **menunggu foto**. Owner `lib/laporan-draft-store.js`; dipindai saat boot + tiap 5 menit oleh `message/handlers/smart-report-handler.pindaiDraftLaporanTertunda` (dijadwalkan di `lib/app-runtime.js`) sehingga laporan **selamat dari `pm2 restart`** — sebelumnya draft hanya hidup di `stateStore` memori dengan promosi `setTimeout` 15 menit dan hilang tanpa jejak log. Bukan di-bootstrap `lib/database.js`.
...
- Ledger transaksi saldo & antrian topup (JSON): `database/saldo_transactions.json` + `database/topup_requests.json` — owner `lib/saldo/transactions-store.js` / `lib/saldo/topup-store.js` (konstanta path di `lib/saldo/shared.js`). Path via `getDatabasePath` (→ `*_test.json` saat `NODE_ENV=test`, #b319) supaya `npm test` tak menimpa data PRODUKSI. Bukan di-bootstrap `lib/database.js`.
...
- [Fix 2026-08-09 (Tiga gerbang yang tak pernah menyala: tutup tiket, bayar-dua-kali, dan tujuan transfer saldo)](docs/boundary-log.md#b200)
...
- [Feat 2026-08-10 (Gaji: menutup komisi historis yang sudah dibayar di luar sistem)](docs/boundary-log.md#b215)
...
- [Fix 2026-08-14 (PSB: modem "sedang dipakai" jadi PENEGASAN, bukan penolakan — plus koreksi bukti basi)](docs/boundary-log.md#b224)
...
- [Fix 2026-08-15 (Invoice = bukti LUNAS, bukan tagihan: blok "metode diterima" jadi opsional + label metode berhenti mencetak kode internal)](docs/boundary-log.md#b236)
...
- [Fix 2026-08-16 (Bot↔pelanggan: penjaga data internal di jalur OLT, kata batal, dan pesan dobel)](docs/boundary-log.md#b246)
- [Fix 2026-08-16 (Identitas internal jaringan tak lagi dioper ke jalur pelanggan — starvasi data)](docs/boundary-log.md#b247)
...
- [Fix 2026-08-24 (Keluhan di komentar survei diteruskan berdasarkan ISI, bukan skor)](docs/boundary-log.md#b263)
...
- [Fix 2026-08-24 (Alarm & laporan ke admin memanggil pengirim yang tidak ada — gagal diam-diam)](docs/boundary-log.md#b270)
...
- [Fix 2026-08-28 (Alarm redaman buta untuk pelanggan belum terdaftar — kepemilikan kini dua sumber)](docs/boundary-log.md#b288)
- [Fix 2026-08-28 (Penyaring Monitor OLT hilang sendiri saat detail pelanggan dibuka)](docs/boundary-log.md#b289)
...
- [Fix 2026-09-06 (WA kejujuran kirim — lib/whatsapp-delivery-verdict.js helper penanda gagal wrapper; retryFailedDeliveries tak lagi tandai dead-letter terkirim buta (kode voucher/saldo aman), delivery-service.sendMessage sent jujur pada {status:error})](docs/boundary-log.md#b318)
- [Fix 2026-09-06 (8 quick-win audit — getDatabasePath jaga ekstensi (test tak timpa topup prod), isolir.js re-read cron.json saat tick (anti bom isolir massal), isolir-notif skip user.paid, public.js voucher callback 200 bukan 500, topup await+cek addKoinUser, response-template-helper cek unresolved, raf.js cancel butuh state, olt-los fallback isi {penanganan}+sadar recovery)](docs/boundary-log.md#b319)
