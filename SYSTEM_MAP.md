...
- [Bugfix 2026-07-15 (Intake bukti bayar: foto keluhan/koordinasi salah-tangkap jadi "bukti pembayaran" → keputusan pindah ke fungsi murni lib/payment-proof-intake-policy dengan GERBANG UANG (tagihan terbuka/isolir); jejak admin-aktif kini DURABEL di chat_activity/message_logs.sqlite di-hydrate saat boot fail-closed; confirmProof tolak no_outstanding; notif admin iklankan ok/tolak/HAPUS)](docs/boundary-log.md#b151)
- [Bugfix 2026-07-15 (Gerbang bukti bayar #b152: pengecualian ISOLIR kini berlaku juga di cabang caption — pelanggan terisolir kirim foto ber-caption "internet mati" tak lagi tertelan jadi keluhan, tapi CAPTURE sebagai kandidat bukti bayar; guard `!isolir` di step 4 classifyIncomingPhoto)](docs/boundary-log.md#b152)
- [Fitur 2026-07-18 (Survei kepuasan CSAT bulanan: cron `lib/cron/jobs/rating-survey.js` kirim rating 1-5 ke pelanggan layak (filter isolir.js:89) + tangkap balasan DURABEL di `database/csat.sqlite` via hook `message/raf.js` gated `config.csatSurvey.enabled` — tahan restart, `unclear` tak membajak; digest rekap ke owner via getAdminJids; owner `repositories/csat.repository.js` + `lib/csat/*`; deploy GELAP default OFF)](docs/boundary-log.md#b153)
- [Fitur 2026-07-18 (CSAT GO-LIVE 2 bot: P1 alert detractor real-time skor ≤2 ke owner via sendCritical (notifikasi, bukan auto-tiket; gate `csatSurvey.alertDetractor`) + P2 No HP di rekap + mode uji `csatSurvey.testPhone` (kirim 1 nomor + tembus gerbang staf); GOTCHA nomor owner=staf jadi hook lewati; enabled=true + cron armed `0 9 22`/`0 9 27`, teruji live)](docs/boundary-log.md#b154)
- [Fitur 2026-07-18 (Halaman web admin `/survei` — rangkuman CSAT on-demand: `routes/admin-csat-routes.js` GET `/api/owner/csat` + `csat.repository.getTrend` + `views/sb-admin/survei.php`/`static/js|css/survei.*` (tiles+sebaran+tabel detractor+No HP+tren, dark-aware); pages.js `/survei` checkRole + nav; verified live 2 bot)](docs/boundary-log.md#b155)
- [Fitur 2026-07-18 (Anti-ban broadcast di `lib/cron/wa-send-queue.sendQueueWithRetry`: (1)jitter (2)jeda-batch (3)cap-per-run+defer (4)circuit-breaker stop+alert-admin (5)ledger `broadcast_ledger.sqlite` skip-terblokir/min-gap fail-open; opt-in `config.broadcastGuard.enabled` default OFF=INERT byte-identik; berlaku SEMUA broadcast via global.config fallback; owner `lib/broadcast-ledger.js`)](docs/boundary-log.md#b156)
- [Fitur 2026-07-18 (Spintax variasi isi pesan `{a|b|c}` anti-ban: `lib/spintax.expandSpintax` di-hook post-render ke renderTemplate/renderResponseTemplate; hanya grup ber-pipe → `${slot}` aman; INERT tanpa sintaks; 5 template CSAT ditulis ulang spintax)](docs/boundary-log.md#b157)
...
- [Fix 2026-07-19 (UI dark-mode: token SEMANTIK sadar-mode di tokens.css (--surface/--ink/--muted/--line, light :root → flip body.tk-dark) + @import tokens.css?v cache-bust + migrasi survei.css/owner-cockpit.css + guard scripts/check-theme-tokens.js "npm run check:theme" menangkap primitif-tetap; akar "halaman baru gelap-di-gelap")](docs/boundary-log.md#b162)
- [Feat 2026-07-20 (Request ganti paket — keputusan admin dari WhatsApp: `message/handlers/package-request-admin-handler`+`state-domains/package-request-admin.state` (owner `package-request`/`PKGREQ_`) + `admin.service.approvePackageChange` +aksi `cancel` diam-diam + `listPendingPackageChangeRequests` + notif ke `getAdminJids`∪ownerNumber; admin balas `ok`/`tolak <alasan>`/`batalkan` via quote-`req_pkg_…`/kode/`request paket`; hook SEBELUM bukti-bayar; verb pakai `batalkan` krn `batal` dicegat guard universal)](docs/boundary-log.md#b163)
- [Feat 2026-07-20 (LOS gerbang MATI-LISTRIK-AREA + notif pelanggan ramah: `olt-los-verifier` +`areaDgClusterByMac` (klaster dying-gasp tetangga intra-log, kebal drift) → `olt-los-broadcaster.broadcastGroup` suppress `suppressed_area_dg` bila ≥`dgClusterThreshold`; + notif PULIH ke pelanggan; template pelanggan RINGAN tanpa "fiber putus"; `normalizeLosConfig` +`current` fallback anti-wipe + `verifyViaScrape.areaOutage` + `notifyCustomer.{notifyOnRecovery,recoveryMessageTemplate}`; akar: ONU gagal-gasp saat mati-listrik massal salah-vonis LOS, verify+cluster-gate lama buta thd banjir DG)](docs/boundary-log.md#b164)
...
- [Fix+Feat 2026-07-21 (Peta jaringan — 3 penyaring disatukan + ODP melapor sendiri: `static/js/map-filter-core.js` BARU (murni/teruji) jadi SATU sumber kebenaran filter utk peta admin & teknisi; legenda tak lagi hilang senyap tiap auto-refresh, quick Online/Offline TIDAK lagi menyembunyikan ODC/ODP, garis mengikuti yang benar-benar tampil; ikon ODP + lencana hunian `3/8` & cincin merah bila mayoritas penghuni offline; popup: vonis dulu, panjang jalur kabel, tombol `/broadcast?mode=odp&target=`; GOTCHA tak-terbaca ≠ mati ≠ sehat)](docs/boundary-log.md#b168)
...
- [Fix+Feat 2026-07-21 (Template pesan — guard integritas key + SOD pindah owner: `message/__tests__/response-template-key-integrity.test.js` memindai SELURUH repo (bukan daftar manual) shg key yang dirujuk kode tapi hilang dari JSON ketahuan — 11 key hilang, termasuk seluruh konfirmasi transfer saldo, "sehat" krn fallback runtime menutupinya; `lib/template-usage-scanner.js` BARU (yatim/hilang/slot tak dikenal) dipakai `diagnostics?health=1`; `saveCategory` kini backup + tolak penghapusan massal (409); **`state-domains/speed-boost.state.js` BARU** (`SODB_*`) jadi owner Speed on Demand — jalur legacy + template `other_sod_payment_success` berisi REKENING DUMMY dihapus, rekening kini dari `config.bankAccounts`; `speed-boost-handler.js` TIDAK disambungkan krn melanggar 3 invariant (state sendiri, key `@lid` mentah, kirim ke `remoteJid`); `isCleanConsent(text,{onTopic})` di affirmative-parser; GOTCHA 4 signature render bikin analisis statis salah 11→98)](docs/boundary-log.md#b169)
