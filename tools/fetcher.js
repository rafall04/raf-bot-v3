...
const uploadImages = (buffData, fileName) => {
    return new Promise(async (resolve, reject) => {
        // Dynamically import the ESM 'file-type' package
        const { fileTypeFromBuffer } = await import('file-type');
        const fileTypeResult = await fileTypeFromBuffer(buffData);

        if (!fileTypeResult) {
            return reject(new Error('Could not determine file type from buffer.'));
        }
