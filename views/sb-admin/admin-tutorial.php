                ...
                <div class="container-fluid">
                    ...
                    <div class="tut tut-wide">
                        ...
                        <section class="flow f-admin" id="hari-pertama">
                            ...
                            <ol class="steps">
                                <li>
                                    ...
                                    <div class="note warn"><span class="ic">⚠️</span><span>Kalau Anda salah ketik role, akun bisa jadi teknisi/agen dan tidak bisa membuka halaman admin. API akun menerima peran admin/owner/superadmin; yang ditolak hanya teknisi/agen.</span></div>
                                </li>
                                <li>
                                ...
                                <li>
                                    ...
                                    <div class="note warn"><span class="ic">💡</span><span>Ketiga halaman panduan tersedia dari sidebar: <a href="/admin-tutorial">Panduan Admin</a> (halaman ini), <a href="/teknisi-tutorial">Panduan Teknisi</a>, dan <a href="/agen-tutorial">Panduan Agen</a> — arahkan tiap peran ke halamannya sendiri.</span></div>
                                </li>
                            </ol>
                        </section>
                        ...
                        <section class="flow f-admin" id="mulai-dari-nol-akun-konfigurasi-dan-samb">
                            ...
                            <div class="grid">
                                ...
                                <div class="card">
                                    ...
                                    <div class="trap"><span class="ic">⚠️</span><span><b>Jebakan:</b> 1) Kolom jadwal cron TIDAK bisa disimpan dari sini — kunci milik cron sengaja diabaikan; pakai /cron. 2) Dropdown grup WhatsApp kosong (503) bila bot sedang tidak terhubung. 3) 'Komisi Collection Teknisi/Agen' hanya berlaku ke depan — nyalakan SEBELUM mulai mengotorisasi. 4) Di produksi config disunting per-kunci; jangan menimpanya dengan salinan dari mesin lain. 5) Halaman /config dipagari peran di route-nya (admin/owner/superadmin); tapi API /api/config bergerbang STAF (termasuk teknisi), jadi teknisi bisa memanggil API config walau tak bisa membuka halamannya.</span></div>
                                </div>
                                <div class="card">
                                ...
                                <div class="card">
                                    <h3>Panduan Admin (halaman ini)</h3>
                                    <span class="where">/admin-tutorial</span>
                                    <p>Rujukan lengkap penggunaan panel — halaman yang sedang Anda baca, terbuka dari menu 'Panduan Admin' di sidebar.</p>
                                    <p class="when"><b>Kapan dipakai:</b> Saat admin baru mencari panduan bawaan dari dalam panel.</p>
                                    <div class="trap"><span class="ic">💡</span><span><b>Catatan:</b> Route dipagari peran admin/owner/superadmin dan view-nya ter-render normal. Teknisi dan agen punya halaman sendiri: /teknisi-tutorial dan /agen-tutorial — arahkan mereka ke sana.</span></div>
                                </div>
                            </div>
                        </section>
                        ...
                        <section class="flow f-admin" id="tiket-gangguan-pekerjaan-teknisi-di-lapa">
                            ...
                            <div class="grid">
                                ...
                                <div class="card">
                                    ...
                                    <div class="trap"><span class="ic">⚠️</span><span><b>Jebakan:</b> Semua langkah butuh akun teknisi/owner. <code class="kbd">list tiket</code> hanya menampilkan status 'baru'. Selama alur ini state TERLINDUNGI — mengetik <code class="kbd">menu</code> tidak akan menjatuhkan tiket di tengah jalan. Verifikasi OTP juga bisa dilakukan lewat panel web bila diperlukan. Jalur alternatif menutup tiket: <code class="kbd">tiketdone &lt;ID&gt;</code> (batas unggah foto 10 menit).</span></div>
                                </div>
                                <div class="card">
                        ...
                        <section class="flow f-admin" id="sdm-gaji-teknisi-kasbon-komisi">
                            ...
                            <div class="grid">
                                ...
                                <div class="card">
                                    ...
                                    <div class="trap"><span class="ic">⚠️</span><span><b>Jebakan:</b> Hanya akun ber-role teknisi yang dikenali dari nomor WhatsApp-nya yang dibalas; nomor yang tak terdaftar di /accounts dijawab 'akun belum bisa dikenali'. Perintah ini mengembalikan ringkasan gaji pokok, komisi penarikan, komisi marketing PSB, potongan &amp; sisa kasbon.</span></div>
                                </div>
                            </div>
                        </section>
                        ...
                        <section class="flow f-admin" id="berbicara-ke-pelanggan-broadcast-pengumu">
                            ...
                            <div class="grid">
                                <div class="card">
                                    ...
                                    <div class="trap"><span class="ic">⚠️</span><span><b>Jebakan:</b> 1) Ada PENJAGA DATA INTERNAL: pesan yang memuat jumlah pelanggan terdampak, nama PPPoE, atau nama ODP/ODC akan DITOLAK sebelum satu pesan pun terkirim — pemeriksaan dilakukan pada teks mentah maupun teks yang sudah jadi. Perbaiki kalimatnya; jangan mencari cara menembusnya. 2) Pelanggan yang opt-out dilewati kecuali kotak 'sertakan yang opt-out' dicentang — dan itu keputusan sadar. 3) Halaman /broadcast kini admin-only (dilayani handler halaman fail-closed), TAPI endpoint POST /api/broadcast masih bergerbang 'staf' termasuk teknisi — jadi teknisi bisa memanggil API broadcast langsung meski tak bisa membuka halamannya; perlakukan kewenangan ini setara admin. 4) Pengiriman diberi jeda + variasi waktu anti-blokir, jadi broadcast besar tidak selesai seketika; jangan klik kirim dua kali.</span></div>
                                </div>
                                <div class="card">
                        ...
                        <section class="faq" id="tanya-jawab">
                            ...
                            <details class="qa">
                                <summary>Di mana halaman panduan untuk tiap peran?</summary>
                                <div class="qa-body">Tiga halaman terpisah, semua dari sidebar: Panduan Admin (/admin-tutorial, halaman ini, dipagari admin/owner/superadmin), Panduan Teknisi (/teknisi-tutorial), dan Panduan Agen (/agen-tutorial). Arahkan tiap peran ke halamannya sendiri.</div>
                            </details>
                            <details class="qa">
                                <summary>Teknisi mengetik `gaji saya` di WhatsApp tapi bot diam. Kenapa?</summary>
                                <div class="qa-body">Pastikan nomor WhatsApp teknisi terdaftar di akunnya di /accounts dan akun itu ber-role teknisi — hanya nomor yang dikenali yang dibalas; nomor asing dijawab 'akun belum bisa dikenali'. Perintah <code class="kbd">gaji saya</code> (juga <code class="kbd">cek gaji</code>/<code class="kbd">slip gaji</code>/<code class="kbd">komisi saya</code>) aktif dan mengembalikan ringkasan gaji bulan berjalan.</div>
                            </details>
                            <details class="qa">
